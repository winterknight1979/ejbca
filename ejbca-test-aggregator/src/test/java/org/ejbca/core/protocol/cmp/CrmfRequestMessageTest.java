...
/**
 * Test to verify that CrmfRequestMessage can be properly Serialized. Previously
 * the fields in the inherited class BaseCmpMessage could not be properly
 * Serialized.
 *
 * @version $Id: CrmfRequestMessageTest.java 30985 2019-01-04 14:36:16Z samuellb
 *     $
 */
 ...
 * @throws IllegalStateException fail
   */
  @Test
  public void testCrmfRequestMessageSerialization()
      throws IOException, ClassNotFoundException, NoSuchAlgorithmException,
          NoSuchProviderException, InvalidAlgorithmParameterException {
    // Create a bogus PKIMessage
    PKIMessage myPKIMessage =
        createPKIMessage("CN=bogusIssuer", "CN=bogusSubject");

    // Create a bogus CrmfRequestMessage
    CrmfRequestMessage crmf =
        new CrmfRequestMessage(myPKIMessage, "CN=SomeCA", true, null);
    ...
    assertTrue(
        "The deserialized object was not of type CrmfRequestMessage.",
        o instanceof CrmfRequestMessage);
    CrmfRequestMessage crmf2 = (CrmfRequestMessage) o;
    assertEquals(
        "Inherited object was not properly deserilized: ",
        "macAlg",
        crmf2.getPbeMacAlg());
  }
 ...
 * @throws IllegalStateException fail
  ...
  public void testCrmfRequestUsernameGeneratorFromDN()
    ...
    CryptoProviderTools.installBCProviderIfNotAvailable();
       ...
       PKIMessage myPKIMessage =
          createPKIMessage(
              "CN=bogusIssuer", "CN=subject,SN=000106716,O=Org,C=SE");
       CrmfRequestMessage crmf =
          new CrmfRequestMessage(myPKIMessage, "CN=SomeCA", true, null);
       X500Name dnname = crmf.getRequestX500Name();
       UsernameGeneratorParams params = new UsernameGeneratorParams();
      ...
      assertEquals(
          "Username was not constructed properly from DN (CN)",
          "subject",
          username);
      ...
      // DN order the other way around, should give username the other way
      // around as well
      myPKIMessage =
          createPKIMessage(
              "CN=bogusIssuer", "C=SE,O=Org,SERIALNUMBER=000106716,CN=subject");
      crmf =
          new CrmfRequestMessage(myPKIMessage, "CN=SomeCA", true, null);
      dnname = crmf.getRequestX500Name();
      params = new UsernameGeneratorParams();
      params.setMode(UsernameGeneratorParams.DN);
      gen = UsernameGenerator.getInstance(params);
      username = gen.generateUsername(dnname.toString());
      assertEquals(
  ...
  private PKIMessage createPKIMessage(
    ...
    org.bouncycastle.asn1.x509.Time na =
        new org.bouncycastle.asn1.x509.Time(new Date());
    optionalValidityV.add(new DERTaggedObject(true, 0, nb));
    optionalValidityV.add(new DERTaggedObject(true, 1, na));
    OptionalValidity myOptionalValidity =
        OptionalValidity.getInstance(new DERSequence(optionalValidityV));
 ...
 * @throws IllegalStateException fail
 * @throws InvalidCmpProtectionException fail
   */
  @Test
  public void testNovosecRARequest()
      throws IOException, InvalidKeyException, NoSuchAlgorithmException,
          ...
          IllegalStateException, InvalidCmpProtectionException {
    // Check that we can parse a request from  Novosec (patched by EJBCA).
    // Read an initialization request with RAVerifiedPOP and PBE protection to
    // see that we can process it
    ...
    try {
      ...
      // Verify should be false if we do not allow RA verify POP here, since we
      // don't have any normal POP
      CrmfRequestMessage msg =
          new CrmfRequestMessage(req, "CN=AdminCA1", false, "CN");
      ...
      assertEquals(
          "CN=abc123rry-4371939543913639881,O=PrimeKey Solutions AB,C=SE",
          msg.getRequestDN());
 ...
 * @throws CertificateException fail
  ...
  @Test
  public void testNovosecClientRequestSHA256()
      throws IOException, InvalidKeyException, NoSuchAlgorithmException,
          NoSuchProviderException, InvalidAlgorithmParameterException,
          SignatureException, IllegalStateException, OperatorCreationException,
          CertificateException {
  ...
  private void doNovosecClientRequest(
      final String sigAlg, final String digestAlg, final String expectedAlgOid)
      throws IOException, InvalidKeyException, NoSuchAlgorithmException,
          NoSuchProviderException, InvalidAlgorithmParameterException,
          SignatureException, IllegalStateException, OperatorCreationException,
          CertificateException {
    // Check that we can parse a request from  Novosec (patched by EJBCA).
    // Read an initialization request with a signature POP and signature
    // protection to see that we can process it
      ...
      try {
        ...
        assertEquals("CN=AdminCA1,O=EJBCA Sample,C=SE", msg.getIssuerDN());
        assertEquals(
            "CN=abc123rry2942812801980668853,O=PrimeKey Solutions AB,C=SE",
            msg.getRequestDN());
        assertEquals("abc123rry2942812801980668853", msg.getUsername());
        assertEquals("foo123", msg.getPassword());
        // Verify signature protection
        AlgorithmIdentifier algId =
            msg.getMessage().getHeader().getProtectionAlg();
        String oid = algId.getAlgorithm().getId();
        assertEquals(PKCSObjectIdentifiers.sha1WithRSAEncryption.getId(), oid);
        // Check that this is an old message, created before ECA-2104, using
        // null instead of DERNull as algorithm parameters.
        ASN1Encodable pp = algId.getParameters();
        assertNull(pp);
        // Try to verify, it should work good even though the small bug in
        // ECA-2104, since we don't use algorithm parameters for RSA-PKCS
        // signatures
    ...
    // Re-protect the message, now fixed by ECA-2104
      ...
      in = new ASN1InputStream(novosecsigpopir);
      try {
        ASN1Primitive derObject = in.readObject();
        ...
        // We have to do this twice, because Novosec caches ProtectedBytes in
        // the PKIMessage object, so we need to
        // encode it and re-decode it again to get the changes from ECA-2104
        // encoded correctly.
        // Not needed when simply signing a new message that you create, only
        // when re-signing
        newmsg =
            CmpMessageHelper.signPKIMessage(
                pkimsg, signCertChain, keys.getPrivate(), digestAlg, "BC");
        in.close();
        in = new ASN1InputStream(newmsg);
        derObject = in.readObject();
        ...
        // Check that we have DERNull and not plain java null as algorithm
        // parameters.
        ASN1Encodable pp = algId.getParameters();
        assertNotNull(pp);
        assertEquals(DERNull.class.getName(), pp.getClass().getName());
        // Try to verify, also verify at the same time that encoding decoding of
        // the signature works
        assertTrue(
            CmpMessageHelper.verifyCertBasedPKIProtection(
                pkimsg, keys.getPublic()));
        // Verify that our verification routine does not give positive result
        // for any other keys
        CrmfRequestMessage msg =
            new CrmfRequestMessage(pkimsg, "CN=AdminCA1", false, "CN");
        assertTrue(msg.verify());
        PublicKey pubKey = msg.getRequestPublicKey();
        assertFalse(
            CmpMessageHelper.verifyCertBasedPKIProtection(pkimsg, pubKey));
      } finally {
        in.close();
      }
 ...
 * @throws InvalidCmpProtectionException fail
  ...
  @Test
  public void testBc146RARequest()
      throws IOException, InvalidKeyException, NoSuchAlgorithmException,
 ...
 * @throws InvalidCmpProtectionException fail
   */
  @Test
  public void testBc147RARequest()
      throws IOException, InvalidKeyException, NoSuchAlgorithmException,
          NoSuchProviderException, SignatureException,
          InvalidCmpProtectionException {
 ...
 * @throws InvalidCmpProtectionException fail
   */
  private void internalBcRARequestTest(final byte[] message)
      throws IOException, InvalidKeyException, NoSuchAlgorithmException,
          NoSuchProviderException, SignatureException,
          InvalidCmpProtectionException {
    // Check that we can parse request from BouncyCastle version 1.46.
    // Read an initialization request with RAVerifiedPOP with PBE protection to
    // see that we can process it
   ...
   */
  @Test
  public void testBc146ClientRequest()
      throws IOException, InvalidKeyException, NoSuchAlgorithmException,
          NoSuchProviderException, SignatureException {
   ...
   */
  @Test
  public void testBc147ClientRequest()
      throws IOException, InvalidKeyException, NoSuchAlgorithmException,
          NoSuchProviderException, SignatureException {
    internalBcClientRequestTest(bc147sigpopir);
  }

  private void internalBcClientRequestTest(final byte[] message)
      throws IOException, InvalidKeyException, NoSuchAlgorithmException,
          NoSuchProviderException, SignatureException {
    // Check that we can parse request from BouncyCastle version 1.46.
    // Read an initialization request with a signature POP, and signature
    ...
    try {
      ...
      CrmfRequestMessage msg =
          new CrmfRequestMessage(req, "CN=AdminCA1", false, "CN");
      // BC messages in BC1.46 uses POPOSigningKeyInput for POPO, not the 3rd
      // case in RFC4211 section 4.1, like everyone else...
      ...
      assertTrue(
          CmpMessageHelper.verifyCertBasedPKIProtection(
              msg.getMessage(), msg.getRequestPublicKey()));
    } finally {
      in.close();
    }
  }
   ...
   */
  @Test
  public void testHuaweiEnodeBClientRequest()
      throws IOException, InvalidKeyException, NoSuchAlgorithmException,
          NoSuchProviderException, SignatureException {
    ...
    try {
      ...
      // Does not work for this Huawei message, is it signed by the same key as
      // in the request at all?
      // We will wait for another huawei message to test
      // assertTrue(CmpMessageHelper.verifyCertBasedPKIProtection(
      //   msg.getMessage(), pubKey));

      // Read the CertConf (certificate confirmation) CMP message that the
      // client sends to
      // the CA after receiving the certificate. RFC4210 section "5.3.18.
  ...
  public void testBlueXIR()
    ...
    try {
      ...
      // Use a default CA instead
      msg = new CrmfRequestMessage(req, "CN=AdminCA1", false, "CN");
      // Verifying POP does not work on this very old BlueX request, check that
      // it fails
      assertFalse(msg.verify());
      assertEquals("CN=AdminCA1", msg.getIssuerDN());
      ...
      String oid = algId.getAlgorithm().getId();
      assertEquals(
          "1.2.840.113533.7.66.13", oid); // HMAC password based protection
      // Get the public key from the request, this does not work either on this
      // message, there is something wrong with the public key
      try {
        msg.getRequestPublicKey();
        fail(
            "We expect public key to be invalid in this very old BlueX"
                + " message");
      } catch (InvalidKeyException e) {
        assertEquals(
            "The message should be that the public key is",
  ...
  private static byte[] bluexir =
      ...
      ("MIICIjCB1AIBAqQCMACkVjBUMQswCQYDVQQGEwJOTDEbMBkGA1UEChMSQS5FLlQu"
           ...
           + "FQAy/vSoNUevcdUxXkCQx3fvxkjh6A==")
          .getBytes());

  /**
   * CMP initial request message, created with Huawei eNode B, with signature
...
+ "OZck9LLi")
  ...
  /**
   * CMP Cert Confirmation message, created with Huawei eNode B, with signature
   * POP (no POPOSigningKey) and signature protection.
   */
...
+ "Um6i30qhLj/1YtCx/5guQwG+Fu4tcsE6dI98R23Pd5dbWg==")
          .getBytes());

  /*
   * CMP Cert Response message, sent to Huawei eNode B Not used, just kept for
   * reference.
   *//*
  private static byte[] huaweiresponse =
      Base64.decode(
("MIILtTCCARECAQKkVDBSMQswCQYDVQQGEwJjbjELMAkGA1UECBMCc2gxCzAJBgNVBAcTAnFjMQsw"
...
+ "GCsbCtLscLeYallqTuvg/0O2zZITN5wcoQOjackHjIJg3eAz8A==")
  .getBytes()); */

  /**
   * CMP initial request message, created with BouncyCastle 1.46, with
  ...
  private static byte[] bc146rapopir =
      Base64.decode(
          ("MIIBcjCBowIBAqQRMA8xDTALBgNVBAMMBHVzZXKkFTATMREwDwYDVQQDDAhBZG1pbk"
              ...
              + "BwQFS2V5SWSkCgQI9oHh9MJNp/qlCgQI9oHh9MJNp/qggbAwga0wgaowgaU"
              + "CAXswgYijFTATMREwDwYDVQQDDAhBZG1pbkNBMaURMA8xDTALBgNVBAMMBH"
              + "VzZXKmXDANBgkqhkiG9w0BAQEFAANLADBIAkEAzPE7ZaY6jiyeELeotH4rH"
              + "A3imjvDwrzgcrefX3gbNiOKiz9/CJwQOU/V2jCc8pbAm02TS41ZSwrL7B7v"
              + "4rtWbQIDAQABMBUwEwYJKwYBBQUHBQEBDAZmb28xMjOAAKAXAxUA49wkvBj"
              + "dn5ENdJJnJb3wU1bHmcs=")
          .getBytes());
   ...
   private static byte[] bc146sigpopir =
      Base64.decode(
          ("MIICNzByAgECpBEwDzENMAsGA1UEAwwEdXNlcqQVMBMxETAPBgNVBAMMCEFkbWluQ0"
              ...
              + "MIGIoxUwEzERMA8GA1UEAwwIQWRtaW5DQTGlETAPMQ0wCwYDVQQDDAR1c2V"
              + "yplwwDQYJKoZIhvcNAQEBBQADSwAwSAJBAI7LgKEDN49fq08G3v8kXa8Gml"
              + "qqLtAy4OKDkbfvtQG/rHvki19isRCat3GPNKhFp5hUrVApPOkC7pwKrtCh3"
              + "u8CAwEAATAVMBMGCSsGAQUFBwUBAQwGZm9vMTIzoYHHoHOgE6QRMA8xDTAL"
              + "BgNVBAMMBHVzZXIwXDANBgkqhkiG9w0BAQEFAANLADBIAkEAjsuAoQM3j1+"
              + "rTwbe/yRdrwaaWqou0DLg4oORt++1Ab+se+SLX2KxEJq3cY80qEWnmFStUC"
              + "k86QLunAqu0KHe7wIDAQABMA0GCSqGSIb3DQEBBQUAA0EAFqKulRt/zRzvd"
              + "+HzPpYTBWd4g7tKaFAZEb+vHbt0hojMLq20kqIE1t2aYpezjGZdV4zyQJPU"
              + "Oi5VYfLesujvjqBDA0EAX8AwJDr+yzjxv3067lsiINO75x5GqqR644GogxE"
              + "IH1cyKkRbhlixw8qLdfWt2VShw1TdliLRkuZA5tCOtWlhlw==")
          .getBytes());

  /**
   * CMP initial request message, created with BouncyCastle 1.47, with
   * RAVerified POP and PBE protection.
   */
  ...
  Base64.decode(
      ("MIIBcjCBowIBAqQRMA8xDTALBgNVBAMMBHVzZXKkFTATMREwDwYDVQQDDAhBZG1pbk"
              ...
              + "BwQFS2V5SWSkCgQIP80XoNAlmuylCgQIP80XoNAlmuyggbAwga0wgaowgaU"
              + "CAXswgYijFTATMREwDwYDVQQDDAhBZG1pbkNBMaURMA8xDTALBgNVBAMMBH"
              + "VzZXKmXDANBgkqhkiG9w0BAQEFAANLADBIAkEAmNU532N0oqncVerEF6QUu"
              + "gLlyC9RfFwO9yRgeLlejbV86j/0sqIs//Zc1Y0SHLqV0pIHTuf3pke0jPJY"
              + "VHIN6wIDAQABMBUwEwYJKwYBBQUHBQEBDAZmb28xMjOAAKAXAxUAeZsIGZF"
              + "SEUpsNBtfA17aHxwjs/Y=")
          .getBytes());
  /**
   * CMP initial request message, created with BouncyCastle 1.47, with
   * POPOSigningKey POP and signature protection.
  ...
  private static byte[] bc147sigpopir =
      ...
      ("MIIBvzByAgECpBEwDzENMAsGA1UEAwwEdXNlcqQVMBMxETAPBgNVBAMMCEFkbWluQ0"
              ...
              + "iKMVMBMxETAPBgNVBAMMCEFkbWluQ0ExpREwDzENMAsGA1UEAwwEdXNlcqZ"
              + "cMA0GCSqGSIb3DQEBAQUAA0sAMEgCQQCY1TnfY3SiqdxV6sQXpBS6AuXIL1"
              + "F8XA73JGB4uV6NtXzqP/Syoiz/9lzVjRIcupXSkgdO5/emR7SM8lhUcg3rA"
              + "gMBAAEwFTATBgkrBgEFBQcFAQEMBmZvbzEyM6FSMA0GCSqGSIb3DQEBBQUA"
              + "A0EAGsbIFtJIwveAYvRzLtFwx9M2CFqr/tLbllNgogcljzhJ3bab4gsh25P"
              + "ZC2bvG7T3ieQXj8fR2KGp6xUQjl4Ii6BDA0EASsMjeH6FutbH9wIGr/5smC"
              + "59E3+z+RjnhhFyUxgMMLV7wKZbKtoJUzKjjc2Hh7IzCjt6sBmcf0dBMpR1Q"
              + "q8OWA==")
          .getBytes());

  /**
   * CMP initial request message, created with EJBCA 4.0.2 code, with RAVerified
  ...
  private static byte[] novosecrapopir =
      ...
      ("MIICwjCCAQ8CAQKkVzBVMSYwJAYDVQQDDB1hYmMxMjNycnktNDM3MTkzOTU0MzkxMz"
              ...
              + "DQSBTYW1wbGUxCzAJBgNVBAYTAlNFoBEYDzIwMTEwNTMwMDkwNTQyWqEwMC"
              + "4GCSqGSIb2fQdCDTAhBAZmb28xMjMwBwYFKw4DAhoCAgI3MAoGCCqGSIb3D"
              + "QIHogkEB215a2V5aWSkEgQQJV0C01odEXcv7BG4uW6bO6USBBD2+lerPhUq"
              + "wF4Az6Vemh2yoIIBkjCCAY4wggGKMIIBawIBBDCCAWSjOTA3MREwDwYDVQQ"
              + "DDAhBZG1pbkNBMTEVMBMGA1UECgwMRUpCQ0EgU2FtcGxlMQswCQYDVQQGEw"
              + "JTRaQkoBEYDzIwMDMwMjExMDAyMTIwWqEPFw0xMTA1MzAwOTA1NDJapVcwV"
              + "TEmMCQGA1UEAwwdYWJjMTIzcnJ5LTQzNzE5Mzk1NDM5MTM2Mzk4ODExHjAc"
              + "BgNVBAoMFVByaW1lS2V5IFNvbHV0aW9ucyBBQjELMAkGA1UEBhMCU0WmXDA"
              + "NBgkqhkiG9w0BAQEFAANLADBIAkEAkM6qeA9Vkaz+NHyejepEgBrwvbCKIt"
              + "a3CpG+mp+0zayL7NA90AknRsW7A3sX0i3IEDY3wvUhJ1+yc53M89OaKQIDA"
              + "QABqUowOwYDVR0RBDQwMoEQZm9vZW1haWxAYmFyLmNvbaAeBgorBgEEAYI3"
              + "FAIDoBAMDmZvb3VwbkBiYXIuY29tMAsGA1UdDwQEAwIF4KACBQAwFTATBgk"
              + "rBgEFBQcFAQEMBmZvbzEyM6AXAxUAnJ0lrTWxB+sKIdj1oCSYfJ1/Fpk=")
          .getBytes());

  /**
   * CMP initial request message, created with EJBCA 4.0.2 code, with signature
   * POP (no POPOSigningKey) and signature protection.
   */
  private static byte[] novosecsigpopir =
      Base64.decode(
         ...
         + "DVQQGEwJTRaARGA8yMDExMDUzMDEyNTQyMVqhDTALBgkqhkiG9w0BAQWkEgQQFoG/D"
         + "zbmQiksb5Yrrih6xqUSBBCcgBl5fXla0Ilj/Qib+iVPoIIB4TCCAd0wggHZMIIBagIB"
         + "BDCCAWOjOTA3MREwDwYDVQQDDAhBZG1pbkNBMTEVMBMGA1UECgwMRUpCQ0EgU2FtcG"
         + "xlMQswCQYDVQQGEwJTRaQkoBEYDzIwMDMwMjExMDAyMTIwWqEPFw0xMTA1MzAxMjU0"
         + "MjFapVYwVDElMCMGA1UEAwwcYWJjMTIzcnJ5Mjk0MjgxMjgwMTk4MDY2ODg1MzEeMB"
         + "wGA1UECgwVUHJpbWVLZXkgU29sdXRpb25zIEFCMQswCQYDVQQGEwJTRaZcMA0GCSqG"
         + "SIb3DQEBAQUAA0sAMEgCQQDFiXtcgIBhL9FFnNASa0ezOX513vxlkWLTgkVqS0TEv/"
         + "wmTxNZ4wgTM9zRYGDK/MVKiOzPREoHPwib0F8X4OwdAgMBAAGpSjA7BgNVHREENDAy"
         + "gRBmb29lbWFpbEBiYXIuY29toB4GCisGAQQBgjcUAgOgEAwOZm9vdXBuQGJhci5jb2"
         + "0wCwYDVR0PBAQDAgXgoVIwUDALBgkqhkiG9w0BAQUDQQANpvmT5H0S9nh7O2WD5+Mg"
         + "Hvu0FT7umak741nuPo3fAgKJJbzMF6rG5/lwE7tNAnAZKVzuhAF9pSk5KjxDIl0UMB"
         + "UwEwYJKwYBBQUHBQEBDAZmb28xMjOgQwNBAA+6Is7z6dNIMMQjEpwk2CEx44rE1Khe"
         + "hrrSS4wqPgkcE6MUt7+IBq/zOD3IE4DT/YoyDIx9bIugIJj8pJiJAbahggE/MIIBOz"
         + "CCATcwgeKgAwIBAgIIN5Wx1B7SXAEwDQYJKoZIhvcNAQEFBQAwGDEWMBQGA1UEAwwN"
         + "Q01QIFNpZ24gVGVzdDAeFw0xMTA1MzAxMjQ0MjFaFw0yMTA1MjcxMjU0MjFaMBgxFj"
         + "AUBgNVBAMMDUNNUCBTaWduIFRlc3QwXDANBgkqhkiG9w0BAQEFAANLADBIAkEAxYl7"
         + "XICAYS/RRZzQEmtHszl+dd78ZZFi04JFaktExL/8Jk8TWeMIEzPc0WBgyvzFSojsz0"
         + "RKBz8Im9BfF+DsHQIDAQABoxAwDjAMBgNVHRMBAf8EAjAAMA0GCSqGSIb3DQEBBQUA"
         + "A0EAvdqiow8CoVzeupH6AtR6IbfM87KPV8TyxiPk5Qt3Mwst8g2nLP7CqMYjlufPFH"
         + "+FvGoZN00zEJZcfeqwhtClCQ==")
              .getBytes());
}
