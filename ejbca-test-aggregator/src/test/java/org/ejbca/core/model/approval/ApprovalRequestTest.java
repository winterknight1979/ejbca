...
/**
 * @version $Id: ApprovalRequestTest.java 27117 2017-11-10 13:04:39Z mikekushner
 *     $
 */
public class ApprovalRequestTest {
  ...
  public void testWriteExternal() throws Exception {
    X509Certificate testcert =
        CertTools.getCertfromByteArray(testcertenc, X509Certificate.class);
    AuthenticationToken token =
        new X509CertificateAuthenticationToken(testcert);

    AccumulativeApprovalProfile approvalProfile =
        new AccumulativeApprovalProfile("AccumulativeApprovalProfile");
    approvalProfile.initialize();
    approvalProfile.setNumberOfApprovalsRequired(2);
    DummyApprovalRequest ar =
        new DummyApprovalRequest(token, null, 1, 2, false, approvalProfile);
    ...
    assertTrue(
        readrequest.getApprovalType() == ApprovalDataVO.APPROVALTYPE_DUMMY);
    assertTrue(
        readrequest.getApprovalRequestType()
            == ApprovalRequest.REQUESTTYPE_SIMPLE);
    assertTrue(readrequest.getRequestSignature() == null);
    assertTrue(
        CertTools.getSerialNumber(readrequest.getRequestAdminCert())
            .equals(CertTools.getSerialNumber(testcert)));
    ...
    assertTrue(
        readrequest
            .getApprovalProfile()
            .getProfileName()
            .equals(approvalProfile.getProfileName()));
    assertTrue(!readrequest.isExecutable());
  }

  private ApprovalRequest getApprovalRequest(final String data)
  ...
  public void testEditedByMe() throws Exception {
    ...
    X509Certificate testcertDup =
        CertTools.getCertfromByteArray(testcertenc, X509Certificate.class);
    AuthenticationToken tokenDup =
        new X509CertificateAuthenticationToken(testcertDup);

    X509Certificate testcert2 =
        CertTools.getCertfromByteArray(testcertenc2, X509Certificate.class);
    AuthenticationToken token2 =
        new X509CertificateAuthenticationToken(testcert2);

    AuthenticationToken token3 =
        new PublicAccessAuthenticationToken("127.0.0.1", true);
    ...
    assertFalse(
        "Fresh approval request should not say it has been edited.",
        ar.isEditedByMe(token));
    ...
    assertFalse(
        ar.isEditedByMe(
            token)); // no longer the last admin who edited it, so this admin
                     // may edit again
    assertFalse(ar.isEditedByMe(tokenDup));
