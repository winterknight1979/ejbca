 ...
 *
 * @version $Id: AccessRulesHelperTest.java 25407 2017-03-07 15:08:07Z jeklund $
 */
public class AccessRulesHelperTest {
  ...
  private void testUnion(
    ...
    assertEquals(
        ERRMSG_UNEXPECTED_STATE, Role.STATE_ALLOW, accessRules.get("/a/"));
    ...
    assertEquals(
        ERRMSG_UNEXPECTED_STATE, Role.STATE_ALLOW, accessRules.get("/b/a/"));
    assertEquals(
        ERRMSG_UNEXPECTED_STATE, Role.STATE_ALLOW, accessRules.get("/c/"));
    assertEquals(
        ERRMSG_UNEXPECTED_STATE, Role.STATE_DENY, accessRules.get("/c/d/"));
    // Verify that no other rules are present
    final HashMap<String, Boolean> accessRulesToClean =
        new HashMap<>(accessRules);
    ...
    assertTrue(
        ...
        accessRulesToClean.isEmpty());
    // Verify that the merged rules gives the expected access
    assertFalse(
        ERRMSG_ALLOWED_TO_DENIED,
        AccessRulesHelper.hasAccessToResource(accessRules, "/"));
    assertTrue(
        ERRMSG_DENIED_TO_ALLOWED,
        AccessRulesHelper.hasAccessToResource(accessRules, "/a/"));
    assertTrue(
        ERRMSG_DENIED_TO_ALLOWED,
        AccessRulesHelper.hasAccessToResource(accessRules, "/a/b/"));
    assertTrue(
        ERRMSG_DENIED_TO_ALLOWED,
        AccessRulesHelper.hasAccessToResource(accessRules, "/a/c/"));
    assertFalse(
        ERRMSG_ALLOWED_TO_DENIED,
        AccessRulesHelper.hasAccessToResource(accessRules, "/b/"));
    assertTrue(
        ERRMSG_DENIED_TO_ALLOWED,
        AccessRulesHelper.hasAccessToResource(accessRules, "/b/a/"));
    assertTrue(
        ERRMSG_DENIED_TO_ALLOWED,
        AccessRulesHelper.hasAccessToResource(accessRules, "/c/"));
    assertFalse(
        ERRMSG_ALLOWED_TO_DENIED,
  ...
  private void testIntersection(
    ...
    assertEquals(
        ERRMSG_UNEXPECTED_STATE, Role.STATE_ALLOW, accessRules.get("/a/"));
    assertEquals(
        ERRMSG_UNEXPECTED_STATE, Role.STATE_DENY, accessRules.get("/a/b/"));
    assertEquals(
        ERRMSG_UNEXPECTED_STATE, Role.STATE_DENY, accessRules.get("/a/c/"));
    assertEquals(ERRMSG_UNEXPECTED_STATE, null, accessRules.get("/b/"));
    assertEquals(ERRMSG_UNEXPECTED_STATE, null, accessRules.get("/b/a/"));
    assertEquals(
        ERRMSG_UNEXPECTED_STATE, Role.STATE_ALLOW, accessRules.get("/b/b/"));
    ...
    assertTrue(
        ERRMSG_DENIED_TO_ALLOWED,
        AccessRulesHelper.hasAccessToResource(accessRules, "/b/b/"));
    assertFalse(
        ERRMSG_ALLOWED_TO_DENIED,
        AccessRulesHelper.hasAccessToResource(accessRules, "/c/"));
    assertFalse(
  ...
  public void testSorting() {
    ...
    final Iterator<Entry<String, Boolean>> iterator =
        accessRules.entrySet().iterator();
