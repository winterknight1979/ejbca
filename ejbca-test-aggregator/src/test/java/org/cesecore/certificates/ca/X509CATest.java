...
public class X509CATest {
  ...
  public void testX509CABasicOperationsRSA() throws Exception {
    ...
    doTestX509CABasicOperations(
        AlgorithmConstants.SIGALG_SHA256_WITH_RSA_AND_MGF1);
    // AlgorithmConstants.SIGALG_SHA256_WITH_RSA_AND_MGF1 uses small w in With.
    // Test with capital as well
    // because this was used previously so need to be supported for upgraded
    // systems.
  ...
  private void doTestX509CABasicOperations(final String algName)
      throws Exception {
    ...
    GeneralNames san =
        CertTools.getGeneralNamesFromAltName("dNSName=foobar.bar.com");
    ...
    PKCS10RequestMessage p10msg =
        new PKCS10RequestMessage(new JcaPKCS10CertificationRequest(p10));
    ...
    assertEquals(
        getTestKeyPairAlgName(algName).toUpperCase(),
        AlgorithmTools.getCertSignatureAlgorithmNameAsString(usercert)
            .toUpperCase());
    assertEquals(
        new String(CertTools.getSubjectKeyId(cacert)),
        new String(CertTools.getAuthorityKeyId(usercert)));
    assertEquals("user@user.com", CertTools.getEMailAddress(usercert));
    // directoryName is turned around, but it's just for string reasons in cert
    // objects because it is gotten (internally in BC) getRFC2253Name().
    assertEquals(
        "rfc822name=user@user.com, dNSName=foo.bar.com,"
            + " directoryName=c=SE\\,o=PrimeKey\\,cn=Tomas",
        CertTools.getSubjectAlternativeName(usercert));
    ...
    revcerts.add(
        new RevokedCertInfo(
            CertTools.getFingerprintAsString(usercert).getBytes(),
            CertTools.getSerialNumber(usercert).toByteArray(),
            revDate.getTime(),
            RevokedCertInfo.REVOCATION_REASON_CERTIFICATEHOLD,
            CertTools.getNotAfter(usercert).getTime()));
    ...
    assertEquals(
        CertTools.getSerialNumber(usercert).toString(),
        entry.getSerialNumber().toString());
  ...
  public void testCRLDistPointOnCRL() throws Exception {
    ...
    byte[] cdpDER =
        xcrl.getExtensionValue(Extension.issuingDistributionPoint.getId());
    ...
    IssuingDistributionPoint cdp =
        IssuingDistributionPoint.getInstance(aIn.readObject());
    ...
    assertEquals(
        ...
        cdpURL,
        ((DERIA5String)
                ((GeneralNames) distpoint.getName()).getNames()[0].getName())
            .getString());
    ...
    assertNull(
        ...
        xcrl.getExtensionValue(Extension.cRLDistributionPoints.getId()));
  }
  ...
  public void testCRLFreshestCRL() throws Exception {
    ...
    ASN1InputStream aIn =
        new ASN1InputStream(new ByteArrayInputStream(cFreshestDpDER));
    ASN1OctetString octs = (ASN1OctetString) aIn.readObject();
    aIn = new ASN1InputStream(new ByteArrayInputStream(octs.getOctets()));
    ...
    assertEquals(
        "More CRL Freshest distributions points than expected",
        1,
        distpoints.length);
    assertEquals(
    ...
    xcrl = CertTools.getCRLfromByteArray(crl.getEncoded());
    assertNull(
        "CRL has freshest crl extension",
        xcrl.getExtensionValue(Extension.freshestCRL.getId()));
  }
  ...
  private void doTestStoreAndLoad(final String algName) throws Exception {
    final CryptoToken cryptoToken = getNewCryptoToken();
    ...
    CAData cadata =
        new CAData(
            cainfo.getSubjectDN(), cainfo.getName(), cainfo.getStatus(), ca);

    CA ca2 = cadata.getCA();
    Certificate usercert2 =
        ca.generateCertificate(
            cryptoToken,
            user,
            keypair.getPublic(),
    ...
    assertEquals(
        ...
        caToken1.getSignatureAlgorithm());
    assertEquals(
        AlgorithmConstants.SIGALG_SHA256_WITH_RSA,
    ...
    assertEquals(
        AlgorithmConstants.SIGALG_SHA256_WITH_RSA,
        caToken2.getSignatureAlgorithm());
    assertEquals(
        AlgorithmConstants.SIGALG_SHA256_WITH_RSA,
  ...
  public void testExtendedCAServices() throws Exception {
    ...
    assertNull(ca.getExtendedCAServiceInfo(1));
    assertNotNull(
        "org.cesecore.certificates.ca.MyExtendedCAServiceInfo",
   ...
   */
  private GeneralNames swapGeneralNames(
      final GeneralNames gns, final int index1, final int index2) {
  ...
  public void testCTRedactedLabels() throws Exception {
    ...
    gns =
        swapGeneralNames(
            gns, 0,
            5); // Swap iPAddress and rfc822Name to test that the order is
                // preserved
    Extension ext =
        new Extension(
            Extension.subjectAlternativeName,
            false,
            gns.toASN1Primitive().getEncoded(ASN1Encoding.DER));
    ...
    assertEquals(
        "altName is not what it should be",
        "iPAddress=192.0.2.123, dNSName=foo.bar.com, dNSName=hidden.secret.se,"
            + " dNSName=hidden1.hidden2.ultrasecret.no,"
            + " directoryName=CN=Tomas\\,O=PrimeKey\\,C=SE,"
    ...
    ASN1Sequence seq =
        ASN1Sequence.getInstance(ctext.getExtnValue().getOctets());
    assertEquals("should be three dnsNames", 3, seq.size());
    ASN1Integer derInt = ASN1Integer.getInstance(seq.getObjectAt(0));
    assertEquals(
        "first dnsName should have 0 redacted labels",
        0,
        derInt.getValue().intValue());
    derInt = ASN1Integer.getInstance(seq.getObjectAt(1));
    assertEquals(
        "second dnsName should have 1 redacted labels",
        1,
        derInt.getValue().intValue());
    derInt = ASN1Integer.getInstance(seq.getObjectAt(2));
    assertEquals(
        "third dnsName should have 2 redacted labels",
        2,
        derInt.getValue().intValue());
  ...
  public void testCTRedactedLabelsInPreCert() throws Exception {
    ...
    assertEquals(
        "altName is not what it should be",
        "iPAddress=192.0.2.123, dNSName=foo.bar.com,"
            + " dNSName=(PRIVATE).secret.se, dNSName=(PRIVATE).ultrasecret.no,"
            + " directoryName=CN=Tomas\\,O=PrimeKey\\,C=SE,"
            + " rfc822name=foo@bar.com",
        altName);
  }
  ...
  public void testInvalidSignatureAlg() throws Exception {
    ...
    try {
      ...
      fail(
          "This should throw because md5withRSA is not an allowed signature"
              + " algorithm. It is vulnerable.");
  ...
  public void doTestWrongCAKey(final String algName) throws Exception {
    ...
    // certificate with this CA, when the
    // issued cert can not be verified by the CA certificate
    cryptoToken.generateKeyPair(
        getTestKeySpec(algName), CAToken.SOFTPRIVATESIGNKEYALIAS);
    ...
    X509Certificate cacert =
        CertTools.genSelfCert(
            CADN, 10L, "1.1.1.1", privateKey, publicKey, "SHA256WithRSA", true);
  ...
  public void testRfc822NameWithPlus() throws Exception {
    ...
    try {
      ...
      fail(
          "Certificate could not be created or AIA could not be parsed: "
              + e.getMessage());
    }
    ...
    try {
      ...
      // handled as "rfc822Name=user" and "plus@user.com". Since the second part
      // does not map to any known fields, the resulting SubjectAltName is
      // "rfc822Name=user"
      assertFalse(
          StringUtils.equals(
              "rfc822name=" + emailUnescaped,
      ...
      assertFalse(
          StringUtils.equals(
              ...
              CertTools.getSubjectAlternativeName(certificate)));
      assertEquals(
          "rfc822name=user", CertTools.getSubjectAlternativeName(certificate));
    } catch (CAOfflineException e) {
   ...
   */
  @Test
  public void testAuthorityInformationAccessCertificateExtension()
      throws Exception {
    ...
    // 1. test with all values filled and both 'Use CA defined' switches are
    // true
    ...
    // 2. test with all values filled and both 'Use CA defined' switches are
    // false
    ...
    // 3a. test with all values filled and 'Use CA defined' CA issuer switch
    ...
    // 3b. test with all values filled and 'Use CA defined' OCSP service switch
    // true, the other false,
    profile.setUseDefaultCAIssuer(false);
    profile.setUseDefaultOCSPServiceLocator(true);
    try {
  ...
  private void assertCertificateAuthorityInformationAccess(
    ...
    List<String> testList =
        CertTools.getAuthorityInformationAccessCAIssuerUris(certificate);
    assertTrue(
        "Certificate CA issuer URIs "
            + Arrays.toString(caIssuerUris.toArray())
            + " expected but was "
            + Arrays.toString(testList.toArray()),
        caIssuerUris.equals(testList));
    testList = CertTools.getAuthorityInformationAccessOcspUrls(certificate);
    assertTrue(
        "Certificate OCSP service locators "
            + Arrays.toString(ocspUrls.toArray())
            + " expected but was "
            + Arrays.toString(testList.toArray()),
        ocspUrls.equals(testList));
  }
  ...
  public void testAuthorityInformationAccessCrlExtension() throws Exception {
    ...
    assertEquals(
        "Number of URLs do not match",
        authorityInformationAccess.size(),
        result.size());
   ...
   */
  @Test
  public void testAuthorityInformationAccessCrlExtensionWithEmptyList()
      throws Exception {
    final CryptoToken cryptoToken = getNewCryptoToken();
    X509CA testCa = createTestCA(cryptoToken, "CN=foo");
    Collection<RevokedCertInfo> revcerts = new ArrayList<RevokedCertInfo>();
    ...
    Collection<String> result = CertTools.getAuthorityInformationAccess(xcrl);
    assertEquals(
        "A list was returned without any values present.", 0, result.size());
  }

  /**
   * Test that the CA refuses to issue certificates outside of the
   * PrivateKeyUsagePeriod, but that it does issue a cert within this period.
   * This test has some timing, so it sleeps in total 11 seconds during the
   * test.
   *
   * @throws Exception Fail
   */
  ...
  public void testCAPrivateKeyUsagePeriodRequest() throws Exception {
    ...
    notAfter.add(
        Calendar.SECOND,
        10); // 10 seconds in the future gives us a 5 second window to generate
             // a cert
    X509CA testCa =
        createTestCA(
            cryptoToken,
    ...
    try {
      ...
      fail(
          "Should not throw CAOfflineException when issuing a certificate"
              + " within PrivateKeyUsagePeriod.");
    }
    // Issue a certificate after private key usage period expires
    // Sleep 5 seconds, now it should not work again since PrivateKeyUsagePeriod
    // has expired
    Thread.sleep(5000);
    try {
      testCa.generateCertificate(
  ...
  public void testCustomCertificateExtension() throws Exception {
    ...
    props2.put(
        "value", "301a300c060a2b060104018237140202300a06082b06010505070302");
    cceConfig.addCustomCertExtension(
        2,
        "1.2.3.4",
        "RawProper",
    ...
    assertEquals(
        cceConfig.getCustomCertificateExtension(1).getOID(),
        "2.16.840.1.113730.1.13");
    assertEquals(
        cceConfig.getCustomCertificateExtension(2).getOID(), "1.2.3.4");
    assertEquals(
        cceConfig.getCustomCertificateExtension(3).getOID(), "1.2.3.5");
  ...
  /**
   * Tests default value of "use printable string" option (should be disabled by
   * default) and tests that the option works.
   *
   * @throws Exception Fail
   */
  @Test
  public void testPrintableString() throws Exception {
    ...
    assertTrue(
        ...
        getValueFromDN(cert, X509ObjectIdentifiers.commonName)
            instanceof DERUTF8String);
    assertTrue(
        "Certificate C was not PrintableString encoded.",
        getValueFromDN(cert, X509ObjectIdentifiers.countryName)
    ...
    assertTrue(
        ...
        getValueFromDN(cert, X509ObjectIdentifiers.commonName)
            instanceof DERUTF8String);
    assertTrue(
        "Certificate O was not UTF-8 encoded by default.",
        getValueFromDN(cert, X509ObjectIdentifiers.organization)
            instanceof DERUTF8String);
    assertTrue(
        "Certificate JurisdictionState was not UTF-8 encoded.",
        getValueFromDN(cert, CeSecoreNameStyle.JURISDICTION_STATE)
            instanceof DERUTF8String);
    assertTrue(
        "Certificate JurisdictionLocality was not UTF-8 encoded.",
        getValueFromDN(cert, CeSecoreNameStyle.JURISDICTION_LOCALITY)
            instanceof DERUTF8String);
    assertTrue(
        "Certificate C was not PrintableString encoded.",
        getValueFromDN(cert, X509ObjectIdentifiers.countryName)
            instanceof DERPrintableString); // C is always PrintableString
    assertTrue(
        "Certificate JurisdictionCountry was not PrintableString encoded.",
        getValueFromDN(cert, CeSecoreNameStyle.JURISDICTION_COUNTRY)
            instanceof DERPrintableString); // C is always PrintableString
    ...
    assertTrue(
        ...
        getValueFromDN(cert, X509ObjectIdentifiers.commonName)
            instanceof DERPrintableString);
    assertTrue(
        "Certificate O was not encoded as PrintableString.",
        getValueFromDN(cert, X509ObjectIdentifiers.organization)
            instanceof DERPrintableString);
    assertTrue(
        "Certificate JurisdictionState was not encoded as PrintableString.",
        getValueFromDN(cert, CeSecoreNameStyle.JURISDICTION_STATE)
            instanceof DERPrintableString);
    assertTrue(
        "Certificate JurisdictionLocality was not encoded as PrintableString.",
        getValueFromDN(cert, CeSecoreNameStyle.JURISDICTION_LOCALITY)
            instanceof DERPrintableString);
    assertTrue(
        "Certificate C was not PrintableString encoded.",
...
+ " B.V.,L=Åmål,ST=Norrland,C=SE";
    ...
    // businesscategory) comes before C and in other aspects are x500 order
  ...
  public void testDNOverride() throws Exception {
    ...
    PKCS10RequestMessage p10msg =
        new PKCS10RequestMessage(new JcaPKCS10CertificationRequest(req));
  ...
  /**
   * Testing that the certificate's serial number is generated with the correct
   * length, according to "CA serial number octet size" that is configured to
   * CA. "CA serial number octet size" is left to be default 20 bytes here
   *
   * @throws Exception Fail
   */
  @Test
  public void testCaSerialNumberWithDefaultOctetSize20() throws Exception {
    ...
    BigInteger lowestBound =
        new BigInteger("0080000000000000000000000000000000000000", 16);
    BigInteger highestBound =
        new BigInteger("7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF", 16);
    assertTrue(
        serialNumber.compareTo(lowestBound) >= 0
            && serialNumber.compareTo(highestBound) <= 0);
  }

  /**
   * Testing that the certificate's serial number is generated with the correct
   * length, according to "CA serial number octet size" that is configured to
   * CA. "CA serial number octet size" is configured to be 4 bytes here
   *
   * @throws Exception Fail
   */
  @Test
  ...
  /**
   * Testing generating certificate with public key from providedRequestMessage
   * (providedPublicKey and
   * endEntityInformation.extendedInformation.certificateRequest must be null).
   *
   * @throws Exception Fail
   */
  @Test
  public void testGeneratingCertificateWithPublicKeyFromProvidedRequestMessage()
      throws Exception {
    final String algName = AlgorithmConstants.SIGALG_SHA256_WITH_RSA;
    final CryptoToken cryptoToken = getNewCryptoToken();
    final X509CA x509ca = createTestCA(cryptoToken, CADN, algName, null, null);
    ...
    PKCS10RequestMessage requestMessage =
        new PKCS10RequestMessage(
            new JcaPKCS10CertificationRequest(certificationRequest));
    assertEquals(
        "CN=RequestMessageCn,O=PrimeKey,C=SE", requestMessage.getRequestDN());
    EndEntityInformation endEntityInformation =
        new EndEntityInformation(
            "username",
            "CN=EndEntityInformationCn,O=PrimeKey,C=SE",
            666,
            null,
            "user@user.com",
            new EndEntityType(EndEntityTypes.ENDUSER),
    ...
    assertEquals(
        "2048", AlgorithmTools.getKeySpecification(usercert.getPublicKey()));
    assertEquals(
        AlgorithmConstants.KEYALGORITHM_RSA,
        AlgorithmTools.getKeyAlgorithm(usercert.getPublicKey()));
  }

  /**
   * Testing that CSR algorithm is enforced from end entity information if there
   * is one.
   *
   * @throws Exception Fail
   */
  @Test
  public void
      testProvidedPublicKeyAlgorithmEnforcedOverOneFromProvidedRequestMessage()
          throws Exception {
    final String algName = AlgorithmConstants.SIGALG_SHA256_WITH_RSA;
    final CryptoToken cryptoToken = getNewCryptoToken();
    final X509CA x509ca = createTestCA(cryptoToken, CADN, algName, null, null);
  ...
  public void testEndEntityInformationCsrAlgorithmEnforced() throws Exception {
    ...
    PKCS10CertificationRequest certificationRequestEnforcedAlg =
        CertTools.genPKCS10CertificationRequest(
            ...
            keyPairEnforcedAlg.getPublic(),
            null,
            keyPairEnforcedAlg.getPrivate(),
            BouncyCastleProvider.PROVIDER_NAME);
    PKCS10RequestMessage requestMessageEnforcedAlg =
        new PKCS10RequestMessage(
            new JcaPKCS10CertificationRequest(certificationRequestEnforcedAlg));
    assertEquals(
        "CN=EnforcedAlgCn,O=PrimeKey,C=SE",
        requestMessageEnforcedAlg.getRequestDN());
    EndEntityInformation endEntityInformation =
        new EndEntityInformation(
            "username",
            "CN=EndEntityInformationCn,O=PrimeKey,C=SE",
    ...
    endEntityInformation
        ...
        .setCertificateRequest(certificationRequestEnforcedAlg.getEncoded());
  ...
  private static ASN1Encodable getValueFromDN(
      final Certificate cert, final ASN1ObjectIdentifier oid) {
  ...
  private static X509CA createTestCA(
      final CryptoToken cryptoToken, final String cadn) throws Exception {
    return createTestCA(
        cryptoToken,
        cadn,
        AlgorithmConstants.SIGALG_SHA256_WITH_RSA,
  ...
  private static X509CA createTestCA(
      ...
      throws Exception {
    cryptoToken.generateKeyPair(
        getTestKeySpec(sigAlg), CAToken.SOFTPRIVATESIGNKEYALIAS);
    cryptoToken.generateKeyPair(
        getTestKeySpec(sigAlg), CAToken.SOFTPRIVATEDECKEYALIAS);
    // Create CAToken
  ...
  private CryptoToken getNewCryptoToken() {
    ...
    cryptoTokenProperties.setProperty(
        CryptoToken.AUTOACTIVATE_PIN_PROPERTY, "foo1234");
  ...
  private static KeyPair genTestKeyPair(final String algName)
   ...
   */
  private static String getTestKeyPairAlgName(final String algName) {
    if (algName.equals(AlgorithmConstants.SIGALG_GOST3411_WITH_ECGOST3410)
