...
/**
 * Unit tests for the X509CertificateAuthenticationToken class. Note that this
 * test class subverts the unit test concept slightly by using 'real'
 * certificates.
 *
 * @version $Id: X509CertificateAuthenticationTokenTest.java 27307 2017-11-23
 *     16:21:38Z anatom $
 */
 ...
 * @throws IOException fail
  ...
  public void testCreateAuthenticationToken() {
    X509CertificateAuthenticationToken authenticationToken =
        new X509CertificateAuthenticationToken(certificate);
    assertTrue(authenticationToken != null);
  }

  /**
   * This test attempts to subvert X509CertificateAuthenticationToken's
   * constructor by not supplying any credentials.
   */
  @Test
  public void testCreateAuthenticationTokenWithoutCredential() {
    ...
    try {
      authenticationToken =
          new X509CertificateAuthenticationToken(
              new HashSet<X500Principal>(), new HashSet<X509Certificate>());
      fail(
          "X509CertificateAuthenticationToken was created without a"
   ...
   */
  @Test
  public void testCreateAuthenticationTokenWithMultipleCredentials()
    ...
    fail(
        "X509CertificateAuthenticationToken was created multiple certificates."
  ...
  public void testMatchCaIdFail() {
    ...
    AccessUserAspect accessUser =
        EasyMock.createMock(AccessUserAspectData.class);
    EasyMock.expect(accessUser.getCaId()).andReturn(-1).anyTimes();
    EasyMock.expect(accessUser.getTokenType())
        .andReturn(X509CertificateAuthenticationTokenMetaData.TOKEN_TYPE);
    EasyMock.replay(accessUser);
    assertFalse(
        "AccessUser matched in spit of incorrect CaIDs",
  ...
  public void testMatchWithFullDN() {
    ...
    X509CertificateAuthenticationToken authenticationToken =
        getAuthenticationToken();
    int caid =
        (CertTools.stringToBCDNString(certificate.getIssuerDN().toString()))
            .hashCode();

    accessUser = EasyMock.createMock(AccessUserAspectData.class);

    EasyMock.expect(accessUser.getCaId()).andReturn(caid);
    EasyMock.expect(accessUser.getMatchValue())
        .andReturn(CertTools.getSubjectDN(certificate));
    EasyMock.expect(accessUser.getMatchWith())
        .andReturn(X500PrincipalAccessMatchValue.WITH_FULLDN.getNumericValue());
    EasyMock.expect(accessUser.getMatchValue())
        .andReturn(CertTools.getSubjectDN(certificate));
    EasyMock.expect(accessUser.getMatchTypeAsType())
        .andReturn(AccessMatchType.TYPE_EQUALCASEINS);
    EasyMock.expect(accessUser.getTokenType())
        .andReturn(X509CertificateAuthenticationTokenMetaData.TOKEN_TYPE);
    EasyMock.replay(accessUser);
  ...
  public void testMatchWithAllValues() {
    ...
    int caid =
        (CertTools.stringToBCDNString(certificate.getIssuerDN().toString()))
            .hashCode();
    AccessUserAspect accessUser;
    X500PrincipalAccessMatchValue[] allValues =
    ...
    for (X500PrincipalAccessMatchValue matchValue : allValues) {
      switch (matchValue) {
        case WITH_SERIALNUMBER:
          ...
          EasyMock.expect(accessUser.getMatchValue())
              .andReturn(certificate.getSerialNumber().toString(16))
              .times(2);
          EasyMock.expect(accessUser.getMatchWith())
              .andReturn(
          ...
          EasyMock.expect(accessUser.getMatchValue())
              ...
              .times(2);
          EasyMock.expect(accessUser.getMatchTypeAsType())
              .andReturn(AccessMatchType.TYPE_EQUALCASE);
          EasyMock.expect(accessUser.getMatchTypeAsType())
              .andReturn(AccessMatchType.TYPE_NOT_EQUALCASEINS);
          EasyMock.expect(accessUser.getTokenType())
              .andReturn(X509CertificateAuthenticationTokenMetaData.TOKEN_TYPE)
              .times(2);
          EasyMock.replay(accessUser);
        ...
        case WITH_UPN:
          break;
        default:
          AccessMatchType match =
              AccessMatchType.values()[(matchValue.ordinal() % 4) + 1];

          accessUser = EasyMock.createMock(AccessUserAspectData.class);
          EasyMock.expect(accessUser.getCaId()).andReturn(caid);
          EasyMock.expect(accessUser.getMatchValue()).andReturn("Test");
          EasyMock.expect(accessUser.getMatchWith())
              .andReturn(matchValue.getNumericValue());
          EasyMock.expect(accessUser.getMatchTypeAsType()).andReturn(match);
          EasyMock.expect(accessUser.getMatchValue()).andReturn("Test");
          EasyMock.expect(accessUser.getTokenType())
              .andReturn(X509CertificateAuthenticationTokenMetaData.TOKEN_TYPE);
          EasyMock.replay(accessUser);
          if (match == AccessMatchType.TYPE_EQUALCASE
              || match == AccessMatchType.TYPE_EQUALCASEINS) {
            assertTrue(
                "Could not match for value " + matchValue,
                authenticationToken.matches(accessUser));
          } else {
            assertFalse(
                "False match for value " + matchValue,
                authenticationToken.matches(accessUser));
          }
          EasyMock.verify(accessUser);
          break;
  ...
  /**
   * Verify that the token does not match after serialization and
   * deserialization.
   *
   * @throws IOException fail
   ...
   */
  @Test
  public void testAuthFailAfterSerialization()
      throws IOException, ClassNotFoundException {
    final X509CertificateAuthenticationToken authenticationToken =
        getAuthenticationToken();
    int caid =
        (CertTools.stringToBCDNString(certificate.getIssuerDN().toString()))
            .hashCode();
    final AccessUserAspect accessUser =
    ...
    assertTrue(
        "Regular matching was not successful.",
        authenticationToken.matches(accessUser));
    // Simulate remote EJB call using serialization. This should destroy the
    // "transient" shared secret.
    final ByteArrayOutputStream buf = new ByteArrayOutputStream();
    new ObjectOutputStream(buf).writeObject(authenticationToken);
    final X509CertificateAuthenticationToken authenticationToken2 =
        (X509CertificateAuthenticationToken)
            new ObjectInputStream(new ByteArrayInputStream(buf.toByteArray()))
                .readObject();
    // Verify that the shared secret was destroyed by trying to match the object
    // again.
    assertFalse(
        "Shared secret in JVM was not destroyed in local object during"
            + " serialization.",
        authenticationToken2.matches(accessUser));
  }
  ...
  public void testBadSerialNumber() {
    ...
    assertFalse(
        "matching was succesful, should not have been.",
        authenticationToken.matches(accessUser));
  }
