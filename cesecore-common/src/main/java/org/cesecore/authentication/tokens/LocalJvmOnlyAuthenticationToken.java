...
/**
 * Common base class for tokens that are only valid
 * in the JVM they are created and could
 * otherwise be spoofed. E.g. X509 client certificate
 * validation AuthenticationToken could
 * otherwise be created and sent to a remote EJB interface.
 *
 * @version $Id: LocalJvmOnlyAuthenticationToken.java
...
public abstract class LocalJvmOnlyAuthenticationToken
    ...
    /** A random token that is unique to this JVM (e.g. the application server
     * JVM and a CLI JVM does not have the same token). */
    ...
    protected final boolean isCreatedInThisJvm() {
        boolean isCreatedInThisJvm
            = ArrayUtils.isEquals(authToken, RANDOM_TOKEN);
