...
/**
 * The sole purpose of this class is to standardize mapping in JNDI of our
 * Stateless Session Beans.
 *
 * <p>Use like this:
 * {@literal @}Stateless(mappedName=(JndiConstants.APP_JNDI_PREFIX +
 * RemoteInterfaceClass.class.getSimpleName()))
 *
...
public abstract class JndiHelper {
   ...
   *
   * <p>Example usage: CAAdminSessionRemote caadminsession =
   * JndiHelper.getRemoteSession(CAAdminSessionRemote.class);
   *
   * @param <T> type
   * @param module the module where the bean is deployed, i.e. ejbca-ejb or
   *     systemtests-ejb.
   * @param remoteInterface interface
   * @return Session
   */
  @SuppressWarnings("unchecked")
  public static <T> T getRemoteSession(
      final Class<T> remoteInterface, final String module) {
    // JEE5, JBoss 5 and 6 and Glassfish 2
    final String jndiNameJEE5 =
        JndiConstants.APP_JNDI_PREFIX + remoteInterface.getSimpleName();
    // JEE6, JBoss 7
    final String viewClassName = remoteInterface.getName();
    // Get the remote interface class, GlobalConfigurationSessionRemote, and
    // return GlobalConfigurationSessionBean
    // This works when we follow our own naming standard
    final String beanName =
        remoteInterface.getSimpleName().replace("Remote", "Bean");
    final String jndiNameJEE6 =
        "ejb:ejbca" + "/" + module + "//" + beanName + "!" + viewClassName;
    ...
    try {
      try {
        ...
        if (!isJEE6) {
          // If that did not work and we are trying with JEE5 jndi names, try
          // with JEE6 naming
          try {
