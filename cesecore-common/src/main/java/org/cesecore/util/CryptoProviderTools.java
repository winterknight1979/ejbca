...
public final class CryptoProviderTools {
   ...
   * "implicitlyCA" key encoding. The curve parameters is then defined outside
  ...
  public static synchronized void installBCProvider() {
    ...
    // A flag that ensures that we install the parameters for implcitlyCA only
    // when we have installed a new provider
    ...
    if (installImplicitlyCA) {
      // Install EC parameters for implicitlyCA encoding of EC keys, we have
      // default curve parameters if no new ones have been given.
      // The parameters are only used if implicitlyCA is used for generating
      // keys, or verifying certs
      final ECCurve curve =
          new ECCurve.Fp(
              new BigInteger(IMPLICITLYCA_Q), // q
              new BigInteger(IMPLICITLYCA_A, 16), // a
              new BigInteger(IMPLICITLYCA_B, 16)); // b
      final org.bouncycastle.jce.spec.ECParameterSpec implicitSpec =
      ...
      final ConfigurableProvider config =
          (ConfigurableProvider) Security.getProvider("BC");
      if (config != null) {
        config.setParameter(
    ...
    // 2007-05-25
    // Finally we must configure SERIALNUMBER behavior in BC >=1.36 to be the
    // same
