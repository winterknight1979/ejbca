...
public abstract class QueryResultWrapper {
  ...
  /**
   * Query.getSingleResult that on with older Hibernate JPA 1.0 provider bundled
   * with EJBCA returns null and on newer JPA implementations throws
   * NoResultException (as it should according to the specifications) is not
   * suitable for EJBCA. We always want to return null when an object does not
   * exist, since exceptions will mess with the garbage collection and lead to
   * worse performance.
   *
   * <p>We still throws NonUniqueResultException and IllegalStateException, just
   * as getSingleResult would.
   *
   * @param query query
