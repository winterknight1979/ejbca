...
/**
 * Default ECC key validator using the Bouncy Castle BCECPublicKey
 * implementation (see
...
public class EccKeyValidator extends KeyValidatorBase {
  ...
  public void initDynamicUiModel() {
    ...
    final DynamicUiProperty<String> curves =
        new DynamicUiProperty<String>(
            String.class,
            CURVES,
            getCurvesAsString(),
            new ArrayList<String>(
                AlgorithmTools.getFlatNamedEcCurvesMap(false).keySet())) {
          @Override
          public boolean isDisabled() {
            return isCurvesDisabled();
          }
        };
    ...
    uiModel.add(
        new DynamicUiProperty<Boolean>(
            Boolean.class,
            USE_FULL_PUBLIC_KEY_VALIDATION_ROUTINE,
            isUseFullPublicKeyValidationRoutine()) {
          @Override
          public boolean isDisabled() {
   ...
   *
   * @see <a
   *     href="https://cabforum.org/wp-content/uploads/CA-Browser-Forum-BR-1.4.2.pdf">CA/B-Forum
...
private void validateQ(final List<String> messages, 
    ...
    if (LOG.isDebugEnabled()) {
      ...
      // performing FIPS 186-3/SP 800-89 Assurances self-test"
      // FSM_TRANS:5.14, "CONDITIONAL TEST", "FIPS 186-3/SP 800-89 ASSURANCES
      // CHECK", "Invoke FIPS 186-3/SP 800-89 Assurances test"
      if (q == null) {
