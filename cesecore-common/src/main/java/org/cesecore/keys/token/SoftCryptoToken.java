 ...
 * private key and stored in database.
 *
...
public class SoftCryptoToken extends BaseCryptoToken {
  ...
  /**
   * This property value denotes whether the default soft token password is
   * usable on this token (as defined by ca.keystorepass, defaults to 'foo123',
   * which allows it to be activated/deactivated/edited without providing a
   * password. Setting this property (it does not matter what it's value is set
   * to as long as it is present) means that the default password is not
   * available for use.
   */
 ...
 /** Data. */
  ...
  /**
   * Sets up some basic properties used in soft keystores and calls init on
   * BaseCryptoToken in order to set up all key string etc.
   */
  @Override
  public void init(
  ...
  public void activate(final char[] oauthCode)
    ...
    if (keystoreData != null) {
      try {
        ...
        // If everything was OK we cache the load/save password so we can store
        // the keystore
   ...
   *
   * @param authCode code
   * @throws CryptoTokenAuthenticationFailedException if the authentication code
   *     is incorrect.
   * @throws CryptoTokenOfflineException if the crypto token is offline or an
  ...
  public void generateKey(
    ...
    if (StringUtils.isNotEmpty(alias)) {
      // Soft crypto tokens must do very special things for secret keys, since
      // PKCS#12 keystores are not designed to hold
      // symmetric keys, we wrap the symmetric key with an RSA key and store it
      // in properties

      // Generate the key
      KeyGenerator generator =
          KeyGenerator.getInstance(algorithm, getEncProviderName());
