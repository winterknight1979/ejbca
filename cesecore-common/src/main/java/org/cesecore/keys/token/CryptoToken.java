...
/**
 * Handles operations of the device producing signatures and handling the
 * private key. All Crypto Token plug-ins must implement this interface.
 *
...
public interface CryptoToken extends Serializable {
 ...
 /** sep.*/
   ...
   *
   * <p>Info contains Crypto Token properties, as entered for all tokens.
   * Properties can create properties used by extending or wrapping classes so
   * the implementing class should not delete or remove properties that are
   * unknown to it, but keep them transparently. Data contains Crypto Token
   * data, can be null for tokens that don't need it.
   *
   * @param properties Properties info used to create token, new token or init
   *     existing token
   * @param data byte[] data as created internally for tokens, can be null for
   *     tokens that don't need it
   * @param id unique ID of the user of the token, the id is user defined and
   *     not used internally for anything but logging.
   ...
   *
   * @param authenticationcode used to unlock crypto token, i.e PIN for
   *     smartcard HSMs. This parameter might be ignored, e.g. for
   *     auto-activated soft tokens.
   * @throws CryptoTokenOfflineException if Crypto Token is not available or
   *     connected.
   * @throws CryptoTokenAuthenticationFailedException with error message if
   *     authentication to tokens fail.
   */
  void activate(char[] authenticationcode)
      throws CryptoTokenOfflineException,
          CryptoTokenAuthenticationFailedException;

  /**
   * Method used to deactivate tokens. Used to set a Crypto Token to offline
   * status and to reset the HSMs authorization code.
   */
  void deactivate();
   ...
   *     alias in the CryptoToken
   */
  boolean isAliasUsed(String alias);

  /**
   * Returns the private key (if possible) of token. Note that this method does
   * NOT check if the key is permitted to be extracted.
   *
  ...
  /**
   * Returns the key (if possible) of token, used for symmetric keys. For
   * assymmetric keys getPrivateKye and getPublic key is recommended..
   *
   * @param alias the key alias to retrieve from the token
   * @throws CryptoTokenOfflineException if Crypto Token is not available or
   *     connected, or key with alias does not exist.
  ...
  /**
   * Generates a key pair (asymmetric keys) in the crypto token. This method is
   * used when you have an existing PublicKey and want to generate a key of the
   * same type. You can use KeyTools to get the AlgorithmParameterSpec from an
   * existing PublicKey.
   *
   * <pre>
  ...
  /**
   * Returns the signature Provider that should be used to sign things with the
   * PrivateKey object returned by this signing device implementation.
   *
  ...
  /**
   * Returns the crypto Provider that should be used to encrypt/decrypt things
   * with the PrivateKey object returned by this signing device implementation.
  ...
  /**
   * Resets token. Might cure HW failures. It is up to each implementation to
  ...
  /**
   * Updates dynamic properties for the crypto token. Call this method when a
   * new key string, autoactivation PIN has been set and the init method on the
   * crypto token is not called. Does not update properties that is only used
   * when token is created, for example P11 slot, this is only updated on
   * recreation of the token.
   *
   * @param properties Properties containing the new key properties or other
   *     properties, such as activation PIN
   */
   ...
   *     (loaded).
   * @throws CryptoTokenOfflineException if the keystore has not been
   *     initialized (loaded).
   */
