...
/**
 * Each instance of this class represents a slot on a P11 module. Use an
 * instance of this class for all your access of a specific P11 slot. Use {@link
 * P11Slot#getProvider()} to get a provider for the slot.
 *
...
public class P11Slot {
   ...
   * token (slot). In this case only one lib could be used.
   */
  ...
  /**
   * Add a PKCS11 Crypto Provider to Java Security.addProvider, if it is not
   * already added, and add==true. The add parameter can seem redundant, but it
   * is here in order to centralize control of what is added as a Crypto
   * Provider, instead if distributing it to multiple places in the code.
   *
  ...
  private void addProviderIfNotExisting(boolean add) {
    ...
    // We can't remove the already existing provider as that will cause the
    // existing one to stop working.
    ...
    // Some HSMs (like Thales) only have one slot so in many cases it is a must
    // to be able to use the same slot for
    // database integrity protection and a Crypto Token for CAs
    if (Security.getProvider(provider.getName()) != null) {
      // Because of the above, and how Sun P11 Provider works, we can re-use the
      // existing provider
      // Of course, of this provider is not working (disconnected to HSM was
      // disconnected or similar), this
      ...
      // Give the possibility to create a P11 slot without actually adding the
  ...
  /**
   * Reset the HSM. Could be done if it has stopped working in a try to get it
   * working again.
   */
  public void reset() {
    ...
    if (provider instanceof AuthProvider) {
      ...
      log.warn(
          "Not possible to logout from PKCS#11 provider '"
              ...
              + AuthProvider.class.getCanonicalName()
              + "'.");
    }
  }
  ...
  /**
   * Get P11 slot instance. Only one instance (provider) will ever be created
   * for each slot regardless of how many times this method is called.
   *
   * @param slotLabel the labeling of the slot, regardless of label type.
   * @param sharedLibrary file path of shared
   * @param slotLabelType The type of the label. May be a slot number [0...9]*,
   *     slot index i[0...9]* or a label.
   * @param attributesFile Attributes file. Optional. Set to null if not used
   * @param token Token that should use this object
   * @param id unique ID of the user of the token. For EJBCA this is the caid.
   *     For the OCSP responder this is fixed since then there is only one user.
   * @param addProvider default value should be true, set to false to create a
   *     P11 slot without actually adding the P11 provider to Java Security
   * @return P11Slot
   * @throws CryptoTokenOfflineException if token can not be activated
   * @throws NoSuchSlotException if no slot with the label defined by slotLabel
   *     could be found
   */
  public static P11Slot getInstance(
      final String slotLabel,
      final String sharedLibrary,
      final Pkcs11SlotLabelType slotLabelType,
      final String attributesFile,
  ...
  /**
   * As {@link #getInstance(String, String, Pkcs11SlotLabelType, String,
   * P11SlotUser, int, boolean)} but is using config file instead parameters. Do
   * only use this method if the P11 shared library is ony specified in this
   * config file.
   *
   ...
   * @throws NoSuchSlotException if no slot defined by the label in
   *     configFileName could be found.
   */
  public static P11Slot getInstance(
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "P11Slot.getInstance(): '"
              + sunP11ConfigFileName
              + "', "
              + Pkcs11SlotLabelType.SUN_FILE.toString()
              + ", "
              + id);
    }
    return getInstance(
  ...
  private static P11Slot getInstance(
      final Pkcs11SlotLabelType slotLabelType,
      final String friendlyName,
      final String slotLabel,
      final String sharedLibrary,
    ...
    try {
      ...
      if (Pkcs11SlotLabelType.SUN_FILE.equals(slotLabelType)) {
        if (sunP11ConfigFileName == null) {
          throw new IllegalStateException(
              "Can't initialize PKCS#11 slot of type "
                  + slotLabelType.name()
                  + " without providing a config file.");
        }
        ...
        if (slotLabel == null || sharedLibrary == null) {
          throw new IllegalStateException(
              "Can't initialize PKCS#11 slot of type "
                  + slotLabelType.name()
                  + " without providing library and slot label.");
        }
      ...
      throw new CryptoTokenOfflineException(
          InternalResources.getInstance()
              .getLocalizedMessage("token.errorcreatetoken", id),
          e);
    }
  }
}
