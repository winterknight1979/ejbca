...
/**
 * This class is responsible for managing a list of CT logs. The CT logs backed
 * by this class can be grouped or reordered and there is functionality for
 * adding new CT logs or removing existing ones. This class does not load or
 * store changes in a persistent storage, and relies on its creator to load the
 * CT logs from e.g. a database or a file.
 *
...
public class CtLogManager {
  ...
  public LinkedHashMap<String, List<CTLogInfo>> getCtLogGroups() {
    final LinkedHashMap<String, List<CTLogInfo>> logLogMap =
        new LinkedHashMap<>();
  ...
  /**
   * Get a list of CT logs labelled as specified. If no logs have the label
   * specified, an empty list will be returned.
   *
  ...
  /**
   * Moves the specified CT log up one step. This method does nothing if the CT
   * log is already on top or if the CT log group to which the CT log belongs
   * only contains one log.
   *
   * @param ctLog Log
   * @throws IllegalArgumentException if the CT log given as argument is not
   *     managed by this CT log manager
   */
  public void moveUp(final CTLogInfo ctLog) {
    ...
    if (!ctLogGroup.contains(ctLog)) {
      throw new IllegalArgumentException(
          "The CT log "
              + ctLog.toString()
              + " is not managed by this CT log manager.");
    }
    if (ctLogGroup.size() == 1 || isOnTop(ctLog)) {
      return;
    }
    final CTLogInfo previousCtLog =
        ctLogGroup.get(ctLogGroup.indexOf(ctLog) - 1);
    Collections.swap(
        ctLogs, ctLogs.indexOf(ctLog), ctLogs.indexOf(previousCtLog));
  }

  /**
   * Moves the specified CT log down one step. This method does nothing if the
   * CT log is already on the bottom or if the CT log group to which the CT log
   * belongs only contains one log.
   *
   * @param ctLog Log
   * @throws IllegalArgumentException if the CT log given as argument is not
   *     managed by this CT log manager
  ...
  /**
   * Add a new CT log to this CT log manager. This method will prevent duplicate
   * logs from being added.
   *
   * @param ctLog the CT log to add
   * @throws DuplicateCtLogException if the CT log to add is a duplicate
   *     according to {@link #canAdd(CTLogInfo)}
   */
  public void addCtLog(final CTLogInfo ctLog) {
    if (!canAdd(ctLog)) {
      throw new DuplicateCtLogException(
          "The CT log "
              + ctLog.toString()
              + " already exists in the log group '"
              + ctLog.getLabel()
              + "'.");
    }
    ctLogs.add(ctLog);
   ...
   * @throws IllegalArgumentException if the CT log is not managed by this CT
   *     log manager
   */
  ...
  /**
   * Determine whether the CT log given as input can be added to this CT log
   * manager. A CT log cannot be added if any of the following conditions hold
   * for another CT log:
   *
   * <ul>
   *   <li>The other log has an ID identical to the new CT log
   *   <li>The other log is has an identical URL and label as the new CT log
   * </ul>
   *
  ...
  public boolean canAdd(final CTLogInfo ctLog) {
    for (CTLogInfo existing : ctLogs) {
      ...
      final boolean urlExistsInCtLogGroup =
          StringUtils.equals(existing.getUrl(), ctLog.getUrl())
  ...
  /**
   * Determine whether the CT log given as input is on the top in its CT log
   * group.
   *
  ...
  /**
   * Determine whether the CT log given as input is on the bottom in its CT log
   * group.
   *
   * @param ctLog the CT log to check
  ...
  public boolean isOnBottom(final CTLogInfo ctLog) {
    final List<CTLogInfo> ctLogGroup = getCtLogsByLabel(ctLog.getLabel());
    return !ctLogGroup.isEmpty()
        && ctLogGroup.get(ctLogGroup.size() - 1).equals(ctLog);
  }

  /**
   * Rename a label. This will effectively set the label of CT logs to the new
   * label for every CT log with the old label.
   *
  ...
  /**
   * Returns the string representation of this object containing the CT logs
   * currently managed by this CT log manager.
   */
  @Override
