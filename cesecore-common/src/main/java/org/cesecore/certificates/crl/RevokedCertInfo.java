...
/**
 * Holds information about a revoked certificate. The information kept here is
 * the information that goes into a CRLEntry.
 *
...
public class RevokedCertInfo implements Serializable {
  ...
  /**
   * A default constructor is needed to instantiate RevokedCertInfo objects
   * using &lt;jsp:useBean&gt; by Tomcat 5.
   */
  ...
  /**
   * @return true is there is a revocationDate set (getRevocationDate() !=
  ...
  public String toString() {
    return this.userCertificate == null
        ? "null"
        : new BigInteger(userCertificate).toString();
  }
  ...
  /**
   * @return true if the certificate is permanently revoked (i.e. revoked and
   *     not just "on hold")
   */
  ...
  /**
   * This method returns the revocation reason as a text string that is
   * understandable. TODO: The strings in the enum should be easier for users to
   * change, used from "publicweb/retrieve/check_status_result.jsp"
   *
  ...
  public String getHumanReadableReason() {
    RevocationReasons revocationReason =
        RevocationReasons.getFromDatabaseValue(reason);
  ...
  /**
   * Merges two collections of RevokedCertInfo. Note that the parameters are
   * slightly different. Duplicates are removed according to these rules:
   *
   * <ul>
   *   <li>Older permanent status changes win over newer ones
   *   <li>Permanent status changes always win over temporary ones ("on hold" /
   *       "re-activate")
   *   <li>More recent temporary status changes win over older ones
   * </ul>
   *
   * @param a First collection of RevokedCertInfo. May <b>not</b> contain
   *     duplicates for the same serial number.
   * @param b Second collection of RevokedCertInfo. May contain duplicates
   * @param lastBaseCrlDate Entries in unrevoked state will only be included if
   *     they are more recent than this date. (&lt;= 0 means never include them)
   * @return CompressionCollection of certificates. May simply be a reference to
   *     <code>a</code> if <code>b</code> is empty, or a new merged
   *     CompressedCollection with any duplicates removed.
   */
  public static Collection<RevokedCertInfo> mergeByDateAndStatus(
      final Collection<RevokedCertInfo> a,
      final Collection<RevokedCertInfo> b,
      final long lastBaseCrlDate) {
    // We can optimize this case, but not the reverse, since b can contain
    // duplicates that should be filtered.
    if (b.isEmpty()) {
      return a;
    }
    ...
    for (final RevokedCertInfo revoked : b) {
      ...
      if (permDate != null) {
        ...
        if (permDate.getRevocationDate().after(revdate)
            && revoked.isPermanentlyRevoked()) {
          permRevoked.put(serial, revoked);
