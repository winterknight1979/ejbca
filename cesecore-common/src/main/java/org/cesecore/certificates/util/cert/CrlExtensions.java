...
public final class CrlExtensions {
  ...
  public static BigInteger getCrlNumber(final X509CRL crl) {
    ...
    try {
      ASN1Primitive obj =
          CrlExtensions.getExtensionValue(crl, Extension.cRLNumber.getId());
      if (obj != null) {
  ...
  public static BigInteger getDeltaCRLIndicator(final X509CRL crl) {
    ...
    try {
      ASN1Primitive obj =
          CrlExtensions.getExtensionValue(
              crl, Extension.deltaCRLIndicator.getId());
      if (obj != null) {
        CRLNumber crlnum = CRLNumber.getInstance(obj);
        if (crlnum != null) {
          ret = crlnum.getCRLNumber();
  ...
  /**
   * @param crlEntry CRL
   * @return the revocation reason code as defined in
   *     RevokedCertInfo.REVOCATION_REASON_...
   */
  public static int extractReasonCode(final X509CRLEntry crlEntry) {
    ...
    if (crlEntry.hasExtensions()) {
      final byte[] extensionValue =
          crlEntry.getExtensionValue(Extension.reasonCode.getId());
      if (extensionValue != null) {
        try {
          ...
          log.debug(
              "Failed to parse reason code of CRLEntry: " + e.getMessage());
        }
      }
    }
    return reasonCode;
  ...
  /**
   * @param crl CRL
   * @return a list of URLs in String format with present freshest CRL
   *     extensions or an empty List
   */
  public static List<String> extractFreshestCrlDistributionPoints(
      final X509CRL crl) {
    final List<String> freshestCdpUrls = new ArrayList<String>();
    final byte[] extensionValue =
        crl.getExtensionValue(Extension.freshestCRL.getId());
    if (extensionValue != null) {
   ...
   * @param clazz class
   * @param <T> type
   * @return the first object found when treating the provided byte array as an
   *     ASN1InputStream
   */
    ...
    try {
      ...
      if (asn1InputStream != null) {
        try {
          ...
          log.info(
              "Failed to extract expected ASN1 object from bytes array.", e);
        }
      }
    }
    return ret;
