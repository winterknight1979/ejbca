 ...
 *
 * @version $Id: QCStatementExtension.java 23659 2016-06-10 15:16:35Z anatom $
 */
public final class QCStatementExtension extends CertTools {
  ...
  public static boolean hasQcStatement(final Certificate cert) {
    ...
    if (cert instanceof X509Certificate) {
      ...
      final ASN1Primitive obj =
          getExtensionValue(x509cert, Extension.qCStatements.getId());
      if (obj != null) {
        ret = true;
      }
    }
   ...
   *
   * @param cert Certificate containing the extension
   * @return Collection of String with the oid, for example "1.1.1.2", or empty
   *     Collection if no identifier is found, never returns null.
   * @throws IOException if there is a problem parsing the certificate
   */
  public static Collection<String> getQcStatementIds(final Certificate cert)
      throws IOException {
   ...
   * @return String with the value and currency (ex '50000 SEK')or null if the
   *     extension is not present
   * @throws IOException if there is a problem parsing the certificate
   */
  public static String getQcStatementValueLimit(final Certificate cert)
      throws IOException {
    String ret = null;
    if (cert instanceof X509Certificate) {
      final X509Certificate x509cert = (X509Certificate) cert;
      ...
      // Look through all the QCStatements and see if we have a stadard ETSI
      // LimitValue
      for (int i = 0; i < seq.size(); i++) {
        final QCStatement qc = QCStatement.getInstance(seq.getObjectAt(i));
        final ASN1ObjectIdentifier oid = qc.getStatementId();
      ...
      if (mv != null) {
        ...
        // A possibly gotcha here if the monetary value is larger than what fits
        // in a long...
        final long value =
   ...
   *     rfc822Name=bar2foo.se' etc. Supports email, dns and uri name, or null
   *     of no RAs are found.
   * @throws IOException if there is a problem parsing the certificate
   */
  public static String getQcStatementAuthorities(final Certificate cert)
      throws IOException {
    String ret = null;
    if (cert instanceof X509Certificate) {
      final X509Certificate x509cert = (X509Certificate) cert;
      ...
      if (si != null) {
        ...
        for (int i = 0; i < gns.length; i++) {
          ...
          if (strBuf.length() != 0) {
            // Append comma so we get nice formatting if there are more than one
            // authority
            strBuf.append(", ");
          }
   ...
   */
  public static String getStatementStringValue(
      final ASN1Sequence seq, final String statementoid, final int pos)
      throws IOException {
    // Look through all the QCStatements and see if we have a standard ETSI PDS
    // URL
    for (int i = 0; i < seq.size(); i++) {
      final QCStatement qc = QCStatement.getInstance(seq.getObjectAt(i));
      final ASN1ObjectIdentifier oid = qc.getStatementId();
