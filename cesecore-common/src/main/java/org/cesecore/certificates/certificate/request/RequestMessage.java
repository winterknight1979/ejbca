...
/**
 * Base interface for request messages sent to the CA. Implementors of this
...
public interface RequestMessage extends Serializable {
  ...
  /**
   * Gets the issuer DN if contained in the request (the CA the request is
   * targeted at).
   *
  ...
  /**
   * Gets the number (of CA cert) from IssuerAndSerialNumber. Combined with
   * getIssuerDN to identify the CA-certificate of the CA the request is
   * targeted for.
   *
  ...
  /**
   * Gets the requested DN if contained in the request (the desired DN for the
   * user).
   *
  ...
  /**
   * Gets the requested DN in the pure form of an X508Name (the desired DN for
   * the user).
   *
   * @return requested DN or null.
   */
  ...
  /**
   * Gets the requested altNames if contained in the request (the desired
   * altNames for the user).
   *
  ...
  /**
   * Gets a validity date from the request, if the request contains a desired
   * validity. The requested validity may, or may not be used, it depends if
   * allowValidityOverride is set in the certificate profile.
   *
   * @return A date now or in the future for notBefore validity in the
   *     certificate, or null if no desired validity is in the certificate.
   */
   ...
   * allowValidityOverride is set in the certificate profile.
   *
   * @return A date in the future for notAfter validity in the certificate, or
   *     null if no desired validity is in the certificate.
   */
  Date getRequestValidityNotAfter();

  /**
   * Gets any requested extensions, if the request message type is able to
   * contain request extensions and if there are any. Requested extensions are
   * (currently) in the form of X509Extensions.
   *
  ...
  /**
   * Gets the issuer DN (of CA cert) from IssuerAndSerialNumber when this is a
   * CRL request.
   *
  ...
  /**
   * Gets the number (of CA cert) from IssuerAndSerialNumber when this is a CRL
   * request.
   *
   * @return serial number of CA certificate for CA issuing CRL or null.
   */
   ...
   *
   * @return The key from a certification request. Returns null if request
   *     content hasn't been set, or there is no public key in the request (can
   *     be ok if server generated keys are requested).
  ...
  /**
   * Verifies signatures, popo etc on the request message. If verification fails
  ...
  /**
   * indicates if this message needs recipients and private key to verify,
   * decrypt etc. If this returns true, setKeyInfo() should be called.
   *
  ...
  /**
   * Sets the and private key needed to decrypt/verify the message. Must be set
   * if requireKeyInfo() returns true.
   *
  ...
  /**
   * Returns the name of the preferred Digest algorithm to be used in the
   * response if applicable. Defaults to CMSSignedGenerator.DIGEST_SHA1 for
   * normal messages.
   *
   * @return oid of digest algorithm ex CMSSignedGenerator.DIGEST_SHA1, SHA256
   *     etc
   */
   ...
   * true = yes. Not applicable for all request/response types.
   *
   ...
   * identifier was used, even if the messages themselves are the same messages.
   *
  ...
  /**
   * For some types of request-responses there is a need for a requestId to
