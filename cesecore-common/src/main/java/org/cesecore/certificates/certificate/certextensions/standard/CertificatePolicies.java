 ...
 * of this extension.
 *
 * @version $Id: CertificatePolicies.java 24112 2016-08-04 12:54:30Z anatom $
 */
public class CertificatePolicies extends StandardCertificateExtension {
  ...
  public ASN1Encodable getValue(
    ...
    final Map<ASN1ObjectIdentifier, ASN1EncodableVector> policiesMap =
        new LinkedHashMap<ASN1ObjectIdentifier, ASN1EncodableVector>();
    // Each Policy OID can be entered several times, with different qualifiers,
    // because of this we make a map of oid and qualifiers, and we can add a new
    // qualifier
    ...
    for (final Iterator<CertificatePolicy> it = policies.iterator();
      ...
      final PolicyQualifierInfo pqi =
          getPolicyQualifierInformation(policy, displayencoding);
      if (pqi != null) {
        qualifiers.add(pqi);
      }
   ...
   *     DisplayText.CONTENT_TYPE_BMPSTRING, CONTENT_TYPE_UTF8STRING,
   *     CONTENT_TYPE_IA5STRING or CONTENT_TYPE_VISIBLESTRING
   * @return PolicyQualifierInfo
   */
  private PolicyQualifierInfo getPolicyQualifierInformation(
    ...
    if (qualifierId != null && !StringUtils.isEmpty(qualifierId.trim())) {
      ...
      if (qualifier != null && !StringUtils.isEmpty(qualifier.trim())) {
        if (qualifierId.equals(PolicyQualifierId.id_qt_cps.getId())) {
          ...
          // Normally we would just use 'DisplayText(unotice)' here. IE has
          // problems with UTF8 though, so lets stick with BMSSTRING to satisfy
          // Bills sick needs.
          final UserNotice un =
              new UserNotice(null, new DisplayText(displayencoding, qualifier));
