 ...
 * QCStatement (rfc3739)
 *
 * <p>Class for standard X509 certificate extension. This extension have some
 * basics defined in RFC 3739, but the majority of fields are used in EU
 * purposes and specified in EU standards. ETSI EN 319 412-5 (v2.1.1, 2016-02 or
 * later)
 ...
 *
 * <p>qcStatements EXTENSION ::= { SYNTAX QCStatements IDENTIFIED BY
 * id-pe-qcStatements } id-pe-qcStatements OBJECT IDENTIFIER ::= { id-pe 3 }
 *
 * <p>QCStatements ::= SEQUENCE OF QCStatement QCStatement ::= SEQUENCE {
 * statementId QC-STATEMENT.&amp;Id({SupportedStatements}), statementInfo
 * QC-STATEMENT.&amp;Type ({SupportedStatements}{@literal @}statementId})
 * OPTIONAL }
 *
 * <p>SupportedStatements QC-STATEMENT ::= { qcStatement-1,...}
 *
...
public class QcStatement extends StandardCertificateExtension {
  ...
  public ASN1Encodable getValue(
    ...
    if (StringUtils.isNotEmpty(certProfile.getQCEtsiType())) {
      ...
      qc =
          new QCStatement(
              ...
              seq); // ETSIQCObjectIdentifiers.id_etsi_qcs_QcType in BC > 1.54
      qcs.add(qc);
    }
    if (certProfile.getQCEtsiPds() != null) {
      ...
      qc =
          new QCStatement(
              new ASN1ObjectIdentifier("0.4.0.1862.1.5"),
              new DERSequence(
                  locations)); // ETSIQCObjectIdentifiers.id_etsi_qcs_QcPds in
                               // BC > 1.54
      qcs.add(qc);
    }
    ...
    //   -- This statement gives you the possibility to define your own
    // QC-statement
    //   -- using an OID and a simple UTF8String, with describing text. A sample
    // text could for example be:
    //   -- This certificate, according to Act. No. xxxx Electronic Signature
    // Law is a qualified electronic certificate
    //
    // YourCustomUTF8String ::= UTF8String
    if (certProfile.getUseQCCustomString()
        && !StringUtils.isEmpty(certProfile.getQCCustomStringOid())
        && !StringUtils.isEmpty(certProfile.getQCCustomStringText())) {
      final DERUTF8String str =
          new DERUTF8String(certProfile.getQCCustomStringText());
      final ASN1ObjectIdentifier oid =
          new ASN1ObjectIdentifier(certProfile.getQCCustomStringOid());
    ...
    if (ret == null) {
      ...
      throw new CertificateExtensionException(
          "If qualified certificate statements extension has been enabled, at"
