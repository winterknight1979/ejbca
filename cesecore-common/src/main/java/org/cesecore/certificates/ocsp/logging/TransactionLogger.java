...
public class TransactionLogger extends PatternLogger {
  ...
  /**
   * The BC normalized issuer Distinguished Name of the certificate used to sign
   * the request.
   */
  public static final String SIGN_ISSUER_NAME_DN = "SIGN_ISSUER_NAME_DN";
  /**
   * The BC normalized Subject Distinguished Name of the certificate used to
   * sign the request.
   */
   ...
   * (see OCSPResponseItem).
   */
  public static final String CERT_STATUS = "CERT_STATUS";

  /**
   * The requested certificate revocation reason, or NOT_REVOKED, if unrevoked
   * (see RevocationReasons). Set to 6 (certificateHold) when certificate is
   * unknown, even if status returned is good.
   */
  public static final String REV_REASON = "REV_REASON";

  /**
   * The id of the certificate profile that was used to issue the requested
   * certificate.
   */
  ...
  public TransactionLogger(
    ...
    super(
        ...
        OcspConfiguration.getTransactionLogPattern(),
        OcspConfiguration.getTransactionLogOrder(),
        OcspConfiguration.getLogDateFormat(),
        OcspConfiguration.getLogTimeZone());
    ...
    paramPut(
        CERT_PROFILE_ID,
        String.valueOf(CertificateProfileConstants.CERTPROFILE_NO_PROFILE));
    paramPut(FORWARDED_FOR, "");
    paramPut(
        REV_REASON,
        String.valueOf(RevocationReasons.NOT_REVOKED.getDatabaseValue()));
  }
}
