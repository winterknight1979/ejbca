 ...
 *
 * <p>When doing secure audit log it is necessary to identify the event being
 * logged.
 *
 * @see org.cesecore.audit.enums.ModuleTypes
 * @see org.cesecore.audit.enums.ServiceTypes
 * @version $Id: EventTypes.java 26591 2017-09-20 14:19:01Z anatom $
 */
public enum EventTypes implements EventType {
  ...
  CA_DELETION,
  /**
   * Internal application name change of a Certificate Authority. Unrelated to
   * Certificate Authority's Subject Distinguisher Name.
   */
  CA_RENAMING,
  ...
  CA_KEYACTIVATE,
  /**
   * Generation of a new key pair that can be used by the Certificate Authority
   * during renewal or update.
   */
  CA_KEYGEN,
  /**
   * Certificate Authority state change to start serving requests. Unrelated to
   * CA private key availability.
   */
  CA_SERVICEACTIVATE,
  /**
   * Certificate Authority state change to stop serving requests. Unrelated to
   * CA private key availability.
   */
  ...
  CERT_REVOKED,
  /**
   * Change of a certificate's status to unassigned, inactive, active, notified
   * about expiration, revoked or archived.
   */
  CERT_CHANGEDSTATUS,
  /**
   * A request for certificate issuance from a Certificate Authority is
   * submitted.
   */
  CERT_REQUEST,
  ...
  /**
   * Activation of a Crypto Token, making the key material available for use by
   * the application.
   */
  CRYPTOTOKEN_ACTIVATION,
  /**
   * Deactivation of a Crypto Token, making the key material unavailable for use
   * by the application.
   */
  CRYPTOTOKEN_DEACTIVATION,
  /**
   * Attempted reactivation of a Crypto Token. Since this occurs automatically,
   * it may fail.
   */
   ...
   * stores, this also implies changes of the protection of the key material.
   */
