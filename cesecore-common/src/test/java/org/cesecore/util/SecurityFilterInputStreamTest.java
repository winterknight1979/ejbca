...
public class SecurityFilterInputStreamTest {
   ...
   *     high value for exploit (etc. 0x1FFFFFFF); also be aware that some high
   *     values could throw heap error on attacker's JVM
   * @throws IOException fail
   */
  public static void prepareExploitStream(
      final OutputStream outputStream, final long payloadSize)
      throws IOException {
    ...
    try {
      ...
      fail(
          "No Java heap error happened for StringBuilder exploit (MaxHeap = "
              ...
              + " input stream during testPreventingTheStringBuilderExploit");
      ...
      fail(
          "Unexpected exception: "
              + e.getMessage()
              + " during testPreventingTheStringBuilderExploit");
    } finally {
  ...
  public void testAcceptedSizeInputStream() throws Exception {
    ...
    try {
      byteArrayOutputStream = new ByteArrayOutputStream();
      prepareExploitStream(
          byteArrayOutputStream, 0xFFFF); // 0xFFFF simulates safe stream
      ...
      fail(
          "Unexpected exception: "
              + e.getMessage()
              + " during testAcceptedSizeInputStream");
    } finally {
      if (byteArrayOutputStream != null) {
        byteArrayOutputStream.close();
      }
