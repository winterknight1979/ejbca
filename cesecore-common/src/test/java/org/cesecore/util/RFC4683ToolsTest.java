...
public class RFC4683ToolsTest {
  ...
  public void testGenerateSimForInternalSanFormat()
    ...
    // SIM parameters.
    san =
        "SUBJECTIDENTIFICATIONMETHOD=2.16.840.1.101.3.4.2.1::MyStrongPassword::SsiType::abc::abc,"
            + " DNSNAME=localhost";
    try {
    ...
    san =
        "SUBJECTIDENTIFICATIONMETHOD=2.16.840.1.101.3.4.2.1::MyStrongPassword::SsiType::SsiValue,"
            + " DNSNAME=localhost";
    RFC4683Tools.generateSimForInternalSanFormat(san);
    // Calculated SIM (3 tokens) -> nothing happens
    san =
        "SUBJECTIDENTIFICATIONMETHOD=2.16.840.1.101.3.4.2.1::CB3AE7FBFFFD9C85A3FB234E51FFFD2190B1F8F161C0A2873B998EFAC067B03A::6D9E6264DDBD0FC997B9B40524247C8BC319D02A583F4B499DD3ECAF06C786DF,"
            + " DNSNAME=localhost";
    assertEquals(RFC4683Tools.generateSimForInternalSanFormat(san), san);
  }
  ...
  public void testGenerateInternalSimString() {
    ...
    // 2a. Use invalid hash algorithm OID (unknown | empty | null) -> IAE("Hash
    // algorithm OID string must not be null or empty.")
    simParameters =
        new String[] {
          null,
          "MyStrongPassword",
    ...
    assertIAEForGenerateInternalSimString(
        simParameters, "Hash algorithm OID string must not be null or empty");
    simParameters =
        new String[] {
          "2.16.840.1.101a.A.B.C",
          "MyStrongPassword",
          "SsiType",
          "Sensitive identifier information"
        };
    assertIAEForGenerateInternalSimString(
        simParameters, "Hash algorithm with OID");
    // 2b. Use invalid password ( empty | null | ... ) -> FIPS 112 and FIPS
    // 180-1 compliance is not tested -> IAE("The user chosen password must not
    // be null or empty")
    simParameters =
        new String[] {
          "2.16.840.1.101.3.4.2.1",
    ...
    assertIAEForGenerateInternalSimString(
        simParameters, "The user chosen password must not be null or empty");
    ...
    simParameters =
        new String[] {
          "2.16.840.1.101.3.4.2.1", "MyStrongPassword", "SsiType", null
        };
    assertIAEForGenerateInternalSimString(
        simParameters,
  ...
  public void testAsn1ReadWrite()
    ...
    final ASN1Primitive generalName =
        testCreateSimGeneralName(simParameters, simTokens);
    testGetSimStringSequence(internalSimString, generalName);
  }

  private ASN1Primitive testCreateSimGeneralName(
    ...
    final DERSequence simSequence =
        ((DERSequence)
            ((DERTaggedObject)
                    ((DERSequence)
                            ((DERTaggedObject) generalName.toASN1Primitive())
                                .getObjectParser(0, true))
                        .getObjectAt(1))
                .getObjectParser(0, true));
    final String algorithmIdentifier =
        ((AlgorithmIdentifier) simSequence.getObjectAt(0))
            .getAlgorithm()
            .getId();
    final DEROctetString authorityRandom =
        (DEROctetString) simSequence.getObjectAt(1);
    final DEROctetString sim = (DEROctetString) simSequence.getObjectAt(2);
    assertEquals(
        "The SIM algorithm identifier must match.",
        simParameters[0],
        algorithmIdentifier);
    assertEquals(
        "The SIM authority random must match.",
        simTokens[1],
        new String(authorityRandom.getOctets()));
    assertEquals(
        "The SIM algorithm identifier must match.",
        simTokens[2],
        new String(sim.getOctets()));
    return generalName;
  }
