...
public final class ConcurrentCacheTest {
  ...
  public void testSingleThreaded() throws Exception {
    ...
    assertFalse(
        "isInCache should return false for non-existent entry",
        entry.isInCache());
    try {
      entry.getValue();
    ...
    assertNotNull("openCacheEntry timed out in single-threaded test", entry);
    assertTrue(
        "isInCache should return true for existing entry", entry.isInCache());
    ...
    // Try to read it now. It should have expired and a fresh entry should be
    // returned.
    entry = cache.openCacheEntry("A", 1);
    assertFalse(
        "isInCache should return false for expired entry", entry.isInCache());
    try {
      entry.getValue();
  ...
  public void testDisabled() throws Exception {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      entry = cache.openCacheEntry("A", 1);
      assertTrue(
          "openCacheEntry took too long. It should be a no-op when cache is"
              + " disabled",
          System.currentTimeMillis() < timeBefore + 10);
      assertNotNull("openCacheEntry timed out when cache is disabled", entry);
      assertFalse(
          "isInCache should return false when cache is disabled",
          entry.isInCache());
      try {
        entry.getValue();
  ...
  public void testMultiThreaded() throws Exception {
    ...
    final Thread thread1 =
        new Thread(new CacheTestRunner(cache, THREAD_ONE), "CacheTestThread1");
    final Thread thread2 =
        new Thread(new CacheTestRunner(cache, THREAD_TWO), "CacheTestThread2");
  ...
  public void testExpiryBehavior() throws Exception {
    ...
    final ConcurrentCache<String, Integer>.Entry entry1 =
    ...
    final ConcurrentCache<String, Integer>.Entry entry2 =
        cache.openCacheEntry("A", 1000);
    assertFalse(
    ...
    final ConcurrentCache<String, Integer>.Entry entry3 =
        cache.openCacheEntry("A", 1000);
    final long stopTime = System.currentTimeMillis();
    assertTrue("openCacheEntry should not block.", stopTime - startTime < 500L);
    assertTrue(
        "Should be able to use the existing cached data, because someone is"
    ...
    final ConcurrentCache<String, Integer>.Entry entry4 =
        cache.openCacheEntry("A", 1000);
  ...
  public void testRandomMultiThreaded() throws InterruptedException {
    ...
    try {
      ...
      // but neither of the getLevel/getEffectiveLevel methods return the
      // correct value
      ...
      try {
        switch (id) {
          case THREAD_ONE:
            ...
            entry = cache.openCacheEntry("A", 1);
            assertFalse(
                "non-existent entry should not be in cache", entry.isInCache());
            Thread.sleep(200);
            entry.putValue(123);
          ...
          case THREAD_RANDOM:
            ...
            while (!shouldExit) {
              ...
              if (entry.isInCache()) {
                assertNotNull(
                    "got null value in cache entry", entry.getValue());
