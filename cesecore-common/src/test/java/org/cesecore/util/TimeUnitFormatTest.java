 ...
 *
 * @version $Id: TimeUnitFormatTest.java 29662 2018-08-16 11:37:46Z anatom $
 */
public class TimeUnitFormatTest {
   ...
   * Both methods of the test object (a.parseMillis() and a.format()) are tested
   * here, to not to double code. Values with '0' are not rendered (i.e. '1y
   * 0mo' -&gt; '1y').
   *
  ...
  public void testParserAndFormatter() throws Exception {
    ...
    assertEquals(
        "'" + time + "' input formatted.",
        "0" + type,
        parser.format(millis, MILLISECONDS_FACTOR, type));

    // test positive part string
    time = "1y2mo3d4h7ms";
    millis = parser.parseMillis(time);
    assertEquals(
        "'" + time + "' input long value.",
        1 * MILLISECONDS_PER_YEAR
            + 2 * MILLISECONDS_PER_MONTH
    ...
    assertEquals(
        "'" + time + "' input formatted.",
        "1y 2mo 3d 4h 7ms",
        parser.format(millis, MILLISECONDS_FACTOR, TYPE_DAYS));
    ...
    // test canonical form for positive results no negative values! (i.e.:
    // '400d' -> '1y 1mo 5d' NOT '1y 2mo -25d')
    time = "400d";
    millis = parser.parseMillis(time);
    assertEquals(
        "'" + time + "' input long value.", 400 * MILLISECONDS_PER_DAY, millis);
    assertEquals(
        "'" + time + "' input formatted.",
        "1y 1mo 5d",
        parser.format(millis, MILLISECONDS_FACTOR, TYPE_DAYS));

    // test negative canonical form -> only negative values! (i.e.: '-400d' ->
    // '-1y -1mo -5d' NOT '-1y -2mo +25d')
    time = "-400d";
    millis = parser.parseMillis(time);
    assertEquals(
        "'" + time + "' input long value.",
        -400 * MILLISECONDS_PER_DAY,
        millis);
    assertEquals(
        "'" + time + "' input formatted.",
        "-1y -1mo -5d",
    ...
    try {
      ...
      assertTrue(
          "Expected NumberFormatException: " + e.toString(),
          e instanceof NumberFormatException
              && "Illegal characters.".equals(e.getMessage()));
    }
    ...
    try {
      ...
      assertTrue(
          "Expected NumberFormatException: " + e.toString(),
          e instanceof NumberFormatException
              && "Cannot parse a blank string.".equals(e.getMessage()));
    }
    try {
      parser.parseMillis("   ");
      fail("NumberFormatException ('Cannot parse a blank string.') expected!");
