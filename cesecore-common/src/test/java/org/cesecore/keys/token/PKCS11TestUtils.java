...
public final class PKCS11TestUtils {
  ...
  public static String getHSMProvider() {
    ...
    final File protectServerLinux64 =
        new File(PROTECTSERVER_PKCS11_LINUX64_LIB);
    final File protectServerLinux32 =
        new File(PROTECTSERVER_PKCS11_LINUX32_LIB);
    final File protectServerWindows =
        new File(PROTECTSERVER_PKCS11_WINDOWS_LIB);
    String ret = null;
    if (utimacoCSLinux.exists()) {
      ...
      ret = "SunPKCS11-libCryptoki2.so-slot1";
        ...
        || protectServerLinux64.exists()
        || protectServerLinux.exists()) {
  ...
  /**
   * @return properties defined in systemtests.properties for override of
   *     non-default environments. See also
   *     org.cesecore.SystemTestsConfiguration.
   */
    ...
    if (properties == null) {
      ...
      try {
        final InputStream is =
            PKCS11TestUtils.class.getResourceAsStream(PROPERTYFILE);
        if (is != null) {
          properties.load(is);
          is.close();
        }
