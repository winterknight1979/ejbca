...
public class CryptoTokenFactoryTest {
  ...
  public void testAvailableCryptoToken() throws Exception {
    ...
    AvailableCryptoToken token1 =
        new AvailableCryptoToken(
            SoftCryptoToken.class.getName(), "SOFT", true, true);
    AvailableCryptoToken token2 =
        new AvailableCryptoToken(
            PKCS11CryptoToken.class.getName(), "PKCS#11", true, true);
    AvailableCryptoToken token3 =
        new AvailableCryptoToken(
            NullCryptoToken.class.getName(), "Null", true, true);
    // A token with the same classpath but different name should count as the
    // same
    AvailableCryptoToken token4 =
        new AvailableCryptoToken(
            SoftCryptoToken.class.getName(), "FOO", true, true);
    // A token that should not exist, but with a real classpath
    AvailableCryptoToken token5 =
        new AvailableCryptoToken(
            MockCryptoToken.class.getName(), "FOO", true, true);
    // A token that should not exist, with a non existing classpath
    AvailableCryptoToken token6 =
        new AvailableCryptoToken("foo.bar.CryptoToken", "FOO", true, true);
    ...
    mgr.addAvailableCryptoToken(
        MockCryptoToken.class.getCanonicalName(), "FOO", true, true);
