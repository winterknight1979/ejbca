 ...
 *
 * @version $Id: CachingKeyStoreWrapperTest.java 22132 2015-11-02 13:52:46Z
 *     mikekushner $
 */
 ...
 * @throws Exception fail
  ...
  private void testGenerateUseDelete(final boolean cache) throws Exception {
    ...
    final CachingKeyStoreWrapper cachingKeyStoreWrapper =
        new CachingKeyStoreWrapper(keyStore, cache);
 ...
 * @throws Exception fail
  ...
  private void testPersistLoad(final boolean cache) throws Exception {
    ...
    final CachingKeyStoreWrapper cachingKeyStoreWrapper2 =
        new CachingKeyStoreWrapper(keyStore2, cache);
    ...
    final KeyStore keyStore3 =
        KeyStore.getInstance("PKCS12", BouncyCastleProvider.PROVIDER_NAME);
    keyStore3.load(new ByteArrayInputStream(baos3.toByteArray()), PASSWORD);
    // Test loaded key store
    final CachingKeyStoreWrapper cachingKeyStoreWrapper3 =
        new CachingKeyStoreWrapper(keyStore3, cache);
    testRemove(cachingKeyStoreWrapper3, ALIAS);
  }

  private void testGenerate(
      final CachingKeyStoreWrapper cachingKeyStoreWrapper, final String alias)
      throws Exception {
    final int aliasCountBefore = getAliasCount(cachingKeyStoreWrapper);
    // Generate a key pair
    final KeyStoreTools keyStoreTools =
        new KeyStoreTools(
    ...
    Assert.assertEquals(
        "Number of aliases should have increased by 1 after generation.",
        aliasCountBefore + 1,
        getAliasCount(cachingKeyStoreWrapper));
    Assert.assertTrue(
        "Generated key pair alias was not found.",
        isContainsAlias(cachingKeyStoreWrapper, alias));
    final Key key = cachingKeyStoreWrapper.getKey(alias, null);
    Assert.assertNotNull(
        "No private key for generated key pair could be found.", key);
    Assert.assertTrue(key instanceof PrivateKey);
    final Certificate certificate =
        cachingKeyStoreWrapper.getCertificate(alias);
    Assert.assertNotNull(
        "No certificate for generated key pair could be found.", certificate);
  }
  ...
  private void testRemove(
    ...
    Assert.assertEquals(
        "Number of aliases should have decreased by 1 after removal.",
        aliasCountBefore - 1,
        getAliasCount(cachingKeyStoreWrapper));
    Assert.assertFalse(
        "Alias of removed key pair still exist.",
        isContainsAlias(cachingKeyStoreWrapper, alias));
  }
   ...
   */
  private int getAliasCount(final CachingKeyStoreWrapper cachingKeyStoreWrapper)
      throws KeyStoreException {
    final Enumeration<String> aliasEnumeration =
        cachingKeyStoreWrapper.aliases();
    int aliasCount = 0;
   ...
   * @throws KeyStoreException fail
   */
  private boolean isContainsAlias(
      final CachingKeyStoreWrapper cachingKeyStoreWrapper, final String alias)
      throws KeyStoreException {
    final Enumeration<String> aliasEnumeration =
        cachingKeyStoreWrapper.aliases();
