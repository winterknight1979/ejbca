...
public class ExternalScriptsWhitelistTest {
  ...
  public void testConstructionFromText() {
    final ExternalScriptsWhitelist whitelist =
        ExternalScriptsWhitelist.fromText("/foo/allowed\n#/foo/disabled");
    assertEquals(1, whitelist.size());
    assertTrue(
        "/foo/allowed should be permitted",
        whitelist.isPermitted("/foo/allowed"));
    assertFalse(
        "/foo/forbidden should not be permitted",
        whitelist.isPermitted("/foo/forbidden"));
    assertFalse(
        "Extension should be considered",
        whitelist.isPermitted("/foo/allowed.sh"));
    assertFalse(
  ...
  public void testConstructionFromPaths() {
    final ExternalScriptsWhitelist whitelist =
        new ExternalScriptsWhitelist("/foo/allowed");
    assertEquals(1, whitelist.size());
    assertTrue(
        "/foo/allowed should be permitted",
        whitelist.isPermitted("/foo/allowed"));
  ...
  public void testInvalidPaths() {
    final ExternalScriptsWhitelist whitelist =
        new ExternalScriptsWhitelist("/foo/invalid");
    assertEquals(1, whitelist.size());
    assertTrue(
        "/foo/invalid should not be a valid path", whitelist.hasInvalidPaths());
  }
}
