...
public class RsaKeyValidatorTest {
  ...
  public void testNoNegativeNumbers() throws Exception {
    ...
    RsaKeyValidator keyValidator =
        (RsaKeyValidator)
            ValidatorTestUtil.createKeyValidator(
                RsaKeyValidator.class,
                "rsa-parameter-validation-input_test",
                "Description",
                null,
                -1,
                null,
    ...
    Assert.assertEquals(
        "Test should not fail, validator field can not be set to a negative"
            + " value ",
        keyValidator.getPublicKeyExponentMin(),
        new BigInteger(numStringPos));
    ...
    Assert.assertEquals(
        "Test should not fail, validator field can not be set to a negative"
            + " value ",
        keyValidator.getPublicKeyExponentMax(),
        new BigInteger(numStringPos));
    ...
    Assert.assertEquals(
        "Test should not fail, validator field can not be set to a negative"
            + " value ",
        keyValidator.getPublicKeyModulusMin(),
        new BigInteger(numStringPos));
    ...
    Assert.assertEquals(
        "Test should not fail, validator field can not be set to a negative"
            + " value ",
        keyValidator.getPublicKeyModulusMax(),
        new BigInteger(numStringPos));
    // Test that a negative number can not be set for
    // setPublicKeyModulusMinFactor
  ...
  /**
   * Tests that it is not possible to set a smaller maximum exponent than
   * currently set minimum exponent and vice versa.
   *
   * @throws Exception Exception
   */
  @Test
  public void testPublicKeyExponentMinSmallerThanMax() throws Exception {
    ...
    RsaKeyValidator keyValidator =
        (RsaKeyValidator)
            ValidatorTestUtil.createKeyValidator(
                RsaKeyValidator.class,
                "rsa-parameter-set-min-smaller-max-test",
                "Description",
                null,
                -1,
                null,
    ...
    Assert.assertEquals(
        "It should be possible to set minimum exponent to 2 if maximum is null",
        keyValidator.getPublicKeyExponentMinAsString(),
        "2");
    Assert.assertEquals(
        "It should be possible to set maximum exponent to 3 if miniimum is 2",
    ...
    Assert.assertEquals(
        "It should not be possible to set maximum exponent to 1 if minimum is"
            + " 2",
        keyValidator.getPublicKeyExponentMaxAsString(),
        "3");
    ...
    Assert.assertEquals(
        "It should not be possible to set minimum exponent to 4 if maximum is"
            + " 3",
        keyValidator.getPublicKeyExponentMinAsString(),
        "2");
    ...
    Assert.assertEquals(
        "It should be possible to set minimum exponent to 3 if maximum is 3",
        keyValidator.getPublicKeyExponentMinAsString(),
        "3");
    Assert.assertEquals(
        "It should be possible to set maximum exponent to 5 if minimum is 3",
        keyValidator.getPublicKeyExponentMaxAsString(),
        "5");
    // Test possible to set same max as min.
    keyValidator.setPublicKeyExponentMaxAsString("3");
    Assert.assertEquals(
        "It should be possible to set maximum exponent to 3 if minimum is 3",
        keyValidator.getPublicKeyExponentMaxAsString(),
        "3");
  ...
  /**
   * Tests that it is not possible to set a smaller maximum modulus than
   * currently set minimum modulus and vice versa.
   *
   * @throws Exception Exception
   */
  @Test
  public void testPublicKeyModulusMinSmallerThanMax() throws Exception {
    ...
    Assert.assertEquals(
        "It should be possible to set minimum modulus to 20 if maximum is null",
        keyValidator.getPublicKeyModulusMinAsString(),
        "20");
    Assert.assertEquals(
        "It should be possible to set maximum modulus to 30 if miniimum is 20",
        keyValidator.getPublicKeyModulusMaxAsString(),
        "30");
    // Test not possible to set smaller max than min.
    keyValidator.setPublicKeyModulusMaxAsString("10");
    Assert.assertEquals(
        "It should not be possible to set maximum modulus to 10 if minimum is"
            + " 20",
        keyValidator.getPublicKeyModulusMaxAsString(),
        "30");
    // Test not possible to set larger min than max.
    ...
    Assert.assertEquals(
        "It should not be possible to set minimum modulus to 40 if maximum is"
            + " 30",
        keyValidator.getPublicKeyModulusMinAsString(),
        "20");
    // Test possible to set same min as max.
    ...
    Assert.assertEquals(
        "It should be possible to set minimum modulus to 30 if maximum is 30",
        keyValidator.getPublicKeyModulusMinAsString(),
        "30");
    Assert.assertEquals(
        "It should be possible to set maximum modulus to 50 if minimum is 30",
        keyValidator.getPublicKeyModulusMaxAsString(),
        "50");
    // Test possible to set same max as min.
    keyValidator.setPublicKeyModulusMaxAsString("30");
    Assert.assertEquals(
        "It should be possible to set maximum modulus to 30 if minimum is 30",
        keyValidator.getPublicKeyModulusMaxAsString(),
        "30");
  ...
  public void test01HasSmallerFactorThan() throws Exception {
    ...
    Assert.assertFalse(
        "Modulus 0 and factor 0 must evaluate to false.",
        RsaKeyValidator.hasSmallerFactorThan(modulus, 0));

    // Factor is smaller than modulus -> false;
    Assert.assertFalse(
        "Modulus 0 and factor 1 must evaluate to false.",
        RsaKeyValidator.hasSmallerFactorThan(modulus, 1));

    // Test both 1 -> false;
    modulus = new BigInteger("1");
    Assert.assertFalse(
        "Modulus 1 and factor 1 must evaluate to false.",
        RsaKeyValidator.hasSmallerFactorThan(modulus, 1));

    // Test both 2 -> false;
    modulus = new BigInteger("2");
    Assert.assertFalse(
        "Modulus 2 and factor 2 must evaluate to false.",
        RsaKeyValidator.hasSmallerFactorThan(modulus, 2));

    // All even numbers have the smallest factor 2 -> false;
    modulus = new BigInteger("12345678902");
    Assert.assertFalse(
        "Even modulus must evaluate to smallest factor 2.",
        RsaKeyValidator.hasSmallerFactorThan(modulus, 2));
    Assert.assertTrue(
        "Even modulus must evaluate to smallest factor 2.",
        RsaKeyValidator.hasSmallerFactorThan(modulus, 3));

    // Prime numbers smallest factor except 1 is itself.
    modulus = new BigInteger("3");
    Assert.assertTrue(
        "A primes smallest factor except 1 is itself.",
        RsaKeyValidator.hasSmallerFactorThan(modulus, 3));
    modulus = new BigInteger("123");
    Assert.assertTrue(
        "A primes smallest factor except 1 is itself.",
        RsaKeyValidator.hasSmallerFactorThan(modulus, 123));
    modulus = new BigInteger("9");
    Assert.assertTrue(
        "The smallest factor of 9 is 3.",
        RsaKeyValidator.hasSmallerFactorThan(modulus, 3));
    modulus = new BigInteger("27");
    Assert.assertTrue(
        "The smallest factor of 27 is 3.",
        RsaKeyValidator.hasSmallerFactorThan(modulus, 3));
    ...
    modulus =
        new BigInteger(
            "135253271074378184290126435546666091495057097246829408541196270645"
            + "2326454779246882256356515986758668088207855729438172373375571963"
            + "7864549756035138362832470653608048618045544899664698566789173834"
            + "6505771576771134762012536044972691177382786401341057394042017796"
            + "805414677173199794244010442024923795849646025390782");
    Assert.assertTrue(
    ...
    BigInteger modulus2048 =
        new BigInteger(
            "135253271074378184290126435546666091495057097246829408541196270645"
            + "2326454779246882256356515986758668088207855729438172373375571963"
            + "7864549756035138362832470653608048618045544899664698566789173834"
            + "6505771576771134762012536044972691177382786401341057394042017796"
            + "805414677173199794244010442024923795849646025390781");
    BigInteger modulus4096 =
        new BigInteger(
            "135253271074378184290126435546666091495057097246829408541196270645"
            + "2326454779246882256356515986758668088207855729438172373375571963"
            + "7864549756035138362832470653608048618045544899664698566789173834"
            + "6505771576771134762012536044972691177382786401341057394042017796"
            + "8054146771731997942440104420249237958496460253907811352532710743"
            + "7818429012643554666609149505709724682940854119627064523264547792"
            + "468822563565159867586680882078557294381723733755719637864549756"
            + "0351383628324706536080486180455448996646985667891738346505771576"
            + "7711347620125360449726911773827864013410573940420177968054146771"
            + "73199794244010442024923795849646025390781");
    BigInteger modulus8192 =
        new BigInteger(
            "135253271074378184290126435546666091495057097246829408541196270645"
            + "2326454779246882256356515986758668088207855729438172373375571963"
            + "7864549756035138362832470653608048618045544899664698566789173834"
            ...
            + "8054146771731997942440104420249237958496460253907811352532710743"
            + "7818429012643554666609149505709724682940854119627064523264547792"
            + "4688225635651598675866808820785572943817237337557196378645497560"
            + "3513836283247065360804861804554489966469856678917383465057715767"
            + "7113476201253604497269117738278640134105739404201779680541467717"
            + "3199794244010442024923795849646025390781135253271074378184290126"
            + "4355466660914950570972468294085411962706452326454779246882256356"
            + "5159867586680882078557294381723733755719637864549756035138362832"
            + "4706536080486180455448996646985667891738346505771576771134762012"
            + "5360449726911773827864013410573940420177968054146771731997942440"
            + "1044202492379584964602539078113525327107437818429012643554666609"
            + "1495057097246829408541196270645232645477924688225635651598675866"
            + "8088207855729438172373375571963786454975603513836283247065360804"
            + "8618045544899664698566789173834650577157677113476201253604497269"
            + "1177382786401341057394042017796805414677173199794244010442024923"
            + "795849646025390781");
  ...
  public void test03RsaParameterValidations() throws Exception {
    ...
    RsaKeyValidator keyValidator =
        (RsaKeyValidator)
            ValidatorTestUtil.createKeyValidator(
                RsaKeyValidator.class,
                "rsa-parameter-validation-test-1",
                "Description",
                null,
                -1,
                null,
    ...
    Assert.assertTrue(
        ...
        messages.size() == 4);
    Assert.assertEquals(
        "RSA parameters bounds failure message isn't right",
        "Invalid: RSA public key exponent is odd.",
        messages.get(0));
    Assert.assertEquals(
        "RSA parameters bounds failure message isn't right",
        "Invalid: RSA public key exponent is smaller than 5",
        messages.get(1));
    Assert.assertEquals(
        "RSA parameters bounds failure message isn't right",
        "Invalid: RSA public key modulus is odd.",
        messages.get(2));
    Assert.assertEquals(
        "RSA parameters bounds failure message isn't right",
        "Invalid: RSA public key modulus is smaller than 17",
        messages.get(3));
    ...
    Assert.assertEquals(
        "RSA parameters bounds failure message isn't right",
        "Invalid: RSA public key exponent is greater than 3",
        keyValidator.validate(publicKey, null).get(1));
    Assert.assertEquals(
        "RSA parameters bounds failure message isn't right",
        "Invalid: RSA public key modulus is greater than 15",
        keyValidator.validate(publicKey, null).get(3));

    // A-3: Test RSA key validation failed because of modulus factor
    // restriction.
    ...
    Assert.assertTrue(
        ...
        messages.size() == 1);
    Assert.assertEquals(
        "smallest factor failure message isn't right",
        "Invalid: RSA public key modulus smallest factor is less than 6",
        messages.get(0));

    // A-4: Test RSA key validation failed because of modulus power of prime
    // restriction.
    ...
    Assert.assertTrue(
        ...
        messages.size() == 1);
    Assert.assertEquals(
        "Power of prime failure message isn't right.",
  ...
  /**
   * Tests public key validation for the ROCA vulnerable key generation.
   * CVE-2017-15361
   *
   ...
   */
  @Test
  public void testRocaWeakKeys()
      throws CertificateParsingException, InstantiationException,
    ...
    X509Certificate roca =
        CertTools.getCertfromByteArray(rocaCert, X509Certificate.class);
