 ...
 *
 * @version $Id: KeyToolsTest.java 22633 2016-01-22 13:01:31Z jeklund $
 */
 ...
 * @throws Exception fail
  ...
  public void testGenKeysECDSAx9() throws Exception {
    ...
    assertTrue(
        "Private key is not encoded with named curves, but using explicit"
            + " parameters",
        X962Parameters.getInstance(
                priv2.getPrivateKeyAlgorithm().getParameters())
            .isNamedCurve());
    SubjectPublicKeyInfo pub2 =
        SubjectPublicKeyInfo.getInstance(keys.getPublic().getEncoded());
    assertTrue(
        "Public key is not encoded with named curves, but using explicit"
            + " parameters",
        X962Parameters.getInstance(pub2.getAlgorithm().getParameters())
            .isNamedCurve());
 ...
 * @throws Exception fail
  ...
  public void testGetECDSACvcPubKeyParams() throws Exception {
    ...
    // with domain parameters from either another EC public key or from the
    // curve name
  ...
  private CVCertificate createCVTestCertificate(
 ...
 * @throws Exception fail
  ...
  public void testGenKeysGOSTAlgorithmSpec() throws Exception {
    ...
    assertEquals(
        AlgorithmConstants.KEYALGORITHM_ECGOST3410,
        keys2.getPublic().getAlgorithm());
    assertEquals(
        AlgorithmConstants.KEYALGORITHM_ECGOST3410,
        keys3.getPublic().getAlgorithm());
    ...
    KeyFactory.getInstance("ECGOST3410")
 ...
 * @throws Exception fail
  ...
  public void testGenKeysDSTU4145AlgorithmSpec() throws Exception {
    ...
    ECPublicKeySpec ecspec = new ECPublicKeySpec(pk2.getW(), pk2.getParams());
    KeyFactory.getInstance("DSTU4145")
        .generatePublic(ecspec); // return value not tested
    KeyFactory.getInstance("EC")
        .generatePublic(ecspec); // return value not tested
 ...
 * @throws Exception fail
  ...
  public void testNotBlackListedEcCurves() throws Exception {
    ...
    assertTrue(
        "Failed to generate EC key pair using " + sb.toString(),
        sb.length() == 0);
  }
}
