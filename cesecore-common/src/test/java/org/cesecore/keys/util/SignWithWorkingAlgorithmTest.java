 ...
 *
 * @version $Id: SignWithWorkingAlgorithmTest.java 26055 2017-06-22 07:36:41Z
 *     anatom $
 */
@FixMethodOrder(MethodSorters.NAME_ASCENDING)
public class SignWithWorkingAlgorithmTest {
  /**
   * This list starts on purpose with long hashing algorithms that will not be
   * handled by a 512bit key. Since first test is the 512 bit key some
   * algorithms must be tried before finding the working one. If the test should
   * have started with 2048 bit key then the signing with 512 should have failed
   * since the chosen algorithm had to long hash for this key. So in real life
   * the list should start with the short algorithms.
   */
  ...
  private class SignOperation implements ISignOperation {
    ...
    @Override
    public void taskWithSigning(final String algorithm, final Provider provider)
    ...
    public boolean verifySignature() throws GeneralSecurityException {
      final Signature signature =
          Signature.getInstance(
              this.usedAlgorithm, BouncyCastleProvider.PROVIDER_NAME);
  ...
  private static KeyPair generateKeyPair(final int size)
  ...
  @Test
  public void n1BC512()
      throws NoSuchProviderException, GeneralSecurityException,
    ...
    assertTrue(
        SignWithWorkingAlgorithm.doSignTask(
            SIG_ALGS_RSA, BouncyCastleProvider.PROVIDER_NAME, operation));
    assertTrue(operation.verifySignature());
    assertEquals(5, operation.getNrOfCalls());
    assertEquals(
        AlgorithmConstants.SIGALG_SHA1_WITH_RSA_AND_MGF1,
        operation.getUsedAlgorithm());
  }
  /**
  ...
  /**
   * The provider SunRsaSign is not supporting MGF1 so we have to try one more
   * time.
   *
   * @throws NoSuchProviderException fail
   * @throws GeneralSecurityException fail
   ...
   */
  @Test
  public void n3SunRsaSign512()
      throws NoSuchProviderException, GeneralSecurityException,
          TaskWithSigningException {
    final SignOperation operation = new SignOperation(generateKeyPair(512));
    assertTrue(
        SignWithWorkingAlgorithm.doSignTask(
            SIG_ALGS_RSA, "SunRsaSign", operation));
    assertTrue(operation.verifySignature());
  ...
  /**
   * Just checking that right algorithm is stilled picked for the SunRsaSign
   * provider.
   *
   * @throws NoSuchProviderException fail
   * @throws GeneralSecurityException fail
