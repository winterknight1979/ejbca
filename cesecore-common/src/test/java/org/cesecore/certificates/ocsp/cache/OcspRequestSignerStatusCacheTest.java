...
/**
 * Test of the cache that hold revocation information for certificates that
 * signs OCSP requests.
 *
 * @version $Id: OcspRequestSignerStatusCacheTest.java 22253 2015-11-24
 *     13:41:45Z jeklund $
 */
public class OcspRequestSignerStatusCacheTest {
  ...
  public void testCacheDisabled() throws Exception {
    ...
    final String key1 =
        OcspRequestSignerStatusCache.INSTANCE.createCacheLookupKey(
            "CN=Test1", new BigInteger("1"));
    assertNull(
        "Cache should be empty from start.",
    ...
    OcspRequestSignerStatusCache.INSTANCE.updateCachedCertificateStatus(
        key1, CertificateStatus.NOT_AVAILABLE);
    assertNull(
        "Cache entry should have been expired.",
        OcspRequestSignerStatusCache.INSTANCE.getCachedCertificateStatus(key1));
    OcspRequestSignerStatusCache.INSTANCE.updateCachedCertificateStatus(
        key1, CertificateStatus.OK);
    assertNull(
        "Cache entry should have been expired.",
        OcspRequestSignerStatusCache.INSTANCE.getCachedCertificateStatus(key1));
    OcspRequestSignerStatusCache.INSTANCE.updateCachedCertificateStatus(
        key1, CertificateStatus.REVOKED);
    assertNull(
        "Cache entry should have been expired.",
        OcspRequestSignerStatusCache.INSTANCE.getCachedCertificateStatus(key1));
  }
  ...
  public void testCacheExpired() throws Exception {
    ...
    OcspRequestSignerStatusCache.INSTANCE.updateCachedCertificateStatus(
        key1, CertificateStatus.NOT_AVAILABLE);
    assertEquals(
        "Cache should have returned non-expired entry.",
        OcspRequestSignerStatusCache.INSTANCE.getCachedCertificateStatus(key1),
        CertificateStatus.NOT_AVAILABLE);
    Thread.sleep(1000);
    // First call should return null to signal to the caller that it should
    // update the cache
    assertNull(
        "Cache entry should have been expired.",
        OcspRequestSignerStatusCache.INSTANCE.getCachedCertificateStatus(key1));
    // Second call should return the stale data, since it assumed that the
    // previous caller will update the cache
    assertEquals(
        "Cache should have returned stale entry.",
        OcspRequestSignerStatusCache.INSTANCE.getCachedCertificateStatus(key1),
        CertificateStatus.NOT_AVAILABLE);
  }
  ...
  public void testCacheEnabled() throws Exception {
    ...
    final String key2 =
        OcspRequestSignerStatusCache.INSTANCE.createCacheLookupKey(
            "CN=Test2", new BigInteger("2"));
    final String key3 =
        OcspRequestSignerStatusCache.INSTANCE.createCacheLookupKey(
            "CN=Test2", new BigInteger("3"));
    final String key4 =
        OcspRequestSignerStatusCache.INSTANCE.createCacheLookupKey(
            "CN=Test3", new BigInteger("1"));
    assertNull(
        "Cache should be empty from start.",
        OcspRequestSignerStatusCache.INSTANCE.getCachedCertificateStatus(key1));
    assertNull(
        "Cache should be empty from start.",
        OcspRequestSignerStatusCache.INSTANCE.getCachedCertificateStatus(key2));
    assertNull(
        "Cache should be empty from start.",
        OcspRequestSignerStatusCache.INSTANCE.getCachedCertificateStatus(key3));
    assertNull(
        "Cache should be empty from start.",
        OcspRequestSignerStatusCache.INSTANCE.getCachedCertificateStatus(key4));
    OcspRequestSignerStatusCache.INSTANCE.updateCachedCertificateStatus(
        key1, CertificateStatus.NOT_AVAILABLE);
    OcspRequestSignerStatusCache.INSTANCE.updateCachedCertificateStatus(
        key2, CertificateStatus.OK);
    OcspRequestSignerStatusCache.INSTANCE.updateCachedCertificateStatus(
        key3, CertificateStatus.REVOKED);
    OcspRequestSignerStatusCache.INSTANCE.updateCachedCertificateStatus(
        key4, CertificateStatus.OK);
    assertEquals(
        "Cache should have returned non-expired entry.",
        OcspRequestSignerStatusCache.INSTANCE.getCachedCertificateStatus(key1),
        CertificateStatus.NOT_AVAILABLE);
