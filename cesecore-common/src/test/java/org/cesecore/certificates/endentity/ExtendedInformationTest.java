 ...
 *
 * @version $Id: ExtendedInformationTest.java 27576 2017-12-18 15:04:00Z
 *     bastianf $
 */
public class ExtendedInformationTest {
  ...
  public void testSetGetCopyFields() {
    ...
    final BigInteger certificateSerialNumber =
        new BigInteger("13813134560128420686215030629780757563");
    ...
    extendedInformation.setExtensionData(
        customExtensionDataKey, customExtensionDataValue);
    ...
    extendedInformation.setSubjectDirectoryAttributes(
        subjectDirectoryAttributes);

    assertEquals(
        approvalRequestId,
        extendedInformation.getAddEndEntityApprovalRequestId());
    assertEquals(
        certificateEndTime, extendedInformation.getCertificateEndTime());
    assertArrayEquals(
        certificateRequest, extendedInformation.getCertificateRequest());
    assertEquals(
        certificateSerialNumber, extendedInformation.certificateSerialNumber());
    assertEquals(
        customDataValue, extendedInformation.getCustomData(customDataKey));
    assertEquals(
        customExtensionDataValue,
        extendedInformation.getExtensionData(customExtensionDataKey));
    assertEquals(
        issuanceRevocationReason,
        (Integer) extendedInformation.getIssuanceRevocationReason());
    assertEquals(
        keyStoreSubAlgorithmType,
        extendedInformation.getKeyStoreAlgorithmSubType());
    assertEquals(
        keyStoreAlgorithm, extendedInformation.getKeyStoreAlgorithmType());
    assertEquals(mapDataValue, extendedInformation.getMapData(mapDataKey));
    assertEquals(
        maxLoginAttempts, (Integer) extendedInformation.getMaxLoginAttempts());
    assertEquals(
        nameConstraintsExcluded,
        extendedInformation.getNameConstraintsExcluded());
    assertEquals(
        nameConstraintsPermitted,
        extendedInformation.getNameConstraintsPermitted());
    assertEquals(
        remainingLoginAttempts,
        (Integer) extendedInformation.getRemainingLoginAttempts());
    assertEquals(
        subjectDirectoryAttributes,
        extendedInformation.getSubjectDirectoryAttributes());

    final ExtendedInformation extendedInformation2 =
        new ExtendedInformation(extendedInformation);
    assertEquals(
        approvalRequestId,
        extendedInformation2.getAddEndEntityApprovalRequestId());
    assertEquals(
        certificateEndTime, extendedInformation2.getCertificateEndTime());
    assertArrayEquals(
        certificateRequest, extendedInformation2.getCertificateRequest());
    assertEquals(
        certificateSerialNumber,
        extendedInformation2.certificateSerialNumber());
    assertEquals(
        customDataValue, extendedInformation2.getCustomData(customDataKey));
    assertEquals(
        customExtensionDataValue,
        extendedInformation2.getExtensionData(customExtensionDataKey));
    assertEquals(
        issuanceRevocationReason,
        (Integer) extendedInformation2.getIssuanceRevocationReason());
    assertEquals(
        keyStoreSubAlgorithmType,
        extendedInformation2.getKeyStoreAlgorithmSubType());
    assertEquals(
        keyStoreAlgorithm, extendedInformation2.getKeyStoreAlgorithmType());
    assertEquals(mapDataValue, extendedInformation2.getMapData(mapDataKey));
    assertEquals(
        maxLoginAttempts, (Integer) extendedInformation2.getMaxLoginAttempts());
    assertEquals(
        nameConstraintsExcluded,
        extendedInformation2.getNameConstraintsExcluded());
    assertEquals(
        nameConstraintsPermitted,
        extendedInformation2.getNameConstraintsPermitted());
    assertEquals(
        remainingLoginAttempts,
        (Integer) extendedInformation2.getRemainingLoginAttempts());
    assertEquals(
        subjectDirectoryAttributes,
  ...
  public void testIntegerFieldsAsStrings() {
    ...
    extendedInformation
        .getRawData()
        .put("remainingloginattempts", remainingLoginAttempts);
    extendedInformation
        .getRawData()
        .put("maxfailedloginattempts", maxFailedLoginAttempts);
    assertEquals(
        Integer.valueOf(remainingLoginAttempts),
        (Integer) extendedInformation.getRemainingLoginAttempts());
    assertEquals(
        Integer.valueOf(maxFailedLoginAttempts),
        (Integer) extendedInformation.getMaxLoginAttempts());
  }
}
