...
/**
 * Unit test of {@link AvailableCustomCertificateExtensionsConfiguration}. see
 * org.cesecore.certificates.certificate.AvailableCustomCertExtensionsConfigTest
 * AvailableCustomCertExtensionsConfigTest (related system test)
 *
 * @version $Id: AvailableCustomCertificateExtensionsConfigurationUnitTest.java
 *     30621 2018-11-26 09:44:13Z samuellb $
 */
public class AvailableCustomCertificateExtensionsConfigurationUnitTest {
  ...
  /**
   * Tests the method that is used for parsing old certextensions.properties
   * files from EJBCA 6.3.x and earlier.
   *
  ...
  public void testUpgradeParsing() throws Exception {
    ...
    oldFileProperties.put(
        "id123.property.value", "aBcD"); // value did not allow padding
    ...
    assertTrue(
        "'Required' is the default. Was false.", upgraded.isRequiredFlag());
    assertFalse("Wrong value of 'Critical'", upgraded.isCriticalFlag());
    assertEquals(
        "Wrong value of 'translatable' property",
        true,
        upgraded.getProperties().get("translatable"));
    final EndEntityInformation dummyEndEntity = new EndEntityInformation();
    dummyEndEntity.setDN(
        "CN=Dummy AvailableCustomCertificateExtensionsConfigurationUnitTest");
    final byte[] valueEncoded =
        upgraded.getValueEncoded(
            dummyEndEntity,
            null,
            new CertificateProfile(), /*userPublicKey*/
            null, /*caPublicKey*/
            null, /*validity*/
            null);
    assertArrayEquals(
