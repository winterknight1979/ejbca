...
public class QcStatementTest {
  ...
  public void testQcStatement()
    ...
    // QC ETSI type from eIDAS EN 319 412-5, eSign = 0.4.0.1862.1.6.1, eseal =
    // 0.4.0.1862.1.6.2, web = 0.4.0.1862.1.6.3
    prof.setQCEtsiType("0.4.0.1862.1.6.1");
    prof.setQCEtsiPds(
        Arrays.asList(
            new PKIDisclosureStatement("http://qcs.localhost/QcPDS", "en")));
    // id-qcs-pkixQCSyntax-v2 (OID = 1.3.6.1.55.7.11.2) with SematicsID =
    // 0.4.0.194121.1.2
    ...
    byte[] value =
        statement.getValueEncoded(null, null, prof, null, null, null);
    @SuppressWarnings("resource")
    final String dump =
        ASN1Dump.dumpAsString(new ASN1InputStream(value).readObject(), true);
    log.info(dump);
    ...
    // Dump included IDs
    // This is just a loop to get all the statement IDs in the QcStatements
    // extension, so we can view them and count them
    ...
    assertTrue(
        oids.contains(
            ETSIQCObjectIdentifiers.id_etsi_qcs_QcCompliance.getId()));
    assertTrue(
        oids.contains(ETSIQCObjectIdentifiers.id_etsi_qcs_QcSSCD.getId()));
    assertTrue(
        oids.contains(
            RFC3739QCObjectIdentifiers.id_qcs_pkixQCSyntax_v2.getId()));
    assertTrue(
        oids.contains(
            "0.4.0.1862.1.6")); // ETSIQCObjectIdentifiers.id_etsi_qcs_QcType
    assertTrue(
        oids.contains(
            "0.4.0.1862.1.5")); // ETSIQCObjectIdentifiers.id_etsi_qcs_QcPds
    // Check the values we set
    assertEquals(
        "0.4.0.1862.1.6.1",
        QCStatementExtension.getStatementStringValue(seq, "0.4.0.1862.1.6", 0));
    assertEquals(
        "[http://qcs.localhost/QcPDS, en]",
