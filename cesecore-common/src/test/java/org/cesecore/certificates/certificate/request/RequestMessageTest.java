 ...
 */
  ...
  public void test01Pkcs10RequestMessage()
    ...
    PKCS10RequestMessage msg =
        new PKCS10RequestMessage(basicpkcs10.toASN1Structure().getEncoded());
    ...
    assertEquals("Test", username);
    assertEquals(
        "CN=Test,unstructuredName=AttrValue1 AttrValue2", msg.getRequestDN());
    ...
    assertEquals("Test", username);
    assertEquals(
        "CN=Test+1.2.840.113549.1.9.3=AttrValue1 AttrValue2",
        msg.getRequestDN());
    ...
    dn =
        new X500Name(
            "1.2.840.113549.1.9.3=AttrValue1 AttrValue2+CN=Test+O=abc");
    basicpkcs10 =
        CertTools.genPKCS10CertificationRequest(
            "SHA1WithRSA",
            dn,
  ...
  public void testOpenSSLChallengepassword() {
    ...
    assertEquals(
        "Username from P10 message is not what we expect",
        "p10test",
        msg1.getUsername());
    assertEquals(
        "Challenge password (UTF8 encoded) from P10 message is not what we"
            + " expect",
        "fSREp8pnxtx3CuV",
        msg1.getPassword());
    ...
    assertEquals(
        ...
        msg2.getUsername());
    assertEquals(
        "Challenge password (PrintableString encoded) from P10 message is not"
            + " what we expect",
        "fSREp8pnxtx3CuVLBjyfuXJAbImBuP",
        msg2.getPassword());
    ...
    assertEquals(
        ...
        msg3.getUsername());
    assertEquals(
        "Challenge password (IA5String encoded which is invalid according to"
            + " the standard but we handle it anyway) from P10 message is not"
            + " what we expect",
        "Wgi_L-O,gTkfFgE.uKB9T?XIZ9tW7E",
        msg3.getPassword());
  }
  ...
  private PKCS10CertificationRequest createP10(final String subjectDN)
    ...
    PKCS10CertificationRequest basicpkcs10 =
        CertTools.genPKCS10CertificationRequest(
            "SHA1WithRSA",
            dn,
            keyPair.getPublic(),
