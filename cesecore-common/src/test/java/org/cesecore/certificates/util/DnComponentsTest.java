...
public class DnComponentsTest {
  ...
  public void test01CheckObjects() throws Exception {
    ...
    assertEquals(
        "2.5.4.97", DnComponents.getOid("organizationidentifier").toString());
    assertEquals("2.5.4.13", DnComponents.getOid("description").toString());
    assertEquals(
        "2.5.4.13",
        DnComponents.getOid("DeScRiPtIoN").toString()); // case insensitive
    assertEquals(
        "1.3.6.1.4.1.311.60.2.1.3",
        DnComponents.getOid("jurisdictioncountry").toString());
    assertEquals(
        "1.3.6.1.4.1.311.60.2.1.2",
        DnComponents.getOid("jurisdictionstate").toString());
    assertEquals(
        "1.3.6.1.4.1.311.60.2.1.1",
        DnComponents.getOid("jurisdictionlocality").toString());
    ...
    assertEquals(
        17,
        (int)
            DnComponents.getDnIdFromAltName(
                "rfc822name")); // should be case insensitive
    assertEquals(
  ...
  public void testEnterpriseProperties() {
    ...
    assertEquals(
        "JURISDICTIONLOCALITY=", DnComponents.getDnExtractorFieldFromDnId(103));
    assertEquals(
        "JURISDICTIONSTATE=", DnComponents.getDnExtractorFieldFromDnId(104));
    assertEquals(
        "JURISDICTIONCOUNTRY=", DnComponents.getDnExtractorFieldFromDnId(105));
    assertEquals(
  ...
  public void test02() {
    String dn =
        CertTools.stringToBCDNString("uri=fff,CN=oid,SN=12345,NAME=name,C=se");
    ...
    assertEquals(
        "description=test,CN=oid,Name=name,SN=12345,SURNAME=Json,C=se", dn1);
