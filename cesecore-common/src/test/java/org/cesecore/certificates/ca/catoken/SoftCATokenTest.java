...
/** Constructor. */
  ...
  public void testDefaultEjbcaSoftTokenProperties() throws Exception {
    ...
    assertEquals(
        "crypto token status should be active after key generation",
  ...
  public void testTokenStatusDifferentAliases() throws Exception {
    ...
    caTokenProperties.setProperty(
        CATokenConstants.CAKEYPURPOSE_CERTSIGN_STRING, "signAlias");
    caTokenProperties.setProperty(
        CATokenConstants.CAKEYPURPOSE_CRLSIGN_STRING, "signAlias");
    caTokenProperties.setProperty(
        CATokenConstants.CAKEYPURPOSE_TESTKEY_STRING, "testAlias");
    caTokenProperties.setProperty(
        CATokenConstants.CAKEYPURPOSE_DEFAULT_STRING, ENCRYPTION_KEY);
    caTokenProperties.setProperty(
        CATokenConstants.CAKEYPURPOSE_CERTSIGN_STRING_PREVIOUS, "deletedAlias");
    caTokenProperties.setProperty(
        CATokenConstants.CAKEYPURPOSE_CERTSIGN_STRING_NEXT,
        "anotherDeletedAlias");
    final CAToken catoken = new CAToken(cryptoToken.getId(), caTokenProperties);
    assertEquals(
        CryptoToken.STATUS_OFFLINE, catoken.getTokenStatus(false, cryptoToken));
    assertEquals(
        CryptoToken.STATUS_OFFLINE, catoken.getTokenStatus(true, cryptoToken));
    ...
    assertEquals(
        CryptoToken.STATUS_ACTIVE, catoken.getTokenStatus(false, cryptoToken));
    assertEquals(
  ...
  public void testTokenStatusCommonAliases() throws Exception {
    ...
    caTokenProperties.setProperty(
        CATokenConstants.CAKEYPURPOSE_CRLSIGN_STRING, "signAlias");
    caTokenProperties.setProperty(
        CATokenConstants.CAKEYPURPOSE_TESTKEY_STRING, "signAlias");
    caTokenProperties.setProperty(
        CATokenConstants.CAKEYPURPOSE_DEFAULT_STRING, "signAlias");
    caTokenProperties.setProperty(
        CATokenConstants.CAKEYPURPOSE_CERTSIGN_STRING_PREVIOUS, "signAlias");
    caTokenProperties.setProperty(
        CATokenConstants.CAKEYPURPOSE_CERTSIGN_STRING_NEXT, "signAlias");
    final CAToken catoken = new CAToken(cryptoToken.getId(), caTokenProperties);
    assertEquals(
        CryptoToken.STATUS_OFFLINE, catoken.getTokenStatus(false, cryptoToken));
    assertEquals(
  ...
  /**
   * When nodefaultpwd == true, the property SoftCryptoToken.NODEFAULTPWD is set
   * in order to avoid trying to use default pwd, if pwd is not specified. Also
   * the auto activation pin is not set when nodefaultpwd == false.
   *
  ...
  private CryptoToken createSoftToken(final boolean useAutoActivationPin) {
    ...
    if (useAutoActivationPin) {
      cryptoTokenProperties.setProperty(
          CryptoToken.AUTOACTIVATE_PIN_PROPERTY, TOKEN_PIN);
    }
  ...
  private Properties getCaTokenProperties(final String signAlias) {
    ...
    caTokenProperties.setProperty(
        ...
        signAlias); // does not exist and never will, will be moved to new keys
    caTokenProperties.setProperty(
        CATokenConstants.CAKEYPURPOSE_CRLSIGN_STRING,
        signAlias); // does not exist and never will, will be moved to new keys
    caTokenProperties.setProperty(
        CATokenConstants.CAKEYPURPOSE_DEFAULT_STRING,
        ENCRYPTION_KEY); // does not exist but will soon, after first generate
