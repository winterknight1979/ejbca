 ...
 * @throws InvalidAlgorithmParameterException fail
  ...
  protected void doCaTokenRSA(
    ...
    try {
      ...
      // Try to delete something that does not exist, it should work without
      // error
      cryptoToken.deleteEntry("sdkfjhsdkfjhsd777");
      cryptoToken.deactivate();
      // We have no keys generated according to the labels above, so the status
      // will be offline
      assertEquals(
          "Expected CryptoToken to be offline.",
      ...
      // Generate the first key, will get name of the certsign property when
      // generating with renew=false
      ...
      assertEquals(
          null, p.getProperty(CATokenConstants.CAKEYPURPOSE_KEYENCRYPT_STRING));
      assertEquals(
          "Expected to use default key.",
          ENCRYPTION_KEY,
          catoken.getAliasFromPurpose(
              CATokenConstants.CAKEYPURPOSE_KEYENCRYPT));
      assertEquals(
          CAToken.DEFAULT_KEYSEQUENCE,
          p.getProperty(CATokenConstants.PREVIOUS_SEQUENCE_PROPERTY));
      assertEquals(
          "rsatest0000" + (seq + 1),
          p.getProperty(CATokenConstants.CAKEYPURPOSE_CERTSIGN_STRING));
      assertEquals(
          "rsatest0000" + (seq + 1),
          p.getProperty(CATokenConstants.CAKEYPURPOSE_CRLSIGN_STRING));
      assertEquals(
          ENCRYPTION_KEY,
          p.getProperty(CATokenConstants.CAKEYPURPOSE_DEFAULT_STRING));
      assertEquals(
          "rsatest0000" + (seq),
          p.getProperty(
      ...
      // When generating keys with renew = false, we generate initial keys,
      // which means generating the key aliases
      // we have specified for signature and encryption keys
      // After this all needed CAToken keys are generated and status will be
      // active
      assertEquals(CryptoToken.STATUS_ACTIVE, cryptoToken.getTokenStatus());
      PrivateKey priv =
      ...
      try {
        ...
        assertTrue(
            "Should have thrown because the key should not exist", false);
      ...
      final PublicKey currentSingKey =
          cryptoToken.getPublicKey(
              catoken.getAliasFromPurpose(
                  CATokenConstants.CAKEYPURPOSE_CERTSIGN));
      ...
      catoken.activateNextSignKey();
      // Now we move away the rsatest00001 key alias from our mappings, so we
      // are now active
      assertEquals(CryptoToken.STATUS_ACTIVE, cryptoToken.getTokenStatus());
      ...
      assertEquals(
          ...
          p.getProperty(
              CATokenConstants.CAKEYPURPOSE_CERTSIGN_STRING_PREVIOUS));
      assertNull(
          p.getProperty(CATokenConstants.CAKEYPURPOSE_KEYENCRYPT_STRING));
      ...
      // Generate new keys, not activating them, this should create a "next
      // key", keeping the current and previous as they are
      // Generate new keys, moving the old ones to "previous key"
      final String nextSignKeyAlias2 = catoken.generateNextSignKeyAlias();
      final PublicKey currentSingKey2 =
          cryptoToken.getPublicKey(
              catoken.getAliasFromPurpose(
                  CATokenConstants.CAKEYPURPOSE_CERTSIGN));
      final String keySpec2 =
          AlgorithmTools.getKeySpecification(currentSingKey2);
      cryptoToken.generateKeyPair(keySpec2, nextSignKeyAlias2);

      p = catoken.getProperties();
      String previousSequence2 =
          p.getProperty(CATokenConstants.PREVIOUS_SEQUENCE_PROPERTY);
      assertEquals(previousSequence, previousSequence2);
      // Now sequence should still be 2, generated but did not activate the new
      // keys
      assertEquals(seq, Integer.valueOf(catoken.getKeySequence()));
      priv =
          cryptoToken.getPrivateKey(
      ...
      String nextSequence =
          p.getProperty(CATokenConstants.NEXT_SEQUENCE_PROPERTY);
      ...
      // Make sure the properties was set correctly so we did not get the
      // "default" key as next
      priv =
          cryptoToken.getPrivateKey(
              catoken.getAliasFromPurpose(
      ...
      // finally activate the "next key" moving that to current and moving the
      // current to previous
      catoken.activateNextSignKey();
      p = catoken.getProperties();
      String previousSequence3 =
          p.getProperty(CATokenConstants.PREVIOUS_SEQUENCE_PROPERTY);
      // The former active sequence (2) should have been moved to "previous
      // sequence" now
      ...
      // Next should now return the encryption key instead, since it is the
      // default
      priv =
          cryptoToken.getPrivateKey(
              catoken.getAliasFromPurpose(
  ...
  protected void doCaTokenDSA(
    ...
    seq += 1;
    assertEquals(seq, Integer.valueOf(catoken.getKeySequence()));
    // When generating keys with renew = false, we generate initial keys, which
    // means generating the key aliases
    // we have specified for signature and encryption keys
    // After this all needed CAToken keys are generated and status will be
    // active
    assertEquals(CryptoToken.STATUS_ACTIVE, cryptoToken.getTokenStatus());
    ...
    // The encryption key is always RSA and we generate it as 1024 bits to speed
    // up the test
    PrivateKey privenc =
        cryptoToken.getPrivateKey(
            catoken.getAliasFromPurpose(
  ...
  protected void doCaTokenECC(
    ...
    try {
      ...
      // Generate the first key, will get name rsatest+nextsequence =
      // rsatest00001
      Integer seq = Integer.valueOf(CAToken.DEFAULT_KEYSEQUENCE);
      cryptoToken.generateKeyPair("1024", ENCRYPTION_KEY);
      final String firstSignKeyAlias = catoken.generateNextSignKeyAlias();
      ...
      // renew = false
      // Encryption key should be an RSA key with 2048 bit, since signature key
      // is ECDSA
      PublicKey encPub =
          cryptoToken.getPublicKey(
              catoken.getAliasFromPurpose(
      ...
      final String keySpec2 =
          AlgorithmTools.getKeySpecification(currentSingKey2);
      LOG.debug(
          "currentSingKey2: " + currentSingKey2 + " keySpec2: " + keySpec2);
      cryptoToken.generateKeyPair(keySpec2, nextSignKeyAlias2);
      catoken.activateNextSignKey();
      Properties p = catoken.getProperties();
      String previousSequence =
      ...
      final PublicKey currentSingKey3 =
          cryptoToken.getPublicKey(
              catoken.getAliasFromPurpose(
                  CATokenConstants.CAKEYPURPOSE_CERTSIGN));
      final String keySpec3 =
          AlgorithmTools.getKeySpecification(currentSingKey3);
  ...
  protected void doActivateDeactivate(
    ...
    try {
      ...
      // We have not set auto activate, so the internal key storage in
      ...
      try {
        ...
        assert (e.getMessage().equals(strsoft)
  ...
  protected void doSaveAndLoad(
    ...
    try {
      ...
      KeyUtil.testKey(
          ...
          cryptoToken.getSignProviderName());
      String newkeyhash =
          CertTools.getFingerprintAsString(newPublicKey.getEncoded());
