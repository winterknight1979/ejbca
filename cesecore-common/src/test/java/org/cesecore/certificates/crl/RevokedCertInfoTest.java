...
public class RevokedCertInfoTest {
  ...
  private static RevokedCertInfo revCertInfo(
    ...
    return new RevokedCertInfo(
        new byte[] {1, 2, 3, 4},
        BigInteger.valueOf(0x1234ABCDL).toByteArray(),
        date(revocationDate),
        reason.getDatabaseValue(),
        date("2017-12-31"));
  }
  ...
  private static void assertRCIEquals(
      final String message,
      final RevokedCertInfo expected,
      final RevokedCertInfo actual) {
    assertEquals(
  ...
  public void mergeEmpty() {
    ...
    final CompressedCollection<RevokedCertInfo> b =
        new CompressedCollection<>(RevokedCertInfo.class);
    assertSame(
        "Empty 'b' collection should cause 'a' to be returned.",
        a,
  ...
  public void mergeWithDuplicates() {
    ...
    assertRCIEquals(
        "Should contain entry REVINFO_4_UNSPECIFIED.",
  ...
  public void mergeWithDuplicates2() {
    ...
    b.add(REVINFO_5_REMOVEFROMCRL);
    final Collection<RevokedCertInfo> res =
        RevokedCertInfo.mergeByDateAndStatus(a, b, System.currentTimeMillis());
    assertEquals("Items should have been de-duplicated.", 1, res.size());
    assertRCIEquals(
        "Should contain entry REVINFO_4_UNSPECIFIED.",
        REVINFO_4_UNSPECIFIED,
  ...
  public void mergeWithDuplicates3() {
    ...
    assertEquals("Items should have been de-duplicated.", 1, res.size());
    assertRCIEquals(
        "Should contain entry REVINFO_2_ONHOLD.",
        REVINFO_2_ONHOLD,
        res.iterator().next());
  }
  ...
  public void mergeWithDuplicates4() {
    ...
    assertEquals("Items should have been de-duplicated.", 1, res.size());
    assertRCIEquals(
        "Should contain entry REVINFO_3_UNSPECIFIED.",
  ...
  public void mergeWithRemoveFromCrl1() {
    ...
    Collection<RevokedCertInfo> res =
        RevokedCertInfo.mergeByDateAndStatus(a, b, 0);
    ...
    assertEquals(
        "REMOVEFROMCRL should not be removed when Base CRL is older than"
            + " revocation date.",
        1,
        res.size());
    assertRCIEquals(
        "Should contain entry REVINFO_5_REMOVEFROMCRL",
        REVINFO_5_REMOVEFROMCRL,
        res.iterator().next());

    res = RevokedCertInfo.mergeByDateAndStatus(a, b, date("2017-12-31"));
    assertEquals(
        "REMOVEFROMCRL should be removed when Base CRL is more recent than"
            + " revocation date.",
        0,
        res.size());
  }
