...
public class OcspKeyBindingTest {
  ...
  public void testOcspSigningCertificateValidationPositives()
    ...
    AvailableExtendedKeyUsagesConfiguration ekuConfig =
        new AvailableExtendedKeyUsagesConfiguration();
    assertTrue(
        "KU=digitalSignature and EKU=id_kp_OCSPSigning should be treated as a"
            + " valid OCSP singing certificate.",
        OcspKeyBinding.isOcspSigningCertificate(
            getCertificate(X509KeyUsage.digitalSignature, ekuExtensionOnly),
            ekuConfig));
    assertTrue(
        "KU=digitalSignature and EKU=id_kp_OCSPSigning should be treated as a"
            + " valid OCSP singing certificate.",
        OcspKeyBinding.isOcspSigningCertificate(
            getCertificate(
                X509KeyUsage.digitalSignature + X509KeyUsage.cRLSign,
                ekuExtensionOnly),
            ekuConfig));
    assertTrue(
        "KU=nonRepudiation and EKU=id_kp_OCSPSigning should be treated as a"
            + " valid OCSP singing certificate.",
        OcspKeyBinding.isOcspSigningCertificate(
            getCertificate(
                X509KeyUsage.nonRepudiation + X509KeyUsage.cRLSign,
                ekuExtensionOnly),
            ekuConfig));
    assertTrue(
        "KU=digitalSignature+nonRepudiation and EKU=id_kp_OCSPSigning should"
            + " be treated as a valid OCSP singing certificate.",
        OcspKeyBinding.isOcspSigningCertificate(
            getCertificate(
  ...
  public void testOcspSigningCertificateAssertionPositives()
    ...
    try {
      new OcspKeyBinding()
          .assertCertificateCompatability(
              getCertificate(X509KeyUsage.digitalSignature, ekuExtensionOnly),
              new AvailableExtendedKeyUsagesConfiguration());
    } catch (CertificateImportException e) {
      fail(
          "KU=digitalSignature and EKU=id_kp_OCSPSigning should be treated as"
              + " a valid OCSP singing certificate.");
    }
  }
  ...
  public void testOcspSigningCertificateValidationNegatives()
    ...
    assertFalse(
        "KU!=digitalSignature|nonRepudiation and EKU=id_kp_OCSPSigning should"
            + " be treated as an invalid OCSP singing certificate.",
        OcspKeyBinding.isOcspSigningCertificate(
            getCertificate(
                X509KeyUsage.keyAgreement + X509KeyUsage.cRLSign,
                ekuExtensionOnly),
            ekuConfig));
    assertFalse(
        "KU=digitalSignature and EKU!=id_kp_OCSPSigning should be treated as"
            + " an invalid OCSP singing certificate.",
        OcspKeyBinding.isOcspSigningCertificate(
            getCertificate(X509KeyUsage.digitalSignature, null), ekuConfig));
    assertFalse(
        "KU=nonRepudiation and EKU!=id_kp_OCSPSigning should be treated as an"
            + " invalid OCSP singing certificate.",
        OcspKeyBinding.isOcspSigningCertificate(
  ...
  public void testOcspSigningCertificateAssertionNegatives()
    ...
    try {
      new OcspKeyBinding()
          .assertCertificateCompatability(
              getCertificate(X509KeyUsage.cRLSign, null),
              new AvailableExtendedKeyUsagesConfiguration());
      fail(
          "KU=cRLSign and EKU!=id_kp_OCSPSigning should be treated as an"
              + " invalid OCSP singing certificate.");
    } catch (CertificateImportException e) {
      // Expected outcome
    }
  ...
  private X509Certificate getCertificate(
      final int keyUsage, final List<Extension> extensions)
  ...
  private static Extension getExtendedKeyUsageExtension() throws IOException {
    final ASN1Encodable usage =
        KeyPurposeId.getInstance(KeyPurposeId.id_kp_OCSPSigning);
  ...
  public void testProperties() {
    ...
    assertFalse(
        "getRequireTrustedSignature", keybind.getRequireTrustedSignature());
    assertEquals(
        "getResponderIdType",
        ResponderIdType.KEYHASH,
        keybind.getResponderIdType());
    ...
    assertTrue(
        "setRequireTrustedSignature", keybind.getRequireTrustedSignature());
    assertEquals(
        "setResponderIdType",
        ResponderIdType.NAME,
        keybind.getResponderIdType());
  }
}
