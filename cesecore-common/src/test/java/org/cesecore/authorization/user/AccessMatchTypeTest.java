...
public class AccessMatchTypeTest {
  ...
  public void testAccessMatchEnum() throws SecurityException {
    ...
    // Types before the role rewrite in EJBCA 6.9, must be kept for upgrade
    // reasons,
    ...
    // Types that were used earlier than EJBCA 4, must also be kept for upgrade
    // reasons,
    // since we support 100% uptime upgrades, we can not NPE if these are in the
    // database
    assertEquals(
        "name is incorrect", "SPECIALADMIN_PUBLICWEBUSER", type2000.name());
    assertEquals(
        "name is incorrect",
        "SPECIALADMIN_CACOMMANDLINEADMIN",
        type2001.name());
    assertEquals("name is incorrect", "SPECIALADMIN_RAADMIN", type2002.name());
    assertEquals(
        "name is incorrect",
        "SPECIALADMIN_BATCHCOMMANDLINEADMIN",
        type2003.name());
    assertEquals(
        "name is incorrect", "SPECIALADMIN_INTERNALUSER", type2004.name());
