...
public interface RoleMemberDataSessionLocal extends RoleMemberDataSession {
  ...
  /**
   * Saves a RoleMember object to the database, either overwriting an existing
   * one with the same primary key or editing an existing instance.
   *
   ...
   *
   * @param primaryKey The ID of the RoleMemberData to remove.
   ...
   * @return all roleId matching the specified valid AuthenticationToken or an
   *     empty list otherwise
   */
  Set<Integer> getRoleIdsMatchingAuthenticationToken(
   ...
   * @return all roleId matching the specified valid AuthenticationToken or an
   *     empty list otherwise
   * @throws AuthenticationFailedException if there is problem matching the
   *     AuthenticationToken to a RoleMember
   */
  Set<Integer> getRoleIdsMatchingAuthenticationTokenOrFail(
      AuthenticationToken authenticationToken)
  ...
  Map<Integer, Integer> getRoleIdsAndTokenMatchKeysMatchingAuthenticationToken(
      AuthenticationToken authenticationToken)
      throws AuthenticationFailedException;

  /**
   * @param authenticationToken Auth token
   * @return all role members matching the specified valid AuthenticationToken
   *     or an empty list otherwise
   */
  Set<RoleMember> getRoleMembersMatchingAuthenticationToken(
      AuthenticationToken authenticationToken);
}
