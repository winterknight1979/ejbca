 ...
 *
 * <p>The sequence number is guaranteed to be unique as long as the read "node
 * identifier" read on first access is unique among the nodes sharing the
 * database.
 *
...
public enum NodeSequenceHolder {
  ...
  public interface OnInitCallBack {
    ...
    /**
     * @param nodeId ID
     * @return the highest known sequence number for the node identifier
     *     returned by {@link OnInitCallBack#getNodeId()}
     */
  ...
  public long getNext(final OnInitCallBack callBack) {
    if (lastSequenceNumberAtomic.get() == -1L) {
      try {
        // Lock threads during initializations of value from the database (only
        // one thread needs to/should do the lookup)
        lockDataBaseUpdate.lock();
        if (lastSequenceNumberAtomic.get() == -1L) {
          // Note that it very important that the nodeId is cached to avoid gaps
          // in the sequence if the nodeId changes
          nodeId = callBack.getNodeId();
          lastSequenceNumberAtomic.set(
              callBack.getMaxSequenceNumberForNode(nodeId));
        }
      } finally {
  ...
  /**
   * Trigger a re-read of the nodeId and last sequence number for the node on
   * next call to {@link NodeSequenceHolder#getNext(OnInitCallBack)}.
   *
   * <p>WARNING: This class is NOT safe to reset in an concurrent environment
   * and this method should only be used from unit tests.
   */
  public void reset() {
