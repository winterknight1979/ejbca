 ...
 *
 * @version $Id: SecurityEventsLoggerSession.java 167 2011-01-27 09:11:21Z
 *     tomas$
 */
public interface SecurityEventsLoggerSession {
   ...
   *
   * @param authToken The authentication token of the entity that invoked the
   *     operation.
   ...
   * @param module The module where the operation took place.
   * @param service The service (application) that performed the operation.
   * @param customId A custom identifier related to this event (e.g. a CA's
   *     identifier)
   * @param searchDetail1 A detail of this event that can be queried for using
   *     QueryCriteria (database) searches (e.g. a certificate serialnumber)
   * @param searchDetail2 A detail of this event that can be queried for using
   *     QueryCriteria (database) searches (e.g. a username)
   * @param additionalDetails Additional details of this event to be stored in a
   *     non-searchable manner.
   * @throws AuditRecordStorageException On fail
   * @throws AuthorizationDeniedException If access denied
   */
  void log(
