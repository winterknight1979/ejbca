 ...
 *
 * @version $Id: CertificateDataSessionLocal.java 28981 2018-05-21 14:10:49Z
 *     jekaterina_b_helmes $
 */
@Local
public interface CertificateDataSessionLocal extends CertificateDataSession {
   ...
   */
  Set<String> findUsernamesBySubjectDNAndIssuerDN(
   ...
   * @return return the query results as a List.
   */
  List<CertificateData> findByIssuerDNSerialNumber(
   ...
   * @return return the query results as a List.
   */
  CertificateInfo findFirstCertificateInfo(
      String issuerDN, String serialNumber);
   ...
   */
  String findLastUsernameByIssuerDNSerialNumber(
      String issuerDN, String serialNumber);
   ...
   * @return return the query results as a List.
   */
  List<CertificateData> findByUsernameAndStatusAfterExpireDate(
   ...
   * @return return the query results as a List.
   */
  Set<String> findUsernamesBySubjectKeyIdOrDnAndIssuer(
      String issuerDN, String subjectKeyId, String subjectDN);
   ...
   */
  Collection<RevokedCertInfo> getRevokedCertInfos(
   ...
   * @return return the query results as a List.
   */
  List<CertificateData> findByExpireDateWithLimit(
      long expireDate, int maxNumberOfResults);
   ...
   * @return return the query results as a List.
   */
  List<CertificateData> findByExpireDateWithLimitAndOffset(
      long expireDate, int maxNumberOfResults, int offset);
   ...
   * @return return the query results as a List.
   */
  List<CertificateData> findByExpireDateAndIssuerWithLimit(
      long expireDate, String issuerDN, int maxNumberOfResults);
  ...
  /**
   * Get a list of {@link Certificate} from a list of list of {@link
   * CertificateData}.
   *
   * @param cdl data
  ...
  /**
   * @param fingerprint FP
   * @return the CertificateInfo representation (all fields except the actual
   *     cert) or null if no such fingerprint exists.
   */
  CertificateInfo getCertificateInfo(String fingerprint);

  /**
   * @param certificateTypes Types
   * @return a List&lt;Certificate&gt; of SecConst.CERT_ACTIVE and
   *     CERT_NOTIFIEDABOUTEXPIRATION certs that have one of the specified
   *     types.
   */
  List<Certificate> findActiveCertificatesByType(
      Collection<Integer> certificateTypes);

  /**
   * @param certificateTypes Types
   * @param issuerDN Issuer DN
   * @return a List&lt;Certificate&gt; of SecConst.CERT_ACTIVE and
   *     CERT_NOTIFIEDABOUTEXPIRATION certs that have one of the specified types
   *     for the given issuer.
   */
  List<Certificate> findActiveCertificatesByTypeAndIssuer(
      Collection<Integer> certificateTypes, String issuerDN);

  /**
   * Fetch a List of all certificate fingerprints and corresponding username
   *
   * <p>We want to accomplish two things:
   *
   * <p>1. Notify for expirations within the service window 2. Notify _once_ for
   * expirations that occurred before the service window like flagging
   * certificates that have a shorter life-span than the threshold (pathologic
   * test-case...)
   *
   * <p>The first is checked by:
   *
   * <p>notify = currRunTimestamp + thresHold &lt;= ExpireDate &lt;
   * nextRunTimestamp + thresHold AND (status = ACTIVE OR status =
   * NOTIFIEDABOUTEXPIRATION)
   *
   * <p>The second can be checked by:
   *
   * <p>notify = currRunTimestamp + thresHold &gt; ExpireDate AND status =
   * ACTIVE
   *
