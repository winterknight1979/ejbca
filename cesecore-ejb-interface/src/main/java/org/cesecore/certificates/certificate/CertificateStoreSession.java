...
/**
 * Interface for certificate store operations Stores certificate in the local
 * database using Certificate JPA Beans.
 *
 * <p>IMPORTANT: No publishing to publishersession from setRevokeStatus
 *
 * @version $Id: CertificateStoreSession.java 29011 2018-05-23 13:41:10Z
 *     jekaterina_b_helmes $
 */
public interface CertificateStoreSession {

  /**
   * Lists fingerprint (primary key) of ALL certificates in the database. NOTE:
   * Caution should be taken with this method as execution may be very heavy
   * indeed if many certificates exist in the database (imagine what happens if
   * there are millions of certificates in the DB!). Should only be used for
   * testing purposes.
   *
   ...
   *
   * @param issuerdn the dn of the certificates issuer.
   * @param lastbasecrldate a date (Date.getTime()) of last base CRL or -1 for a
   *     complete CRL
   * @return Collection of RevokedCertInfo, reverse ordered by expireDate where
   *     last expireDate is first in array.
   */
   ...
   *
   * @param subjectDN the DN of the subject whose certificates will be
   *     retrieved.
   * @param issuerDN the DN of the certificates issuer.
   * @return Collection of Certificates (java.security.cert.Certificate) in no
   *     specified order or an empty Collection.
   */
  List<Certificate> findCertificatesBySubjectAndIssuer(
   ...
   *     retrieved.
   * @param issuerDN the DN of the certificates issuer.
   * @param onlyActive set to true to limit the search to active (non revoked,
   *     unexpired) certificates.
   * @return Collection of Certificates (java.security.cert.Certificate) in no
   *     specified order or an empty Collection.
   */
  List<Certificate> findCertificatesBySubjectAndIssuer(
      String subjectDN, String issuerDN, boolean onlyActive);

  /**
   * @param issuerDN Issuer DN
   * @param subjectDN Subject DN
   * @return set of users with certificates with specified subject DN issued by
   *     specified issuer.
   */
  Set<String> findUsernamesByIssuerDNAndSubjectDN(
      String issuerDN, String subjectDN);

  /**
   * @param issuerDN Issuer DB
   * @param subjectKeyId Key
   * @return set of users with certificates with specified key issued by
   *     specified issuer.
   */
  Set<String> findUsernamesByIssuerDNAndSubjectKeyId(
      String issuerDN, byte[] subjectKeyId);

  /**
   * Return the username of a certificate data object matching the given issuer
   * DN and serial number (there may be only one).
   *
  ...
  /**
   * Finds certificates expiring within a specified time and that have status
   * "active" or "notifiedaboutexpiration".
   *
   ...
   * @see
   *     org.cesecore.certificates.certificate.CertificateConstants#CERT_NOTIFIEDABOUTEXPIRATION
   * @return Collection of maximum 500 certificates, never null
   */
  Collection<CertificateWrapper> findCertificatesByExpireTimeWithLimit(
      Date expireTime);
   ...
   */
  List<Certificate> findCertificatesByExpireTimeWithLimit(
      Date expireTime, int maxNumberOfResults);
  ...
  List<Certificate> findExpiringCertificates(
      Date expirationDate, int maxNumberOfResults, int offset);

  /**
   * Finds certificates expiring within a specified time, issued by a specified
   * issuer and have status "active" or "notifiedaboutexpiration".
   *
   * @param expireTime The time by which the certificates will be expired
   * @param issuerDN The SubjectDN of the CA that issued the certificates that
   *     will expire within the given time
   * @param maxNumberOfResults The maximum number of certificates to be returned
   * @see org.cesecore.certificates.certificate.CertificateConstants#CERT_ACTIVE
   * @see
   *     org.cesecore.certificates.certificate.CertificateConstants#CERT_NOTIFIEDABOUTEXPIRATION
   * @return List of certificates (java.security.cert.Certificate), never null
   */
  List<Certificate> findCertificatesByExpireTimeAndIssuerWithLimit(
      Date expireTime, String issuerDN, int maxNumberOfResults);

  /**
   * Finds certificates expiring within a specified time, of a specified type
   * and have status "active" or "notifiedaboutexpiration".
   *
   * @param expireTime The time by which the certificates will be expired
   * @param certificateType The type of the certificates that will expire within
   *     the given time
   * @param maxNumberOfResults The maximum number of certificates to be returned
   * @see org.cesecore.certificates.certificate.CertificateConstants#CERT_ACTIVE
   * @see
   ...
   */
  List<Certificate> findCertificatesByExpireTimeAndTypeWithLimit(
      Date expireTime, int certificateType, int maxNumberOfResults);

  /**
   * Finds usernames of users having certificate(s) expiring within a specified
   * time and that has status "active" or "notifiedaboutexpiration".
   *
   ...
   */
  Certificate findCertificateByIssuerAndSerno(
   ...
   */
  Collection<Certificate> findCertificatesByIssuerAndSernos(
      String issuerDN, Collection<BigInteger> sernos);
   ...
   * Note that this method does not check if the returned certificate was issued
   * by a rollover CA.
   *
   ...
   *
   * @param subjectDN The subject DN to match.
   * @param rolloverCA The rollover certificate of the issuing CA, or null to
   *     not check if issued by a rollover certificate.
   * @param findRollover Whether a rollover or normal certificate should be
   *     returned.
   * @return the sought result, or null if none exists.
   */
  X509Certificate findLatestX509CertificateBySubject(
      String subjectDN, X509Certificate rolloverCA, boolean findRollover);
   ...
   * @return Collection of Certificates ordered by expire date, with last expire
   *     date first, or null if none found.
   */
   ...
   * @param excludedStatuses a list of statuses to exclude in the query or null
   *     to return all
   ...
   */
  List<CertificateDataWrapper> getCertificateDataByUsername(
      String username, boolean excludeExpired, List<Integer> excludedStatuses);
   ...
   * @return Collection of Certificates ordered by expire date, with last expire
   *     date first, or empty list if user can not be found
   */
  Collection<Certificate> findCertificatesByUsernameAndStatus(
      String username, int status);

  /**
   * Finds certificate(s) for a given username and status if the expireDate is
   * after the provided one.
   *
   * @param username the username of the certificate(s) that will be retrieved
   * @param status the status from the CertificateConstants.CERT_ constants
   * @param afterExpireDate only return entries that has an expireDate larger
   *     than or equal to this one
   * @return Collection of Certificates ordered by expire date, with last expire
   *     date first, or empty list if user can not be found
   */
  Collection<Certificate> findCertificatesByUsernameAndStatusAfterExpireDate(
      String username, int status, long afterExpireDate);

  /**
   * Gets certificate info, which is basically all fields except the certificate
   * itself. Note: this method should not be used within a transaction where the
   * reading of this info might depend on something stored earlier in the
   * transaction. This is because this method uses direct SQL.
   *
  ...
  /**
   * Finds a certificate based on fingerprint. You can get fingerprint by for
   * example "String fingerprint =
  ...
  /**
   * Lists all active (status = 20) certificates of a specific type and if given
   * from a specific issuer.
   *
   * @param issuerDN get all certificates issued by a specific issuer. If {@code
   *     null} or empty return certificates regardless of the issuer.
   * @param type CERTTYPE_* types from CertificateConstants
   * @throws IllegalArgumentException when admin is null or type is not one or
   *     more of of SecConst.CERTTYPE_SUBCA, SecConst.CERTTYPE_ENDENTITY,
   *     SecConst.CERTTYPE_ROOTCA
   * @return Collection of Certificate, never {@code null}
   */
  Collection<CertificateWrapper> findCertificatesByType(
      int type, String issuerDN);
   ...
   * @return List containing certificate chain, starting with the given
   *     certificate.
   */
  List<Certificate> getCertificateChain(CertificateInfo certinfo);

  /**
   * Method revoking all certificates generated by the specified issuerdn. Sets
   * revocationDate to current time. Should only be called by when a CA is about
   * to be revoked.
   *
   ...
   */
  void revokeAllCertByCA(AuthenticationToken admin, String issuerdn, int reason)
   ...
   * @return CertificateStatus status of the certificate, never null,
   *     CertificateStatus.NOT_AVAILABLE if the certificate is not found.
   */
  CertificateStatus getStatus(String issuerDN, BigInteger serno);

  /**
   * Performs the same operation as getStatus, but returns a richer object which
   * also contains the certificate, in order to save on database lookups when
   * both objects are required. Issuer + serial number are always unique.
   *
   * @param issuerDN the issuer of the sought certificate
   * @param serno the serial number of the sought certificate
   * @return a {@link CertificateStatusHolder} object containing the status and
   *     the sought certificate.
   */
  CertificateStatusHolder getCertificateAndStatus(
      String issuerDN, BigInteger serno);
   ...
   * @return true if the status was updated, false if not, for example if the
