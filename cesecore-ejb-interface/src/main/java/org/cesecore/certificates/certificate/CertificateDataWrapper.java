...
/**
 * Wrapper class for returning cloned specific CertificateData and
 * Base64CertData objects.
 *
...
public class CertificateDataWrapper
    implements CertificateWrapper,
        Comparable<CertificateDataWrapper>,
        Serializable {
...
/** Data. */
  ...
  /**
   * @return the BaseCertificateData entity object, may be CertificateData or
   *     NoConflictCertificateData entity object.
   *     <p>Note that the NoConflictCertificateData table is append only, so no
   ...
   * @throws ClassCastException If certificate is stored in
  ...
  /**
   * Returns the CertificateData object, or a copy converted to CertificateData,
   * if it was a NoConflictCertificateData entity object.
   *
   * @return CertificateData object. Do not modify the returned object!
   *     Modifications might not be written back to the database.
   */
  ...
  public int compareTo(final CertificateDataWrapper other) {
    if (getCertificate() != null && other.getCertificate() != null) {
      ...
      return Long.valueOf(
              CertTools.getNotBefore(other.getCertificate()).getTime())
          .compareTo(CertTools.getNotBefore(getCertificate()).getTime());
    } else {
      // Sort descending by expiration date if certificates are not available
      return Long.valueOf(other.getCertificateData().getExpireDate())
          .compareTo(getCertificateData().getExpireDate());
    }
  }
