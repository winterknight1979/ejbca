 ...
 *
 * <p>In order to add or edit CertificateProfiles the admin needs access to: -
 * /ca_functionality/edit_certificate_profiles
 * (StandardRules.CERTIFICATEPROFILEEDIT) - the CAs set as "available CAs" with
 * CA rules as /ca/&lt;caid&gt; (StandardRules.CAACCESS) - if "available CAs"
 * contains ANYCA (-1) the -1 caid privilege is needed.
 *
 * @version $Id: CertificateProfileSession.java 29410 2018-06-29 09:44:31Z
 *     mikekushner $
 */
public interface CertificateProfileSession {
   ...
   *
   * @param admin administrator performing the task
   * @param id internal ID of new certificate profile, use only if you know it's
   *     right.
   * @param name readable name of new certificate profile
   * @param profile the profile to be added
   * @return the generated certificate profile id
  ...
  /**
   * Adds a certificate profile with the same content as the original
   * certificate profile.
   *
   * @param admin Administrator performing the operation
   * @param orgname name of old certificate profile
   * @param newname name of new certificate profile
   * @param authorizedCaIds list of CAid to replace the original list with, or
   *     null.
   * @throws CertificateProfileExistsException If a profile already exists with
   *     the new name
   * @throws CertificateProfileDoesNotExistException if the profile with the
   *     given ID didn't exist.
   * @throws AuthorizationDeniedException If access denied
   */
  void cloneCertificateProfile(
      ...
      String orgname,
      String newname,
      List<Integer> authorizedCaIds)
      throws CertificateProfileExistsException,
          CertificateProfileDoesNotExistException, AuthorizationDeniedException;

  /**
   * Retrieves a Collection of id:s (Integer) to authorized profiles. Only
   * profiles that refer to CA's that the authentication token is authorized to
   * will be returned.
   *
   * @param admin Administrator performing the operation
   * @param certprofiletype should be either
   *     CertificateConstants.CERTTYPE_ENDENTITY,
   *     CertificateConstants.CERTTYPE_SUBCA,
   *     CertificateConstants.CERTTYPE_ROOTCA,
   *     CertificateConstants.CERTTYPE_HARDTOKEN (i.e EndEntity certificates and
   *     Hardtoken fixed profiles) or CertificateConstants.CERTTYPE_UNKNOWN for
   *     all. Retrieves certificate profile names sorted.
   * @return Collection of id:s (Integer)
   */
  List<Integer> getAuthorizedCertificateProfileIds(
      AuthenticationToken admin, int certprofiletype);

  /**
   * Retrieves a Collection of id:s (Integer) of all certificate profiles which
   * have non-existent CA Ids. This requires access to the root resource (i.e.
   * superadmin access). If access is denied then an empty list is returned.
   *
   * @param admin Auth token
   * @return List
   */
  List<Integer> getAuthorizedCertificateProfileWithMissingCAs(
      AuthenticationToken admin);
   ...
   *
   * @param id certificate profile id
   * @return certificate profile name or null if certificate profile id does not
   *     exist.
   */
   ...
   * @throws CertificateProfileExistsException If a profile with the new name
   *     already exists
   * @throws AuthorizationDeniedException If access denied
   */
  ...
  /**
   * Removes a certificate profile from the database, does not throw any errors
   ...
   * profiles. Used to avoid desynchronization of publisher data.
   *
   ...
   * @throws CertificateProfileDoesNotExistException if the profile with the
   *     given ID didn't exist.
   * @throws AuthorizationDeniedException if the profile contained CAs that the
   *     admin wasn't authorized to, or the admin wasn't authorized to view
   *     profiles
   */
