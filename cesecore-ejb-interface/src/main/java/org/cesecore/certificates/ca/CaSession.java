 ...
 *
 * @version $Id: CaSession.java 29306 2018-06-21 14:07:02Z andresjakobs $
 */
public interface CaSession {
   ...
   */
  void addCA(AuthenticationToken admin, CA ca)
   ...
   *
   * <p>There's also CAAdminSession.editCA() which handles some special cases,
   * e.g. changing the Subject DN of uninitialized CAs.
   *
   * @param admin AuthenticationToken of admin
   ...
   *
   * <p>Does not log access control to all CAs it checks, because this does not
  ...
  /**
   * Like {@link #getAuthorizedCaNames(AuthenticationToken)}, but returns a
   * TreeMap which includes the CA Id as well.
   *
   * @param admin AuthenticationToken of admin
   * @return a Collection&lt;String&gt; of available CA names
  ...
  /**
   * Method returning info objects for all CA's available to the system that the
   * administrator is authorized to.
   *
   * <p>Does not log access control to all CAs it checks, because this does not
  ...
  /**
   * Method returning info objects for all active CA's available to the system,
   * i.e. not having status "external", uninitialized or "waiting for
   * certificate response" and that the administrator is authorized to,
   *
   * <p>Does not log access control to all CAs it checks, because this does not
   * give access to the CAs but only returns CAInfo of CAs.
   *
   ...
   */
  List<CAInfo> getAuthorizedAndEnabledCaInfos(
      AuthenticationToken authenticationToken);

  /**
   * Method returning info objects for all non-external CA's available to the
   * system, i.e. not having status "external", and that the administrator is
   * authorized to.
   *
   * <p>Does not log access control to all CAs it checks, because this does not
   * give access to the CAs but only returns CAInfo of CAs.
   ...
   */
  List<CAInfo> getAuthorizedAndNonExternalCaInfos(
      AuthenticationToken authenticationToken);
   ...
   */
  CAInfo getCAInfo(AuthenticationToken admin, String name)
   ...
   * @throws AuthorizationDeniedException if admin not authorized to CA
   */
  CAInfo getCAInfo(AuthenticationToken admin, int caid)
      throws AuthorizationDeniedException;
   ...
   *     does not exist.
   */
  Collection<CertificateWrapper> getCaChain(
      AuthenticationToken authenticationToken, String caName)
      throws AuthorizationDeniedException, CADoesntExistsException;

  /**
   * Method used to remove a CA from the system. You should first check that the
   * CA isn't used by any EndEntity, Profile or AccessRule before it is removed.
   * CADataHandler for example makes this check. Should be used with care. If
   * any certificate has been created with the CA use revokeCA instead and don't
   * remove it.
   *
   * @param admin AuthenticationToken of admin
   * @param caid numerical unique id of CA
   *     <p>If the CA does not exist, nothing happens the method return
   *     silently.
   * @throws AuthorizationDeniedException if not authorized to remove CA
   */
  void removeCA(AuthenticationToken admin, int caid)
      throws AuthorizationDeniedException;

  /**
   * Renames the short name of CA (used in administrators interfaces). This name
   * does not have to be the same as SubjectDN and is only used for reference.
   *
   * @param admin AuthenticationToken of admin
   ...
   */
  void renameCA(AuthenticationToken admin, String oldname, String newname)
  ...
  /**
   * Returns the future key/certificate rollover if a rollover is pending, or
   * otherwise null.
   *
   * @param caid numerical unique id of CA
   * @return Certificate
