 ...
 *
 * @version $Id: KeyValidatorSessionLocal.java 27643 2017-12-22 09:56:06Z
 *     mikekushner $
 */
@Local
public interface KeyValidatorSessionLocal
    extends KeyValidatorSession, CertificateValidationDomainService {
  ...
  /**
   * Retrieves a Map of key validator names which are applicable for the type of
   * CA (X509 or CVC see {@link CAInfo#CATYPE_X509} or {@link
   * CAInfo#CATYPE_CVC}).
   *
  ...
  /**
   * Returns the ids for validators conflicting with a given key validator which
   * is to be added.
   *
   * <p>A new validator is conflicting either if it has the same id or if has
   * the same name and type as an existing validator. A validator can be added
   * to this session iff this method returns an empty list.
   *
   ...
   */
  void importValidator(AuthenticationToken admin, Validator validator)
   ...
   *     access to
   * @param keyValidatorAccessRule an additional access rule which is required
   *     in order for it to be returned, for example REGULAR_EDITVALIDATOR to
   *     only return profiles only if the admin have validator create rights
   ...
   * defined in it and the CA reference to it. The method is invoked while
   * certificate issuance for user certificates and CA certificates.
   *
   * @param admin the AuthenticationToken of the admin who requested the
   *     operation resulting in validation, used for audit logging, for example
   *     the admin requesting cert issuance
   ...
   * @return true if all matching key validators could validate the public key
   *     successfully. If false #getMessage().size() is greater than 0.
   * @throws ValidationException if the key validation failed. If the key
   *     validators failed action is set to abort certificate issuance {@link
   *     KeyValidationFailedActions#ABORT_CERTIFICATE_ISSUANCE} and validation
   *     fails, or the wrong algorithm type is chosen, message is NOT null.
   *     Exception of any technical errors are stored in the cause, and message
   *     is null.
   * @throws IllegalValidityException if the certificate validity could not be
   *     determined.
   */
  ...
  /**
   * Validates dnsName fields defined in the SubjectAltName field of the end
   * entity against CAA rules.
   *
   ...
   * @throws ValidationException if the validation failed. If the validation
   *     failed action is set to abort certificate issuance {@link
   *     KeyValidationFailedActions#ABORT_CERTIFICATE_ISSUANCE} and validation
   *     fails, message is NOT null. Exception of any technical errors are
  ...
  /**
   * Removes the key validator data if it is not referenced by a CA. If the
   * validatorId does not exist, the method returns without doing anything.
   *
   * @param admin AuthenticationToken of admin.
   * @param validatorId the ID of the validator to remove
   * @throws AuthorizationDeniedException required access rights are
   *     ca_functionality/edit_validators
  ...
  /**
   * Replaces the settings of a key validator without changing its name or
   * profile ID.
   *
