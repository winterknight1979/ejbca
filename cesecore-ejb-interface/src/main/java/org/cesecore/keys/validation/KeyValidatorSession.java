...
public interface KeyValidatorSession {
   ...
   * @return a BaseKeyValidator or null if a key validator with the given id
   ...
   *
   * @param id ID
   * @return the name of the key validator with the given id or null if none was
   *     found.
   */
   ...
   * @throws AuthorizationDeniedException required access rights are
   *     ca_functionality/edit_validator
   * @throws KeyValidatorDoesntExistsException if there's no key validator with
   *     the given name.
   */
  ...
  /**
   * Removes the key validator data equal if it is not referenced by a CA. If
   * the validatorId does not exist, the method returns without doing anything.
   *
   * @param admin AuthenticationToken of admin.
   * @param validatorName the name of the validator to remove
   * @throws AuthorizationDeniedException required access rights are
   *     ca_functionality/edit_validators
   * @throws CouldNotRemoveKeyValidatorException if the key validator is
  ...
  /**
   * Flushes the key validators cache to ensure that next time they are read
   * from database.
   */
