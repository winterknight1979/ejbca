 ...
 * (OCSP) For a detailed description of OCSP refer to RFC2560.
 *
...
public class OCSPServlet extends HttpServlet {
  ...
  /** Param. */
  @EJB
  private OcspResponseGeneratorSessionLocal
      integratedOcspResponseGeneratorSession;
  ...
  public void doGet(
    ...
    try {
      ...
      final boolean performKeyRenewal =
          keyRenewalSignerDN != null && keyRenewalSignerDN.length() > 0;
      // We have a command to force reloading of keys that can only be run from
      // localhost
      final boolean doReload =
          StringUtils.equals(request.getParameter("reloadkeys"), "true");
      final String newConfig = request.getParameter("newConfig");
      final boolean doNewConfig = newConfig != null && newConfig.length() > 0;
      final boolean doRestoreConfig =
          request.getParameter("restoreConfig") != null;
      final String remote = request.getRemoteAddr();
      if (doReload || doNewConfig || doRestoreConfig) {
        if (!StringUtils.equals(remote, "127.0.0.1")) {
          LOG.info(
              "Got reloadkeys or updateConfig of restoreConfig command from"
                  + " unauthorized ip: "
                  + remote);
      ...
      if (performKeyRenewal) {
        final Set<String> rekeyingTriggeringHosts =
            OcspConfiguration.getRekeyingTriggingHosts();
        if (!rekeyingTriggeringHosts.contains(remote)) {
          LOG.info(
              INTRES.getLocalizedMessage(
                  "ocsp.rekey.triggered.unauthorized.ip", remote));
          response.sendError(HttpServletResponse.SC_UNAUTHORIZED);
          return;
        }
        final String rekeyingTriggingPassword =
            OcspConfiguration.getRekeyingTriggingPassword();
        if (rekeyingTriggingPassword == null) {
          LOG.info(
              INTRES.getLocalizedMessage(
                  "ocsp.rekey.triggered.not.enabled", remote));
          response.sendError(HttpServletResponse.SC_UNAUTHORIZED);
          return;
        }
        ...
        if (!rekeyingTriggingPassword.equals(requestPassword)) {
          LOG.info(
              INTRES.getLocalizedMessage(
                  "ocsp.rekey.triggered.wrong.password", remote));
          response.sendError(HttpServletResponse.SC_UNAUTHORIZED);
          return;
        }
        try {
          ...
          LOG.info(
              INTRES.getLocalizedMessage(
                  "ocsp.rekey.keystore.notactivated", remote));
          response.sendError(HttpServletResponse.SC_UNAUTHORIZED);
          return;
        } catch (CryptoTokenOfflineException e) {
          LOG.info(
              INTRES.getLocalizedMessage(
                  "ocsp.rekey.cryptotoken.notactivated", remote));
          response.sendError(HttpServletResponse.SC_UNAUTHORIZED);
          return;
        } catch (InvalidKeyException e) {
          LOG.info(
              INTRES.getLocalizedMessage("ocsp.rekey.invalid.key", remote));
          response.sendError(HttpServletResponse.SC_UNAUTHORIZED);
          return;
        }
        return;
  ...
  public void doPost(
    ...
    try {
      ...
      if (contentType != null
          && contentType.equalsIgnoreCase("application/ocsp-request")) {
        processOcspRequest(request, response, HttpMethod.POST);
        return;
      }
      final String remoteAddr = request.getRemoteAddr();
      // Legacy support for activation using ClientToolBox. We will only use
      // this once for upgrading the installation.
      final String activationPassword = request.getHeader("activate");
      if (activationPassword != null && remoteAddr.equals("127.0.0.1")) {
        try {
          LOG.warn(
              "'active' will only be used for initial one-time upgrade. Use"
                  + " regular CryptoToken activation in EJB CLI or Admin GUI"
                  + " to active your responder keystores.");
          integratedOcspResponseGeneratorSession.adhocUpgradeFromPre60(
              activationPassword.toCharArray());
        } catch (Exception e) {
      ...
      if (contentType != null) {
        final String sError =
            "Content-type is not application/ocsp-request. It is \'"
  ...
  private void processOcspRequest(
    ...
    try {
      if (auditLogger.isEnabled()) {
        auditLogger.paramPut(
            PatternLogger.LOG_ID, Integer.valueOf(localTransactionId));
      ...
      if (transactionLogger.isEnabled()) {
        ...
        transactionLogger.paramPut(
            TransactionLogger.FORWARDED_FOR, xForwardedFor);
      }
      ...
      try {
        ...
        ocspResponseInformation =
            integratedOcspResponseGeneratorSession.getOcspResponse(
                requestBytes,
                requestCertificates,
                remoteAddress,
                xForwardedFor,
        ...
        ocspResponseInformation =
            new OcspResponseInformation(
                responseGenerator.build(
                    OCSPRespBuilder.MALFORMED_REQUEST, null),
        ...
        if (auditLogger.isEnabled()) {
          ...
          Throwable
              e) { // NOPMD, we really want to catch everything here to return
                   // internal error on unexpected errors
        if (transactionLogger.isEnabled()) {
          transactionLogger.paramPut(
              IPatternLogger.PROCESS_TIME, IPatternLogger.PROCESS_TIME);
        }
        if (auditLogger.isEnabled()) {
          auditLogger.paramPut(
              IPatternLogger.PROCESS_TIME, IPatternLogger.PROCESS_TIME);
        ...
        if (auditLogger.isEnabled()) {
          auditLogger.paramPut(
              AuditLogger.STATUS, OCSPRespBuilder.INTERNAL_ERROR);
        }
      }
      ...
      if (HttpMethod.GET.equals(httpMethod)) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              "Will not add RFC 5019 cache headers: \"clients MUST use the GET"
                  + " method (to enable OCSP response caching)\"");
        }
      }
  ...
  private void addOscpPostHeaders(
      final HttpServletResponse response,
      final OcspResponseInformation ocspResponseInformation) {
    ...
    // last modified the response. == thisUpdate
    ...
    response.setDateHeader(
        "Expires",
        nextUpdate); // This is overridden by max-age on HTTP/1.1 compatible
                     // components
  ...
  /**
   * RFC 2560 does not specify how cache headers should be used, but RFC 5019
   * does. Therefore we will only add the headers if the requirements of RFC
   * 5019 is fulfilled: A GET-request, a single embedded response, the response
   * contains a nextUpdate and no nonce is present.
   *
   * @param request is the incoming HttpServletRequest
   * @param response is the outgoing HttpServletResponse
   * @param ocspResponseInformation is the previously parsed data carrier that
   *     wraps the contents of an OCSPResp
   * @throws IOException org.bouncycastle.cert.ocsp.OCSPException
   * @throws org.bouncycastle.cert.ocsp.OCSPException fail
   */
  private void addRfc5019CacheHeaders(
      final HttpServletRequest request,
      final HttpServletResponse response,
      final OcspResponseInformation ocspResponseInformation)
      throws IOException, org.bouncycastle.cert.ocsp.OCSPException {
    ...
    if (ocspResponseInformation.getMaxAge() <= 0) {
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Will not add RFC 5019 cache headers: RFC 5019 6.2: max-age should"
                + " be 'later than thisUpdate but earlier than nextUpdate'.");
      }
      return;
    }
    ...
    final long thisUpdate = ocspResponseInformation.getThisUpdate();
    long nextUpdate = ocspResponseInformation.getNextUpdate();
    // Max-age and nextUpdate will be truncated if they point longer than the
    // OCSP signer certificate is valid
    ...
    // the HTTP response.
    // On JBoss AS the "Date"-header is cached for 1 second, so this value will
    // be overwritten and off by up to a second
    ...
    // HEX representation of the SHA1 hash of the OCSPResponse structure.
    response.setHeader(
        "ETag", "\"" + ocspResponseInformation.getResponseHeader() + "\"");
    if (ocspResponseInformation.isExplicitNoCache()) {
      // Note that using no-cache here is not conforming to RFC5019, but with
      // more recent CABForum discussions it seems RFC5019 will not
      // be followed, or will be changed. (See ECA-3289)
      response.setHeader(
          "Cache-Control", "no-cache, must-revalidate"); // HTTP 1.1
      response.setHeader("Pragma", "no-cache"); // HTTP 1.0
    } else {
      // Max age is retrieved in milliseconds, but it must be in seconds in the
  ...
  /**
   * Reads the request bytes and verifies min and max size of the request. If an
   * error occurs it throws a MalformedRequestException. Can get request bytes
   * both from a HTTP GET and POST request
   *
   ...
   */
  private byte[] checkAndGetRequestBytes(
      final HttpServletRequest request, final HttpMethod httpMethod)
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          ">checkAndGetRequestBytes. Received "
              + httpMethod.name()
    ...
    if (n > LimitLengthASN1Reader.MAX_REQUEST_SIZE) {
      String msg =
          INTRES.getLocalizedMessage(
              "ocsp.toolarge", LimitLengthASN1Reader.MAX_REQUEST_SIZE, n);
      LOG.info(msg);
      throw new MalformedRequestException(msg);
    }
    // So we passed basic tests, now we can read the bytes, but still keep an
    // eye on the size
    // we can not fully trust the sent content length.
    if (HttpMethod.POST.equals(httpMethod)) {
      final ServletInputStream in =
          request
              .getInputStream(); // ServletInputStream does not have to be
                                 // closed, container handles this
      LimitLengthASN1Reader limitLengthASN1Reader =
          new LimitLengthASN1Reader(in, n);
      try {
        ret = limitLengthASN1Reader.readFirstASN1Object();
        if (n > ret.length) {
          // The client is sending more data than the OCSP request. It might be
          // slightly broken or trying to bog down the server on purpose.
          // In the interest of not breaking existing systems that might have
          // slightly broken clients we just log for a warning for now.
          String msg =
              INTRES.getLocalizedMessage("ocsp.additionaldata", ret.length, n);
          LOG.warn(msg);
        }
      } finally {
      ...
      // RFC2560 A.1.1 says that request longer than 255 bytes SHOULD be sent by
      ...
      if (url.length() <= LimitLengthASN1Reader.MAX_REQUEST_SIZE) {
        ...
        try {
          // We have to extract the pathInfo manually, to avoid multiple slashes
          // being converted to a single
          // According to RFC 2396 2.2 chars only have to encoded if they
          // conflict with the purpose, so
          // we can for example expect both '/' and "%2F" in the request.
          final String fullServletpath =
              request.getContextPath() + request.getServletPath();
          final int paramIx =
              Math.max(url.indexOf(fullServletpath), 0)
                  + fullServletpath.length()
                  + 1;
          final String requestString =
              paramIx < url.length() ? url.substring(paramIx) : "";
          decodedRequest =
              URLDecoder.decode(requestString, "UTF-8").replaceAll(" ", "+");
        } catch (Exception e) {
        ...
        if (decodedRequest != null && decodedRequest.length() > 0) {
          if (LOG.isDebugEnabled()) {
              final int max = 2048;
            // Don't log the request if it's too long, we don't want to cause
            // denial of service by filling log files or buffers.
            if (decodedRequest.length() < max) {
              LOG.debug("decodedRequest: " + decodedRequest);
            } else {
              LOG.debug(
                  "decodedRequest too long to log: " + decodedRequest.length());
            }
          }
          try {
      ...
      String msg =
          INTRES.getLocalizedMessage("ocsp.unknownmethod", request.getMethod());
      LOG.info(msg);
      throw new MalformedRequestException(msg);
    }
    ...
    if (ret == null || ret.length == 0) {
      String msg =
          INTRES.getLocalizedMessage("ocsp.emptyreq", request.getRemoteAddr());
      LOG.info(msg);
      throw new MalformedRequestException(msg);
    }
    return ret;
