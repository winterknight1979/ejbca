...
/**
 * ASN.1 OCSP extension used to return hash values for certificates as part of
 * an OCSP response.
 *
 * <p>Certificates will be encoded with SHA256
 *
 * <p>OID for this extension is 1.3.36.8.3.13
 *
 * <p>This extension is specified in the German Common PKI standard. <a
 * href="http://www.t7ev.org/T7-de/Common-PKI">Common PKI</a> SigG CertHash OCSP
 * extension.
 *
 * <p>CertHashExtension is a SINGLE_RESPONSE extension, which means it's inside
 * the responseItem, not generic for the Response (which can contain several
 * responseItems)
 *
 * @version $Id: OcspCertHashExtension.java 28881 2018-05-08 18:18:11Z anatom $
 */
public class OcspCertHashExtension implements OCSPExtension {
  ...
  private static final Logger LOG =
      Logger.getLogger(OcspCertHashExtension.class);
  ...
  public Map<ASN1ObjectIdentifier, Extension> process(
      ...
      final org.bouncycastle.cert.ocsp.CertificateStatus status,
    ...
    try {
      ...
      throw new IllegalStateException(
          "Could not create MessageDigest with algorithm SHA256", e);
    }
    CertHash certHash;
    try {
      certHash =
          new CertHash(
              new AlgorithmIdentifier(SHA256), md.digest(cert.getEncoded()));
    } catch (CertificateEncodingException e) {
      // This state can't be handled, shouldn't return null
      LOG.error("Could not encode certificate " + cert, e);
      throw new IllegalStateException(
          "Could not encode certificate " + cert, e);
    }
    HashMap<ASN1ObjectIdentifier, Extension> result =
        new HashMap<ASN1ObjectIdentifier, Extension>();
    try {
      result.put(
          new ASN1ObjectIdentifier(CERT_HASH_OID),
          new Extension(
              new ASN1ObjectIdentifier(CERT_HASH_OID),
              false,
      ...
      throw new IllegalStateException(
          "Could not construct an ASN.1Primitive.", e);
    }
    return result;
  }
