...
/**
 * Helper class used all bytes for the first ASN.1 object in the stream. Limits
 * the size that is ever read to MAX_REQUEST_SIZE. This class is used for
 * example to read requests from POSTs to servlets, when you want to make sure
 * that you never read too many bytes that might have been sent by an attacker.
 * Can only be used to read ASN.1 sequences, will throw MalformedException if
 ...
 * </pre>
 *
 * @version $Id: LimitLengthASN1Reader.java 22152 2015-11-04 15:42:43Z
 *     mikekushner $
 */
public class LimitLengthASN1Reader extends ASN1InputStream {
   ...
   * @throws MalformedRequestException if the number of bytes read is less than
   *     length, i.e. asn.1 length tag was invalid
   */
  private byte[] readTopASN1(final int length)
      throws IOException, MalformedRequestException {
    // This small code snippet is inspired/copied from apache IO utils by Tomas
    // Gustavsson...
    final byte[] buf =
        new byte
            [length]; // buf of length length, normal optimal case is only one
                      // read operation below
    int n = 0;
    int bytesRead = 0;
    // We must always read until it returns -1, make sure we read maximum length
    // bytes
  ...
  /**
   * Reads all bytes for the first ASN.1 object in the stream. Limits the size
   * that is ever read to MAX_REQUEST_SIZE.
   *
   * @return all bytes for the first ASN.1 object in the stream.
   * @throws IOException IO fail
   * @throws MalformedRequestException if the request is too large or not
   *     correctly GET encoded.
   */
  public byte[] readFirstASN1Object()
    ...
    // the classic way, limiting the size of bytes read.
    if (length < 0) { // undefined length
      ...
      if (this.contentLength
        ...
        while (((len = read(t)) != -1)
            && (r
                < LimitLengthASN1Reader
                    .MAX_REQUEST_SIZE)) { // never read more than
                                          // MAX_OCSP_REQUEST_SIZE bytes
      ...
      return readTopASN1(
          this.contentLength
              - tlByteLength); // 'tlByteLength' bytes already read.
                               // 'this.contentLength <= MAX_REQUEST_SIZE'
                               // tested above.
    }
