...
/**
 * Class Holding cache variable for SCEP configuration. Needed because EJB spec
 * does not allow volatile, non-final fields in session beans. This is a trivial
 * cache, too trivial, it needs manual handling of setting the cache variable,
 * this class does not keep track on if the cache variable is null or not, the
 * using class must ensure that it does not try to use a null value. Only the
 * method "needsUpdate will return true of the cache variable is null.
 *
 * @version $Id: ScepConfigurationCache.java 22740 2016-02-05 13:28:45Z
 *     mikekushner $
 */
public final class ScepConfigurationCache implements ConfigurationCache {
  ...
  /**
   * help variable used to control that ScepConfiguration update isn't performed
   * to often.
   */
  ...
  public boolean needsUpdate() {
    if (scepConfigurationCache != null
        && lastupdatetime
