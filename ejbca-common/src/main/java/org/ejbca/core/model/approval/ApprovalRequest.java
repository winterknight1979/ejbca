...
/**
 * Abstract Base class representing one approval request created when an
 * administrator performs an action that requires an approval.
 *
 * <p>Contains information like: Admin that performs the request Data necessary
 * to display the request to the approver Eventual data necessary to execute the
 * request.
 *
...
public abstract class ApprovalRequest implements Externalizable {
  ...
  /**
   * Admins who have edited the request, in order of time. The last admin will
   * not be allowed to approve the request.
   */
  ...
  /**
   * Generate an approval hash (called ID which is confusing since there is a
   * unique requestID as well) for this type of approval, the same request i.e
   * the same admin want's to do the same thing twice should result in the same
   * approval hash. This is the value that will be stored in the
   * ApprovalData.approvalId column. This hash is not used to identify a
   * specific request, but is used to be able to compare to requests if they are
   * for the same thing. As an example trying to add the exact same user twice
   * will result in the same approval hash so it is possible to find an already
   * existing request for adding this user.
   *
   * @return a hash code for the action the request is for, should be the same
   *     code every time the same action is performed.
   */
   ...
   * a decision about the request. The AddEndEntityApprovalRequest and
   * EditEndEntityApprovalRequest classes have a more detailed overloaded
   * version, that performs database queries to fill in the CA and profile
   * names, etc.
   *
   ...
   */
  public abstract List<ApprovalDataText> getNewRequestDataAsText(
   ...
   * @return a List of ApprovalDataText
   */
  public abstract List<ApprovalDataText> getOldRequestDataAsText(
      AuthenticationToken admin);

  /**
   * This method is used to check if this is an allowed transition between two
   * states, so that it does not require approval. Override this method to add
   * allowed transitions.
   *
   * @return true if this transition does not require approval, false by
   *     default.
   */
  ...
  /**
   * Method returning the number of required approvals in order to execute the
   * request.
   *
   * @return int
   */
  ...
  /**
   * @return the related ca id. The approving administrator must be authorized
   *     to this ca in order to approve it.
   */
  public int getCAId() {
    return cAId;
  }
  ...
  /**
   * @return the related end entity profile id. The approving administrator must
   *     be authorized to this profile in order to approve it.
   */
  ...
  /**
   * Returns the certificate of the request admin, if there is any. Walks
   * through credentials of the request admins AuthenticationToken and returns
   * the first certifciate encountered.
   *
   ...
   * @return true if admin was the last of admins who edited this approval
  ...
  public boolean isEditedByMe(final AuthenticationToken admin) {
    ...
    if (editedByAdmins.isEmpty()) {
      ...
      final AuthenticationToken lastEditedBy =
          editedByAdmins.get(editedByAdmins.size() - 1).getAdmin();
   ...
   * @param step to query
   * @return bool
   * @deprecated this method denotes a outdated feature pertaining to hard
   *     tokens, and should not be used.
   */
  @Deprecated
  public boolean isStepDone(final int step) {
   ...
   * @param step to query
   */
  public void markStepAsDone(final int step) {
  ...
  public void readExternal(final ObjectInput in)
    ...
    if (version == 3) {
      // Version 2 and 3 only care about the certificate from the old Admin
      // object
      final Admin admin = (Admin) in.readObject();
      final X509Certificate x509cert =
          (X509Certificate) admin.getAdminInformation().getX509Certificate();
    ...
    if (version == 4) {
      ...
      if (this.requestAdmin instanceof LocalJvmOnlyAuthenticationToken) {
        ...
        LocalJvmOnlyAuthenticationToken localtoken =
            (LocalJvmOnlyAuthenticationToken) this.requestAdmin;
        localtoken.initRandomToken();
      } else if (this.requestAdmin instanceof CliAuthenticationToken) {
        // A Cli authentication token was probably used already and must thus be
        // "re-registered"
        CliAuthenticationToken ctok =
            (CliAuthenticationToken) this.requestAdmin;
