...
/**
 * UserDataSourceVO is a value object returned from the fetch method of a user
 * data source.
 *
 * <p>Contains two things one is the EndEntityInformationand the other is a set
 * of constants indicating which fields that should be modifyable by the RA.
 *
 * @version $Id: UserDataSourceVO.java 25216 2017-02-08 12:27:16Z anatom $
 */
public class UserDataSourceVO implements Serializable {
  ...
  public static final int[] AVAILABLEMODIFYABLEFIELDS = {
    ...
    ISMODIFYABLE_CAID,
    ISMODIFYABLE_EMAILDATA,
    ISMODIFYABLE_TYPE,
    ISMODIFYABLE_ENDENTITYPROFILE,
    ISMODIFYABLE_CERTIFICATEPROFILE,
    ISMODIFYABLE_TOKENTYPE,
    ISMODIFYABLE_HARDTOKENISSUER,
    DNFieldExtractor.UID,
    DNFieldExtractor.CN,
    DNFieldExtractor.SN,
    DNFieldExtractor.GIVENNAME,
    DNFieldExtractor.INITIALS,
    DNFieldExtractor.SURNAME,
    DNFieldExtractor.T,
    DNFieldExtractor.OU,
    DNFieldExtractor.O,
    DNFieldExtractor.L,
    ...
    DNFieldExtractor.DC,
    DNFieldExtractor.C,
    DNFieldExtractor.UNSTRUCTUREDADDRESS,
    DNFieldExtractor.UNSTRUCTUREDNAME,
    DNFieldExtractor.DNSNAME,
    DNFieldExtractor.IPADDRESS,
    DNFieldExtractor.DIRECTORYNAME,
    DNFieldExtractor.URI,
    DNFieldExtractor.UPN,
    DNFieldExtractor.GUID,
    DNFieldExtractor.KRB5PRINCIPAL,
    DNFieldExtractor.PERMANTIDENTIFIER,
    DNFieldExtractor.SUBJECTIDENTIFICATIONMETHOD,
    DNFieldExtractor.DATEOFBIRTH,
    DNFieldExtractor.PLACEOFBIRTH,
    DNFieldExtractor.GENDER,
   ...
   *     DNFieldExtractor constants defined in the AVAILABLEMODIFYABLEFIELDS
   *     array.
   * @return true if the field should be modifyable.
   */
  public boolean isFieldModifyable(final int field) {
    return isModifyableSet.contains(field);
  }

  /**
   * Method returning the complete isModifyable Set, it is recommended that the
   * isFieldModifyable(int field) method is used instead.
   *
