 ...
 *     $
 */
public class PublicKeyBlacklistEntry extends BlacklistEntry
  ...
  /**
   * Public key fingerprint digest algorithm. Matching blacklists created by
   * debian: https://launchpad.net/ubuntu/+source/openssl-blacklist/
   * https://launchpad.net/ubuntu/+source/openssh-blacklist
   * https://launchpad.net/ubuntu/+source/openvpn-blacklist/ The blacklists
   * contains the SHA1 hash with the first 20 bytes remove, i.e. the last 20
   * bytes
   */
   ...
   */
  public PublicKeyBlacklistEntry(
      final int id, final String fingerprint, final String keyspec) {
  ...
  /**
   * Gets the fingerprint. Fingerprint of the key to compare, this is not just a
   * normal fingerprint over the DER encoding For RSA keys this is the hash (see
   * {@link #DIGEST_ALGORITHM}) over the binary bytes of the public key modulus.
   * This because blacklist is typically due to weak random number generator
   * (Debian weak keys) and we then want to capture all keys generated by this,
   * so we don't want to include the chosen e, only the randomly generated n.
   * For other keys (ECDSA and DSA) it's a fingerprint over the public key
   * encoding.
   *
   * @return fingerprint
   */
  ...
  /**
   * Sets the fingerprint Fingerprint of the key to compare, this is not just a
   * normal fingerprint over the DER encoding For RSA keys this is the hash (see
   * {@link #DIGEST_ALGORITHM}) over the binary bytes of the public key modulus.
   * This because blacklist is typically due to weak random number generator
   * (Debian weak keys) and we then want to capture all keys generated by this,
   ...
   *     fingerprint can be created (due to unhandled key type for example)
   */
  public static String createFingerprint(final PublicKey pk) {
    ...
    if (pk instanceof RSAPublicKey) {
      // Fingerprint of the key to compare, this is not just a normal
      // fingerprint over the DER encoding
      // For RSA keys this is the hash (see {@link #DIGEST_ALGORITHM}) over the
      // binary bytes of the public key modulus.
      // This because blacklist is typically due to weak random number generator
      // (Debian weak keys) and we then want to capture all keys
      // generated by this, so we don't want to include the chosen e, only the
      ...
      try {
        digest =
            MessageDigest.getInstance(PublicKeyBlacklistEntry.DIGEST_ALGORITHM);
      } catch (NoSuchAlgorithmException e) {
        throw new RuntimeException(
            "Unable to create hash " + PublicKeyBlacklistEntry.DIGEST_ALGORITHM,
            e);
      }
      digest.reset();
  ...
  /**
   * Gets the public key, have to be set transient with {@link
   * #setPublicKey(PublicKey)}, not available after serialization or storage.
   *
