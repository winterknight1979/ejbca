 ...
 *     aveen4711 $
 */
public class HardTokenEncryptCAService extends ExtendedCAService
  ...
  public ExtendedCAServiceResponse extendedService(
    ...
    if (!(request instanceof HardTokenEncryptCAServiceRequest)) {
      throw new IllegalExtendedCAServiceRequestException(
          "Not a HardTokenEncryptCAServiceRequest: "
              + request.getClass().getName());
    }

    final HardTokenEncryptCAServiceRequest serviceReq =
    ...
    if (serviceReq.getCommand()
      ...
      try {
        returnval =
            new HardTokenEncryptCAServiceResponse(
                HardTokenEncryptCAServiceResponse.TYPE_ENCRYPTRESPONSE,
                getCa()
                    .encryptData(
                        cryptoToken,
                        serviceReq.getData(),
      ...
      if (serviceReq.getCommand()
          == HardTokenEncryptCAServiceRequest.COMMAND_DECRYPTDATA) {
        try {
          returnval =
              new HardTokenEncryptCAServiceResponse(
                  HardTokenEncryptCAServiceResponse.TYPE_DECRYPTRESPONSE,
                  getCa()
                      .decryptData(
                          cryptoToken,
                          serviceReq.getData(),
