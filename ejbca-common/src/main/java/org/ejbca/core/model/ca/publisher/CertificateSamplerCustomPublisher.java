 ...
 *
 * <p>The configured sampling method and its parameters (if any) determines
 * which certificates that should be sampled.
 *
 * @see SamplingMethod
 * @version $Id: CertificateSamplerCustomPublisher.java 23625 2016-06-07
 *     19:32:11Z anatom $
 */
public class CertificateSamplerCustomPublisher implements ICustomPublisher {
  ...
  public void testConnection() throws PublisherConnectionException {
    ...
    for (String name : config.stringPropertyNames()) {
      if (name.startsWith(PROPERTYPREFIX_PROFILEID)) {
        if (name.endsWith(PROPERTYSUFFIX_SAMPLINGMETHOD)
            && name.length()
                > PROPERTYPREFIX_PROFILEID.length()
                    + PROPERTYSUFFIX_SAMPLINGMETHOD.length()) {
          final String profileIdString =
              name.substring(
                  PROPERTYPREFIX_PROFILEID.length(),
                  name.indexOf(PROPERTYSUFFIX_SAMPLINGMETHOD));
          int profileId;
          try {
          ...
          try {
            SamplingMethod profileMethod =
                SamplingMethod.valueOf(config.getProperty(name));

            // Check that there is an pvalue for this profile
            if (SamplingMethod.SAMPLE_PROBABILISTIC.equals(profileMethod)) {
              getPvalue(
                  profileId); // Throws exception if the value (or default
        ...
        } else if (name.endsWith(PROPERTYSUFFIX_PVALUE)
            && name.length()
                > PROPERTYPREFIX_PROFILEID.length()
                    + PROPERTYSUFFIX_PVALUE.length()) {
          final String profileIdString =
              name.substring(
                  PROPERTYPREFIX_PROFILEID.length(),
                  name.indexOf(PROPERTYSUFFIX_PVALUE));
          try {
  ...
  public boolean storeCRL(
    ...
    return true; // This ICustomPublisher does not care about CRLs and chooses
                 // to simply return SUCCESS for those.
  }
  ...
  public boolean storeCertificate(
      ...
      final org.cesecore.certificates.endentity.ExtendedInformation
          extendedinformation)
      throws PublisherException {
    if (LOG.isTraceEnabled()) {
      LOG.trace(
          ">storeCertificate, Storing Certificate of profileid: "
    ...
    return true; // If we got this far either we chose to not store the
   ...
   */
  private boolean isTimeToSample(final int profileId)
      throws PublisherException {
      ...
      case SAMPLE_PROBABILISTIC:
        {
          ...
          // Gets an pseudorandomly value between 0 and 1 from an approximately
          // uniform distribution
          // Notice: This method call is thread-safe, however uses
          // synchronization which might decrease performance
          //         We can not use a random object as instance variable as a
          // new instance of this class is created for every request and
          // requests at the same time would then be initialized with the same
          // seed
          //         When we switch to Java 7 we could possibly use
  ...
  private SamplingMethod getMethod(final int profileId)
      throws PublisherException {
    final SamplingMethod result;

    String propertyKey =
        PROPERTYPREFIX_PROFILEID + profileId + PROPERTYSUFFIX_SAMPLINGMETHOD;
  ...
  private Double getPvalue(final int profileId) throws PublisherException {
    final Double result;

    String propertyKey =
        PROPERTYPREFIX_PROFILEID + profileId + PROPERTYSUFFIX_PVALUE;
    String propertyString = config.getProperty(propertyKey);
    if (propertyString == null) {
  ...
  public enum SamplingMethod {
    ...
    /**
     * Every certificate is sampled with the probability as specified as the
     * pvalue.
     */
