...
/**
 * This is a help class used to manipulate SVG images. It replaces all
 * occurrenses of specified variables in the images with the corresponding
 * userdata.
 *
...
public class SVGImageManipulator {
  ...
  private static final Pattern STARTDATE =
      Pattern.compile("\\$STARTDATE", Pattern.CASE_INSENSITIVE);
...
/** Congig. */
  ...
  // Pattern.compile("\\$CUSTOMTEXTROW2", Pattern.CASE_INSENSITIVE);
  //    private static final Pattern CUSTOMTEXTROW3 =
  // Pattern.compile("\\$CUSTOMTEXTROW3", Pattern.CASE_INSENSITIVE);
  //    private static final Pattern CUSTOMTEXTROW4 =
  // Pattern.compile("\\$CUSTOMTEXTROW4", Pattern.CASE_INSENSITIVE);
  //    private static final Pattern CUSTOMTEXTROW5 =
  // Pattern.compile("\\$CUSTOMTEXTROW5", Pattern.CASE_INSENSITIVE);
  //    private static final Pattern COPYOFSN = Pattern.compile("\\$COPYOFSN",
  // Pattern.CASE_INSENSITIVE);
  //    private static final Pattern COPYOFSNWITHOUTPREFIX =
  ...
  public Printable print(
    ...
    // DNFieldExtractor subaltnamefields = new
    // DNFieldExtractor(dn,DNFieldExtractor.TYPE_SUBJECTALTNAME);
    Date currenttime = new Date();
    String startdate =
        DateFormat.getDateInstance(DateFormat.SHORT).format(currenttime);

    String enddate =
        DateFormat.getDateInstance(DateFormat.SHORT)
            .format(new Date(currenttime.getTime() + (this.validityms)));
    String hardtokensnwithoutprefix =
        hardtokensn.substring(this.hardtokensnprefix.length());
    String copyoftokensnwithoutprefix =
        copyoftokensn.substring(this.hardtokensnprefix.length());
  ...
  private void process(
      ...
      final String[] pukcodes,
      final String hardtokensn,
      final String hardtokensnwithoutprefix,
      final String copyoftokensn,
      final String copyoftokensnwithoutprefix,
      final String startdate,
      final String enddate,
      final SVGDocument clone) {
  ...
  private String processString(
    ...
    text =
        UID.matcher(text)
            .replaceAll(dnfields.getField(DNFieldExtractor.UID, 0));
    text =
        CN.matcher(text).replaceAll(dnfields.getField(DNFieldExtractor.CN, 0));
    text =
        OU.matcher(text).replaceAll(dnfields.getField(DNFieldExtractor.OU, 0));
    ...
    text =
        LOCATION
            .matcher(text)
            .replaceAll(dnfields.getField(DNFieldExtractor.L, 0));
    text =
        TITLE
            .matcher(text)
            .replaceAll(dnfields.getField(DNFieldExtractor.T, 0));
    text =
        INITIALS
            .matcher(text)
            .replaceAll(dnfields.getField(DNFieldExtractor.INITIALS, 0));
    text =
        SN.matcher(text).replaceAll(dnfields.getField(DNFieldExtractor.SN, 0));
    text =
        SURNAME
            .matcher(text)
            .replaceAll(dnfields.getField(DNFieldExtractor.SURNAME, 0));
    text =
        GIVENNAME
            .matcher(text)
            .replaceAll(dnfields.getField(DNFieldExtractor.GIVENNAME, 0));
    ...
    text =
        HARDTOKENSNWITHOUTPREFIX
            .matcher(text)
            .replaceAll(hardtokensnwithoutprefix);
