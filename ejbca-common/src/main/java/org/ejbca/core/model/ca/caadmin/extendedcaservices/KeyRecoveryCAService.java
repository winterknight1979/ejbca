 ...
 */
public class KeyRecoveryCAService extends ExtendedCAService
  ...
  public ExtendedCAServiceResponse extendedService(
    ...
    if (!(request instanceof KeyRecoveryCAServiceRequest)) {
      throw new IllegalExtendedCAServiceRequestException(
          "Not a KeyRecoveryCAServiceRequest: " + request.getClass().getName());
    }

    final KeyRecoveryCAServiceRequest serviceReq =
        (KeyRecoveryCAServiceRequest) request;
    ExtendedCAServiceResponse returnval = null;
    if (serviceReq.getCommand()
      ...
      try {
        ...
        try {
          keyId =
            ...
            Exception
                e) { // NOPMD: we catch wide here because we do not want this to
                     // cause a transaction failure
          log.warn(
              "Error creating subjectKeyId for key recovery, cryptoToken: "
                  + cryptoToken.getId()
                  + ", keyAlias: "
                  + keyAlias,
              e);
        ...
        returnval =
            new KeyRecoveryCAServiceResponse(
                KeyRecoveryCAServiceResponse.TYPE_ENCRYPTKEYSRESPONSE,
                X509CA.encryptKeys(
                    cryptoToken, keyAlias, serviceReq.getKeyPair()),
                cryptoToken.getId(),
                keyAlias,
                keyId);
      } catch (Exception e) {
      ...
      if (serviceReq.getCommand()
        ...
        try {
          ...
          if (StringUtils.isEmpty(keyAlias)) {
            // If we haven't stored any key alias, in the entry, use the default
            // one
          ...
          try {
            ...
            keys =
                X509CA.decryptKeys(
                    cryptoToken, keyAlias, serviceReq.getKeyData());
          } catch (
              Exception
                  e) { // NOPMD: we have to catch wide here, using the wrong key
                       // to decrypt can result in several different errors
            if (log.isDebugEnabled()) {
              log.debug(
                  "Decryption with alias '"
                      + keyAlias
                      + "' failed, trying defaultAlias: ",
                  e);
            }
            // Did we use the wrong key alias? Try with the default one, if we
            // din't do that already
            if (!StringUtils.equals(keyAlias, defaultAlias)) {
              if (log.isDebugEnabled()) {
                log.debug(
                    "Trying to decrypt using default alias '"
                        + defaultAlias
                        + "' from crypto token "
                        + cryptoToken.getId());
              }
              keys =
                  X509CA.decryptKeys(
                      cryptoToken, defaultAlias, serviceReq.getKeyData());
            } else {
              // Just re-throw if we have nothing to test here
              throw e;
            }
          }
          // Creating the KeyId in String format may just throw an exception, we
          // will log this but store the cert and ignore the error
          String keyId = null;
          try {
            keyId =
                new String(
          ...
          returnval =
              new KeyRecoveryCAServiceResponse(
                  KeyRecoveryCAServiceResponse.TYPE_DECRYPTKEYSRESPONSE,
                  keys,
                  cryptoToken.getId(),
                  keyAlias,
                  keyId);
        ...
        throw new IllegalExtendedCAServiceRequestException(
            "Illegal command: " + serviceReq.getCommand());
      }
    }
    if (log.isTraceEnabled()) {
      log.trace("<extendedService");
