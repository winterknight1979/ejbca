...
public class OCSPUnidResponse {
  ...
  //
  /**
   * This is the standard code when no error occurred. Ideally this should
   * always be the returned value.
   */
  public static final int ERROR_NO_ERROR = 0;
  /**
   * An unknown error has occurred (for example internal server error on the
   * OCSP responder) .
   */
  ...
  /**
   * This error is returned when the signature of the OCSP-response sent by the
   * server has an invalid signature. This should typically never happen unless
   * the OCSP-server is compromised in someway, a fake OCSP-server is installed
   * or something went wrong with the communication so the response was
   * truncated.
   */
  public static final int ERROR_INVALID_SIGNATURE = 4;
  /**
   * This error is returned when the signerId in the OCSP-response sent by the
   * server does not match the first certificate in the chain in the response.
   * This should typically never happen unless the OCSP-server is broken.
   */
  public static final int ERROR_INVALID_SIGNERID = 5;
  /**
   * This error is returned when the OCSP signers certificate can not be
   * verified using the CA-certificate. This should typically never happen
   * unless the OCSP-server is broken or compromised.
   */
  public static final int ERROR_INVALID_SIGNERCERT = 6;
  /**
   * You did not specify a URI in the call, and there is not one embedded in the
   * certificate.
   */
