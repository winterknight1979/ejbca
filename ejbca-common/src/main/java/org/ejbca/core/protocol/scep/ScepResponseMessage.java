 ...
 *
 * @version $Id: ScepResponseMessage.java 28201 2018-02-07 08:33:29Z
 *     andresjakobs $
 */
public class ScepResponseMessage implements CertificateResponseMessage {
  ...
  public boolean create() throws CertificateEncodingException, CRLException {
    ...
    try {
      ...
      CMSTypedData msg;
      // Create encrypted response if this is success and NOT a CRL response
      // message
      if (status.equals(ResponseStatus.SUCCESS)) {

        CMSEnvelopedDataGenerator edGen = new CMSEnvelopedDataGenerator();
        // Add the issued certificate to the signed portion of the CMS (as
        // signer, degenerate case)
        List<X509Certificate> certList = new ArrayList<X509Certificate>();
        if (cert != null) {
          log.debug("Adding certificates to response message");
          certList.add((X509Certificate) cert);
          // Add the CA cert, it's optional but Cisco VPN client complains if it
          // isn't there
          if (includeCACert) {
            if (caCert != null) {
              // If we have an explicit CAcertificate
              log.debug(
                  "Including explicitly set CA certificate in SCEP response.");
              certList.add((X509Certificate) caCert);
            } else {
              // If we don't have an explicit caCert, we think that the signCert
              // is the CA cert
              // If we have an explicit caCert, the signCert is probably the RA
              // certificate, and we don't include that one
              log.debug(
                  "Including message signer certificate in SCEP response.");
        ...
        gen.addCertificates(
            new CollectionStore<>(
                CertTools.convertToX509CertificateHolder(certList)));
        if (crl != null) {
          ...
          try {
            X509Certificate rec =
                CertTools.getCertfromByteArray(
                    recipientKeyInfo, X509Certificate.class);
            log.debug(
                "Added recipient information - issuer: '"
                    + CertTools.getIssuerDN(rec)
                    + "', serno: '"
                    + CertTools.getSerialNumberAsString(rec));
            edGen.addRecipientInfoGenerator(
                new JceKeyTransRecipientInfoGenerator(rec)
                    .setProvider(BouncyCastleProvider.PROVIDER_NAME));
          } catch (CertificateParsingException e) {
            throw new IllegalArgumentException(
                "Can not decode recipients self signed certificate!", e);
          }
        } else {
          edGen.addRecipientInfoGenerator(
              new JceKeyTransRecipientInfoGenerator((X509Certificate) cert)
                  .setProvider(BouncyCastleProvider.PROVIDER_NAME));
        }
        try {
          ...
          CMSEnvelopedData ed =
              edGen.generate(
                  new CMSProcessableByteArray(s.getEncoded()),
                  jceCMSContentEncryptorBuilder.build());
          if (log.isDebugEnabled()) {
            log.debug(
                "Enveloped data is " + ed.getEncoded().length + " bytes long");
          }
      ...
      // add authenticated attributes...status, transactionId, sender- and
      // recipientNonce and more...
      ...
      log.debug(
          "Signing SCEP message with cert: " + CertTools.getSubjectDN(cacert));
      String signatureAlgorithmName =
          AlgorithmTools.getAlgorithmNameFromDigestAndKey(
              digestAlg, signKey.getAlgorithm());
      try {
        ...
        gen1.addSignerInfoGenerator(
            builder.build(contentSigner, (X509Certificate) cacert));
