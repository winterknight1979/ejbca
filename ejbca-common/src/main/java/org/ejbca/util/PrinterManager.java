...
public final class PrinterManager {
  ...
  /**
   * Method that returns the names of all the available printers or an empty
   * list if no printers were found or an error occurred.
   *
   * @return printers
   */
  public static String[] listPrinters() {
    ...
    try {
      ...
      for (int i = 0; i < printService.length; i++) {
        ...
        Throwable e) { // NOPMD: printers will give all sorts of wacky errors.
      String msg =
          "There might be a problem with one of the printers' configuration: ";
  ...
  /**
   * Method performin the actual processing and printing of an SVG Template
   *
   * <p>Imporant the method is caching template data so to subsekvent calls with
   * the samt svtTemplate name will result in the same SVG data being processed.
   * Avoid the same svgTemplate name for two different SVG files
   *
  ...
  public static void print(
      ...
      final String svtTemplateName,
      final String sVGData,
      final int copies,
      final int visualVaildity,
      final EndEntityInformation endEntityInformation,
      final String[] pINs,
      final String[] pUKs,
      final String hardTokenSerialPrefix,
      final String hardTokenSN,
      final String copyOfHardTokenSN)
      throws PrinterException {
    if (log.isTraceEnabled()) {
      log.trace(
          ">print: " + endEntityInformation.getUsername() + ", " + printerName);
    }
    if (currentService == null
      ...
      while (i < printService.length
          && !trimemdPrinterName.equalsIgnoreCase(printService[i].getName())) {
        i++;
      }
    ...
    try {
      if (currentSVGTemplateName == null
          || !currentSVGTemplateName.equals(svtTemplateName)) {
        sVGImagemanipulator =
            new SVGImageManipulator(
                new StringReader(sVGData),
                visualVaildity,
                hardTokenSerialPrefix);
      }
      ...
      throw new PrinterException(
          "Error occured when processing the SVG data :" + e.getMessage());
    }
    if (log.isTraceEnabled()) {
      log.trace(
          "<print: " + endEntityInformation.getUsername() + ", " + printerName);
