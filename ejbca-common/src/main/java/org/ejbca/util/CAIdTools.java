 ...
 *
 * <p>The methods always take an object to change, as well as the old and new CA
 * Ids and the new Subject DN. They return true if the object was changed and
 * should be updated in the database.
 *
 * <p>The reason for having the methods here and not in the respective objects
 * is:
 *
 * <ol>
 *   <li>Access rules are simply a list of access rule objects, so it can't have
 *       any methods directly on it anyway.
 *   <li>Some objects do not know about all of their properties. The
 *       ServiceConfiguration object does not know about the different types of
 *       services it can store.
 *   <li>Some objects do not have a single base class, such as
 *       InternalKeyBindings which has two.
 * </ol>
 *
...
public final class CAIdTools {
   ...
   * @return True if the certificate profile was changed. If so it should be
   *     persisted to the database.
   */
  public static boolean updateCAIds(
      final CertificateProfile certProfile,
      final int fromId,
      final int toId,
      final String toSubjectDN) {
    boolean changed = false;
    final List<Integer> availableCAs =
        new ArrayList<>(certProfile.getAvailableCAs());
   ...
   * @param toId New CA Id to replace with.
   * @param toSubjectDN New CA Subject DN.
   * @return True if the end entity profile was changed. If so it should be
   *     persisted to the database.
   */
  public static boolean updateCAIds(
   ...
   * @param toId New CA Id to replace with.
   * @param toSubjectDN New CA Subject DN.
   * @return True if the data source was changed. If so it should be persisted
   *     to the database.
   */
  public static boolean updateCAIds(
      ...
      final String toSubjectDN) {
    boolean changed = false;
    final List<Integer> applicableCAs =
        new ArrayList<>(dataSource.getApplicableCAs());
    // The list is modified so we can't use an iterator
   ...
   * @param toId New CA Id to replace with.
   * @param toSubjectDN New CA Subject DN.
   * @return True if the service was changed. If so it should be persisted to
   *     the database.
   */
  public static boolean updateCAIds(
    ...
    final String idsToCheckStr =
        workerProps.getProperty(IWorker.PROP_CAIDSTOCHECK);
    if (!StringUtils.isEmpty(idsToCheckStr)) {
      ...
      if (changed) {
        workerProps.setProperty(
            IWorker.PROP_CAIDSTOCHECK, StringUtils.join(caIds, ';'));
   ...
   * @param toId New CA Id to replace with.
   * @param toSubjectDN New CA Subject DN.
   * @return True if the key binding was changed. If so it should be persisted
   *     to the database.
   */
  public static boolean updateCAIds(
    ...
    for (InternalKeyBindingTrustEntry trustentry
      ...
      trustentries.add(
          new InternalKeyBindingTrustEntry(
              trustCaId, trustentry.fetchCertificateSerialNumber()));
    }
    if (changed) {
   ...
   * @param toId New CA Id to replace with.
   * @param toSubjectDN New CA Subject DN.
   * @return True if the configuration was changed. If so it should be persisted
   *     to the database.
   */
  public static boolean updateCAIds(
   ...
   * @param toId New CA Id to replace with.
   * @param toSubjectDN New CA Subject DN.
   * @return True if the configuration changed. If so it should be persisted to
   *     the database.
   */
  public static boolean updateCAIds(
   ...
   */
  public static boolean updateCAIds(
      final GlobalOcspConfiguration ocspConfig,
      final int fromId,
      final int toId,
      final String toSubjectDN) {
    boolean changed = false;
  ...
  /**
   * Updates any references to a CA's CAId and Subject DN in the given role and
  ...
  public static boolean updateCAIds(
      ...
      final String toSubjectDN) {
    boolean changed = false;
    // Look for references from access rules (currently only the /ca/<CA ID>
    // rule) */
    final String oldResource =
        AccessRulesHelper.normalizeResource(
            StandardRules.CAACCESS.resource() + String.valueOf(fromId));
    final String newResource =
        AccessRulesHelper.normalizeResource(
            StandardRules.CAACCESS.resource() + String.valueOf(toId));
    ...
    for (final RoleMember roleMember : roleMembers) {
      if (roleMember.getTokenIssuerId() == fromId) {
        // Also check that tokenIssuerId refers to a CA. This check is more
        // expensive performance-wise so it's done last
   ...
   * Updates any references to a CA's CAId and Subject DN.
   *
   * @param roleName Name of the role. Used when creating roles to replace the
   *     old roles with.
  ...
  public static boolean updateCAIds(
      ...
      final Map<Integer, AccessRuleData> rules,
      final Map<Integer, AccessUserAspectData> users,
      final int fromId,
      final int toId,
      final String toSubjectDN) {
    ...
    for (int id : new ArrayList<>(rules.keySet())) {
      ...
      if (accessRuleName.equals(toReplace)
          || accessRuleName.startsWith(toReplaceSlash)) {
        final String newName =
            StandardRules.CAACCESS.resource()
                + String.valueOf(toId)
                + accessRuleName.substring(toReplace.length());
        final int state = rule.getState();
        rule =
            new AccessRuleData(
                roleName,
                newName,
                rule.getInternalState(),
                rule.getRecursive());
        rule.setState(state);
        rules.put(id, rule);
        changed = true;
      ...
      if (extsvc instanceof CmsCAServiceInfo) {
        ...
        extsvc =
            new CmsCAServiceInfo(
                extsvc.getStatus(),
                "CN=CMSCertificate, " + casubjdn,
                cmssvc.getSubjectAltName(),
                cmssvc.getKeySpec(),
                cmssvc.getKeyAlgorithm());
      }
      extsvcs.add(extsvc);
    }
