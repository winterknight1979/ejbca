 ...
 * tables.
 *
 * <p>It's main function is getQueryString which returns a string which should
 * be placed in the 'WHERE' clause of a SQL statement.
 *
 * @version $Id: Query.java 25704 2017-04-18 14:42:43Z jeklund $
 */
public class Query implements Serializable {
  ...
  /**
   * @deprecated Type of query is no longer support and only kept as a reminder
   *     not to re-reuse the value 0
   */
  ...
  /**
   * Adds a time restraint to the query. Both start and enddate parameters
   * cannot be null This method should only be used in ra user queries.
   *
   * @param matchwith should indicate which field to match with, on of the
   *     TimeMatch.MATCH_WITH constants.
   ...
   */
  public void add(
      final int matchwith, final Date startdate, final Date enddate) {
    matches.add(new TimeMatch(type, matchwith, startdate, enddate));
  }

  /**
   * Adds a time restraint and a connector to the query. Both parameter cannot
   * be null. This method should only be used in log queries.
   *
   * @param startdate gives the start date of the query or null if it no
   *     startdate.
   * @param enddate gives the end date of the query or null if it no startdate.
   ...
   */
  public void add(
      final Date startdate, final Date enddate, final int connector) {
    matches.add(new TimeMatch(type, startdate, enddate));
    connectors.add(Integer.valueOf(connector));
  }

  /**
   * Adds a time restraint and a connector to the query. Both start and enddate
   * parameters cannot be null. This method should only be used in ra user
   * queries.
   *
   * @param matchwith should indicate which field to match with, on of the
   *     TimeMatch.MATCH_WITH constants.
   ...
   *
   * @param matchwith should be one of the the UserMatch.MATCH_WITH or
   *     LogMatch.MATCH_WITH constants depending on query type.
   ...
   * @throws NumberFormatException if there is an illegal character in
   *     matchvalue string.
   */
  public void add(
  ...
  /**
   * Adds a match of type UserMatch or ApprovalMatch and a connector to the
   * query.
   *
   * @param matchwith should be one of the the UserMatch.MATCH_WITH or
   *     ApprovalMatch.MATCH_WITH depending on query type.
   * @param matchtype should be one of BasicMatch.MATCH_TYPE constants.
   * @param matchvalue should be a string representation to match against.
   * @param connector should be one of the 'CONNECTOR' constants.
   * @throws NumberFormatException if there is an illegal character in
   ...
   *     matchvalue string.
   */
  public void add(final int connector) {
    connectors.add(Integer.valueOf(connector));
  }
  ...
  /**
   * Checks if the present query is legal by checking if every match is legal
   * and that the number of connectors is one less than matches.
   *
