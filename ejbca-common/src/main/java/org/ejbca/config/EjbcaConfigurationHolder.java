 ...
 *
 * @version $Id: EjbcaConfigurationHolder.java 27627 2017-12-21 12:08:15Z
 *     henriks $
 */
public final class EjbcaConfigurationHolder {
  ...
  /**
   * ejbca.properties must be first in this file, because CONFIGALLOWEXTERNAL is
   * defined in there.
   */
   ...
   */
  public static final String CONFIGALLOWEXTERNAL =
  ...
  private static CompositeConfiguration instanceInternal() {
    // read ejbca.properties, from config file built into jar, and see if we
    // allow configuration by external files
    boolean allowexternal = false;
    try {
      final URL url =
          EjbcaConfigurationHolder.class.getResource(
              "/conf/" + CONFIG_FILES[0]);
      if (url != null) {
    ...
    if (allowexternal) {
      ...
      // Override with file in "application server home directory"/conf, this is
      // prio 2
    ...
    try {
      final URL url =
          EjbcaConfigurationHolder.class.getResource("/internal.properties");
      if (url != null) {
        final PropertiesConfiguration pc = new PropertiesConfiguration(url);
        config.addConfiguration(pc);
  ...
  public static void addConfigurationResource(final String resourcename) {
    ...
    try {
      final URL url =
          EjbcaConfigurationHolder.class.getResource("/conf/" + resourcename);
      if (url != null) {
        final PropertiesConfiguration pc = new PropertiesConfiguration(url);
        config.addConfiguration(pc);
        LOG.debug("Added url to configuration source: " + url);
   ...
   * @return String configured for property, or default value, if defaultValue
   *     is null and property is not found null is returned.
   */
   ...
   *
   * <p>NOTE: This method should only be used by tests through
   * ConfigurationSessionBean!
   *
   * @return bool
   */
