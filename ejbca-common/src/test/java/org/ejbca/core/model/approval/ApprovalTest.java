 ...
 *
 * <p>$Id: ApprovalTest.java 23688 2016-06-17 10:46:42Z mikekushner $
 */
public class ApprovalTest {
  ...
  public void testWriteExternal() throws Exception {
    ...
    X509CertificateAuthenticationToken xtok =
        (X509CertificateAuthenticationToken) rap.getAdmin();
    assertEquals(
        CertTools.getIssuerDN(testcert),
        CertTools.getIssuerDN(xtok.getCertificate()));
    assertEquals(
        CertTools.getSerialNumber(testcert),
        CertTools.getSerialNumber(xtok.getCertificate()));
    assertEquals(CertTools.getIssuerDN(testcert), rap.getAdminCertIssuerDN());
    assertEquals(
        CertTools.getSerialNumber(testcert), rap.getAdminCertSerialNumber());
    assertTrue(rap.isApproved());
    assertEquals("test", rap.getComment());
    assertEquals(apDate, rap.getApprovalDate());
    assertEquals(
        "Sequence identifier was not externalized successfully",
        sequenceIdentifier,
        rap.getStepId());
  }

  private static Collection<Approval> getApprovals(final String stringdata) {
