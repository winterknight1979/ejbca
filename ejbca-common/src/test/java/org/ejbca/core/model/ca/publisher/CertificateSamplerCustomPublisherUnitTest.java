 ...
 *
 * <p>This is a unit test and does not require EJBCA to be running.
 *
 * @version $Id: CertificateSamplerCustomPublisherUnitTest.java 22121 2015-10-29
...
public class CertificateSamplerCustomPublisherUnitTest {
   ...
   * property output folder is missing.
   *
  ...
  public void testNoOutputFolder() throws Exception {
    ...
    try {
      storeCertificate(
          publisher, CertificateConstants.CERT_ACTIVE, ANY_PROFILEID);
  ...
  /**
   * Tests that storeCertificate and testConnection throws Exception as the
   * property for the default sampling method is missing.
   *
   * @throws Exception On fail
   */
  @Test
  ...
  /**
   * Tests that storeCertificate and testConnection throws Exception as the
   * pvalue for a profile is missing.
   *
   * @throws Exception fail
   */
  @Test
  public void testNoPValueForProfileSamplingMethod() throws Exception {
    ...
    config.setProperty("default.samplingmethod", "SAMPLE_ALL");
    config.setProperty(
        "profileid." + PROFILE_A + ".samplingmethod", "SAMPLE_PROBABILISTIC");
  ...
  /**
   * Tests that testConnection and storeCertificate gives error if there is an
  ...
  public void testInvalidSamplingMethod() throws Exception {
    ...
    try {
      publisher.testConnection();
      fail(
          "Should have failed as '_INVALID_' is not an existing sampling"
              + " method");
    } catch (PublisherConnectionException expected) {
    } // NOPMD
    publisher = createMockedPublisher(config);
    ...
    config.setProperty("default.samplingmethod", "SAMPLE_ALL");
    config.setProperty(
        "profileid." + PROFILE_A + ".samplingmethod", "_INVALID_");
  ...
  /**
   * Tests that the method storeCRL always returns true as publishing/sampling
   * of CRLs are currently not supported.
   *
   * @throws Exception fail
   */
  @Test
  public void testStoreCRL() throws Exception {
    assertTrue(
        "Storing CRL is not supported but return status should be success",
        createPublisher(ANY_GOOD_PROPERTIES)
            .storeCRL(
                ANY_ADMIN, ANY_BYTEARRAY, ANY_CAFP, ANY_NUMBER, ANY_SUBJECTDN));
  }
  ...
  public void testStoreCertificateRevoked() throws Exception {
    ...
    success =
        storeCertificate(
            publisher, CertificateConstants.CERT_REVOKED, ANY_PROFILEID);
    assertTrue("Status should be success", success);
    assertFalse(
        "Certificate should not have been stored",
        publisher.isWriteCertificateCalled());

    publisher = createMockedPublisher(ANY_GOOD_PROPERTIES);
    success =
        storeCertificate(
            publisher, CertificateConstants.CERT_INACTIVE, ANY_PROFILEID);
    assertTrue("Status should be success", success);
    assertFalse(
        "Certificate should not have been stored",
        publisher.isWriteCertificateCalled());
  ...
  public void testSampleAll() throws Exception {
    ...
    assertTrue("Status should be success", success);
    assertTrue(
        "Certificate should have been stored",
        publisher.isWriteCertificateCalled());
  }

  /**
   * Tests sampling with different probabilities. This method has a change of
   * false positives but with <code>PROBABILISTIC_TRIES</code> number of tries
  ...
  public void testSampleProbabilistic() throws Exception {
    ...
    for (int i = 0; i < PROBABILISTIC_TRIES; i++) {
      ...
      assertTrue("Status should be success", success);
      assertFalse(
          "Certificate should not have been stored, i=" + i,
          publisher.isWriteCertificateCalled());
    }
    ...
    for (int i = 0; i < PROBABILISTIC_TRIES; i++) {
      ...
      assertTrue("Status should be success", success);
      assertTrue(
          "Certificate should have been stored, i=" + i,
          publisher.isWriteCertificateCalled());
      publisher.reset();
    }
  ...
  public void testDifferentProfiles() throws Exception {
    ...
    assertTrue(
        storeCertificate(
            publisher, CertificateConstants.CERT_ACTIVE, PROFILE_A));
    assertFalse(
        "Certificate in profile A should not be stored",
        publisher.isWriteCertificateCalled());

    publisher = createMockedPublisher(config);
    assertTrue(
        storeCertificate(
            publisher, CertificateConstants.CERT_ACTIVE, PROFILE_B));
    assertTrue(
        "Certificate in profile B should have been stored",
        publisher.isWriteCertificateCalled());
  ...
  public void testDifferentMethodsForProfiles() throws Exception {
    ...
    assertTrue(
        storeCertificate(
            publisher, CertificateConstants.CERT_ACTIVE, PROFILE_B));
    assertFalse(
        "Certificate in profile B should not be stored",
        publisher.isWriteCertificateCalled());
    publisher = createMockedPublisher(config);
    assertTrue(
        storeCertificate(
            publisher, CertificateConstants.CERT_ACTIVE, ANY_PROFILEID));
    assertFalse(
        "Certificate in no profile should not be stored",
        publisher.isWriteCertificateCalled());
    ...
    config.setProperty("default.pvalue", "0.0");
    config.setProperty(
        "profileid." + PROFILE_A + ".samplingmethod", "SAMPLE_ALL");
    ...
    assertTrue(
        storeCertificate(
            publisher, CertificateConstants.CERT_ACTIVE, PROFILE_A));
    assertTrue(
        "Certificate in profile A should be stored",
        publisher.isWriteCertificateCalled());
    publisher = createMockedPublisher(config);
    assertTrue(
        storeCertificate(
    ...
    assertTrue(
        storeCertificate(
            publisher, CertificateConstants.CERT_ACTIVE, ANY_PROFILEID));
    assertFalse(
        "Certificate in other profile should not be stored",
        publisher.isWriteCertificateCalled());
    ...
    config.setProperty(
        "profileid." + PROFILE_A + ".samplingmethod", "SAMPLE_ALL");
    config.setProperty(
        "profileid." + PROFILE_B + ".samplingmethod", "SAMPLE_PROBABILISTIC");
    config.setProperty("profileid." + PROFILE_B + ".pvalue", "1.0");
    ...
    assertTrue(
        storeCertificate(
            publisher, CertificateConstants.CERT_ACTIVE, PROFILE_B));
    assertTrue(
        "Certificate in profile B should be stored",
        publisher.isWriteCertificateCalled());
    publisher = createMockedPublisher(config);
    assertTrue(
        storeCertificate(
  ...
  public void testSampleNone() throws Exception {
    ...
    config.setProperty("default.samplingmethod", "SAMPLE_ALL");
    config.setProperty(
        "profileid." + PROFILE_A + ".samplingmethod", "SAMPLE_NONE");
    ...
    assertTrue(
        storeCertificate(
            publisher, CertificateConstants.CERT_ACTIVE, ANY_PROFILEID));
    assertTrue(
        "Certificate in other profile should be stored",
        publisher.isWriteCertificateCalled());
  }

  /**
   * Tests that a profile can have a different pvalue than the default but the
   * same method will be used.
   *
   * @throws Exception fail
   */
  @Test
  ...
  private boolean storeCertificate(
      final ICustomPublisher publisher, final int status, final int profileId)
   ...
   * @return Publisher
   */
  private CertificateSamplerCustomPublisher createPublisher(
      final Properties properties) {
    CertificateSamplerCustomPublisher result =
        new CertificateSamplerCustomPublisher();
   ...
   * @return Publisher
   */
  private MockedCertificateSamplerCustomPublisher createMockedPublisher(
      final Properties properties) {

    MockedCertificateSamplerCustomPublisher result =
        new MockedCertificateSamplerCustomPublisher();
    result.init(properties);
    return result;
  }
}
