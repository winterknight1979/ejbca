 ...
 *
 * @version $Id: EndEntityProfileTest.java 28619 2018-04-03 12:57:23Z
 *     mikekushner $
 */
public class EndEntityProfileTest {
  ...
  public void testRegexValidation() throws EndEntityFieldValidatorException {
    // The regex validator ignored the dn component, so that can be set to
    // anything
    EndEntityValidationHelper.checkValidator(
        DnComponents.COMMONNAME,
        RegexFieldValidator.class.getName(),
        "[0-9-]*");
    EndEntityValidationHelper.checkValidator(
        DnComponents.COMMONNAME,
        RegexFieldValidator.class.getName(),
    ...
    try {
      EndEntityValidationHelper.checkValidator(
          DnComponents.COMMONNAME, RegexFieldValidator.class.getName(), "*");
    ...
    EndEntityValidationHelper.checkValue(
        DnComponents.COMMONNAME, makeRegexValidator("[0-9]*"), "123");
    try {
      EndEntityValidationHelper.checkValue(
          DnComponents.COMMONNAME, makeRegexValidator("[0-9]*"), "abc");
  ...
  public void testUserFulfillEndEntityProfile() {
    ...
    // Test that email address can be required as well, and that it does not
    // require an @ sign in it
  ...
  private static Map<String, Serializable> makeRegexValidator(
      final String regex) {
