 ...
 *
 * <p>This is a unit test and does not require EJBCA nor an LDAP catalog to be
 * running.
 *
 * @version $Id: CustomerLdapPublisher1UnitTest.java 28087 2018-01-24 09:22:01Z
 *     anatom $
 */
public class CustomerLdapPublisher1UnitTest {
  ...
  static {
    GOOD_PROPERTIES_OMITTED_EXTENDEDINFOOBJECTCLASS.remove(
  ...
  public void testInit() throws Exception {
    ...
    config.setProperty(
        "connectiontimeout", String.valueOf(expectedConnectionTimeout));
    ...
    config.setProperty(
        "logconnectiontests", String.valueOf(expectedLogConnectionTests));
    ...
    assertEquals(
        "hostnames", Arrays.asList(expectedHostnames), instance.getHostnames());
    ...
    assertEquals(
        "loginpassword", expectedLoginPassword, instance.getLoginPassword());
    assertEquals("usessl", expectedUseSSL, instance.isUseSSL());
    assertEquals("timeout", expectedConnectionTimeout, instance.getTimeout());
    assertEquals(
        "connectiontimeout:bind",
        expectedConnectionTimeout,
        instance.getLdapBindConstraints().getTimeLimit());
    assertEquals(
        "connectiontimeout:conn",
        expectedConnectionTimeout,
        instance.getLdapConnectionConstraints().getTimeLimit());
    assertEquals(
        "connectiontimeout:disconn",
        expectedConnectionTimeout,
        instance.getLdapDisconnectConstraints().getTimeLimit());
    assertEquals(
        "readtimeout",
        expectedReadTimeout,
        instance.getLdapSearchConstraints().getTimeLimit());
    assertEquals(
        "storetimeout",
        expectedStoreTimeout,
        instance.getLdapStoreConstraints().getTimeLimit());
    assertEquals(
        "logconnectiontests",
        expectedLogConnectionTests,
        instance.isLogConnectionTests());
  }

  /**
   * Tests that storeCertificate can not be called before it has been
   * initialized.
   *
   * @throws Exception Fail
   */
  ...
  /**
   * Tests the logic in storeCertificate when storing active end entity
   * certificates.
   *
   * @throws Exception Fail
   */
  ...
  public void testStoreCertificate_activeEndentity() throws Exception {
    ...
    final Certificate cert =
        CertTools.getCertfromByteArray(ANY_CERT_BYTES, Certificate.class);
    instance.storeCertificate(
        ANY_ADMIN,
        cert,
        ANY_USERNAME,
        ANY_PASSWORD,
        ...
        null);
    assertTrue(
        "doStoreCertificate called", instance.isDoStoreCertificateCalled());
    assertTrue("storeLog called", instance.isStoreLogCalled());
    assertTrue("log success", instance.getStoreLogParameters().isSuccess());
    assertEquals(
        "info logging", "info", instance.getStoreLogParameters().getLevel());

    // Now similate storage failure
    instance = new MockedCustomerLdapPublisher1();
    instance.setDoStoreCertificateException(
        new PublisherException(
            "SIMULATED_STORE_FAILURE")); // Simulate store failure
    instance.init(GOOD_PROPERTIES);
    ...
    assertEquals(
        "error logging", "err", instance.getStoreLogParameters().getLevel());

    // Now similate logging failure
    instance = new MockedCustomerLdapPublisher1();
    instance.setStoreLogException(
        new PublisherException(
            "SIMULATED_LOG_FAILURE")); // Simulate log failure
    instance.init(GOOD_PROPERTIES);

    // Successfull storage but failure to log of an ACTIVE ENDENTITY certificate
    // This should not result in an exception as the certificate was published
    // and should not end up on publish queue
    try {
      instance.storeCertificate(
          ANY_ADMIN,
          cert,
  ...
  /**
   * Tests the logic in storeCertificate when storing non-active or non end
   * entity certificates.
   *
   * @throws Exception Fail
   */
  ...
  public void testStoreCertificate_NonActiveOrNonEndentity() throws Exception {
    ...
    instance.storeCertificate(
        ...
        null);
    assertFalse(
        "doStoreCertificate called for ROOTCA",
        instance.isDoStoreCertificateCalled());
    ...
    instance.storeCertificate(
        ...
        null);
    assertFalse(
        "doStoreCertificate called for SUBCA",
        instance.isDoStoreCertificateCalled());
    ...
    instance.storeCertificate(
        ...
        null);
    assertFalse(
        "doStoreCertificate called for revokation",
        instance.isDoStoreCertificateCalled());
  ...
  public void testStoreCRL_uninitialized() throws Exception {
    try {
      new CustomerLdapPublisher1()
          .storeCRL(
              ANY_ADMIN, ANY_BYTEARRAY, ANY_TAG, ANY_NUMBER, ANY_SUBJECTDN);
      fail("Expected illegal state as publisher not initialized");
    } catch (IllegalStateException ok) {
      assertEquals("Publisher not initialized", ok.getMessage());
    }
  ...
  public void testStoreCRL() throws Exception {
    ...
    instance.storeCRL(
        ANY_ADMIN, ANY_CRL_BYTES, ANY_CAFP, ANY_NUMBER, ANY_SUBJECTDN);
    ...
    assertTrue(
        "crl bytes",
        Arrays.equals(
            ANY_CRL_BYTES, instance.getDoStoreCRLParameters().getIncrl()));

    // Now similate storage failure
    instance = new MockedCustomerLdapPublisher1();
    instance.setDoStoreCRLException(
        new PublisherException(
            "SIMULATED_STORE_FAILURE")); // Simulate store failure
    instance.init(GOOD_PROPERTIES);
  ...
  public void testConnection() throws Exception {
    ...
    assertTrue("log success", instance.getStoreLogParameters().isSuccess());
    assertEquals(
        "debug logging", "debug", instance.getStoreLogParameters().getLevel());

    // Now similate storage failure
    instance = new MockedCustomerLdapPublisher1();
    instance.setDoTestConnectionException(
    ...
    try {
      instance.testConnection();
      assertTrue(
          "doTestConnection called", instance.isDoTestConnectionCalled());
      assertTrue("storeLog called", instance.isStoreLogCalled());
      assertTrue("log success", instance.getStoreLogParameters().isSuccess());
      assertEquals(
          "info logging", "debug", instance.getStoreLogParameters().getLevel());
  ...
  public void testDoStoreCertificate() throws Exception {
    ...
    final X509Certificate bcCert =
        CertTools.getCertfromByteArray(CERT_BYTES, X509Certificate.class);
    final X509Certificate sunCert =
        CertTools.getCertfromByteArray(
            CERT_BYTES, "SUN", X509Certificate.class);

    // Test storage of an new entry
    SecondMockedCustomerLdapPublisher1 instance =
        new SecondMockedCustomerLdapPublisher1();
    instance.init(GOOD_PROPERTIES);
    instance.setSearchOldEntityReturn(null); // No old enttry

    // Test with BC cert
    instance.doStoreCertificate(
        bcCert, ANY_USERNAME, ANY_PASSWORD, CERT_ISSUERDN, null);
    assertTrue("serachOldEntity called", instance.isSearchOldEntityCalled());
    assertTrue(
        "writeCertEntryToLDAP called", instance.isWriteCertEntryToLDAPCalled());
    SecondMockedCustomerLdapPublisher1.WriteCertEntryToLDAPParameters params =
        instance.getWriteCertEntryToLDAPParameters();
    LDAPEntry newEntry = params.getNewEntry();

    // newEntry.writeDSML(System.out);

    // Note: The DN should be in the reversed order as compared to the
    ...
    // Test with Sun cert
    instance.doStoreCertificate(
        sunCert, ANY_USERNAME, ANY_PASSWORD, CERT_ISSUERDN, null);
    assertTrue("serachOldEntity called", instance.isSearchOldEntityCalled());
    assertTrue(
        "writeCertEntryToLDAP called", instance.isWriteCertEntryToLDAPCalled());
  ...
  public void testDoStoreCRL() throws Exception {
    ...
    if ("CN=TestCA, O=AnaTom, C=SE".equals(crl.getIssuerDN().getName())) {
      throw new Exception(
          "Test assumes the CRL to have the DN: CN=TestCA, O=AnaTom, C=SE but"
              + " was: "
    ...
    assertTrue("serachOldEntity called", instance.isSearchOldEntityCalled());
    assertTrue(
        "writeCrlEntryToLDAP called", instance.isWriteCrlEntryToLDAPCalled());
    SecondMockedCustomerLdapPublisher1.WriteCrlEntryToLDAPParameters params =
        instance.getWriteCrlEntryToLDAPParameters();
    LDAPEntry newEntry = params.getNewEntry();
    ...
    assertEquals(
        "ldapDN",
        "CN=C=SE\\,O=AnaTom\\,CN=TestCA,ou=staging,dc=test.example.com,dc=com",
        newEntry.getDN());
    LDAPAttributeSet attributeSet = newEntry.getAttributeSet();
  ...
  public void testStoreLog() throws Exception {
    // Create mocked instance
    ThirdMockedCustomerLdapPublisher1 instance =
        new ThirdMockedCustomerLdapPublisher1();
    ...
    assertEquals(
        "log DN",
        "logTime=20130906071129.999Z,cn=log,dc=test.example.com,dc=com",
        instance
    ...
    assertEquals(
        "two calls", 2, instance.getWriteCertEntryToLDAPParameters().size());
    LDAPEntry newEntry =
        instance.getWriteCertEntryToLDAPParameters().get(1).getNewEntry();
    String logInfo = newEntry.getAttribute("logInfo").getStringValue();
    assertEquals(
        "log DN",
        "logTime=20130906071130.000Z,cn=log,dc=test.example.com,dc=com",
        newEntry.getDN());
    assertTrue(
        "logInfo is " + logInfo, logInfo.contains("time:20130906071129.999Z"));
   ...
   * @throws Exception Fail
   */
  private void testExtendedInfoObjectClass(
      final Properties properties,
      final String expectedExtendedInfoObjectClassValue)
      throws Exception {
    ...
    LDAPAttributeSet attributeSet = newEntry.getAttributeSet();
    assertNotNull(attributeSet.getAttribute("objectclass"));
    final String[] expectedObjectClassesForCertificates =
        new String[] {
          CustomerLdapPublisher1.INETORGPERSON,
          expectedExtendedInfoObjectClassValue
        };
    assertArrayEquals(
        "Not expected object classes when storing certificates",
        expectedObjectClassesForCertificates,
        attributeSet.getAttribute("objectclass").getStringValueArray());

    // Test with a CRL
    instance.doStoreCRL(CRL_BYTES);
    SecondMockedCustomerLdapPublisher1.WriteCrlEntryToLDAPParameters
        crlEntryParams = instance.getWriteCrlEntryToLDAPParameters();
    attributeSet = crlEntryParams.getNewEntry().getAttributeSet();
    assertNotNull(attributeSet.getAttribute("objectclass"));
    final String[] expectedObjectClassesForCrls =
        new String[] {
          CustomerLdapPublisher1.CRLDISTRIBUTIONPOINT,
          expectedExtendedInfoObjectClassValue
        };
    assertArrayEquals(
        "Not expected object classes when storing CRLs",
        expectedObjectClassesForCrls,
        attributeSet.getAttribute("objectclass").getStringValueArray());
  }
  ...
  public void testExtendedInfoObjectClass() throws Exception {
    testExtendedInfoObjectClass(
        GOOD_PROPERTIES, A_CUSTOM_EXTENDED_INFO_OBJECT_CLASS_VALUE);
  }
  ...
  public void testDefaultExtendedInfoObjectClass() throws Exception {
    testExtendedInfoObjectClass(
        GOOD_PROPERTIES_OMITTED_EXTENDEDINFOOBJECTCLASS, "icaoExtendedInfo");
  }
}
