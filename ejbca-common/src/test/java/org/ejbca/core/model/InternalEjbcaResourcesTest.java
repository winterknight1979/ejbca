...
/**
 * @version $Id: InternalEjbcaResourcesTest.java 25823 2017-05-10 06:57:54Z
 *     anatom $
 */
public class InternalEjbcaResourcesTest {
  ...
  public void testGetLocalizedMessageString() {
    InternalEjbcaResources intres =
        new InternalEjbcaResources(TEST_RESOURCE_PATH);
    String res = intres.getLocalizedMessage("test.testmsg");
    assertEquals("Test en-US", res);
    assertEquals(
        "Test en-US", intres.getLocalizedMessageCs("test.testmsg").toString());
  ...
  public void testMessageStringWithExtraParameter() {
    ...
    assertEquals(
        "Test sv-SE",
        intres
            .getLocalizedMessageCs("test.testmsgsv", "foo $bar \\haaaar")
            .toString());
  }
