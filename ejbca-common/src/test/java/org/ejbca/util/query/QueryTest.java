...
public class QueryTest {
  ...
  public void test01TestUserQuery() throws Exception {
    ...
    query.add(
        ...
        Integer.toString(EndEntityConstants.STATUS_NEW));
    String str = query.getQueryString();
    assertEquals("status = 10", str);

    query = new Query(Query.TYPE_USERQUERY);
    query.add(
        UserMatch.MATCH_WITH_USERNAME, BasicMatch.MATCH_TYPE_EQUALS, "foo");
  ...
  public void testUserQueryTrim() throws Exception {
    ...
    query2.add(
        ...
        "Common Name ");
    assertEquals(
        "UPPER(username) LIKE 'COMMON NAME %'", query2.getQueryString());
    log.trace("<testUserQueryTrim");
  }
  ...
  public void testUserQuerySingleQuote() throws Exception {
    ...
    query2.add(
        ...
        "D'Greatest ");
    assertEquals(
        "UPPER(username) LIKE 'D''GREATEST %'", query2.getQueryString());
    ...
    query3.add(
        ...
        "D''Greatest");
    assertEquals(
        "UPPER(username) LIKE 'D''''GREATEST%'", query3.getQueryString());
    log.trace("<testUserQuerySingleQuote");
  }
}
