 ...
 *   <li>lastcert - gets latest certificate of a user, takes argument
 *       'subject=&lt;subjectDN&gt;'.
 *   <li>listcerts - lists all certificates of a user, takes argument
 *       'subject=&lt;subjectDN&gt;'.
 *   <li>revoked - checks if a certificate is revoked, takes arguments
 *       'subject=&lt;subjectDN&gt;&amp;serno=&lt;serial number&gt;'.
 *   <li>cacert - returns ca certificate in PEM-format, takes argument
 *       'issuer=&gt;issuerDN&lt;&amp;level=&lt;ca-level, 0=root&gt;'
 *   <li>nscacert - returns ca certificate for Firefox, same args as above
 *   <li>iecacert - returns ca certificate for Internet Explorer, same args as
 *       above
 * </ul>
 *
 * cacert, nscacert and iecacert also takes optional parameter level=&lt;int
 * 1,2,...&gt;, where the level is which ca certificate in a hierachy should be
 * returned. 0=root (default), 1=sub to root etc.
 *
 * @version $Id: CertDistServlet.java 34109 2019-12-18 09:08:11Z anatom $
 */
public class CertDistServlet extends HttpServlet {
  ...
  /** Logger. */
  private static final Logger LOG = Logger.getLogger(CertDistServlet.class);
  ...
  /** @Deprecated since EJBCA 6.3.0. MOZILLA_PROPERTY
   *  can be removed in EJBCA 6.4 or 6.5 */
  ...
  /** EJB. */
  @EJB private CertificateStoreSessionLocal storesession;
  /** EJB. */
  ...
  public void doGet(final HttpServletRequest req, final HttpServletResponse res)
    ...
    final AuthenticationToken administrator =
        new AlwaysAllowLocalAuthenticationToken(
            new PublicWebPrincipal(remoteAddr));
    ...
    // See if the client wants the response cert or CRL in PEM format (default
    // is DER)
    ...
    if ((command.equalsIgnoreCase(COMMAND_CRL)
            || command.equalsIgnoreCase(COMMAND_DELTACRL))
        && issuerdn != null) {
      try {
        ...
        if (StringUtils.isNotEmpty(crlNumber)) {
          // Using CRLNumber then we don't care if it's delta or full, it's what
          // it is specified by the number
          if (!StringUtils.isNumeric(crlNumber)
            ...
            res.sendError(
                ...
                "CRLNumber is not a valid positive numeric value.");
          }
        ...
        // moz is only kept for backwards compatibility, can be removed in EJBCA
        // 6.4 or 6.5
        ...
        res.sendError(
            HttpServletResponse.SC_NOT_FOUND, "Error getting latest CRL.");
        return;
      }
      ...
      if (dn == null) {
        ...
        res.sendError(
            ...
            "Usage command=eecert?issuer=<issuerdn>&serno=<serialnumber in"
                + " hex>.");
        return;
      }
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Looking for certificate with issuer/serno '"
                + dn
                + "', '"
                + serno
                + "'.");
      ...
      try {
        ...
        Certificate cert =
            storesession.findCertificateByIssuerAndSerno(
                dn, new BigInteger(serno, 16));
        if (cert == null) {
          LOG.debug("Error getting End Entity certificate, not found: ");
          res.sendError(
              HttpServletResponse.SC_NOT_FOUND,
              "Error getting End Entity certificate, not found.");
          return;
        }
        sendEndEntityCert(administrator, req, res, format, cert);
      } catch (NumberFormatException e) {
        LOG.debug(
            "Error getting End Entity certificate, invalid serial number"
                + " (hex): ",
            e);
        res.sendError(
            HttpServletResponse.SC_NOT_FOUND,
        ...
        LOG.info(
            "Error getting End Entity certificate, invalid certificate?: ", e);
        res.sendError(
        ...
        LOG.info(
            "Error getting End Entity certificate, can not get field to"
                + " generate filename?: ",
            e);
        res.sendError(
            HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
        ...
        LOG.error(
            "Error getting End Entity certificate, not authorized to create"
                + " PKCS7: ",
            e);
        res.sendError(
            HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
        ...
        LOG.info(
            "Error getting End Entity certificate, CA to create PKCS7 does not"
                + " exist, or can not create PKCS7: ",
            e);
        res.sendError(
            HttpServletResponse.SC_NOT_FOUND,
            "Error getting End Entity certificate, CA to create PKCS7 does not"
      ...
      try {
        ...
        if (cert == null) {
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                "No certificate found for requested fingerprint. '"
                    + fp
                    + "'.");
          }
          res.sendError(
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                "Sent certificate with fingerprint '"
                    + fp
                    + "' (and Subject DN '"
                    + dn
                    + "') to client at "
        ...
        LOG.debug(
            "Error getting certificate with fingerprint '"
                + fp
                + "' for "
                + remoteAddr
                + ": ",
            e);
        res.sendError(
            HttpServletResponse.SC_NOT_FOUND, "Error getting certificate.");
        return;
      }
    } else if (command.equalsIgnoreCase(COMMAND_LASTCERT)
      ...
      if (dn == null) {
        LOG.debug(
            "Bad request, no 'subject' arg to 'lastcert' or 'listcert'"
                + " command.");
        res.sendError(
            HttpServletResponse.SC_BAD_REQUEST,
      ...
      try {
        ...
        Collection<Certificate> certcoll =
            storesession.findCertificatesBySubject(dn);
        ...
        if (command.equalsIgnoreCase(COMMAND_LASTCERT)) {
          ...
          for (int i = 0; i < certs.length; i++) {
            if (i == 0) {
              maxdate =
                  CertTools.getNotBefore((Certificate) certs[i]).getTime();
              latestcertno = 0;
            } else if (CertTools.getNotBefore((Certificate) certs[i]).getTime()
                > maxdate) {
              maxdate =
                  CertTools.getNotBefore(((Certificate) certs[i])).getTime();
              latestcertno = i;
        ...
        if (command.equalsIgnoreCase(COMMAND_LISTCERT)) {
          ...
          for (int i = 0; i < certs.length; i++) {
            ...
            BigInteger serno =
                CertTools.getSerialNumber((Certificate) certs[i]);
            ...
            pout.println("</pre>");
            pout.println(
                "<a href=\"certdist?cmd=revoked&issuer="
                    + URLEncoder.encode(issuer, "UTF-8")
                    + "&serno="
                    + serno.toString()
                    + "\">Check if certificate is revoked</a>");
            pout.println("<hr>");
          }
          if (certs.length == 0) {
            pout.println(
                "No certificates exists for '"
                    + HTMLTools.htmlescape(dn)
                    + "'.");
          }
        ...
        LOG.debug(
            "Error getting certificates for '"
                + dn
                + "' for "
                + remoteAddr
                + ": ",
      ...
      try {
        if (lev != null) {
          ...
          pkcs7 = true;
        } // CA is level 0, next over root level 1 etc etc, -1 returns chain as
          // PKCS7
        ...
        String filename =
            RequestHelper.getFileNameFromCertNoEnding(cacert, "ca");
        byte[] enccert = null;
        if (pkcs7) {
        ...
        res.sendError(
            HttpServletResponse.SC_NOT_FOUND,
            "Invalid level, should be a number.");
        return;
      } catch (Exception e) {
    ...
    } else if (command.equalsIgnoreCase(COMMAND_CACHAIN)
        && (issuerdn != null || caid != 0)) {
      ...
      try {
        CertificateStatus revinfo =
            storesession.getStatus(dn, new BigInteger(serno));
        PrintWriter pout = new PrintWriter(res.getOutputStream());
        res.setContentType("text/html");
        ...
        if (revinfo != null) {
          if (!revinfo.isRevoked()) {
            ...
            pout.println(
                "RevocationDate is '"
                    + revinfo.getRevocationDate()
                    + "' and reason '"
                    + revinfo.getRevocationReason()
                    + "'.");
          }
        } else {
        ...
        LOG.debug(
            ...
            e);
        res.sendError(
            HttpServletResponse.SC_NOT_FOUND, "Error checking revocation.");
        return;
      }
    } else {
      res.sendError(
          HttpServletResponse.SC_BAD_REQUEST,
          "Commands=cacert | lastcert | listcerts | crl | deltacrl | revoked"
              + " && issuer=<issuerdn>");
      return;
    }
  }
   ...
   */
  private void sendEndEntityCert(
      final AuthenticationToken administrator,
      final HttpServletRequest req,
      final HttpServletResponse res,
      final String format,
      final Certificate certcert)
      throws CertificateEncodingException, NoSuchFieldException, IOException,
    ...
    if (certcert instanceof CardVerifiableCertificate) {
      ending = ".cvcert";
    } else if (StringUtils.equals(format, "PEM")
        || StringUtils.equals(format, "chain")) {
      ending = ".pem";
    ...
    if (StringUtils.equals(format, "PEM")) {
      ...
      try {
        ...
        throw new CertificateEncodingException(
            "Unable to create certs-only PKCS#7 / CMS.");
      }
      ...
      RequestHelper.sendNewB64File(
          ...
          filename,
          "",
          ""); // chain includes begin/end already
    } else {
   ...
   * @return the full leaf certificate chain of a certificate given that the
   *     IssuerDN hashCode fo the leaf will map to an existing CA Id.
   */
  private List<Certificate> getFullChainOfCertificate(
      final Certificate certificate) {
    final int caId = CertTools.getIssuerDN(certificate).hashCode();
    final LinkedList<Certificate> certificateChain =
        new LinkedList<>(signSession.getCertificateChain(caId));
    certificateChain.addFirst(certificate);
    return certificateChain;
  }

  private Certificate[] getCertificateChain(
      final int caId, final String issuerDn) {
  ...
  private void handleCaChainCommands(
    ...
    try {
      ...
      if (((GlobalConfiguration)
        ...
        filename =
            RequestHelper.getFileNameFromCertNoEnding(chain[0], "ca")
                + "-chain."
                + format.toLowerCase();
      }
      ...
      if ((format == null) || StringUtils.equalsIgnoreCase(format, "pem")) {
        ...
        for (int i = 0; i < chain.length; i++) {
          ...
          alias = StringUtils.substring(alias, 0, len);
      ...
      LOG.debug(
          "Sent CA certificate chain to client, len=" + outbytes.length + ".");
    } catch (CertificateEncodingException e) {
      ...
      res.sendError(
          HttpServletResponse.SC_NOT_FOUND,
          "Error creating JKS with CA certificate chain.");
  ...
  private void printHtmlHeader(final String title, final PrintWriter pout) {
  ...
  private void printHtmlFooter(final PrintWriter pout) {
    pout.println("</body>");
    pout.println("<head>");
    pout.println("<META HTTP-EQUIV=\"Pragma\" CONTENT=\"no-cache\">");
    pout.println("<META HTTP-EQUIV=\"Expires\" CONTENT=\"-1\">");
