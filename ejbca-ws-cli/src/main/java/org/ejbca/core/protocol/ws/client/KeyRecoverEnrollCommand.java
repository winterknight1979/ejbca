...
/**
 * @version $Id: KeyRecoverEnrollCommand.java 28423 2018-03-06 14:43:12Z henriks
 *     $
 */
public class KeyRecoverEnrollCommand extends EJBCAWSRABaseCommand
    ...
    try {
      ...
      try {
        KeyStore result =
            getEjbcaRAWS()
                .keyRecoverEnroll(
                    username, certSn, issuerDn, password, hardTokenSn);
        ...
        if (result == null) {
          ...
          java.security.KeyStore ks =
              KeyStoreHelper.getKeyStore(
                  result.getKeystoreData(), keyStoreType, password);
          ks.store(fos, password.toCharArray());
          fos.close();
          getPrintStream()
              .println(
                  "Key recovery sucessfull!\nKeystore generated, written to "
                      + filepath);
        }
  ...
  protected void usage() {
    ...
    getPrintStream()
        .println(
            "Usage : keyrecover <username> <certSerialNr> <issuerDN>"
                + " <password> <hardtokensn (or NONE)> <outputpath"
                + " (optional)>");
  }
}
