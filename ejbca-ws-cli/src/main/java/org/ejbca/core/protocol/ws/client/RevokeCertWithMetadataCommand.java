 ...
 * this one here allows to include a list of keyvaluepair input parameters for
 * example "reason=REV_SUPERSEDED" "certificateProfileId=12"
 *
 * @version $Id: RevokeCertWithMetadataCommand.java 28395 2018-02-27 14:19:00Z
 *     tarmo_r_helmes $
 */
public class RevokeCertWithMetadataCommand extends EJBCAWSRABaseCommand
   ...
   */
  public RevokeCertWithMetadataCommand(final String[] args) {
    super(args);
  }
  ...
  public void execute()
    ...
    try {
      ...
      final String issuerdn =
          CertTools.stringToBCDNString(this.args[ARG_ISSUERDN]);
      final String certsn = getCertSN(this.args[ARG_CERTSN]);

      final String justRevoke =
          "To revoke the certificate with the current time remove the last"
              + " argument (revocation time).";

      try {
        final RevokeStatus status =
            getEjbcaRAWS().checkRevokationStatus(issuerdn, certsn);
        if (status != null) {
          getEjbcaRAWS().revokeCertWithMetadata(issuerdn, certsn, metadata);
          getPrintStream()
              .println("Certificate revoked (or unrevoked) successfully.");
        } else {
        ...
        getPrintStream()
            .println(
                "The certificate was already revoked, or you tried to unrevoke"
                    + " a permanently revoked certificate.");
  ...
  private String getCertSN(final String certsn) {
    try {
  ...
  protected void usage() {
    getPrintStream()
        .println("Command used to revoke or unrevoke a certificate.");
    getPrintStream()
        .println(
            "Unrevocation is done using the reason REV_REMOVEFROMCRL, and can"
                + " only be done if the certificate is revoked with reason"
                + " REV_CERTIFICATEHOLD.");
    getPrintStream()
        .println(
            "Usage : revokecert <issuerdn> <certificatesn(HEX)>"
                + " reason=<reason> [revocationdate=<revocation date>]"
    ...
    getPrintStream()
        .println(
            "Revocation date is optional. If specified it must be in the past"
                + " and must be a valid ISO8601 string");
