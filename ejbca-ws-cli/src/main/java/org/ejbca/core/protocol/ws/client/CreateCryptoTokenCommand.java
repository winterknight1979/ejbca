...
public class CreateCryptoTokenCommand extends EJBCAWSRABaseCommand
   ...
   */
  public CreateCryptoTokenCommand(final String[] args) {
    super(args);
  }
  ...
  public void execute()
    ...
    try {
      ...
      getEjbcaRAWS()
          .createCryptoToken(
              tokenName, tokenType, pin, autoActivate, properties);
  ...
  protected void usage() {
    ...
    getPrintStream()
        .println(
            "Usage : createcryptotoken <cryptotokenName> <cryptotokenType>"
                + " <autoActivate> <activationPin> "
                + "[<pathToCryptoTokenPropertiesFile>]");
    getPrintStream().println();
    getPrintStream()
        .println("cryptotokenName: The name of the new crypto token.");
    getPrintStream()
        .println(
            "cryptotokenType: Can be one of: "
                + SoftCryptoToken.class.getSimpleName()
                + ", "
                + PKCS11CryptoToken.class.getSimpleName());
    getPrintStream()
        .println(
            "autoActivate: Set to true|false to allow|disallow whether crypto"
                + " token should be autoactivated or not.");
    getPrintStream().println("activationPin: Pin code for the crypto token.");
    getPrintStream()
        .println(
            "pathToCryptoTokenPropertiesFile: The path to a .properties file"
                + " containing the new cryptotoken properties. Optional");
    /*
