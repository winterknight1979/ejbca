 ...
 */
public class CvcPemCommand extends EJBCAWSRABaseCommand
    implements IAdminCommand {

  /** @param args command line arguments */
  public CvcPemCommand(final String[] args) {
    super(args);
  }
  ...
  public void execute()
    ...
    try {
      ...
      if (inform.equals(outform)) {
        getPrintStream()
            .println("No point in converting to the same format, exiting.");
        return;
      }
      getPrintStream()
          .println(
              "converting CV Certificate ("
                  + inform
                  + "): "
                  + infile
                  + " to "
  ...
  protected void usage() {
    getPrintStream()
        .println("Command used to convert between binary and PEM formats.");
    getPrintStream()
        .println(
            "Usage : cvcpem <inform bin/pem> <in filename> <outform bin/pem>"
                + " <out filename> [-req]\n\n");
    getPrintStream()
        .println(
            "If adding the optional parameter -req the PEM output/input is a"
                + " certificate request as opposed to a certificate.");
  }
}
