...
public class CreateCACommand extends EJBCAWSRABaseCommand
   ...
   */
  public CreateCACommand(final String[] args) {
    super(args);
  }
  ...
  public void execute()
    ...
    try {
      ...
      String certprofile =
          StringUtils.equalsIgnoreCase(args[ARG_CERT_PROFILE], "null")
              ? null
              : args[ARG_CERT_PROFILE];
      ...
      mapping.load(
          new FileInputStream(new File(args[ARG_PURPOSE_KEY_MAPPING_PATH])));
      ArrayList<KeyValuePair> purposeKeyMapping =
          getKeyValuePairListFromProperties(mapping);
  ...
  protected void usage() {
    getPrintStream()
        .println(
            "Command used to create a new CA with a specific existing"
                + " cryptotoken");
    getPrintStream()
        .println(
            "Usage : createca <caname> <cadn> <catype> <validity>"
                + " <certprofile> <signalg> <signedByCAID> <cryptotokenName>"
                + " <pathToKeyPurposeMappingFile> [<pathToCAPropertiesFile>]");
    ...
    getPrintStream()
        .println(
            "'certprofile' the certificate profile that should be used when"
                + " issueing the CA certificate instead of the default"
                + " profiles ROOTCA or SUBCA. Use 'null' for default"
                + " profiles.");
    ...
    getPrintStream()
        .println(
            "'signedByCAID' is the ID of a CA that will sign this CA. Use '1'"
                + " for self signed CA (i.e. a root CA). CAs created using the"
                + " WS cannot be signed by external CAs");
    getPrintStream()
        .println(
            "'cryptotokenName' is the name of the cryptotoken associated with"
                + " the CA");
    getPrintStream()
        .println(
            "'pathToKeyPurposeMappingFile' is the path to a .properties file"
                + " containing the mapping of the cryptotoken keys and their"
                + " purpose");
    getPrintStream()
        .println(
            "'pathToCAPropertiesFile' is the path to a .properties file"
                + " containing additional CA properties. Optional");
    getPrintStream().println();
    getPrintStream()
        .println(
            "Please note that the policy ID is specified in the properties"
                + " file containing additional properties with the property"
                + " key 'policyid'.");
    getPrintStream()
        .println(
            "The policy ID can be 'null' if no Certificate Policy extension"
  ...
  private ArrayList<KeyValuePair> getKeyValuePairListFromProperties(
