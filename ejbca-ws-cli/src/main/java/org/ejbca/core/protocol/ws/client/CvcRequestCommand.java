...
public class CvcRequestCommand extends EJBCAWSRABaseCommand
   ...
   */
  public CvcRequestCommand(final String[] args) {
    super(args);
  }
    ...
    final int seedLen = 5;
      ...
      try {
        ...
        if (genrequest) {
          getPrintStream()
              .println(
                  "Generating a new request with base filename: "
                      + basefilename);
          ...
          // CAReferenceField caRef = new
          // CAReferenceField(country,mnemonic,sequence);
          CAReferenceField caRef =
              null; // Don't create a caRef in the self signed request
          // We are making a self signed request, so holder ref is same as ca
          // ref
          ...
          if (authSignKeyFile != null) {
            getPrintStream()
                .println(
                    "Reading private key from pkcs8 file "
                        + authSignKeyFile
            ...
            PrivateKey privKey =
                keyfact.generatePrivate(new PKCS8EncodedKeySpec(keybytes));
            KeyPair authKeyPair =
                new KeyPair(null, privKey); // We don't need the public key
            // Default caRef if we do not pass in a certificate to get caRef
            // from
            CAReferenceField authCaRef =
                new CAReferenceField(country, mnemonic, sequence);
            CVCertificate authCert = null;
            if (authSignCertFile != null) {
              getPrintStream()
                  .println(
                      "Reading cert from cvcert file "
                          + authSignCertFile
                          + " to create an authenticated request");
              CVCObject parsedObject =
                  CvcPrintCommand.getCVCObject(authSignCertFile);
          ...
          fos.close();
          getPrintStream()
              .println("Wrote binary request to: " + basefilename + ".cvreq");
          fos = new FileOutputStream(basefilename + ".pkcs8");
          fos.write(keyPair.getPrivate().getEncoded());
          fos.close();
          getPrintStream()
              .println(
                  "Wrote private key in "
        ...
        // Actually don't do that, leverage the existing commands and force to
        // use the editUser command instead.
        // This also makes this CLI exactly represent the actual WS-API call
        // getEjbcaRAWS().editUser(userdata);

        getPrintStream()
            .println("Submitting CVC request for user '" + username + "'.");
        getPrintStream().println();
        // Use the request and request a certificate
        List<Certificate> resp =
            getEjbcaRAWS().cvcRequest(username, userpassword, cvcreq);
        ...
        getPrintStream()
            .println(
                "Wrote binary certificate to: " + basefilename + ".cvcert");
        getPrintStream()
            .println(
                "You can look at the certificate with the command cvcwscli.sh"
                    + " cvcprint "
                    + basefilename
                    + ".cvcert");
        ...
        getPrintStream()
            .println(
                "Error : Given userdata doesn't fulfill end entity profile. : "
                    + e.getMessage());
      }
  ...
  protected void usage() {
    getPrintStream()
        .println(
            "Command used to make a CVC request. A user must exist, add one"
                + " with 'ejbcawsracli.sh edituser'.");
    getPrintStream()
        .println(
            "Usage : cvcrequest <username> <password> <subjectdn> <sequence>"
                + " <signatureAlg> <keyspec"
                + " (1024|1536|2048|curve)><genreq=true|false> <basefilename>"
                + " [<auth-sign-key>] [<auth-sign-cert>]\n\n");
    getPrintStream()
        .println(
            "SignatureAlg is used when generating a request and can be"
                + " SHA1WithRSA, SHA256WithRSA, SHA256WithRSAAndMGF1,"
                + " SHA1WithECDSA, SHA224WithECDSA, SHA256WithECDSA");
    getPrintStream()
        .println(
            "Keyspec is used when generating a request and is 1024, 1536,"
                + " 2048, etc. for RSA keys and the name of a named curve for"
                + " ECDSA, see User Guide for supported curves.");
    getPrintStream()
        .println(
            "DN is used when generating a request and is of form \"C=SE,"
                + " CN=ISTEST2\", where SE is the country and ISTEST2 the"
                + " mnemonic.");
    getPrintStream()
        .println(
            "Sequence is used when generating a request and is a sequence"
                + " number for the public key, recomended form 00001 etc. If"
                + " 'null' a random 5 number sequence will be generated.");
    getPrintStream()
        .println(
            "If genreq is true a new request is generated and the generated"
                + " request is written to <basefilename>.cvreq, and the"
                + " private key to <basefilename>.pkcs8.");
    getPrintStream()
        .println(
            "If genreq is false a request is read from <reqfilename>.cvreq and"
                + " sent to the CA, the sequence from the command line is"
                + " ignored.");
    getPrintStream()
        .println(
            "The issued certificate is written to <basefilename>.cvcert\n");
    getPrintStream()
        .println(
            "auth-sign-key is optional and if given the CVC request is signed"
                + " by this key to create an authenticated CVC request.");
    getPrintStream()
        .println(
            "auth-sign-cert is optional and if given the caRef of the"
                + " authenticated CVC request is taken from this CVC"
                + " certificate.");
  }
}
