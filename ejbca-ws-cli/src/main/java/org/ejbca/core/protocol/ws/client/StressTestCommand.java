...
/**
 * @version $Id: StressTestCommand.java 28806 2018-04-30 13:06:54Z
 *     jekaterina_b_helmes $
 */
public class StressTestCommand extends EJBCAWSRABaseCommand
      ...
      switch (this.testType) {
        ...
        case REVOKE:
          return new Command[] {
            ...
            this.testType.equals(TestType.REVOKE_BACKDATED)
                ? new RevokeCertBackdatedCommand(ejbcaWS, jobData)
        ...
        case REVOKEALOT:
          return new Command[] {
            new MultipleCertsRequestsForAUserCommand(
                ejbcaWS,
                this.caName,
                this.endEntityProfileName,
                this.certificateProfileName,
  ...
  private class Pkcs10RequestCommand extends BaseCommand implements Command {
    ...
    Pkcs10RequestCommand(
      ...
      this.pkcs10 =
          CertTools.genPKCS10CertificationRequest(
              "SHA1WithRSA",
              CertTools.stringToBcX500Name("CN=NOUSED"),
              keys.getPublic(),
              new DERSet(),
              keys.getPrivate(),
              null);
    ...
    public boolean doIt() throws Exception {
      final CertificateResponse certificateResponse =
          this.ejbcaWS.pkcs10Request(
              this.jobData.userName,
              this.jobData.passWord,
              new String(Base64.encode(this.pkcs10.getEncoded())),
              null,
  ...
  private boolean checkAndLogCertificateResponse(
    ...
    if (cert == null) {
      StressTestCommand.this
          .performanceTest
          .getLog()
          .error("no certificate generated for user " + jobData.userName);
      return false;
    }
    final String commonName =
        CertTools.getPartFromDN(cert.getSubjectDN().getName(), "CN");
    if (!commonName.equals(jobData.userName)) {
      StressTestCommand.this
          .performanceTest
          .getLog()
          .error(
              "Cert not created for right user. Username: \""
                  + jobData.userName
                  + "\" Subject DN: \""
                  + cert.getSubjectDN()
                  + "\".");
    ...
    StressTestCommand.this
        .performanceTest
        .getLog()
        .info("Cert created. Subject DN: \"" + cert.getSubjectDN() + "\".");
    StressTestCommand.this
        .performanceTest
        .getLog()
        .result(CertTools.getSerialNumber(cert));
    return true;
  }
      ...
      for (int i = 0; i < max; i++) {
        ...
        if (!editUserCommand.doIt()) {
          StressTestCommand.this
              ...
              .getLog()
              .error(
                  "MultiplePkcs10RequestsCommand failed for "
                      + this.jobData.userName);
          return false;
        }
        createUser = false;
        Pkcs10RequestCommand pkcs10RequestCommand =
            new Pkcs10RequestCommand(
                this.ejbcaWS, this.kpg.generateKeyPair(), this.jobData);
  ...
  private class FindUserCommand extends BaseCommand implements Command {
    ...
    @Override
    public boolean doIt() throws Exception {
      final org.ejbca.core.protocol.ws.client.gen.UserMatch match =
          new org.ejbca.core.protocol.ws.client.gen.UserMatch();
      ...
      if (result.size() < 1) {
        StressTestCommand.this
            .performanceTest
            .getLog()
            .error("No users found for DN \"" + this.jobData.getDN() + "\"");
        return false;
      }
  ...
  private class ListCertsCommand extends BaseCommand implements Command {
    ...
    public boolean doIt() throws Exception {
      final List<Certificate> result =
          this.ejbcaWS.findCerts(this.jobData.userName, true);
      final Iterator<Certificate> i = result.iterator();
      this.jobData.userCertsToBeRevoked = new X509Certificate[result.size()];
      for (int j = 0; i.hasNext(); j++) {
        this.jobData.userCertsToBeRevoked[j] =
            (X509Certificate)
                CertificateFactory.getInstance("X.509")
                    .generateCertificate(
                        new ByteArrayInputStream(
                            Base64.decode(i.next().getCertificateData())));
      }
      if (this.jobData.userCertsToBeRevoked.length < 1) {
        this.jobData.userCertsToBeRevoked =
            this.jobData.userCertsGenerated.toArray(
                new X509Certificate[this.jobData.userCertsGenerated.size()]);
      }
      ...
      if (this.jobData.userCertsToBeRevoked.length < 1) {
        StressTestCommand.this
            .performanceTest
            .getLog()
            .error("no cert found for user " + this.jobData.userName);
        return false;
      }
  ...
  private class RevokeCertBackdatedCommand extends BaseCommand
    ...
    RevokeCertBackdatedCommand(final EjbcaWS uejbcaWS, final JobData ujobData)
      ...
      StressTestCommand.this
          .performanceTest
          .getLog()
          .info("Revoke time: " + this.revoceTime);
    }

    private void revokeBackdated(final int i) throws Exception {
    ...
    private void revoke(final int i) throws Exception {
      this.ejbcaWS.revokeCert(
          this.jobData.userCertsToBeRevoked[i].getIssuerDN().getName(),
          this.jobData.userCertsToBeRevoked[i].getSerialNumber().toString(16),
          REVOKATION_REASON_UNSPECIFIED);
      ...
      for (int i = 0; i < this.jobData.userCertsToBeRevoked.length; i++) {
        try {
          ...
          StressTestCommand.this
              .performanceTest
              .getLog()
              .info("No back dating since not allowed for the profile.");
          continue;
        }
      }
      return true;
  ...
  private class EditUserCommand extends BaseCommand implements Command {
    ...
    EditUserCommand(
      ...
      this.user.setTokenType(
          org.ejbca.core.protocol.ws.objects.UserDataVOWS
    ...
    public boolean doIt() throws Exception {
      if (this.doCreateNewUser) {
        ...
        this.jobData.userName =
            "WSTESTUSER" + StressTestCommand.this.performanceTest.nextLong();
      }
      if (this.bitsInCertificateSN > 0 && this.doCreateNewUser) {
        this.user.setCertificateSerialNumber(
            new BigInteger(
                this.bitsInCertificateSN,
                StressTestCommand.this.performanceTest.getRandom()));
      }
  ...
  private class CertificateRequestCommand extends BaseCommand
    ...
    public boolean doIt() throws Exception {
      ...
      final CertificateResponse certificateResponse =
          this.ejbcaWS.certificateRequest(
              this.user, requestData, requestType, hardTokenSN, responseType);
      return checkAndLogCertificateResponse(certificateResponse, this.jobData);
    }
  ...
  protected void usage() {
    getPrintStream()
        .println("Command used to perform a \"stress\" test of EJBCA.");
    getPrintStream().println("The command will start up a number of threads.");
    getPrintStream()
        .println(
            "Each thread will continuously add new users to EJBCA. After"
                + " adding a new user the thread will fetch a certificate for"
                + " it.");
    getPrintStream().println();
    getPrintStream()
        .println(
            "Usage : stress <caname> <nr of threads> <max wait time in ms to"
                + " fetch cert after adding user> [<end entity profile name>]"
                + " [<certificate profile name>] [<type of test>]");
    getPrintStream().println();
    getPrintStream()
        .println("Here is an example of how the test could be started:");
    getPrintStream().println("./ejbcawsracli.sh stress ManagementCA 20 5000");
    getPrintStream()
        .println(
            "20 threads is started. After adding a user the thread waits"
                + " between 0-500 ms before requesting a certificate for it."
                + " The certificates will all be signed by the CA"
                + " ManagementCA.");
    getPrintStream()
        .println(
            "You should use the CA with 'Enforce unique public keys' unchecked"
                + " to avoid 'User ... is not allowed to use same key as"
                + " another user is using.' error");
    getPrintStream().println();
    getPrintStream()
        .println(
            "To define a template for the subject DN of each new user use the"
                + " java system property 'subjectDN'.");
    getPrintStream()
        .println(
            "If the property value contains one or several '<userName>' string"
                + " these strings will be substituted with the user name.");
    getPrintStream()
        .println(
            "Example:"
  ...
  public void execute()
    ...
    try {
      ...
      final String subjectDN =
        ...
        try {
          iTmp =
              sTmp != null && sTmp.length() > 0 ? Integer.parseInt(sTmp) : -1;
      ...
      getPrintStream()
          .println(
              "A test key for each thread is generated. This could take some"
                  + " time if you have specified many threads and long keys.");
      synchronized (this) {
        wait();
      }
