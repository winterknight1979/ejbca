 ...
 *
 * @version $Id: GetPublisherQueueLength.java 19902 2014-09-30 14:32:24Z anatom
 *     $
 */
public class GetPublisherQueueLength extends EJBCAWSRABaseCommand
   ...
   */
  public GetPublisherQueueLength(final String[] args) {
    super(args);
  }
  ...
  public void execute()
    ...
    try {
      ...
      if (length < 0) {
        ...
        getPrintStream()
            .println(
                "The length of the queue for the publisher '"
                    + name
                    + "' is "
                    + length
                    + " items.");
      ...
      System.exit(
          length); // return the length so that scripts may use it. // NOPMD,
                   // this is not a JEE app
    } catch (Exception e) {
  ...
  protected void usage() {
    ...
    getPrintStream()
        .println("Usage : getpublisherqueuelength <publisher name>");
