...
public class GetExpiredCertificatesCommand extends EJBCAWSRABaseCommand
    ...
    try {
      ...
      try {
        List<Certificate> result =
            getEjbcaRAWS()
                .getCertificatesByExpirationTime(days, maxNumberOfResults);

        if (result == null || result.size() == 0) {
          getPrintStream()
              .println("No certificate will expire within " + days + " days.");
        } else {
          getPrintStream()
              .println(
                  result.size()
  ...
  protected void usage() {
    ...
    getPrintStream()
        .println(
            "Usage : getexpiredcerts <numberOfDays> <maxNumberOfResults>"
                + " <encoding (DER|PEM)> <outputpath>");
