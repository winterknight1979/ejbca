...
public class GetProfileCommand extends EJBCAWSRABaseCommand
  ...
  public void execute()
    ...
    try {
      ...
      if (!StringUtils.equalsIgnoreCase(profiletype, "eep")
          && !StringUtils.equalsIgnoreCase(profiletype, "cp")) {
        getPrintStream()
            .println(
                "Error: Unknown profile type. Accepted types are 'eep' for end"
                    + " entity profiles and 'cp' for certificate profiles.");
        return;
      }
      ...
      if (!dir.exists() || !dir.isDirectory()) {
        getPrintStream()
            .println(
                "Error: The specified directory is either not a directory or"
                    + " it does not exist");
        return;
      }
      ...
      try {
        ...
        final String outfile =
            directory + "/" + profiletype + "-" + profileid + ".xml";
  ...
  protected void usage() {
    getPrintStream()
        .println(
            "Command used to retrieve a specific end entity or certificate"
                + " profile userdata");
    getPrintStream()
        .println(
            "Usage : getprofile <profileid> <profiletype> <destdirectory>\n");
    getPrintStream()
        .print(
            "Profile type can be either 'eep' for End Entity Profiles or 'cp'"
