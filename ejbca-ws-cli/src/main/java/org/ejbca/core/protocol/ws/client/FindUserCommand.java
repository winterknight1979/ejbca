...
public class FindUserCommand extends EJBCAWSRABaseCommand
  ...
  private static final String[] MATCHWITHTEXTS = {
    ...
    "DN_PKIX_UID",
    "DN_PKIX_COMMONNAME",
    "DN_PKIX_SERIALNUMBER",
    "DN_PKIX_GIVENNAME",
    "DN_PKIX_INITIALS",
    "DN_PKIX_SURNAME",
    "DN_PKIX_TITLE",
    "DN_PKIX_ORGANIZATIONALUNIT",
    "DN_PKIX_ORGANIZATION",
    "DN_PKIX_LOCALITY",
    "DN_PKIX_STATEORPROVINCE",
    "DN_PKIX_DOMAINCOMPONENT",
    "DN_PKIX_COUNTRY"
  };
    ...
    try {
      ...
      try {
        ...
        if (result == null || result.size() == 0) {
          getPrintStream()
              .println("No matching users could be found in database");
        } else {
          ...
          for (int i = 0; i < result.size(); i++) {
            ...
            if (next.getSubjectAltName() == null) {
              ...
              getPrintStream()
                  .println("  Subject Altname: " + next.getSubjectAltName());
            }
            ...
            getPrintStream()
                .println("  Status: " + getStatus(next.getStatus()));
            getPrintStream()
                .println(
                    "  Certificate profile: "
            ...
            getPrintStream()
                .println(
                    "  End entity profile: " + next.getEndEntityProfileName());
  ...
  private int getMatchType(final String matchtype, final int matchwith) {
    if (matchtype.equalsIgnoreCase("EQUALS")) {
      if (matchwith >= org.ejbca.util.query.UserMatch.MATCH_WITH_UID) {
        getPrintStream()
            .println(
                "Error: Matchtype 'EQUALS' cannot be used with DN fields, only"
                    + " 'BEGINSWITH' can be used.");
        usage();
        System.exit(-1); // NOPMD, it's not a JEE app
      } else {
    ...
    if (matchtype.equalsIgnoreCase("BEGINSWITH")) {
      if (matchwith > org.ejbca.util.query.UserMatch.MATCH_WITH_UID
          || matchwith == org.ejbca.util.query.UserMatch.MATCH_WITH_DN
          || matchwith == org.ejbca.util.query.UserMatch.MATCH_WITH_EMAIL) {
        return org.ejbca.util.query.UserMatch.MATCH_TYPE_BEGINSWITH;
      } else {
        getPrintStream()
            .println(
                "Error: Matchtype 'BEGINSWITH' can only be used with DN"
                    + " fields, full DN and EMAIL, use EQUALS");
        usage();
        System.exit(-1); // NOPMD, it's not a JEE app
      }
    }
    if (matchtype.equalsIgnoreCase("CONTAINS")) {
      if ((matchwith == org.ejbca.util.query.UserMatch.MATCH_WITH_DN)
        ...
        getPrintStream()
            .println(
                "Error: Matchtype 'CONTAINS' can only be used with matchwith"
                    + " 'DN' or 'USERNAME'.");
        usage();
        System.exit(-1); // NOPMD, it's not a JEE app
      }
  ...
  private int getMatchWith(final String matchwith) {
  ...
  private EndEntityType getType(final UserDataVOWS userData) {
  ...
  protected void usage() {
    getPrintStream()
        .println(
            "Command used to find userdata, maximum of 100 users will be"
                + " returned ");
    getPrintStream()
    ...
    getPrintStream()
        .println(
            "Matchtype can be : EQUALS (not DN fields), BEGINSWITH (DN"
                + " fields), CONTAINS (matchwith complete DN or USERNAME"
                + " only)");
  }
}
