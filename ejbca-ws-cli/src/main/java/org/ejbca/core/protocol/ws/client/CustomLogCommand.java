 ...
 * @version $Id: CustomLogCommand.java 22553 2016-01-11 13:06:46Z mikekushner $
 */
public class CustomLogCommand extends EJBCAWSRABaseCommand
   ...
   */
  public CustomLogCommand(final String[] args) {
    super(args);
  }
  ...
  public void execute()
    ...
    try {
      ...
      if (!StringUtils.equalsIgnoreCase(certfile, "null")) {
        ...
        getPrintStream()
            .println(
                "Using certificate with subjectDN '"
  ...
  protected void usage() {
    getPrintStream()
        .println("Command used to log a custom log row in the EJBCA log.");
    getPrintStream()
        .println(
            "Usage : customlog logLevel logType logMsg caName username"
                + " certificate\n\n");
    getPrintStream().println("level: log level of the event, INFO or ERROR");
    getPrintStream()
        .println(
            "type: user defined string used as a prefix in the log comment");
    getPrintStream()
        .println(
            "msg: message data used in the log comment. The log comment will"
                + " have a syntax of 'type : msg'");
    getPrintStream()
        .println(
            "caName: of the ca related to the event, use null if no specific"
                + " CA is related. Then will the ca of the administrator be"
                + " used.");
    getPrintStream()
        .println(
            "username of the related user, use null if no related user"
                + " exists.");
    getPrintStream()
        .println(
            "certificate: file path to the certificate that relates to the log"
                + " event, use null if no certificate is related");
  }
}
