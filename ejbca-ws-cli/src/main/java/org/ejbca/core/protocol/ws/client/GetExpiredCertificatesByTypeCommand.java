 ...
 *
 * @version $Id: GetExpiredCertificatesByTypeCommand.java 26057 2017-06-22
 *     08:08:34Z anatom $
 */
public class GetExpiredCertificatesByTypeCommand extends EJBCAWSRABaseCommand
  ...
  public GetExpiredCertificatesByTypeCommand(final String[] args) {
    super(args);
  }
  ...
  public void execute()
    ...
    try {
      ...
      int maxNumberOfResults =
          Integer.parseInt(args[ARG_MAX_NUMBER_OF_RESULTS]);
        ...
        List<Certificate> result =
            getEjbcaRAWS()
                .getCertificatesByExpirationTimeAndType(
                    days, certType, maxNumberOfResults);

        if (result == null || result.size() == 0) {
          getPrintStream()
              .println(
                  "No certificate will expire within "
                      + days
                      + " days and are of type "
                      + certType);
        } else {
          getPrintStream()
              .println(
                  result.size()
                      + " certificate found, written to "
                      + outputPath);
          ...
          while (iter.hasNext()) {
            ...
            if (encoding.equals("DER")) {
              FileOutputStream fos =
                  new FileOutputStream(outputPath + "/" + "cert-" + i + ".cer");
              fos.write(
              ...
              FileOutputStream fos =
                  new FileOutputStream(outputPath + "/" + "cert-" + i + ".pem");
  ...
  protected void usage() {
    getPrintStream()
        .println(
            "Command used to find certificates that will expire within a"
                + " specified number of days and are of a specific type");
    getPrintStream()
        .println(
            "Usage : getexpiredcertsbytype <numberOfDays> <certType>"
                + " <maxNumberOfResults> <encoding (DER|PEM)> <outputpath>");
    getPrintStream().println();
    getPrintStream()
        .println(
            "certType: Use 0=Unknown  1=EndEntity  2=SUBCA  8=ROOTCA "
                + " 16=HardToken");
    getPrintStream()
        .println(
            "maxNumberOfResults: the maximum number of returned certificates");
    getPrintStream()
        .println("outputpath : directory where certificates are written");
    getPrintStream()
        .println(
            "Note that all returned certificates are of status active or"
                + " notified about expiration");
  }
}
