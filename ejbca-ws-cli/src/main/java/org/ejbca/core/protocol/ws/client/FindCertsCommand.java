...
public class FindCertsCommand extends EJBCAWSRABaseCommand
    ...
    try {
      ...
      try {
        List<Certificate> result =
            getEjbcaRAWS().findCerts(username, onlyValid);
  ...
  private boolean getOnlyValid(final String onlyValid) {
  ...
  protected void usage() {
    ...
    getPrintStream()
        .println(
            "Usage : findcerts <username> <onlyvalid (true|false)> <encoding"
                + " (DER|PEM)> <outputpath> \n\n");
    getPrintStream()
        .println(
            "onlyvalid = true only returns nonexired and unrevoked"
                + " certificates ");
    getPrintStream()
        .println(
            "outputpath : directory where certificates are written in form"
                + " username+nn ");
  }
}
