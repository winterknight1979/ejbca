...
/**
 * Imports a certificate issued by an external CA. Used to import certificates
 * sent as a reply to a request created with CaCertRequestCommand.
 *
 * @version $Id: CaCertResponseCommand.java 22553 2016-01-11 13:06:46Z
 *     mikekushner $
 */
public class CaCertResponseCommand extends EJBCAWSRABaseCommand
   ...
   */
  public CaCertResponseCommand(final String[] args) {
    super(args);
  }
  ...
  public void execute()
    ...
    try {
      ...
      getPrintStream()
          .println(
              "Importing certificate with subjectDN '"
                  + CertTools.getSubjectDN(incert)
                  + "', and issuerDN '"
                  + CertTools.getIssuerDN(incert)
                  + "'.");
      ...
      getEjbcaRAWS()
          .caCertResponse(caname, incert.getEncoded(), cachain, keystorepwd);
  ...
  protected void usage() {
    getPrintStream()
        .println(
            "Command used to import a certificate from an external CA. Can be"
                + " X.509 or CVC. Used to receive certificate responses to"
                + " request created with 'cacertrequest' command.");
    getPrintStream()
        .println(
            "Usage : cacertrequest <caname> <certfile> <cachainfile> [<CA"
                + " token password>]\n\n");
    getPrintStream()
        .println(
    ...
    getPrintStream()
        .println(
            "Certfile is a file with the certificate issued by the external"
                + " CA. This is a file with a single PEM or binary"
                + " certificate.");
    getPrintStream()
        .println(
    ...
    getPrintStream()
        .println(
            "CA token password is needed if importing this certificate means"
                + " that a new CA signing key pair must be activated. If not"
                + " given this command will prompt for the input. If you are"
                + " certain that a new key pair will not be activated you can"
                + " give any input as this password will be ignored.");
  }
}
