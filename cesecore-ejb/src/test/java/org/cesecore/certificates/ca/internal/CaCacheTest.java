 ...
 *
 * @version $Id: CaCacheTest.java 26277 2017-08-12 08:25:36Z anatom $
 */
public class CaCacheTest {
  ...
  public void disabledCacheBehavior() {
    ...
    assertEquals(
        ...
        0,
        CaCache.INSTANCE.getNameToIdMap().entrySet().size());
    assertNull(
        "Empty cache returned an entry anyway.",
        CaCache.INSTANCE.getEntry(ca1.getCAId()));
    assertNull(
        "Empty cache returned an entry anyway.",
        CaCache.INSTANCE.getEntry(ca2.getCAId()));
    assertNull(
        "Empty cache returned an entry anyway.",
        CaCache.INSTANCE.getEntry(ca3.getCAId()));
    assertTrue(
        "Cache indicated that non-existing object is valid.",
        CaCache.INSTANCE.shouldCheckForUpdates(ca1.getCAId()));
    assertTrue(
        "Cache indicated that non-existing object is valid.",
        CaCache.INSTANCE.shouldCheckForUpdates(ca2.getCAId()));
    assertTrue(
        "Cache indicated that non-existing object is valid.",
  ...
  public void enabledCacheBehavior() {
    ...
    assertEquals(
        ...
        ca1.getCAId(),
        CaCache.INSTANCE.getNameToIdMap().get(name1).intValue());
    assertEquals(
        "Wrong mapping for name.",
        ca2.getCAId(),
        CaCache.INSTANCE.getNameToIdMap().get(name2).intValue());
    assertEquals(
        "Wrong mapping for name.",
        ca3.getCAId(),
        CaCache.INSTANCE.getNameToIdMap().get(name3).intValue());
    assertEquals(
