 ...
 *
 * @version $Id: InternalSecurityEventsLoggerSessionBean.java 23956 2016-07-20
 *     13:39:31Z anatom $
 */
@Stateless
...
public class InternalSecurityEventsLoggerSessionBean
  ...
  public void log(// NOPMD
    ...
    if (LogServiceState.INSTANCE.isDisabled()) {
      throw new AuditRecordStorageException(
          "Security audit logging is currently disabled.");
    }
    ...
    for (final String loggerId : AuditDevicesConfig.getAllDeviceIds()) {
      try {
        ...
        LOG.error(
            "AuditDevice "
                + loggerId
                + " failed. An event was not logged to this device!",
            e);
      }
    }
    if (anyFailures
      // CESeCore.FAU_STG.4.1: The TSF shall prevent audited events, except
      // those taken by the Auditable and no other actions if the audit trail is
      // full.
      // So even if we failed to produce a proper audit trail for these events,
      // we swallow the exception here to allow the operation to continue.
      && !eventType.equals(EventTypes.LOG_VERIFY)
          && !eventType.equals(EventTypes.LOG_EXPORT)
          && !eventType.equals(EventTypes.LOG_DELETE)
          && !eventType.equals(EventTypes.LOG_SIGN)
          && !eventType.equals(EventTypes.LOG_MANAGEMENT_CHANGE)) {
        throw new AuditRecordStorageException(
            "Failed to write audit log to at least one device.");
  ...
  public boolean auditLogCryptoTest(final String protectThis) {
    if (CesecoreConfigurationHelper.useDatabaseIntegrityProtection(
        AuditRecordData.class.getSimpleName())) {
      if (LOG.isDebugEnabled()) {
        LOG.debug("Performing audit log integrity protection test.");
      }
      // Make a dummy auditRecordData object, to use the "real" code to
      // calculate database integrity protection
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Audit log integrity protection test completed successfully.");
      }
      return true;
    }
    return false;
