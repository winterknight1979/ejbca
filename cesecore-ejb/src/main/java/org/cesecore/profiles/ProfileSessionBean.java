...
public class ProfileSessionBean implements ProfileSessionLocal {
    ...
    if (isFreeProfileId(id)) {
      ...
      final String msg =
          INTRES.getLocalizedMessage(
              "profile.store.error.profile_with_id_exists", id);
  ...
  @Override
  public ProfileData findById(final int id) {
  ...
  public List<ProfileData> findByIds(final Collection<Integer> identifiers) {
    final TypedQuery<ProfileData> query =
        entityManager.createQuery(
            "SELECT a FROM ProfileData a WHERE a.id IN (:ids)",
            ProfileData.class);
  ...
  @Override
  @TransactionAttribute(TransactionAttributeType.REQUIRED)
  public void renameProfile(final Profile profile, final String newName)
  ...
  @TransactionAttribute(TransactionAttributeType.SUPPORTS)
  @Override
  public List<ProfileData> findByNameAndType(
      final String name, final String type) {
    TypedQuery<ProfileData> query =
        entityManager.createQuery(
            "SELECT a FROM ProfileData a WHERE a.profileName=:name AND"
  ...
  public List<ProfileData> findAllProfiles(final String profileType) {
    TypedQuery<ProfileData> query =
        entityManager.createQuery(
            "SELECT a FROM ProfileData a WHERE a.profileType=:profileType",
            ProfileData.class);
