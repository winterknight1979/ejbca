 ...
 *
 * @version $Id: AccessTreeUpdateSessionBean.java 26158 2017-07-18 05:46:19Z
 *     mikekushner $
 */
@Stateless // (mappedName = JndiConstants.APP_JNDI_PREFIX +
           // "AccessTreeUpdateSessionRemote")
...
public class AccessTreeUpdateSessionBean
  ...
  public void signalForAccessTreeUpdate() {
    AccessTreeUpdateData accessTreeUpdateData =
        entityManager.find(
            AccessTreeUpdateData.class,
            AccessTreeUpdateData.AUTHORIZATIONTREEUPDATEDATA);
    if (accessTreeUpdateData == null) {
      // We need to create the database row and incremented the value directly
      // since this is an call to update it
      try {
        accessTreeUpdateData = new AccessTreeUpdateData();
        accessTreeUpdateData.setAccessTreeUpdateNumber(
            AccessTreeUpdateData.DEFAULTACCESSTREEUPDATENUMBER + 1);
        entityManager.persist(accessTreeUpdateData);
        // Additionally we set the marker that this (new) installation should
        // use the new union access rule pattern
        setNewAuthorizationPatternMarker();
      } catch (Exception e) {
        LOG.error(
            InternalResources.getInstance()
                .getLocalizedMessage("authorization.errorcreateauthtree"),
            e);
        throw new EJBException(e);
      }
    } else {
      accessTreeUpdateData.setAccessTreeUpdateNumber(
          accessTreeUpdateData.getAccessTreeUpdateNumber() + 1);
    }
    LOG.debug("Invoking event");
    final AuthorizationCacheReload event =
        new AuthorizationCacheReload(
            accessTreeUpdateData.getAccessTreeUpdateNumber());
  ...
  public boolean isNewAuthorizationPatternMarkerPresent() {
    return entityManager.find(
            AccessTreeUpdateData.class,
            AccessTreeUpdateData.NEW_AUTHORIZATION_PATTERN_MARKER)
        != null;
  }
