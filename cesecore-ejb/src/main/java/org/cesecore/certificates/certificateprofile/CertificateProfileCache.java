 ...
 * volatile, non-final fields in session beans.
 *
 * <p>This cache is designed so only one thread at the time will update the
 * cache if it is too old. Other threads will happily return a bit too old
 * object. If a cache update is forced, for example when a profile is edited, it
 * will always update the cache even if the commit of the transaction fails.
 *
 * <p>Another known issue during forced updates is the race condition exists, so
 * an update in progress might overwrite the result from forced update's
 * database query.
 *
 * <p>The intention of this design is better throughput than fully ordered
 * sequential updates.
 *
 * <p>Probably based on EJBCA's
 * org.ejbca.core.ejb.ca.store.CertificateProfileCache r11155
 *
 * @version $Id: CertificateProfileCache.java 21957 2015-09-30 18:39:55Z jeklund
 *     $
 */
public enum CertificateProfileCache {
  ...
  CertificateProfileCache() {
    ...
    idNameMapCacheTemplate.put(
        Integer.valueOf(CertificateProfileConstants.CERTPROFILE_FIXED_SUBCA),
        CertificateProfile.SUBCAPROFILENAME);
    idNameMapCacheTemplate.put(
        Integer.valueOf(CertificateProfileConstants.CERTPROFILE_FIXED_ROOTCA),
    ...
    idNameMapCacheTemplate.put(
        Integer.valueOf(CertificateProfileConstants.CERTPROFILE_FIXED_SERVER),
  ...
  /**
   * Fetch all profiles from the database, unless cache is enabled, valid and we
   * do not force an update.
   *
   * @param entityManager is required for reading the profiles from the database
  ...
  public void updateProfileCache(
    ...
    if (!force
        && cacheCertificateProfileTime != 0
        && lastUpdate + cacheCertificateProfileTime > now) {
    ...
    try {
      ...
      lastUpdate =
          now; // make sure next thread does not also pass the update test
    } finally {
      lock.unlock();
    }
    final Map<Integer, String> idNameCache =
        new HashMap<Integer, String>(idNameMapCacheTemplate);
    final Map<String, Integer> nameIdCache =
        new HashMap<String, Integer>(nameIdMapCacheTemplate);
    final Map<Integer, CertificateProfile> profCache =
        new HashMap<Integer, CertificateProfile>();
    try {
      final List<CertificateProfileData> result =
          CertificateProfileData.findAll(entityManager);
      for (final CertificateProfileData current : result) {
        ...
        final String certificateProfileName =
            current.getCertificateProfileName();
   ...
   * @return the latest object from the cache or a current database
   *     representation if no caching is used.
   */
