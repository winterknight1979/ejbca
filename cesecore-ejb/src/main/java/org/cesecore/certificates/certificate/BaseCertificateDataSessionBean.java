...
/**
 * Contains common function for CertificateDataSessionBean and
 * NoConflictCertificateDataSessionBean.
 *
 * @version $Id: BaseCertificateDataSessionBean.java 34193 2020-01-07 15:18:15Z
 *     samuellb $
 */
public abstract class BaseCertificateDataSessionBean {
  ...
  /**
   * Returns the name of the table in the database. Either "CertificateData" or
   ...
   * @return a list with information about revoked certificates. Depending on
   *     the table, the result can either contain at most one entry per
   *     certificate, or it may contain duplicates.
   */
  protected Collection<RevokedCertInfo> getRevokedCertInfosInternal(
      final String issuerDN,
      final long lastbasecrldate,
      final boolean forceGetAll) {
    ...
    if (lastbasecrldate > 0) {
      ...
      query =
          getEntityManager()
              .createNativeQuery(
                  "SELECT a.fingerprint as fingerprint, a.serialNumber as"
                      + " serialNumber, a.expireDate as expireDate,"
                      + " a.revocationDate as revocationDate,"
                      + " a.revocationReason as revocationReason FROM "
                      + tableName
                      + " a WHERE a.issuerDN=:issuerDN AND"
                      + " a.revocationDate>:revocationDate AND"
                      + " (a.status=:status1 OR a.status=:status2 OR"
                      + " a.status=:status3)",
      ...
      query.setParameter(
          ...
          CertificateConstants
              .CERT_ACTIVE); // in case the certificate has been changed from on
                             // hold, we need to include it as "removeFromCRL"
                             // in the Delta CRL
      query.setParameter(
          "status3",
          CertificateConstants
              .CERT_NOTIFIEDABOUTEXPIRATION); // could happen if a cert is
                                              // re-activated just before
                                              // expiration
    } else if (forceGetAll) {
      // Base CRL
      query =
          getEntityManager()
              .createNativeQuery(
                  "SELECT a.fingerprint as fingerprint, a.serialNumber as"
                      ...
                      + " a.revocationReason as revocationReason FROM "
                      + tableName
                      + " a WHERE a.issuerDN=:issuerDN AND (a.status=:status1"
                      + " OR a.status=:status2 OR a.status=:status3)",
                  "RevokedCertInfoSubset");
      query.setParameter("issuerDN", issuerDN);
      query.setParameter("status1", CertificateConstants.CERT_REVOKED);
      query.setParameter(
    ...
    while (true) {
      ...
      for (final Object[] current : incompleteCertificateDatas) {
        ...
        final byte[] serialNumber =
            new BigInteger((String) current[1]).toByteArray();
