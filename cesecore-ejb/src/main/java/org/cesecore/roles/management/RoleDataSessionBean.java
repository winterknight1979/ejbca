...
public class RoleDataSessionBean
  ...
  public List<Role> getAllRoles() {
    final TypedQuery<RoleData> query =
        entityManager.createQuery("SELECT a FROM RoleData a", RoleData.class);
  ...
  public Role getRole(final String nameSpace, final String roleName) {
    final Integer roleId =
        RoleCache.INSTANCE
            .getNameToIdMap()
            .get(Role.getRoleNameFullAsCacheName(nameSpace, roleName));
    if (roleId != null) {
    ...
    if (role != null) {
      RoleCache.INSTANCE.updateWith(
          role.getRoleId(),
          role.hashCode(),
          Role.getRoleNameFullAsCacheName(
    ...
    if (StringUtils.isEmpty(nameSpace)) {
      final Query query =
          entityManager.createQuery(
              "SELECT a FROM RoleData a WHERE a.roleName=:roleName AND"
                  + " a.nameSpaceColumn IS NULL");
  ...
  public Role getRole(final int roleId) {
    if (roleId == Role.ROLE_ID_UNASSIGNED) {
      // The reserved ID will never have a database entry, so return quickly
      // with what we know will be the result
      return null;
    }
  ...
  private RoleData getRoleData(final int roleId) {
    final TypedQuery<RoleData> query =
        entityManager.createQuery(
            "SELECT a FROM RoleData a WHERE a.id=:id", RoleData.class);
  ...
  public Role persistRole(final Role role) {
    ...
    if (role.getRoleId() == Role.ROLE_ID_UNASSIGNED) {
      ...
      if (roleData == null) {
        ...
        // persist it, so we proceed (keeping the requested Role ID)
        entityManager.persist(new RoleData(role));
      } else {
        final Role oldRole = roleData.getRole();
        if (role.getAccessRules().equals(oldRole.getAccessRules())) {
          // We will not care if the role has any members, since there is no
    ...
    RoleCache.INSTANCE.updateWith(
        ...
        Role.getRoleNameFullAsCacheName(
            role.getNameSpace(), role.getRoleName()),
        role);
    // If we only created a new Role that has no members yet or the access rules
    // did no change, the authorization would not have changed
  ...
  public boolean deleteRoleNoAuthorizationCheck(final int roleId) {
    // Use an DELETE query instead of entityManager.remove to tolerate
    // concurrent deletion better
    final Query query =
        entityManager.createQuery("DELETE FROM RoleData a WHERE a.id=:id");
    query.setParameter("id", roleId);
