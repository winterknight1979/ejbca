...
/**
 * Sample filter implementation to define a set of Content Security Policies,
 * and other security headers.<br>
 ...
 * specification.<br>
 * and X-XSS-Protection, X-Content-Type-Options, X-FRAME-OPTIONS <br>
 * This implementation is oriented to be easily understandable and easily
 * adapted.<br>
 *
 * @version $Id: ContentSecurityPolicyFilter.java 34257 2020-01-13 09:37:53Z
 *     anatom $
 */
public class ContentSecurityPolicyFilter implements Filter {
  ...
  public void init(final FilterConfig filterConfig) throws ServletException {
    ...
    */
    ...
    // --Define loading policies for javascript, allow inline style elements in
    // order to use onClick attributes
    ...
    cspPolicies.add(
        "script-src " + originLocationRef + " 'unsafe-inline'" + evalstr);
    ...
    // --Define loading policies for Connection, which we don't use, but may use
    // when we move to advanced JSF pages and components
    ...
    // --Define loading policies for Plugins Types, only allow PDF documents, if
    // plug-ins are enabled in web.xml
  ...
  public void doFilter(
    ...
    // With this:
    // Content security policy script-nonce. Only javascripts that are in a
    // self-source js file, or in-line and declare a nonce
    // will be allowed to execute in the browser when the
    // content-security-policy headers are defined with
    // "script-src 'self' 'nonce-$RANDOM".
    // The RANDOM is passed in each request as an attribute CSP_SCRIPT_NONCE.
    // See ContentSecurityPolicyFilter.java
    // Add nonce to in-line java scripts like:
    // <script nonce="<%= (String)request.getAttribute("CSP_SCRIPT_NONCE") %>"
    // type="text/javascript">

    // We can not use this currently because it would require moving all <a
    // id="item-id" ...onclick="callMethod()"/> into setting event handlers in
    // the javascript methods instead:
...
if ((session == null) || (session.getAttribute("CSP_SCRIPT_NONCE") == null)) {
    ...
    */
    ...
    // Used to be in a separate filter, ClickjackFilter, but there is no point
    // in having multiple filters adding security headers
    httpResponse.setHeader("X-FRAME-OPTIONS", frameOptionsMode);
    // New header in 2018, Feature-Policy.
    ...
    httpResponse.setHeader(
        "Feature-Policy",
        "vibrate 'none'; autoplay 'none'; camera 'none'; microphone 'none';"
            + " midi 'none'; gyroscope 'none'; accelerometer 'none';"
            + " magnetometer 'none'; payment 'none'");
