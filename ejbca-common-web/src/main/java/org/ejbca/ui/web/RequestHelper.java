...
public class RequestHelper {
  ...
  /**
   * @deprecated Since 6.1.0, remove in 7.0.0. Use
   *     CertificateResponseType.ENCODED_CERTIFICATE instead
   */
  @Deprecated public static final int ENCODED_CERTIFICATE = 1;
  /**
   * @deprecated Since 6.1.0, remove in 7.0.0. Use
   *     CertificateResponseType.ENCODED_PKCS7 instead
   */
  @Deprecated public static final int ENCODED_PKCS7 = 2;
  /**
   * @deprecated Since 6.1.0, remove in 7.0.0. Use
   *     CertificateResponseType.BINARY_CERTIFICATE instead
   */
  @Deprecated public static final int BINARY_CERTIFICATE = 3;
  /**
   * @deprecated Since 6.1.0, remove in 7.0.0. Use
   *     CertificateResponseType.ENCODED_CERTIFICATE_CHAIN instead
   */
   ...
   */
  public byte[] nsCertRequest(
      final SignSessionLocal signsession,
      final byte[] reqBytes,
      final String username,
      final String password)
      throws ObjectNotFoundException, CADoesntExistsException, EjbcaException,
          AuthorizationDeniedException, CesecoreException,
          CertificateEncodingException, InvalidKeyException,
          NoSuchAlgorithmException, SignatureException,
          NoSuchProviderException {
    if (reqBytes == null || reqBytes.length == 0) {
      throw new IllegalStateException(
          "Invalid request sent from browser (null or zero length).");
    }
    if (Arrays.equals(reqBytes, HIGHGRADE_STRING)
        || Arrays.equals(reqBytes, MEDIUMGRADE_STRING)) {
      throw new IllegalStateException(
          "Key generation failed. If enrolling using a hardware token, please"
    ...
    X509Certificate cert =
        (X509Certificate)
            signsession.createCertificate(
                administrator, username, password, nscr.getPublicKey());
    if (log.isDebugEnabled()) {
   ...
   * @param resulttype should indicate if a PKCS7 or just the certificate is
   *     wanted.
  ...
  public CertificateRequestResponse pkcs10CertRequest(
    ...
    PKCS10RequestMessage req =
        RequestMessageUtils.genPKCS10RequestMessage(b64Encoded);
    req.setUsername(username);
    req.setPassword(password);
    ResponseMessage resp =
        signsession.createCertificate(
            administrator, req, X509ResponseMessage.class, null);
    cert =
    ...
    switch (resulttype) {
      ...
      case ENCODED_CERTIFICATE_CHAIN:
        CAInfo caInfo =
            signsession.getCAFromRequest(administrator, req, false).getCAInfo();
   ...
   * @deprecated Since 6.1.0, remove in 7.0.0. Use the other overloaded version
   *     taking a CertificateResponseType in the resulttype parameter.
   */
  @Deprecated
  public byte[] pkcs10CertRequest(
      final SignSessionLocal signsession,
      final CaSessionLocal caSession,
      final byte[] b64Encoded,
      final String username,
    ...
    return pkcs10CertRequest(
            ...
            username,
            password,
            CertificateResponseType.fromNumber(resulttype),
            doSplitLines)
        .getEncoded();
  }
  ...
  public CertificateRequestResponse pkcs10CertRequest(
      ...
      final String password,
      final CertificateResponseType resulttype)
      throws CertificateEncodingException, CertificateException, EjbcaException,
          CesecoreException, AuthorizationDeniedException,
          CertificateExtensionException {
    return pkcs10CertRequest(
        signsession,
        caSession,
  ...
  /**
   * Handles CVC certificate requests. These are the special certificates for
   * EAC ePassport PKI.
   *
   * @param signsession signsession to get certificate from
   ...
   */
  public byte[] cvcCertRequest(
      final SignSessionLocal signsession,
      final byte[] b64Encoded,
      final String username,
      final String password)
      throws Exception {
    CVCRequestMessage req =
        RequestMessageUtils.genCVCRequestMessage(b64Encoded);
    req.setUsername(username);
    req.setPassword(password);
    // Yes it says X509ResponseMessage, but for CVC it means it just contains
    // the binary certificate blob
    ResponseMessage resp =
        signsession.createCertificate(
            administrator, req, X509ResponseMessage.class, null);
    if (resp.getStatus() == ResponseStatus.FAILURE) {
      final String msg =
          "Failed to generate CVC certificate: " + resp.getFailText();
      log.debug(msg);
      throw new EjbcaException(msg);
    }
    CardVerifiableCertificate cert =
        CertTools.getCertfromByteArray(
            resp.getResponseMessage(), CardVerifiableCertificate.class);
   ...
   * @throws Exception fail
   */
  public static void ieCertFormat(final byte[] bA, final PrintStream out)
      throws Exception {
    BufferedReader br =
   ...
   */
  public static void sendNewCertToIEClient(
      final byte[] b64cert,
    ...
    if (is == null) {
      throw new IOException(
          "Template '(/)" + responseTemplate + "' can not be found or read.");
    }
   ...
   * @throws Exception on error
   */
  public static void sendNewCertToNSClient(
      final byte[] certs, final HttpServletResponse out) throws Exception {
   ...
   * @param beginKey String containing key information, i.e.
   *     BEGIN_CERTIFICATE_WITH_NL or BEGIN_PKCS7_WITH_NL
   * @param endKey String containing key information, i.e.
   *     END_CERTIFICATE_WITH_NL or END_PKCS7_WITH_NL
   * @throws IOException on error
   */
  public static void sendNewB64File(
      final byte[] b64cert,
   ...
   * @throws IOException on error
   */
  public static void sendNewB64Cert(
      final byte[] b64cert,
      final HttpServletResponse out,
      final String beginKey,
      final String endKey)
   ...
   * @throws IOException on error
   */
  public static void sendNewX509CaCert(
      final byte[] cert, final HttpServletResponse out) throws IOException {
   ...
   * @param filename to call the file in a Content-disposition, can be null to
   *     leave out this header
   * @throws IOException on error
   */
  public static void sendBinaryBytes(
      final byte[] bytes,
      final HttpServletResponse out,
   ...
   * @throws IOException on error
   */
  public static void sendB64BinaryBytes(
      final byte[] bytes,
      final HttpServletResponse out,
      final String contentType,
      final String filename)
   ...
   * @param out output stream to send to
   * @param hidemenu whether the menu should be hidden (translates to the
   *     "hidemenu" URL parameter)
   * @param resulttype type of desired result, e.g. cert, certchain, pkcs7...
   * @throws Exception fail
   */
  public static void sendResultPage(
      final byte[] certbytes,
      final HttpServletResponse out,
      final boolean hidemenu,
      final CertificateResponseType resulttype)
      throws Exception {
    Certificate cert =
    ...
    out.sendRedirect(
        "enrol/result_download.jsp?issuer="
            + URLEncoder.encode(issuerDN, "UTF-8")
            + "&serno="
            + serialNumber
            + "&resulttype="
            + resultTypeStr
  ...
  /**
   * Sends a page with certificate information and an automatic redirect to the
   * download page. The issuer must not be a "throw away" CA. The certificate is
   * automatically installed in the browser.
   *
   * @param certbytes bytes
   * @param out response
   * @param hidemenu bool
   * @param installToBrowser Browser type. Only "netscape" is supported, which
   *     means "most browsers except IE".
   * @throws Exception fail
   */
  public static void sendResultPage(
      final byte[] certbytes,
   ...
   * First tries to get the character encoding from the request, if the browser
   * is so kind to tell us which it is using, which it never does... Otherwise,
   * when the browser is silent, it sets the character encoding to the same
   * encoding that we use to display the pages.
   *
  ...
  public static String getFileNameFromCertNoEnding(
    ...
    if (StringUtils.equals(cacert.getType(), "CVC")) {
      ...
      HolderReferenceField chrf =
          cvccert.getCVCertificate().getCertificateBody().getHolderReference();
      if (chrf != null) {
