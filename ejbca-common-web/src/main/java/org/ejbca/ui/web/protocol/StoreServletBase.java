...
public abstract class StoreServletBase extends HttpServlet {
   ...
   * @throws ServletException ServletException
   */
  public abstract void sHash(
      String sHash, HttpServletResponse resp, HttpServletRequest req)
   ...
   * @throws ServletException ServletException
   */
  public abstract void iHash(
      String iHash, HttpServletResponse resp, HttpServletRequest req)
      throws IOException, ServletException;
   ...
   * @throws ServletException ServletException
   */
  public abstract void sKIDHash(
      String sKIDHash, HttpServletResponse resp, HttpServletRequest req)
      throws IOException, ServletException;

  /**
   * Return certificate or CRL for the RFC4387 sKIDHash http parameter. In this
   * case the alias name has been used to get the parameter.
   *
   * @param sKIDHash sKIDHash http parameter
   * @param resp HttpServletResponse
   * @param req HttpServletRequest
   ...
   * @return false if reload wasn't requested, or true if it was requested (even
   *     if it turned out to be unauthorized)
   */
  private boolean performReload(
      final HttpServletRequest req, final HttpServletResponse resp) {
    // We have a command to force reloading of the certificate cache that can
    // only be run from localhost
    // http://localhost:8080/crls/search.cgi?reloadcache=true
    final boolean doReload =
        StringUtils.equals(req.getParameter("reloadcache"), "true");
    if (!doReload) {
      return false;
    }
    try {
   ...
   * @param req the HttpServletRequest
   * @param resp the HttpServletResponse
   * @return true if authorized, or false if not, and sends
   *     HttpServletResponse.SC_UNAUTHORIZED
   * @throws IOException if the HttpServletResponse.SC_UNAUTHORIZED response
   *     couldn't be sent in case if a negative result
   */
  private boolean checkIfAutorizedIP(
      final HttpServletRequest req, final HttpServletResponse resp)
      throws IOException {
   ...
   *
   * <p>The implementation has to escape characters of URL to be HTML safe.
   *
  ...
  public abstract void printInfo(
      X509Certificate cert, String indent, PrintWriter pw, String url);

  /** @return the title of the page */
  public abstract String getTitle();

  private void returnInfoPage(
      final HttpServletResponse response, final String info)
      throws IOException {
    ...
    writer.println(
        "<p>When searching for certificates you can use iHash, sHash and"
            + " sKIDHash. iHash is the ASN1 encoded DN of the issuer in a"
            + " certificate, sHash of the subject and sKIDHash is the"
            + " subjectKeyIdentifier. If you search with it you get all"
            + " certificates that has the same issuer, except for the root"
            + " certificate. You do not find a root certificate if you search"
            + " with the iHash of the root. It has been assumed that sHash"
            + " should be used when searching for a root.</p>");
    writer.println(
        "<p>When searching for CRLs you can use iHash and sKIDHash. iHash is"
            + " the ASN1 encoded DN of the issuer in a certificate and"
            + " sKIDHash is the subjectKeyIdentifier.");
    writer.println(
        "<br/>To get the latest delta CRL you can append the parameter"
            + " 'delta='.");
    writer.println(
        "<br/>To get a CRL with a specific CRL number you can append the"
            + " parameter 'crlnumber=&lt;number&gt;'.</p>");
    writer.println("<hr>");
    writer.println(info);
    writer.println("</td>");
    writer.println("</tr>");
  ...
  private void printInfo(
    ...
    printInfo(
        this.certCache.getRootCertificates(),
        "",
        pw,
  ...
  private class HtmlPrintWriter extends PrintWriter {
    ...
    HtmlPrintWriter(final Writer out) {
      super(out);
    }
