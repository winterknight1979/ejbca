...
/**
 * A class containing some helpful functions used in more than one Servlet,
...
public class ServletUtils {
   ...
   * headers makes IE refuse to save a file that is sent (for example a
   * certificate). No-cache headers are also automatically added by Tomcat by
   * default, so we better make sure they are set to a harmless value.
   *
   * @param res HttpServletResponse parameter as taken from the doGet, doPost
