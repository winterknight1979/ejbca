 ...
 *
 * @version $Id: PublishQueueProcessWorker.java 24487 2016-10-10 09:10:53Z
 *     anatom $
 */
public class PublishQueueProcessWorker extends EmailSendingWorker {
  ...
  /**
   * Semaphore making sure not two identical services run at the same time. This
   * must be decided by serviceName, since we can configure one of these
   * services for every publisher.
   */
  private static HashMap<String, Boolean> runmap =
  ...
  public void work(final Map<Class<?>, Object> ejbs)
    ...
    final PublisherQueueSessionLocal publisherQueueSession =
        ((PublisherQueueSessionLocal)
            ejbs.get(PublisherQueueSessionLocal.class));
        ...
        if (o != null) {
          ...
          for (int i = 0; i < ids.length; i++) {
            ...
            publisherQueueSession
                .plainFifoTryAlwaysLimit100EntriesOrderByTimeCreated(
