...
/**
 * Variables used with userdata ${USERNAME} or ${user.USERNAME} = The users
 * username ${PASSWORD} or ${user.PASSWORD} = The users password ${UID} or
 * ${user.UID} = The user's unique identifier ${CN} or ${user.CN} = The common
 * name of the user. ${SN} or ${user.SN} = The serial number (in DN) of the
 * user. ${O} or ${user.O} = The user's organization ${OU} or ${user.OU} = The
 * user's organization unit ${C} or ${user.C} = The user's country ${user.E} =
 * The user's email address from Subject DN ${user.TIMECREATED} = The time the
 * user was created ${user.TIMEMODIFIED} = The time the user was modified
 *
 * <p>${requestAdmin.CN} = The common name of the requesting administrator.
 * ${requestAdmin.EE.EMAIL} = The email address of the administrator adding the
 * end entity according to the administrator's End Entity data.
 * ${requestAdmin.SAN.EMAIL} = The email address of the administrator adding the
 * end entity according to the administrator's Subject Alternative Name,
 * RFC822Name field.
 *
 * <p>Variables used with expiring certificates. ${expiringCert.CERTSERIAL} =
 * The serial number of the certificate about to expire
 * ${expiringCert.EXPIREDATE} = The date the certificate will expire
 * ${expiringCert.CERTSUBJECTDN} = The certificate subject dn
 * ${expiringCert.CERTISSUERDN} = The certificate issuer dn The variables ${CN},
 * ${SN}, ${O}, ${OU}, ${C} are also available.
 *
 * <p>Variable used with notification where approval is involved in the state
 * change (approvalAdmin = last admin to approve). ${approvalAdmin.USERNAME} =
 * The approving administrator's username ${approvalAdmin.CN} = The common name
 * of the approving administrator. ${approvalAdmin.SN} = The common name of the
 * approving administrator. ${approvalAdmin.O} = The approving administrator's
 * organization ${approvalAdmin.OU} = The approving administrator's organization
 * unit ${approvalAdmin.C} = The approving administrator's country
 * ${approvalRequestID} = The id of the approval request
 *
 * @version $Id: UserNotificationParamGen.java 24838 2016-12-06 10:45:31Z
 *     jeklund $
 */
public class UserNotificationParamGen extends NotificationParamGen {
  ...
  /**
   * Notification parameter generation when notifying about end entity events
   * (new, generated, revoked etc).
   *
   * @param userData The end entity that is being operated on. Populates
   *     USERNAME, CN etc variables
   * @param approvalAdminDN The DN of the administrator that approved the
   *     request, if approvals were used. Populates approvalAdmin variables.
   * @param requestAdmin The end entity that requested the event from the
   *     beginning, either the admin that adds an end entity if no approvals are
   *     used, of if approvals were used the admin requesting (creating the
   *     approval request) to add an end entity. Populates requestAdmin
   *     variables
   * @param approvalRequestID ID
   * @param revokedCertificate the certificate that is revoked, in case of a
   *     revocation event. Populates revokedCertificate variables
   */
  public UserNotificationParamGen(
      final EndEntityInformation userData,
      final String approvalAdminDN,
  ...
  private void populateWithApprovalData(
    ...
    DNFieldExtractor dnfields =
        new DNFieldExtractor(approvalAdminDN, DNFieldExtractor.TYPE_SUBJECTDN);
  ...
  protected void populateWithRevokedCertificate(
    ...
    if (revokedCertificate != null) {
      final CertificateData certificateData =
          revokedCertificate.getCertificateData();
      if (certificateData != null) {
        paramPut(
            "revokedCertificate.CERTSERIAL",
            certificateData.getSerialNumberHex());
        paramPut(
            "revokedCertificate.EXPIREDATE",
            fastDateFormat(new Date(certificateData.getExpireDate())));
        paramPut(
            "revokedCertificate.CERTSUBJECTDN",
            certificateData.getSubjectDnNeverNull());
        paramPut(
            "revokedCertificate.CERTISSUERDN", certificateData.getIssuerDN());
        ...
        switch (RevocationReasons.getFromDatabaseValue(
            certificateData.getRevocationReason())) {
          case UNSPECIFIED:
            newRevocationReason = "unspecified";
            break;
          case KEYCOMPROMISE:
            newRevocationReason = "keyCompromise";
            break;
          case CACOMPROMISE:
            newRevocationReason = "cACompromise";
            break;
          case AFFILIATIONCHANGED:
            newRevocationReason = "affiliationChanged";
            break;
          case SUPERSEDED:
            newRevocationReason = "superseded";
            break;
          case CESSATIONOFOPERATION:
            newRevocationReason = "cessationOfOperation";
            break;
          case CERTIFICATEHOLD:
            newRevocationReason = "certificateHold";
            break;
          case REMOVEFROMCRL:
            newRevocationReason = "removeFromCRL";
            break;
          case PRIVILEGESWITHDRAWN:
            newRevocationReason = "privilegeWithdrawn";
            break;
          case AACOMPROMISE:
            newRevocationReason = "aACompromise";
            break;
          default:
            break;
        }
  ...
  protected void populateWithExpiringCert(final Certificate expiringCert) {
    if (expiringCert != null) {
      paramPut(
          "expiringCert.CERTSERIAL",
          CertTools.getSerialNumberAsString(expiringCert));
      paramPut(
          "expiringCert.EXPIREDATE",
          fastDateFormat(CertTools.getNotAfter(expiringCert)));
      paramPut(
          "expiringCert.CERTSUBJECTDN", CertTools.getSubjectDN(expiringCert));
      paramPut(
          "expiringCert.CERTISSUERDN", CertTools.getIssuerDN(expiringCert));
    }
  }
    ...
    if (userData != null) {
      ...
      DNFieldExtractor dnfields =
          new DNFieldExtractor(
              userData.getDN(), DNFieldExtractor.TYPE_SUBJECTDN);
  ...
  protected void populateWithEmailAddresses(
    ...
    if (userdata != null) {
      ...
      final DNFieldExtractor sanfields =
          new DNFieldExtractor(
              userdata.getSubjectAltName(),
              DNFieldExtractor.TYPE_SUBJECTALTNAME);
      paramPut(
          "user.SAN.EMAIL", sanfields.getField(DNFieldExtractor.RFC822NAME, 0));
    }
    if (requestAdmin != null) {
      paramPut("requestAdmin.EE.EMAIL", requestAdmin.getEmail());
      final DNFieldExtractor sdnFields =
          new DNFieldExtractor(
              requestAdmin.getDN(), DNFieldExtractor.TYPE_SUBJECTDN);
      paramPut("requestAdmin.CN", sdnFields.getField(DNFieldExtractor.CN, 0));
      final DNFieldExtractor sanFields =
          new DNFieldExtractor(
              requestAdmin.getSubjectAltName(),
              DNFieldExtractor.TYPE_SUBJECTALTNAME);
      paramPut(
          "requestAdmin.SAN.EMAIL",
          sanFields.getField(DNFieldExtractor.RFC822NAME, 0));
    }
  }
}
