...
/**
 * Worker used for downloading external CRLs and populating the local database
 * with limited CertificateData entries, that can be used to service OCSP
 * responses and stores the CRL locally, so it could be served for example
 * through the public web.
 *
 * <p>If the freshest CRL extension is present in a full CRL, the delta CRL will
 * be downloaded and processed as well.
 *
 * <p>The worker can be configured to not respect the nextUpdate
 *
...
/** Param. */
  ...
  public void work(final Map<Class<?>, Object> ejbs)
    ...
    final CrlStoreSessionLocal crlStoreSession =
        (CrlStoreSessionLocal) ejbs.get(CrlStoreSessionLocal.class);
    final ImportCrlSessionLocal importCrlSession =
        (ImportCrlSessionLocal) ejbs.get(ImportCrlSessionLocal.class);
    // Parse worker configuration
    Collection<Integer> caIdsToCheck = getCAIdsToCheck(true);
    if (caIdsToCheck != null
        && caIdsToCheck.contains(Integer.valueOf(CAConstants.ALLCAS))) {
      caIdsToCheck = caSession.getAllCaIds();
    }
    final boolean ignoreNextUpdate =
        Boolean.valueOf(
            properties.getProperty(
                PROP_IGNORE_NEXT_UPDATE, Boolean.FALSE.toString()));
    final int maxDownloadSize =
        Integer.parseInt(
            properties.getProperty(
                PROP_MAX_DOWNLOAD_SIZE,
    ...
    for (final int caId : caIdsToCheck) {
      ...
      try {
        ...
        if (caInfo != null
          ...
          final X509CRL lastFullCrl =
              getCRLFromBytes(crlStoreSession.getLastCRL(issuerDn, false));
          ...
          if (!ignoreNextUpdate
            ...
            final X509CRL downloadedFullCrl =
                getAndProcessCrl(
                    url,
                    maxDownloadSize,
                    caCertificate,
          ...
          if (newestFullCrl != null) {
            final List<String> freshestCdps =
                CrlExtensions.extractFreshestCrlDistributionPoints(
                    newestFullCrl);
            if (!freshestCdps.isEmpty()) {
              // Delta CRLs are used and we might already have a valid one
              // stored
              X509CRL lastDeltaCrl =
                  getCRLFromBytes(crlStoreSession.getLastCRL(issuerDn, true));
              if (lastDeltaCrl != null
                  && lastDeltaCrl
                      .getThisUpdate()
                      .before(newestFullCrl.getThisUpdate())) {
                // The last known delta CRL info is already included in the
                // latest full CRL, so treat the last delta as non-existent
                lastDeltaCrl = null;
              }
              if (!ignoreNextUpdate
                  && lastDeltaCrl != null
                ...
                // Check for and process first delta CRL that can be reached
                // over HTTP (if any)
        ...
        throw new ServiceExecutionFailedException(
            "Service should always be authorized to any CA.", e);
      }
    }
  }
  ...
  private X509CRL getAndProcessCrl(
      final URL cdpUrl,
      final int maxSize,
      final X509Certificate caCertificate,
      final CAInfo caInfo,
