...
public class EncryptedValueBuilder {
   ...
   */
  public EncryptedValue build(final char[] revocationPassphrase)
   ...
   *     symmetric key for this value.
   */
  public EncryptedValue build(final X509CertificateHolder holder)
      throws CRMFException {
    try {
      return encryptData(padData(holder.getEncoded()));
    } catch (IOException e) {
      throw new CRMFException(
          "cannot encode certificate: " + e.getMessage(), e);
    }
  }
   ...
   *     symmetric key for this value.
   */
  public EncryptedValue build(final PrivateKeyInfo privateKeyInfo)
      throws CRMFException {
    PKCS8EncryptedPrivateKeyInfoBuilder encInfoBldr =
        new PKCS8EncryptedPrivateKeyInfoBuilder(privateKeyInfo);
    ...
    try {
      ...
      encSymmKey =
          new DERBitString(wrapper.generateWrappedKey(encryptor.getKey()));
      ...
      return new EncryptedValue(
          ...
          keyAlg,
          valueHint,
          new DERBitString(encInfo.getEncoded()));
    } catch (IOException e) {
      throw new CRMFException(
          "cannot encode encrypted private key: " + e.getMessage(), e);
  ...
  private EncryptedValue encryptData(final byte[] data) throws CRMFException {
  ...
  private byte[] padData(final byte[] data) {
    if (padder != null) {
