...
public class RegTokenPasswordExtractor implements ICMPAuthenticationModule {
  ...
  public boolean verifyOrExtract(final PKIMessage msg, final String username) {
    ...
    // If there is "Registration Token Control" in the CertReqMsg regInfo
    // containing a password, we can use that
      ...
      do {
        ...
        if (av != null) {
          ...
          if (StringUtils.equals(
              CRMFObjectIdentifiers.id_regCtrl_regToken.getId(),
              av.getType().getId())) {
    ...
    if (pwd == null) {
      // If there is "Registration Token Control" in the CertRequest controls
      // containing a password, we can use that
      // Note, this is the correct way to use the regToken according to RFC4211,
      // section "6.1.  Registration Token Control"
