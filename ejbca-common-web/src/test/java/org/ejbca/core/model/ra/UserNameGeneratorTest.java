 ...
 *
 * @version $Id: UserNameGeneratorTest.java 22139 2015-11-03 10:41:56Z
 *     mikekushner $
 */
public class UserNameGeneratorTest {
  ...
  public void test01() throws Exception {
    UsernameGeneratorParams usernameGeneratorParams =
        new UsernameGeneratorParams();
    ...
    UsernameGenerator usernameGenerator =
        UsernameGenerator.getInstance(usernameGeneratorParams);

    final String errorMessage = "Did not generate an expected username.";
    assertEquals(
        errorMessage,
        "test",
        usernameGenerator.generateUsername(new X500Name("CN=test").toString()));
    assertEquals(
        errorMessage, null, usernameGenerator.generateUsername("".toString()));
    assertEquals(
        errorMessage, null, usernameGenerator.generateUsername(" ".toString()));
    assertEquals(
        errorMessage,
        ...
        usernameGenerator.generateUsername(
            new X500Name("CN=test, serialNumber=1234").toString()));
    assertEquals(
        errorMessage,
        null,
        usernameGenerator.generateUsername(new X500Name("O=org").toString()));
    assertEquals(
        errorMessage,
        "12345",
        usernameGenerator.generateUsername("CN=test, SN=12345"));
    assertEquals(
        errorMessage, "1234", usernameGenerator.generateUsername("SN=1234"));

    // These wont work since new X509Name converts SN to SERIALNUMBER in
    // toString()
    // Is this something we should compensate for in
  ...
  public void testUsernameGeneratorRandom() {
    UsernameGenerator gen =
        UsernameGenerator.getInstance(UsernameGeneratorParams.RANDOM);
  ...
  public void testUsernameGeneratorDN() {
    ...
    UsernameGenerator gen =
        UsernameGenerator.getInstance(UsernameGeneratorParams.DN);
    ...
    assertEquals(
        ...
        "C=SE, O=FooO, UID=foo, CN=bar",
        u);
    u = gen.generateUsername(dnReverse);
    assertEquals(
        "Username was not constructed properly from DN",
        "CN=bar,UID=foo,O=FooO,C=SE",
        u);
  ...
  public void testUsernameGeneratorUsername() {
    String username = "foo";
    UsernameGenerator gen =
        UsernameGenerator.getInstance(UsernameGeneratorParams.USERNAME);
