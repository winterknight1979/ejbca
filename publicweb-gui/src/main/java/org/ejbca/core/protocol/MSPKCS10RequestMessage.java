...
/**
 * Extends the PKCS10RequestMessgae and contains a few function to parse MS
 * specific information like GUID, DNS, Template etc..
 *
 * @version $Id: MSPKCS10RequestMessage.java 20728 2015-02-20 14:55:55Z
 *     mikekushner $
 */
public class MSPKCS10RequestMessage extends PKCS10RequestMessage {
  ...
  private static final Logger LOG =
      Logger.getLogger(MSPKCS10RequestMessage.class);
   ...
   */
  public MSPKCS10RequestMessage(final byte[] msg) throws IOException {
    super(msg);
  }
   ...
   * @throws IOException fail
   */
  public MSPKCS10RequestMessage(final PKCS10CertificationRequest p10)
      throws IOException {
    super(new JcaPKCS10CertificationRequest(p10));
  }

  /** Param. */
  public static final String SZOID_CERTIFICATE_TEMPLATE_V2 =
      "1.3.6.1.4.1.311.21.7";
  // MicrosoftObjectIdentifiers.microsoftCertTemplateV2?
  /** Param. */
  public static final String SZOID_CMC_REG_INFO = "1.3.6.1.5.5.7.7.18";
  /** Param. */
  public static final String SZOID_REQUEST_CLIENT_INFO =
      "1.3.6.1.4.1.311.21.20";
  /** Param. */
  public static final String SZOID_ENROLL_CERTTYPE_EXTENSION =
      "1.3.6.1.4.1.311.20.2";
  /** Param. */
  public static final String SZOID_EXTENSION_REQUEST = "1.2.840.113549.1.9.14";
  /** Param. */
  public static final String SOME_TEMPL_OID =
      "1.3.6.1.4.1.311.21.8.4014942.3497959.5914804."
      + "3829722.12246394.103.3066650.1537810";
  /** Param. */
  public static final String OID_GUID = "1.3.6.1.4.1.311.25.1";

  /**
   * Returns the MS request client info object (1.3.6.1.4.1.311.21.20) as an
   * ArrayList<String>.
   *
   * <p>E.g. an Machine-template request contains the following structure
   * SEQUENCE { 209 9: OBJECT IDENTIFIER '1 3 6 1 4 1 311 21 20' 220 57: SET {
   * 222 55: SEQUENCE { 224 1: INTEGER 1 227 18: UTF8String 'host.company.local'
   * 247 21: UTF8String 'COMPANY\Administrator' 270 7: UTF8String 'certreq' : }
   * : } : }
   *
   * @return list
   */
  private ArrayList<String> getMSRequestInfo() {
    ...
    Attribute[] attributes =
        pkcs10.getAttributes(
            new ASN1ObjectIdentifier(SZOID_REQUEST_CLIENT_INFO));
    if (attributes.length == 0) {
      ...
      DERSequence seq =
          (DERSequence) DERSequence.getInstance(values.getObjectAt(0));
  ...
  /**
   * Returns the DNS of the MS request client info object
   * (1.3.6.1.4.1.311.21.20) as a String.
   *
   * <p>This is probably the machine from where the reqeust was made.
   *
   * @return DNS
   */
  ...
  /**
   * Returns the name of the Certificate Template or null if not available or
   * not known.
   *
   * @return name
   */
  public String getMSRequestInfoTemplateName() {
    ...
    while (enumeration.hasMoreElements()) {
      DERSequence seq2 =
          (DERSequence) DERSequence.getInstance(enumeration.nextElement());
      ...
      if (SZOID_ENROLL_CERTTYPE_EXTENSION.equals(oid.getId())) {
        try {
          ...
          ASN1InputStream dosAsn1InputStream =
              new ASN1InputStream(new ByteArrayInputStream(dos.getOctets()));
          try {
  ...
  /**
   * Returns a String vector with known subject altnames: [0] Requested GUID [1]
   * Requested DNS.
   *
   * @return vector
   */
  public String[] getMSRequestInfoSubjectAltnames() {
    ...
    if (attributes.length != 0) {
      ASN1Set set = attributes[0].getAttrValues();
      DERSequence seq =
          (DERSequence) DERSequence.getInstance(set.getObjectAt(0));
      Enumeration<?> enumeration = seq.getObjects();
      while (enumeration.hasMoreElements()) {
        DERSequence seq2 =
            (DERSequence) DERSequence.getInstance(enumeration.nextElement());
            ...
            while (ais.available() > 0) {
              ...
              while (enum1.hasMoreElements()) {
                ...
                if (dto.getTagNo() == 0) {
                  ...
                  ASN1ObjectIdentifier doid =
                      (ASN1ObjectIdentifier) ds.getObjectAt(0);
                  if (OID_GUID.equals((doid).getId())) {
                    DEROctetString dos3 =
                        (DEROctetString)
                            ((DERTaggedObject) ds.getObjectAt(1)).getObject();
                    ret[0] =
                        dos3.toString()
                            .substring(1); // Removes the initial #-sign
                  }
