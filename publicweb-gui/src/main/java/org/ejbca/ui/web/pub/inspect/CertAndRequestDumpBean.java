...
/**
 * This bean dumps contents of certificate and certificate request as text. CVC
 * dump or ASN.1 dump.
 *
 ...
 *
 * @version $Id: CertAndRequestDumpBean.java 22553 2016-01-11 13:06:46Z
 *     mikekushner $
 */
public class CertAndRequestDumpBean {
   ...
   *
   * @return String containing raw text output or null of input is null, or
  ...
  public String getDump() {
    ...
    if ((ret == null) && (requestBytes != null) && (requestBytes.length > 0)) {
      ...
      try {
        final PKCS10CertificationRequest pkcs10 =
            new PKCS10CertificationRequest(requestBytes);
        //                ret = pkcs10.toString();
        final ASN1InputStream ais =
            new ASN1InputStream(new ByteArrayInputStream(pkcs10.getEncoded()));
    ...
    if (ret == null) {
      // Not a CVC object or PKCS10 request message, perhaps a X.509
      // certificate?
      try {
  ...
  /**
   * @param returnMessageOnError bool
   * @return dump
   */
  ...
  private static CVCObject getCVCObject(final byte[] cvcdata)
    ...
    try {
      ...
      try {
        ...
        final Collection<Certificate> col =
            CertTools.getCertsFromPEM(
                new ByteArrayInputStream(cvcdata), Certificate.class);
  ...
  private Certificate getCert(final byte[] certbytes)
