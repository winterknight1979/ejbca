...
/**
 * When using some filters, namely the CSRFGuard filter, it will mess up
 * character encoding (if using something else than US-ASCII). We use this
 * filter to set the correct, configured in EJBCA, (i.e. UTF-8 to support intl
 * characters) character encoding.
 *
 * <p>We don't use a WebFilter annotation here, because we must set it in
 * web.xml in order to enforce the order filters are called.
 *
 * @version $Id: EncodingFilter.java 34116 2019-12-18 10:32:41Z anatom $
 */
public class EncodingFilter implements Filter {
  ...
  public void doFilter(
    ...
    // The way we use OWASP CSRF Guard makes all POST requests to our form in
    // the last remaining jsp pages protected against csrf attacks
    // it does however not protect GET requests to jsp pages, and service
    // methods in jsp pages will respond to both GET and POST
    ...
    if (!httpreq.getMethod().equalsIgnoreCase("post")) {
      // It's not a POST request, check if it is an operation that tries to do
      // something
