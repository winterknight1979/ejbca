 ...
 *
 * @version $Id: ApprovalPartitionProfileGuiObject.java 28844 2018-05-04
 *     08:31:02Z samuellb $
 */
public class ApprovalPartitionProfileGuiObject implements Serializable {
  ...
  public List<SelectItem /*<String,String>*/> getPropertyPossibleValues() {
    ...
    if (profilePropertyList != null) {
      final DynamicUiProperty<? extends Serializable> property =
          profilePropertyList.getRowData();
   ...
   *     "APPROVAL_PROFILE_&lt;TYPE&gt;_&lt;property-name&gt;" or property-name
   *     if no key exists.
   */
  public String getPropertyNameLocalized() {
    ...
    final String translatedNameCommon =
        EjbcaJSFHelper.getBean().getEjbcaWebBean().getText(msgKeyCommon);
    if (!translatedNameCommon.equals(msgKeyCommon)) {
      return translatedNameCommon;
    }
    final String msgKey =
        "APPROVAL_PROFILE_"
            + approvalProfileIdentifier.toUpperCase()
            + "_"
            + name.toUpperCase();
    final String translatedName =
        EjbcaJSFHelper.getBean().getEjbcaWebBean().getText(msgKey);
