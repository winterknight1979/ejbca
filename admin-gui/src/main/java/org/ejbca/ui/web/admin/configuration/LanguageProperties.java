 ...
 *
 * @version $Id: LanguageProperties.java 22667 2016-01-28 15:49:56Z samuellb $
 */
public class LanguageProperties extends Properties {
  ...
  public synchronized void load(final InputStream inStream) throws IOException {
    ...
    // Note: This is not true, we try with ISO-8859-1 and try again if it
    // fails??
    BufferedReader in =
        new BufferedReader(
            new InputStreamReader(
                inStream, "ISO-8859-1")); // This sentence is crucial
    try {
      put(in);
    } catch (Throwable t) {
      log.info(
          "Caught exception loading LanguageProperties with system default"
              + " encoding, will try with ISO-8859-1: "
              + t.getMessage());
  ...
  private void put(final BufferedReader in) throws IOException {
    while (true) {
      // Get next line
