 ...
 *
 * @version $Id: RolesBean.java 28844 2018-05-04 08:31:02Z samuellb $ TODO: CDI
 *     beans
 */
...
public class RolesBean extends BaseManagedBean implements Serializable {
  ...
  /**
   * @return true when admin is authorized to edit roles (which implies view
   *     rights)
   */
  ...
  /**
   * @return true if the only the empty namespace currently exists on this
   *     system
   */
  ...
  /**
   * @return true if the admin has access to the empty namespace (and hence is
  ...
  /**
   * @return a list of existing and authorized namespaces that the admin has
   *     access to
   */
  public List<String> getNameSpaceAvailable() {
    if (nameSpacesAvailable == null) {
      nameSpacesAvailable =
          new LinkedList<>(roleSession.getAuthorizedNamespaces(getAdmin()));
  ...
  public void actionEditNewNameSpace() {
    ...
    // namespace however, we set it to something so we can edit it.
    // otherwise the isRenderEditNameSpace will not trigger a possibility to
    // edit
  ...
  /**
   * @return true when creating a new namespace, instead of using on of the
   *     existing ones while adding or renaming a role
   */
  ...
  public String getEditNameSpaceSelected() {
  ...
  /**
   * @return the currently free-text namespace when adding or renaming a role
   *     (or null if no free text editing is currently ongoing)
   */
  public String getEditNameSpace() {
 ...
 */
  ...
  public void actionAddRoleConfirm() {
    if (StringUtils.isEmpty(editRoleName)) {
      super.addGlobalMessage(
          FacesMessage.SEVERITY_ERROR, "ROLES_ERROR_EMPTYNAME");
      return;
    }
    try {
      ...
      actionAddRoleReset();
      super.addGlobalMessage(
          FacesMessage.SEVERITY_INFO, "ROLES_INFO_ROLEADDED");
    } catch (RoleExistsException e) {
      super.addGlobalMessage(FacesMessage.SEVERITY_ERROR, "ROLEEXISTS");
    } catch (AuthorizationDeniedException e) {
      super.addGlobalMessage(
  ...
  public void actionDeleteRoleConfirm() {
    try {
      if (roleSession.deleteRoleIdempotent(
          getAdmin(), roleToDelete.getRoleId())) {
        super.addGlobalMessage(
            FacesMessage.SEVERITY_INFO, "ROLES_INFO_DELETED");
      }
    } catch (AuthorizationDeniedException e) {
      super.addGlobalMessage(
          FacesMessage.SEVERITY_ERROR,
