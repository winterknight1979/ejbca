 ...
 * a BigInteger.
 *
 * @version $Id: HexSerialNumberValidator.java 28844 2018-05-04 08:31:02Z
 *     samuellb $
 */
public class HexSerialNumberValidator implements Validator {
  ...
  public void validate(
    ...
    if ("matchValue".equals(textField.getId())) {
      ...
      final String matchWithType =
          (String) ((UIOutput) textField.findComponent("matchWith")).getValue();
      // Check if the matchWithType is
      // "CertificateAuthenticationToken:WITH_SERIALNUMBER"
      if ((X509CertificateAuthenticationTokenMetaData.TOKEN_TYPE
              + ":"
              + X500PrincipalAccessMatchValue.WITH_SERIALNUMBER)
          .equals(matchWithType)) {
        try {
