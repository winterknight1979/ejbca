 ...
 *
 * @version $Id: UpgradeBean.java 26920 2017-10-27 13:36:23Z mikekushner $ TODO:
 *     use CDI
 */
...
public class UpgradeBean extends BaseManagedBean implements Serializable {
  ...
  /**
   * @return true if the current administrator is authorized to start the
   *     upgrade
   */
  public boolean isAuthorizedToUpgrade() {
    return authorizationSession.isAuthorizedNoLogging(
        getAdmin(), "/system_functionality/edit_systemconfiguration");
  }
  ...
  public boolean isPostUpgradeInProgressInCluster() {
    // The first check requires no DB read (which might return cached/stale
    // data), so we check that first
    return upgradeStatusSingleton.isPostUpgradeInProgress()
        || upgradeSession.getPostUpgradeStarted() != 0L;
  }
  ...
  /**
   * @return true if the administrator is allowed to start the post upgrade
   *     procedure
   */
  public boolean isActionStartUpgradeAllowed() {
    return isPostUpgradeRequired()
        && !isPostUpgradeInProgress()
        && !isPostUpgradeInProgressInCluster();
  }

  /**
   * @return true if the administrator is allowed to force a restart of the post
   *     upgrade procedure (10 minutes has passed and it is not running on this
   *     node)
   */
  ...
  /**
   * @return true is there is specific post-upgrade instructions that should be
   *     rendered
   */
