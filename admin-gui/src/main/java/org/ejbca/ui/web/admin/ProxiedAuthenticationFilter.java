...
/**
 * Servlet filter implementation that can use a certificate belonging to a user
 * in EJBCA to emulate SSL client certificate authentication.
 *
 * @version $Id: ProxiedAuthenticationFilter.java 25421 2017-03-09 02:23:26Z
 *     jeklund $
 */
public class ProxiedAuthenticationFilter implements Filter {
  ...
  public void init(final FilterConfig filterConfig) throws ServletException {
    proxiedAuthenticationEnabled =
        WebConfiguration.isProxiedAuthenticationEnabled();
  }
    ...
    if (request.getAttribute(ATTR_X509CERTIFICATE) == null) {
      if (proxiedAuthenticationEnabled) {
        ...
        if (username != null) {
          ...
          final Collection<Certificate> userCerts =
              ejb.getCertificateStoreSession()
                  .findCertificatesByUsernameAndStatus(
                      username, CertificateConstants.CERT_ACTIVE);
          ...
          for (final Certificate cert : userCerts) {
            if (cert instanceof X509Certificate) {
              ...
              if (latestestIssuance == null
                  || latestestIssuance.after(thisIssuance)) {
          ...
          if (tempCerts[0] == null) {
            final String msg =
                "Authentication failed. No certificate found for admin with"
                    + " subject '"
                    + username
                    + "'.";
            LOG.info(msg);
            ...
            // Create a temporary Admin/AuthenticationToken to make sure it is
            // NOT authorized with Superadmin rights.
            final AuthenticationToken admin =
                new X509CertificateAuthenticationToken(tempCerts[0]);
            if (ejb.getAuthorizationSession()
                .isAuthorizedNoLogging(
                    admin, StandardRules.ROLE_ROOT.resource())) {
              final String msg =
                  "Authentication failed. Superadmin login is only allowed"
  ...
  private void showError(
      final HttpServletResponse httpServletResponse, final String content)
      throws IOException {
