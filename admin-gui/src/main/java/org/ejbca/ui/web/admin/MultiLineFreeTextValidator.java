...
/**
 * JSF validator that check that fields do no contain any ASCII control
 * characters. Newlines are allowed, though.
 *
 * @version $Id: MultiLineFreeTextValidator.java 29899 2018-09-25 08:11:00Z
 *     samuellb $
 */
public class MultiLineFreeTextValidator implements Validator {
  ...
  public void validate(
    ...
    if (textFieldValue != null
        && CharSetUtils.count(textFieldValue, CONTROL_CHARS) != 0) {
      final String msg =
          EjbcaJSFHelper.getBean().getEjbcaWebBean().getText("INVALIDCHARS")
              + "control characters (except for newlines) are not allowed";
