...
public class ListServicesManagedBean extends BaseManagedBean {
  ...
  public List<SortableSelectItem> getAvailableServices() {
    ...
    Collection<Integer> availableServicesIds =
        ejb.getServiceSession().getVisibleServiceIds();
    for (Integer id : availableServicesIds) {
      ServiceConfiguration serviceConfig =
          ejb.getServiceSession().getServiceConfiguration(id.intValue());
      String serviceName =
          ejb.getServiceSession().getServiceName(id.intValue());
      String hidden = "";
      ...
      if (serviceConfig.isActive()) {
        availableServices.add(
            new SortableSelectItem(
                ...
                serviceName
                    + " ("
                    + EjbcaJSFHelper.getBean().getText().get("ACTIVE")
                    + ")"
                    + hidden));
      } else {
        availableServices.add(
            new SortableSelectItem(
                ...
                serviceName
                    + " ("
                    + EjbcaJSFHelper.getBean().getText().get("INACTIVE")
                    + ")"
                    + hidden));
      }
    }
  ...
  public String editService() {
    ...
    if (StringUtils.isNotEmpty(selectedServiceName)) {
      ...
      ServiceConfiguration serviceConf =
          ejb.getServiceSession().getService(selectedServiceName);
    ...
    if (StringUtils.isEmpty(selectedServiceName)) {
      ...
      try {
        ejb.getServiceSession()
            .renameService(getAdmin(), selectedServiceName, newServiceName);
      } catch (ServiceExistsException e) {
        addNonTranslatedErrorMessage(
            EjbcaJSFHelper.getBean().getText().get("SERVICENAMEALREADYEXISTS"));
      }
    }
    newServiceName = "";
    return "listservices";
  ...
  public String addService() {
    if (StringUtils.isEmpty(StringUtils.trim(newServiceName))) {
      ...
      try {
        ...
        ejb.getServiceSession()
            .addService(getAdmin(), newServiceName, serviceConfig);
  ...
  public String cloneService() {
    if (StringUtils.isEmpty(selectedServiceName)) {
      ...
      try {
        ejb.getServiceSession()
            .cloneService(getAdmin(), selectedServiceName, newServiceName);
      } catch (ServiceExistsException e) {
