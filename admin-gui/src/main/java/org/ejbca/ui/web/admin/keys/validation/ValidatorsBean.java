 ...
 *
 * @version $Id: ValidatorsBean.java 28844 2018-05-04 08:31:02Z samuellb $
 */
public class ValidatorsBean extends BaseManagedBean {
  ...
  /**
   * Gets the available key validators taking into account access rules. The
   * admin need access to view vlidators, and to all certificate profiles
  ...
  public ListDataModel<ValidatorItem> getAvailableValidators() {
    if (validatorItems == null) {
      ...
      final Collection<Integer> validatorIds =
      ...
      for (Integer id : validatorIds) {
        ...
        final String accessRule =
            StandardRules.VALIDATORACCESS.resource() + validator.getProfileId();
      ...
      Collections.sort(
          items,
          new Comparator<ValidatorItem>() {
            @Override
            public int compare(final ValidatorItem o1, final ValidatorItem o2) {
  ...
  public void actionAdd() {
    ...
    if (StringUtils.isNotBlank(name)) {
      try {
        keyValidatorSession.addKeyValidator(
            getAdmin(), new RsaKeyValidator(name));
        actionCancel();
  ...
  public void actionAddFromTemplateConfirm() {
    ...
    if (name.length() > 0) {
      try {
        keyValidatorSession.cloneKeyValidator(
            getAdmin(), getValidatorId(), name);
  ...
  public void actionRenameConfirm() {
    ...
    if (name.length() > 0) {
      try {
        keyValidatorSession.renameKeyValidator(
            getAdmin(), getValidatorId(), name);
        setValidatorName(StringUtils.EMPTY);
      } catch (KeyValidatorDoesntExistsException e) {
  ...
  public void validateExternalCommand(
    ...
    final ExternalScriptsWhitelist externalScriptsWhitelist =
        ExternalScriptsWhitelist.fromText(
            globalConfiguration.getExternalScriptsWhitelist(),
