...
public class RoleMembersBean extends BaseManagedBean implements Serializable {
   ...
   * @return Equal Case sensitive unless access match value forces something
   *     else.
   */
  private AccessMatchType getAccessMatchType(
    ...
    if (accessMatchValue.equals(X500PrincipalAccessMatchValue.WITH_SERIALNUMBER)
        ...
        || accessMatchValue.equals(
            X500PrincipalAccessMatchValue.WITH_DNEMAILADDRESS)
        || accessMatchValue.equals(
            X500PrincipalAccessMatchValue.WITH_STATEORPROVINCE)
        || accessMatchValue.equals(
            X500PrincipalAccessMatchValue.WITH_RFC822NAME)) {
  ...
  public boolean isAuthorizedToEditRole() {
    try {
      if (authorizationSession.isAuthorizedNoLogging(
          ...
          && getRole() != null) {
        roleSession.assertAuthorizedToRoleMembers(
            getAdmin(), getRole().getRoleId(), true);
        return true;
      }
    ...
    if (roleMembers == null) {
      ...
      Collections.sort(
          ...
          new Comparator<RoleMember>() {
            @Override
            public int compare(
                final RoleMember roleMember1, final RoleMember roleMember2) {
              final String matchWithItemString1 =
                  getMatchWithItemString(roleMember1);
              final String matchWithItemString2 =
                  getMatchWithItemString(roleMember2);
              final int compareA =
                  matchWithItemString1.compareTo(matchWithItemString2);
              if (compareA != 0) {
                return compareA;
              }
              final String tokenIssuerIdString1 =
                  getTokenIssuerIdString(roleMember1);
              final String tokenIssuerIdString2 =
                  getTokenIssuerIdString(roleMember2);
              final int compareB =
                  tokenIssuerIdString1.compareTo(tokenIssuerIdString2);
              if (compareB != 0) {
                return compareB;
              }
              return roleMember1
        ...
        if (authenticationTokenMetaData.isUserConfigurable()) {
          for (final AccessMatchValue accessMatchValue :
            ...
            if (!X500PrincipalAccessMatchValue.NONE.equals(accessMatchValue)) {
              matchWithItems.add(
                  new SelectItem(
                      tokenType + ":" + accessMatchValue.getNumericValue(),
                      getEjbcaWebBean().getText(tokenType)
                          + ": "
                          + getEjbcaWebBean()
                              .getText(accessMatchValue.name())));
            }
          }
        }
      }
  ...
  public String getMatchWithSelected() {
  ...
  public String getMatchWithItemString(final RoleMember roleMember) {
    ...
    final String tokenTypeString =
        getEjbcaWebBean().getText(authenticationTokenMetaData.getTokenType());
    final String tokenMatchKeyString =
        getEjbcaWebBean()
            .getText(
  ...
  /**
   * @return true if the currently selected tokenType and tokenMatchKey combo
   *     implies that is has been issued by a CA
   */
  public boolean isRenderTokenIssuerIdInput() {
    return getAccessMatchValue(
            getSelectedTokenType(), getSelectedTokenMatchKey())
        .isIssuedByCa();
  }
  ...
  public List<SelectItem> getAvailableCas() {
    ...
    final List<Integer> authorizedCaIds =
        caSession.getAuthorizedCaIds(getAdmin());
  ...
  public Integer getTokenIssuerId() {
  ...
  /**
   * @return the ViewScoped cache of the CA to name map or a fresh copy from the
   *     backend if none is present yet.
   */
  ...
  public String getTokenMatchOperatorString(final RoleMember roleMember) {
    return getEjbcaWebBean()
        .getText(
            AccessMatchType.matchFromDatabase(
                    roleMember.getTokenMatchOperator())
                .name());
  }

  /**
   * @return true if the currently selected tokenType and tokenMatchKey combo
   *     implies that it needs a tokenMatchValue
   */
   ...
   * @param roleMember Member
   * @return true if the currently selected tokenType and tokenMatchKey combo
   *     implies that the tokenMatchValue is a hex certificate serial number
   */
  public boolean isRenderCertificateLink(final RoleMember roleMember) {
    return X509CertificateAuthenticationTokenMetaData.TOKEN_TYPE.equals(
            roleMember.getTokenType())
        && X500PrincipalAccessMatchValue.WITH_SERIALNUMBER.getNumericValue()
  ...
  /**
   * @return the tokenType from the currently selected tokenType and
   *     tokenMatchKey combo
   */
  ...
  /**
   * @return the tokenMatchKey from the currently selected tokenType and
   *     tokenMatchKey combo
   */
   ...
   * @return the AccessMatchValue from the specified tokenType and tokenMatchKey
   *     combo
   */
  private AccessMatchValue getAccessMatchValue(
      final String tokenType, final int tokenMatchKey) {
  ...
  public void actionAddRoleMember() {
    try {
      ...
      final AccessMatchValue accessMatchValue =
          getAccessMatchValue(tokenType, tokenMatchKey);
      ...
      if (AccessMatchType.TYPE_UNUSED.equals(accessMatchType)) {
        ...
        if (StringUtils.isEmpty(tokenMatchValue)) {
          super.addGlobalMessage(
              FacesMessage.SEVERITY_ERROR, "MATCHVALUEREQUIRED");
          return;
        }
      }
      ...
      if (!invalidCharacters.isEmpty()) {
        ...
        super.addGlobalMessage(
            FacesMessage.SEVERITY_ERROR, "INVALIDCHARS", sb.substring(2));
        return;
      }
      ...
      try {
        ...
        super.addGlobalMessage(
            FacesMessage.SEVERITY_ERROR, "AUTHORIZATIONDENIED");
      }
      // Only reset this one, since admin might want to add additional matches
      // of the same type
      tokenMatchValue = "";
      // Trigger a reload of the RoleMember list (if this is an AJAX requests
      // and no new viewscope will be created)
      roleMembers = null;
    } finally {
  ...
  /**
   * @param roleMember Member
   * @return true if the RoleMember's tokenType and tokenMatchKey combo implies
   *     that it is issued by a CA
   */
  ...
  public void actionDeleteRoleMemberConfirm() {
    try {
      ...
      super.addGlobalMessage(
          FacesMessage.SEVERITY_INFO, "ROLEMEMBERS_INFO_REMOVED");
      roleMembers = null;
      roleMemberToDelete = null;
      nonAjaxPostRedirectGet();
    } catch (AuthorizationDeniedException e) {
