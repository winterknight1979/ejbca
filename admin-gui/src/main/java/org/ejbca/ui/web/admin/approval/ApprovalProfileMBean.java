...
@ViewScoped // Local variables will live as long as actions on the backed page
            // return "" or void.
@ManagedBean(name = "approvalProfileMBean")
public class ApprovalProfileMBean extends BaseManagedBean
  ...
  /**
   * This enum field represents the types of data fields that can be added to an
  ...
  private enum FieldType {
    ...
    public static FieldType getFromName(final String name) {
   ...
   *     this view (this will never change in the view)
   */
  ...
  /**
   * Saves the current approval profile to a temporary variable managed by this
   * bean. Should be used to save the current state before the UI is refreshed.
   * This prevents data entry in UI controls to be discarded, causing poor user
   * experience. To permanently save the approval profile, see {@link #save}. In
   * comparison, this method does not require any authorisation, and will not
   * throw any exceptions, nor show any error messages to the user.
   */
  @SuppressWarnings("unchecked")
    ...
    for (final ApprovalStepGuiObject step : steps) {
      for (final ApprovalPartitionProfileGuiObject partition :
        ...
        final Object guiObject =
            partition.getProfilePropertyList().getWrappedData();
        approvalProfile.addPropertiesToPartition(
            stepId,
            partitionId,
            (List<DynamicUiProperty<? extends Serializable>>) guiObject);
      }
  ...
  /**
   * Adds the currently selected field to the partition specified by the
   * parameter. The step that the partition resides in is retrieved from the
   * steps class member.
   *
   * @param partitionId the ID of the partition to add the field to
   * @return an empty string to keep the scope.
   */
  public String addField(final int partitionId) {
    ...
    if (updatedApprovalProfile
      ...
      updatedApprovalProfile.addPropertyToPartition(
          currentStep, partitionId, property);
  ...
  /**
   * A special method for adding rows to radio button arrays. The ID of the
   * partition is required, but the step identity and the radio button field
   * identity can be derived from class members.
   *
   * @param partitionId the ID of the partition that the radio button resides in
   * @param label the label for the new row
   * @return an empty string to keep the scope.
   */
  public String addRowToRadioButton(final int partitionId, final String label) {
    final Integer currentStep = steps.getRowData().getIdentifier();
    ...
    for (ApprovalPartitionProfileGuiObject approvalPartitionProfileGuiObject :
      ...
      if (approvalPartitionProfileGuiObject.getPartitionId() == partitionId) {
        @SuppressWarnings("unchecked")
        DynamicUiProperty<RadioButton> radioButtonProperty =
            (DynamicUiProperty<RadioButton>)
                approvalPartitionProfileGuiObject
                    .getProfilePropertyList()
                    .getRowData();
        Collection<RadioButton> possibleValues =
            radioButtonProperty.getPossibleValues();
        ...
        saveTemporary();
        updatedApprovalProfile.addPropertyToPartition(
            currentStep, partitionId, radioButtonProperty);
        steps = createStepListFromProfile(updatedApprovalProfile);
        break;
      }
    }
  ...
  /**
   * Similar to the above, the below method removes a row from a radio button
   * array.
   *
   * @param partitionId the ID of the partition that the radio button resides in
   ...
   * @return an empty string to keep the scope.
   */
  public String removeRowFromRadioButton(
      final int partitionId, final String encodedRadioButton) {
    final Integer currentStep = steps.getRowData().getIdentifier();
    final ApprovalProfile updatedApprovalProfile = getApprovalProfile();
        ...
        DynamicUiProperty<RadioButton> radioButtonProperty =
            (DynamicUiProperty<RadioButton>)
                approvalPartitionProfileGuiObject
                    .getProfilePropertyList()
                    .getRowData();
        List<RadioButton> oldValues =
            new ArrayList<>(radioButtonProperty.getPossibleValues());
  ...
  public String removeField(final int partitionId, final String propertyName) {
    ...
    saveTemporary();
    ApprovalProfile updatedApprovalProfile = getApprovalProfile();
    updatedApprovalProfile.removePropertyFromPartition(
        currentStep, partitionId, propertyName);
    steps = createStepListFromProfile(updatedApprovalProfile);
    return "";
  }
  ...
  public boolean isPropertyPredefined(
    ...
    return approvalProfile.isPropertyPredefined(
  ...
  public void setCurrentApprovalProfileTypeName(final String typeName) {
    ...
    if (!getApprovalProfile()
        ...
        .equals(typeName)) {
      final ApprovalProfile newApprovalProfile =
          ApprovalProfilesFactory.INSTANCE.getArcheType(typeName);
  ...
  /**
   * Take an approval step and extract its partitions and respective properties,
   * filling in with values from the database where required.
   *
   * @param step an approval step
   * @return a Map linking partitions IDs to lists of each partitions
   *     properties.
   */
  private Map<Integer, List<DynamicUiProperty<? extends Serializable>>>
      getPartitionProperties(final ApprovalStep step) {
    Map<Integer, List<DynamicUiProperty<? extends Serializable>>>
        partitionProperties = new LinkedHashMap<>();
      ...
      for (DynamicUiProperty<? extends Serializable> property :
        ...
        switch (propertyClone.getPropertyCallback()) {
          case ROLES:
            ...
            break;
          case ROLES_VIEW:
            final List<Role> allAuthorizedRolesForViewing =
                roleSession.getAuthorizedRoles(getAdmin());
            final List<RoleInformation> viewingRoleRepresentations =
                new ArrayList<>();
              ...
              if (AccessRulesHelper.hasAccessToResource(
                ...
                try {
                  final List<RoleMember> roleMembers =
                      roleMemberSession.getRoleMembersByRoleId(
                          getAdmin(), role.getRoleId());
                  viewingRoleRepresentations.add(
                      RoleInformation.fromRoleMembers(
                          role.getRoleId(),
                          role.getNameSpace(),
                          role.getRoleName(),
            ...
            if (!viewingRoleRepresentations.contains(
                propertyClone.getDefaultValue())) {
              viewingRoleRepresentations.add(
                  0, (RoleInformation) propertyClone.getDefaultValue());
            }
  ...
  /**
   * @return true of the approval profile is of a type where sequences can be
   *     added
   */
  public boolean isStepSizeFixed() {
    return ApprovalProfilesFactory.INSTANCE
        .getArcheType(getCurrentApprovalProfileTypeName())
        .isStepSizeFixed();
  }
  ...
  public boolean isNotificationEnabled(final int partitionIdentifier) {
    ...
    final ApprovalStep approvalStep =
        approvalProfile.getStep(steps.getRowData().getIdentifier());
    final ApprovalPartition approvalPartition =
        approvalStep.getPartition(partitionIdentifier);
  ...
  public void addNotification(final int partitionIdentifier) {
    ...
    final String hostnameFromRequest =
    ...
    final String defaultBody =
        "Approval Request to ${approvalRequest.TYPE} from"
            ...
            + baseUrl
            + "ra/managerequest.xhtml?id=${approvalRequest.ID}";
    approvalProfile.addNotificationProperties(
        approvalPartition,
        "approval-admin-group@example.org supervisor@example.org",
        "no-reply@" + hostnameFromRequest,
        defaultSubject,
        defaultBody);
    steps = null;
  }
  ...
  public void addUserNotification(final int partitionIdentifier) {
    ...
    final String defaultBody =
        "Approval Request to ${approvalRequest.TYPE} from"
            ...
            + baseUrl
            + "ra/enrollwithrequestid.xhtml?requestId=${approvalRequest.ID}";
    approvalProfile.addUserNotificationProperties(
        approvalPartition,
        "no-reply@" + hostnameFromRequest,
        defaultSubject,
        defaultBody);
