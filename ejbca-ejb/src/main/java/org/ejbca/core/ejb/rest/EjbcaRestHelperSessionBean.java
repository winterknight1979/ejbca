...
/**
 * @version $Id: EjbcaRestHelperSessionBean.java 29240 2018-06-15 14:51:31Z
 *     henriks $
 */
@Stateless(
    mappedName = JndiConstants.APP_JNDI_PREFIX + "EjbcaRestHelperSessionRemote")
 ...
 authorization for individual admins. */
  ...
  public AuthenticationToken getAdmin(
    ...
    if (!raMasterApiProxyBean.isAuthorizedNoLogging(
        raRestAuthCheckToken, AccessRulesConstants.REGULAR_PEERPROTOCOL_REST)) {
      throw new AuthorizationDeniedException(
          "REST resources is not authorized for this Peer connection");
    }
  ...
  public EndEntityInformation convertToEndEntityInformation(
    ...
    CAInfo caInfo =
        getCAInfo(
            enrollcertificateRequest.getCertificateAuthorityName(),
            authenticationToken);
    if (caInfo == null) {
      String errorMessage =
          "CA with name \""
    ...
    int certificateProfileId =
        getCertificateProfileId(
            enrollcertificateRequest.getCertificateProfileName());
    if (certificateProfileId == 0) {
      String errorMessage =
          "Certificate profile with name \""
              + enrollcertificateRequest.getCertificateProfileName()
              + "\" doesn't exist";
    ...
    Integer endEntityProfileId =
        getEndEntityProfileId(
            enrollcertificateRequest.getEndEntityProfileName());
    ...
    boolean isSendNotificationDefaultInProfile =
        EndEntityProfile.TRUE.equals(
            endEntityProfile.getValue(EndEntityProfile.SENDNOTIFICATION, 0));
    endEntityInformation.setSendNotification(
        isSendNotificationDefaultInProfile
  ...
  protected String getSubjectAltName(
    ...
    String altName = null;
    final Extension subjectAlternativeNameExtension =
        CertTools.getExtension(
            pkcs10CertificateRequest, Extension.subjectAlternativeName.getId());
    if (subjectAlternativeNameExtension != null) {
      altName =
          CertTools.getAltNameStringFromExtension(
  ...
  private CAInfo getCAInfo(
      ...
      throws AuthorizationDeniedException {
    CAInfo caInfo =
        caSessionBean.getCAInfo(authenticationToken, certificateAuthorityName);
    return caInfo;
  }
  ...
  public Integer getEndEntityProfileId(final String endEntityProfileName)
    ...
    int endEntityProfileId =
        endEntityProfileSessionBean.getEndEntityProfileId(endEntityProfileName);
  ...
  private int getCertificateProfileId(final String certificateProfileName) {
    int certificateProfileId =
        certificateProfileSessionBean.getCertificateProfileId(
