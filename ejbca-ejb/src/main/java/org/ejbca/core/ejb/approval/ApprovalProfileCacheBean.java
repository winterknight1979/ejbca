 ...
 * sequential updates.
 *
 * @version $Id: ApprovalProfileCacheBean.java 26076 2017-06-26 12:49:12Z
 *     mikekushner $
 */
@Singleton
@Startup
...
public class ApprovalProfileCacheBean {
  ...
  public void initialize() {
    ...
    try {
      ...
      // We don't want to murder the entire deployment if the database happens
      // to be unresponsive during startup, it's something we might
      // recover from
      LOG.error(e);
    }
  }
   ...
   * cache to be reloaded on next read. Required due to the fact that the cache
   * can't reload until whatever transaction performing CRUD ops finishes.
   */
  ...
  public void updateProfileCache(final boolean force) {
    ...
    final long cacheApprovalProfileTime =
        EjbcaConfiguration.getCacheApprovalProfileTime();
    final long now = System.currentTimeMillis();
    // Check before acquiring lock
    if (!force
        && cacheApprovalProfileTime != 0
        && lastUpdate + cacheApprovalProfileTime > now) {
    ...
    final Map<Integer, String> idNameCache =
        new HashMap<>(idNameMapCacheTemplate);
