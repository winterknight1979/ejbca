 ...
 *
 * @version $Id: StartupSingletonBean.java 29695 2018-08-21 09:02:17Z henriks $
 */
@Singleton
@Startup
@ConcurrencyManagement(
    ConcurrencyManagementType.BEAN) // No external access to this bean
...
public class StartupSingletonBean {
  ...
  private void shutdown() {
    ...
    // logSession.log(EjbcaEventTypes.EJBCA_STOPPING, EventStatus.SUCCESS,
    // EjbcaModuleTypes.SERVICE, EjbcaServiceTypes.EJBCA,
    // authenticationToken.toString(), null, null, null, details);
  }

  /**
   * Checks that a class is loaded by a classloader that is local to the
   * ejbca.ear deployment. This check is probably JBoss / WildFly specific but
   * not certain. We want to check this so we can detect if JBoss / WildFly
   * bundles jars that we use so we risk having a version conflict.
   *
   ...
   */
  private static void checkClassLoaderIsEJBCA(
      final Class<? extends X509Certificate> clazz) {
    ...
    results.append(
        "\n"
            ...
            + ").ClassLoader="
            + cl);
    // We want classes (that we check here) to be loaded from the EJBCA
    // application classloader, and not by the
  ...
  /**
   * A test certificate we use to test load a certificate in order to figure out
   * which classloader the BouncyCaslte Provider is using, i.e. who did
   * Security.addProvider(new BouncyCastleProvider()), if it was us or
   * JBoss/WildFly
   *
  ...
  private void startup() {
    ...
    // We really need BC to be installed. This is an attempt to fix a bug where
    ...
    details.put("msg", iMsg);
    logSession.log(
        EjbcaEventTypes.EJBCA_STARTING,
        EventStatus.SUCCESS,
    ...
    if (loggerIds.isEmpty()) {
      ...
      if (!checkForProtectedAudit(authenticationToken, loggerIds)) {
        ...
        final String msg =
            InternalEjbcaResources.getInstance()
                .getLocalizedMessage("startservices.noprotectedauditdevices");
        final Map<String, Object> logdetails = new LinkedHashMap<>();
        logdetails.put("msg", msg);
        logSession.log(
            EventTypes.LOG_MANAGEMENT_CHANGE,
            EventStatus.VOID,
    ...
    // database
    // operation and avoid a performance hit for the first request where this is
    // checked.
    final Boolean unique =
        DatabaseIndexUtil.isIndexPresentOverColumns(
            JDBCUtil.getDataSourceOrNull(),
            "CertificateData",
    ...
    if (unique == null) {
      LOG.info(
          "Unable to read the index meta data from the database. Will detect"
              ...
              + " CertificateData.");
      // Fall-back to testing for a unique index on CertificateData using
      // conflicting INSERTs
    ...
    // Start legacy background service for renewal of OCSP signers via EJBCA WS
    // calls to CA
    ocspKeyRenewalSession.startTimer();
    // Verify that the EJB CLI user (if present) cannot be used to generate
    // certificates
    final String cliUsername = EjbcaConfiguration.getCliDefaultUser();
    try {
      final EndEntityInformation defaultCliUser =
          endEntityAccessSession.findUser(authenticationToken, cliUsername);
      if (defaultCliUser != null
          && defaultCliUser.getStatus() == EndEntityConstants.STATUS_NEW) {
        try {
   ...
   *     startup from properties file)
   * @param loggerIds the configured loggers among which we look for the
   *     protected device
   * @return true if there is an integrity protected audit device and is was
   *     configured during startup (and audit log of this config was made)
   */
  private boolean checkForProtectedAudit(
      final AuthenticationToken admin, final Set<String> loggerIds) {
    boolean ret = false;
    // See if we have IntegrityProtectedDevice configured, due to class loading
    // constraints we can not use
    ...
    for (Iterator<String> iterator = loggerIds.iterator();
      ...
      if (integrityProtectedName.equals(id)) {
        ...
        final Map<String, Object> logdetails = new LinkedHashMap<>();
        try {
          // Use reflection to get the ProtectedDataConfiguration and make some
          // calls to it.
          // This is needed since ProtectedDataConfiguration may not be
          // available during compile time, or runtime
          Class<?> c =
              Class.forName(
                  "org.cesecore.dbprotection.ProtectedDataConfiguration");
          ...
          Method getProtectVersion =
              c.getMethod("getProtectVersion", Integer.TYPE);
          ...
          if ((keyid != null) && (keyid > 0)) {
            if (CesecoreConfigurationHelper.useDatabaseIntegrityProtection(
              ...
              final Integer protectVersion =
                  (Integer) getProtectVersion.invoke(config, keyid);
              // Call ProtectedDataConfiguration.instance().getKeyLabel
              final String keyLabel =
                  (String) getKeyLabel.invoke(config, keyid);
