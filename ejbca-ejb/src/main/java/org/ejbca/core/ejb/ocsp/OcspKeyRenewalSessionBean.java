...
/**
 * @version $Id: OcspKeyRenewalSessionBean.java 26145 2017-07-10 10:45:12Z
 *     mikekushner $
 */
@Stateless(
    mappedName = JndiConstants.APP_JNDI_PREFIX + "OcspKeyRenewalSessionRemote")
...
public class OcspKeyRenewalSessionBean
  ...
  /**
   * @param signerSubjectDN signerSubjectDN subject DN of the signing key to be
   *     renewed. The string "all" will result in all keys being renewed
   * @param safetyMargin the number of seconds before actual expiration that a
   *     keystore should be renewed
   * @throws CryptoTokenOfflineException if Crypto Token is not available or
   *     connected, or key with alias does not exist.
   * @throws InvalidKeyException if the public key in the tokenAndChain can not
   *     be used to verify a string signed by the private key, because the key
   *     is wrong or the signature operation fails for other reasons such as a
   *     NoSuchAlgorithmException or SignatureException.
   */
  private synchronized void renewKeyStores(
      final String signerSubjectDN, final long safetyMargin)
    ...
    try {
      ...
      try {
        target =
            signerSubjectDN.trim().equalsIgnoreCase(RENEW_ALL_KEYS)
                ? null
      ...
      for (final OcspSigningCacheEntry ocspSigningCacheEntry :
        ...
        // (the cache can map each OcspKeyBinding multiple times by SHA-1,
        // SHA-256 and as default responder)
        final int ocspKeyBindingId =
        ...
        if (timeLeftBeforeRenewal < (1000 * safetyMargin)) {
          ...
          matched.append(
              " '"
                  + ocspSigningCertificate.getIssuerX500Principal().getName()
                  + '\'');
          try {
  ...
  @Override
  public synchronized void renewKeyStores(final String signerSubjectDN)
  ...
  /**
   * Generate a new key pair and request a new certificate for this key pair
   * using EJBCA WS.
   *
   * @param ejbcaWS a reference to the remote EJBCA WS
   * @param ocspSigningCacheEntry the cached OCSP signing entry backed by an
   *     OcspKeyBinding
   * @throws InvalidKeyException if the new public key can not be used to verify
   *     a string signed by the private key, because the key is wrong or the
  ...
  /**
   * Get user data for the EJBCA user that will be used when creating the cert
   * for the new key.
   *
   * @param ejbcaWS WS
   * @param signingCertificate The OCSP signing certificate to get the end
   *     entity for
   ...
   * @return the name
   */
  private String getCAName(final EjbcaWS ejbcaWS, final int caId) {
  ...
  /**
   * This method sends a keypair off to be signed by the CA that issued the
   * original keychain.
   *
   * @param ejbcaWS WS
  ...
  private X509Certificate signCertificateByCa(
    ...
    final UserDataVOWS userData =
        getUserDataVOWS(ejbcaWS, ocspSigningCertificate, caId);
    if (userData == null) {
      final String msg =
    ...
    try {
      certificateResponse =
          ejbcaWS.pkcs10Request(
              userData.getUsername(),
              userData.getPassword(),
              new String(Base64.encode(pkcs10CertificationRequest)),
              null,
    ...
    if (certificateResponse == null) {
      throw new KeyRenewalFailedException(
          "Certificate Response was not received");
    }
    ...
    for (X509Certificate certificate : certificates) {
      ...
      // same provider is used for the public keys
    ...
    if (signedCertificate == null) {
      throw new KeyRenewalFailedException(
          "No certificate signed by correct CA generated.");
    }
  ...
  private SSLSocketFactory getSSLSocketFactory() {
      ...
      try {
        ...
        if (internalKeyBinding
          ...
          authenticationKeyBinding =
              (AuthenticationKeyBinding) internalKeyBinding;
          break;
        }
      } catch (AuthorizationDeniedException e) {
    ...
    try {
      ...
      // Now construct a SSLContext using these (possibly wrapped) KeyManagers,
      // and the TrustManagers.
      // We still use a null SecureRandom, indicating that the defaults should
      // be used.
  ...
  private List<X509Certificate> getListOfTrustedCertificates(
    ...
    if (trustedCertificateReferences == null
        || trustedCertificateReferences.isEmpty()) {
      return null;
    }
  ...
  // TODO: This method also exists in OcspResponseGenSSB.. merge! to method call
  // in certificateStoreSession
  ...
  /**
   * When the timer expires, this method will check through the cache and
   * automatically renew keystore matching the predefined criteria, and which
   * expire within the designated time frame.
   *
   * <p>According to JSR 220 FR (18.2.2), this method may not throw any
   * exceptions.
   *
   * <p>Glassfish 2.1.1: "Timeout method ....timeoutHandler(javax.ejb.Timer)must
   * have TX attribute of TX_REQUIRES_NEW or TX_REQUIRED or TX_NOT_SUPPORTED"
  ...
  public void timeoutHandler(final Timer timer) {
    long rekeyingUpdateTime =
        OcspConfiguration.getRekeyingUpdateTimeInSeconds();
    try {
      renewKeyStores(
