...
/**
 * EJB Interceptor that is normally enabled for all EJBs when EJBCA is running
 * in non-production mode.
 *
 * <p>The Interceptor will perform two tasks when active: - Trace log all EJB
 * invocations including input and output if Log4J trace-logging is enabled for
 * the invoked methods EJB. - Collect profiling statistics for all EJB
 * invocations if Log4J debug is enabled for this class.
 *
 * @version $Id: ProfileAndTraceInterceptor.java 29553 2018-08-03 07:46:04Z
 *     samuellb $
 */
    ...
    if (targetLogger.isTraceEnabled()) {
      ...
      for (Object obj : invocationContext.getParameters()) {
        ...
        if (obj instanceof Object[]) {
          // We want to print all arguments, which are stored as an array in the
          // array,
        ...
        if (returnException == null) {
          ...
          targetLogger.trace(
              "<"
                  ...
                  + invocationDuration
                  + "micros, threw "
                  + returnException.getClass().getName()
                  + ": "
