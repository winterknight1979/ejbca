...
/**
 * Global cache of RaStyleInfos fetched via Peers. Each entry is mapped to an
 * administrator, containing its available custom RA Styles (CSS files and
 * logo). This cache prevents multiple requests over the Peers protocol when an
 * administrator loads the RA page and style resources are requested by the
 * servlet filter (RaStyleRequestFilter). Since style archives may be very heavy
 * to transfer, a check for changes is sent via Peers before reloading the
 * styles.
 *
 * @version $Id: RaStyleCacheBean.java 27422 2017-12-05 14:05:42Z bastianf $
 */
@Singleton
@Startup
...
public class RaStyleCacheBean {
   ...
   */
  public boolean needsUpdate(final AuthenticationToken authenticationToken) {
    List<RaStyleInfo> cachedStyle = raStyleCache.get(authenticationToken);
    if (cachedStyle != null) {
      return raMasterApiProxyBean.getAvailableCustomRaStyles(
              authenticationToken, cachedStyle.hashCode())
          != null;
    }
    return true;
  }
  ...
  /**
   * Returns custom RA Styles associated with the role of the requesting
   * administrator. If the administrator has requested styles very recently, the
   * same content will be returned to prevent requests via Peers for for every
   * resource request e.g. in a page load in the RA web.
   *
   * @param authenticationToken of the requesting administrator
   * @return List of custom RA styles available for the requesting administrator
   */
  public List<RaStyleInfo> getAvailableRaStyles(
    ...
    if (raStyleCache.containsKey(authenticationToken)) {
      ...
      // Check for changes
      availableRaStyles =
          raMasterApiProxyBean.getAvailableCustomRaStyles(
              authenticationToken, cachedStyles.hashCode());
   ...
   */
  public long getLastUpdate(final AuthenticationToken authenticationToken) {
