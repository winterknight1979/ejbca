 ...
 */
@Stateless(
    mappedName = JndiConstants.APP_JNDI_PREFIX + "RevocationSessionRemote")
...
public class RevocationSessionBean
  ...
  @Override
  public void revokeCertificateInNewTransaction(
      final AuthenticationToken admin,
      final CertificateDataWrapper cdw,
      final Collection<Integer> publishers,
      ...
      throws CertificateRevokeException, AuthorizationDeniedException {
    revokeCertificate(
        admin, cdw, publishers, revocationDate, reason, userDataDN);
  }
  ...
  public void revokeCertificate(
    ...
    final boolean waschanged =
        noConflictCertificateStoreSession.setRevokeStatus(
            admin, cdw, getRevocationDate(cdw, revocationDate, reason), reason);
    // Only publish the revocation if it was actually performed
    if (waschanged) {
      // Since storeSession.findCertificateInfo uses a native query, it does not
      // pick up changes made above
      // that is part if the transaction in the EntityManager, so we need to get
      // the object from the EntityManager.
      ...
      if (!RevokedCertInfo.isRevoked(reason)) {
        ...
        if (published) {
          ...
          // If it is not possible, only log error but continue the operation of
          // not revoking the certificate
          final String msg =
              "Unrevoked cert:"
                  + serialNumber
                  + " reason: "
          ...
          auditSession.log(
              EjbcaEventTypes.REVOKE_UNREVOKEPUBLISH,
              EventStatus.FAILURE,
              ModuleTypes.CA,
              ServiceTypes.CORE,
              admin.toString(),
   ...
   * @param reason Reason
   * @return revocationDate as is, or null if unrevoking a certificate that's
   *     not on a base CRL in on hold state.
   */
  private Date getRevocationDate(
      final CertificateDataWrapper cdw,
      final Date revocationDate,
      final int reason) {
    if (revocationDate == null
        ...
        || (cdw.getBaseCertificateData().getRevocationReason()
    ...
    if (baseCrlInfo == null
        || baseCrlInfo
            .getCreateDate()
            .before(revocationDate)) { // if not on base CRL
      return null;
    } else {
