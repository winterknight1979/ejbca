...
/**
 * Handles execution of approved tasks. Separated from ApprovealSessionBean to
 * avoid circular dependencies, since execution will require SSBs that
 * originally created the approval request.
 *
 * @version $Id: ApprovalExecutionSessionBean.java 29010 2018-05-23 13:09:53Z
 *     jekaterina_b_helmes $
 */
@SuppressWarnings("deprecation")
@Stateless(
    mappedName =
        JndiConstants.APP_JNDI_PREFIX + "ApprovalExecutionSessionRemote")
...
public class ApprovalExecutionSessionBean
  ...
  public void approve(
      ...
      throws ApprovalRequestExpiredException, ApprovalRequestExecutionException,
    ...
    try {
      // Retrieve the latest non-stale version of the approval profile from the
      // approval request (as the copy of the profile stored in the request may
      // contain metadata which was added during the approval process.
      final Integer approvalProfileId =
          approvalData.getApprovalRequest().getApprovalProfile().getProfileId();
      ...
      if (approvalData.getStatus()
          != ApprovalDataVO.STATUS_WAITINGFORAPPROVAL) {
        throw new ApprovalException(
            "Wrong status of approval request, expected"
                + " STATUS_WAITINGFORAPPROVAL(-1): "
                + approvalData.getStatus());
      }
      // Check if the approval is applicable, i.e belongs to and satisfies a
      // certain partition, as well as that all previous steps have been
      // satisfied
      if (!approvalProfile.isApprovalAuthorized(approvalsPerformed, approval)) {
        throw new AuthorizationDeniedException(
            "Administrator "
                + approval.getAdmin().toString()
                + " was not authorized to partition "
      ...
      if (approvalData.hasRequestOrApprovalExpired()) {
        approvalSession.sendApprovalNotifications(
            approvalData.getApprovalRequest(),
            approvalProfile,
            approvalData,
            false);
        throw new ApprovalRequestExpiredException();
      }
      final boolean readyToCheckExecution =
      ...
      if (readyToCheckExecution) {
        // Kept for legacy reasons to allow for 100% uptime, can be removed once
        // upgrading from 6.6.0 is no longer supported.
        ...
        if (approvalRequest.isExecutable()) {
          try {
            if (approvalRequest instanceof ActivateCATokenApprovalRequest) {
              ((ActivateCATokenApprovalRequest) approvalRequest)
                  .execute(caAdminSession);
              ...
              ((ChangeStatusEndEntityApprovalRequest) approvalRequest)
                ...
                instanceof EditEndEntityApprovalRequest) {
          ...
          approvalData.setExpiredate(
              (new Date()).getTime() + approvalRequest.getApprovalValidity());
        }
      }
  ...
  public void reject(
    ...
    try {
      ...
      approvalSession.setApprovals(approvalData, approvalsPerformed);
      // Retrieve the approval profile just to make sure that the state is still
      // valid
      ...
      if (approvalData.getApprovalRequest().isExecutable()) {
        ...
        approvalData.setExpiredate(
            (new Date()).getTime()
                + approvalData.getApprovalRequest().getApprovalValidity());
      }
      approvalSession.sendApprovalNotifications(
          approvalData.getApprovalRequest(),
          approvalProfile,
      ...
      auditSession.log(
          EjbcaEventTypes.APPROVAL_REJECT,
          EventStatus.SUCCESS,
          EjbcaModuleTypes.APPROVAL,
          EjbcaServiceTypes.EJBCA,
          admin.toString(),
  ...
  /**
   * Verifies that an administrator can approve an action, i.e. that it is not
   * the same admin approving the request as made the request originally. An
   * admin is not allowed to approve his/her own actions.
   *
   ...
   * @throws AdminAlreadyApprovedRequestException if the admin has already
   *     approved the action before
   * @throws SelfApprovalException if the administrator performing the approval
   *     is the same as the one requesting the original action.
   */
  private void checkApprovalPossibility(
      final AuthenticationToken admin,
      final ApprovalData approvalData,
      final Approval approval)
      throws AdminAlreadyApprovedRequestException, SelfApprovalException {
    // Check that the approver's principals don't exist among the existing
    // usernames.
    ...
    if (approvalInformation.getApprovalRequest().isEditedByMe(admin)
      ...
      throw new SelfApprovalException(
          "Can not approve a request that was last edited by oneself");
    }
  }

  @Override
  public void assertAuthorizedToApprove(
      final AuthenticationToken admin, final ApprovalDataVO approvalData)
      throws AuthorizationDeniedException {
    if (approvalData.getEndEntityProfileId()
    ...
    try {
      nextStep =
          approvalProfile.getStepBeingEvaluated(approvalData.getApprovals());
