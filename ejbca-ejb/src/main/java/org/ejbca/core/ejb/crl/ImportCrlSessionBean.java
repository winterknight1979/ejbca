...
@Stateless(
    mappedName = JndiConstants.APP_JNDI_PREFIX + "ImportCrlSessionRemote")
...
public class ImportCrlSessionBean
  ...
  public void importCrl(
    ...
    if (lastCrlOfSameType != null
      ...
      log.info(
          (isDeltaCrl ? "Delta" : "Full")
              ...
              + " for CA '"
              + cainfo.getName()
              + "' is not newer than last known "
              + (isDeltaCrl ? "delta" : "full")
              + " CRL. Ignoring download.");
      return;
    }
    ...
    if (x509crl.getRevokedCertificates() == null) {
      ...
      // For each entry that was updated after the last known CRL, create/update
      // a new database entry with the new status
      for (final X509CRLEntry crlEntry : crlEntries) {
        ...
        final CertificateDataWrapper cdw =
            certStoreSession.getCertificateDataByIssuerAndSerno(
                issuerDn, serialNumber);
        if (isLimitedCertificate(issuerDn, serialNumber, cdw)) {
          // Store as much as possible about what we know about the certificate
          // and its status (which is limited) in the database
          certStoreSession.updateLimitedCertificateDataStatus(
              authenticationToken,
              cainfo.getCAId(),
              issuerDn,
              serialNumber,
          ...
          try {
            ...
            throw new CrlImportException(
                "Failed to revoke certificate with serial number " + serialHex,
                e);
          }
        }
      }
    ...
    if (downloadedCrlNumber == 0) {
      final int lastCrlNumber =
          crlStoreSession.getLastCRLNumber(issuerDn, isDeltaCrl);
      newCrlNumber = lastCrlNumber + 1;
    } else {
      newCrlNumber = downloadedCrlNumber;
    }
    // Last of all, store the CRL if there were no errors during creation of
    // database entries
    crlStoreSession.storeCRL(
        authenticationToken,
        x509crl.getEncoded(),
        caFingerprint,
        newCrlNumber,
  ...
  private boolean isCertAlreadyRevoked(
      final int revocationReason, final CertificateDataWrapper cdw) {
    if (cdw != null) {
  ...
  private X509CRL getLastCrlOfSameType(
      final X509CRL crl, final boolean isDeltaCrl, final String issuerDN) {
  ...
  private boolean isLimitedCertificate(
