 ...
 * sequential updates.
 *
 * @version $Id: EndEntityProfileCache.java 27816 2018-01-09 16:19:36Z samuellb
 *     $
 */
public enum EndEntityProfileCache {
  ...
  public void updateProfileCache(
    ...
    final long cacheEndEntityProfileTime =
        EjbcaConfiguration.getCacheEndEntityProfileTime();
    final long now = System.currentTimeMillis();
    // Check before acquiring lock
    if (!force
        && cacheEndEntityProfileTime != 0
        && lastUpdate + cacheEndEntityProfileTime > now) {
      if (LOG.isTraceEnabled()) {
        LOG.trace(
            "<updateProfileCache returning, cache time not expired: "
                + cacheEndEntityProfileTime);
      }
    ...
    final Map<Integer, EndEntityProfile> profCache =
        new HashMap<Integer, EndEntityProfile>();
    try {
      final List<EndEntityProfileData> result =
