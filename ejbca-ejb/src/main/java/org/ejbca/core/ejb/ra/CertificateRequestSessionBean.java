 ...
 *
 * @version $Id: CertificateRequestSessionBean.java 29635 2018-08-14 10:56:22Z
 *     anatom $
 */
@Stateless(
    mappedName =
        JndiConstants.APP_JNDI_PREFIX + "CertificateRequestSessionRemote")
...
public class CertificateRequestSessionBean
  ...
  public byte[] processCertReq(
      ...
      throws AuthorizationDeniedException, NotFoundException,
          ...
          EndEntityProfileValidationException, ApprovalException,
    ...
    // This is the secret sauce, do the end entity handling automagically here
    // before we get the cert
    ...
    try {
      ...
      if (imsg != null) {
        retval =
            getCertResponseFromPublicKey(
                admin, imsg, hardTokenSN, responseType, userdata);
      }
  ...
  public ResponseMessage processCertReq(
      ...
      throws EndEntityExistsException, AuthorizationDeniedException,
          EndEntityProfileValidationException, EjbcaException,
    ...
    try {
      retval =
          signSession.createCertificate(admin, req, responseClass, userdata);
    } catch (EjbcaException e) {
      sessionContext
          .setRollbackOnly(); // This is an application exception so it wont
                              // trigger a roll-back automatically
   ...
   * @throws CADoesntExistsException if userdata.caId is not a valid caid. This
   *     is checked in editUser or addUserFromWS
  ...
  private void addOrEditUser(
      ...
      throws AuthorizationDeniedException, EndEntityProfileValidationException,
          ApprovalException, EndEntityExistsException, CADoesntExistsException,
          CertificateSerialNumberException, IllegalNameException,
          CustomFieldException, NoSuchEndEntityException {
    ...
    // First we need to fetch the CA configuration to see if we save UserData,
    // if not, we still run addUserFromWS to
    // get all the proper authentication checks for CA and end entity profile.
    // No need to to access control here just to fetch this flag, access control
    // for the CA is done in EndEntityManagementSession
    boolean useUserStorage =
        caSession.getCAInfoInternal(caid, null, true).isUseUserStorage();
    // Add or edit user
    try {
      String username = userdata.getUsername();
      if (useUserStorage && endEntityManagementSession.existsUser(username)) {
        ...
        // addUserfromWS also checks useUserStorage internally, so don't
        // duplicate the check
      ...
      // trigger a roll-back automatically
      String msg =
          "Single transaction enrollment request rejected since approvals are"
              + " enabled for this CA ("
              + caid
   ...
   * @throws AuthorizationDeniedException fail
   * @throws CertificateExtensionException if the request message contained
   *     invalid extensions
   */
  private byte[] getCertResponseFromPublicKey(
      final AuthenticationToken admin,
      final RequestMessage msg,
      final String hardTokenSN,
      final int responseType,
      final EndEntityInformation userData)
      throws EjbcaException, CesecoreException, CertificateEncodingException,
          CertificateException, IOException, AuthorizationDeniedException,
          CertificateExtensionException {
    byte[] retval = null;
    Class<X509ResponseMessage> respClass = X509ResponseMessage.class;
    ResponseMessage resp =
        signSession.createCertificate(admin, msg, respClass, userData);
  ...
  public byte[] processSoftTokenReq(
      ...
      throws ApprovalException, EndEntityExistsException,
          CADoesntExistsException, CertificateSerialNumberException,
          IllegalNameException, CustomFieldException,
          AuthorizationDeniedException, EndEntityProfileValidationException,
          NoSuchAlgorithmException, InvalidKeySpecException,
          CertificateException, InvalidAlgorithmParameterException,
          KeyStoreException, NoSuchEndEntityException {
