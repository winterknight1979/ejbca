...
/**
 * This class gives facilities to populate user data with default values from
 * profile.
 *
...
public class EndEntityInformationFiller {
   ...
   * @return update user.
   */
  public static EndEntityInformation fillUserDataWithDefaultValues(
      final EndEntityInformation userData, final EndEntityProfile profile) {
    ...
    subjectDN =
        mergeSubjectDnWithDefaultValues(
            subjectDN, profile, userData.getEmail());
    userData.setDN(subjectDN);
    String subjectAltName = userData.getSubjectAltName();
    // Processing Subject Altname values
    subjectAltName =
        mergeSubjectAltNameWithDefaultValues(
            subjectAltName, profile, userData.getEmail());
    userData.setSubjectAltName(subjectAltName);
    if (userData.getType().getHexValue() == EndEntityTypes.INVALID.hexValue()) {
      if (StringUtils.isNotEmpty(
          profile.getValue(EndEntityProfile.FIELDTYPE, 0))) {
        final int type =
            Integer.parseInt(profile.getValue(EndEntityProfile.FIELDTYPE, 0));
   ...
   * @return updated DN.
   */
  private static String mergeSubjectDnWithDefaultValues(
    ...
    return CertTools.stringToBCDNString(
        profiledn.mergeDN(userdn, true, dnMap).toString());
  }
  ...
  private static String mergeSubjectAltNameWithDefaultValues(
    ...
    int numberofsubjectAltNamefields =
        profile.getSubjectAltNameFieldOrderLength();
