 ...
 *
 * <p>According to RFC 4210 4.2.2.2: "Where verification of the cert
 * confirmation message fails, the RA/CA MUST revoke the newly issued
 * certificate if it has been published or otherwise made available."
 *
 * <p>However, EJBCA does not keep track of the transaction and always responds
 * with a ResponseStatus.SUCCESS Certificate Confirmation ACK.
 *
 * @version $Id: ConfirmationMessageHandler.java 27740 2018-01-05 07:24:53Z
 *     mikekushner $
 */
public class ConfirmationMessageHandler extends BaseCmpMessageHandler
  ...
  public ResponseMessage handleMessage(
    ...
    if (version == PKIHeader.CMP_1999) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Creating a PKI confirm message response, responseProtection="
                + responseProtection);
      }
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Not Cmp1999 or Cmp2000 or later - Not creating a PKI confirm"
                + " message response");
      }
    }
    return cresp;
  ...
  private void setPbeParameters(
      final BaseCmpMessage cmpResponseMessage,
      final BaseCmpMessage cmpRequestMessage,
      final boolean authenticated)
      throws InvalidCmpProtectionException {
    final String keyId =
        CmpMessageHelper.getStringFromOctets(
            cmpRequestMessage.getHeader().getSenderKID());
    String sharedSecret =
        cmpConfiguration.getAuthenticationParameter(
            CmpConfiguration.AUTHMODULE_HMAC, confAlias);
    if (StringUtils.equals(sharedSecret, "-")) {
      try {
        final X509CAInfo cainfo =
            getCAInfo(cmpRequestMessage.getRecipient().getName().toString());
        sharedSecret = cainfo.getCmpRaAuthSecret();
      } catch (CADoesntExistsException e) {
        LOG.error("Exception during CMP response protection: ", e);
      }
    }
    // We don't need to check the shared secret in client mode (= the EndEntiy
    // password) because PBE protection is only supported in RA mode
    CmpPbeVerifyer verifyer =
        new CmpPbeVerifyer(cmpRequestMessage.getMessage());
    String owfAlg = verifyer.getOwfOid();
    String macAlg = verifyer.getMacOid();
    int iterationCount = verifyer.getIterationCount();
    cmpResponseMessage.setPbeParameters(
        keyId, sharedSecret, owfAlg, macAlg, iterationCount);
  }

  private void signResponse(
      final CmpConfirmResponseMessage cresp,
      final BaseCmpMessage cmpRequestMessage) {
    try {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Using CA '"
                + caInfo.getName()
                + "' to sign Certificate Confirm message");
      }
      X509Certificate cacert =
          (X509Certificate) caInfo.getCertificateChain().iterator().next();
      // We use the actual asn.1 encoding from the cacert subjectDN here. This
      // ensures that the DN is exactly as
      // encoded in the certificate (which it should be).
      // If we use only the cainfo.getSubjectDN we will get "EJBCA encoding",
      // and this may not be the same if the
      // CA certificate comes from an external CA that encodes thing differently
      // than EJBCA.
      cresp.setSender(
          new GeneralName(
      ...
      final AlgorithmIdentifier protectionAlgorithm =
          cmpRequestMessage.getHeader().getProtectionAlg();
      if (protectionAlgorithm != null) {
        cresp.setPreferredDigestAlg(
            AlgorithmTools.getDigestFromSigAlg(
                protectionAlgorithm.getAlgorithm().getId()));
      }
  ...
  private X509CAInfo getCAInfo(final String caDn)
      throws CADoesntExistsException {
    CAInfo caInfo = null;
    if (caDn == null) {
      final String caDnDefault =
          CertTools.stringToBCDNString(
              this.cmpConfiguration.getCMPDefaultCA(this.confAlias));
      ...
      caInfo =
          caSession.getCAInfoInternal(caDnNormalized.hashCode(), null, true);
      if (caInfo == null) {
        final String caDnDefault =
            CertTools.stringToBCDNString(
                this.cmpConfiguration.getCMPDefaultCA(this.confAlias));
        LOG.info(
            "Could not find Recipient CA with DN '"
                + caDnNormalized
                + "'."
                + " Trying to use CMP DefaultCA instead with DN '"
                + caDnDefault
                + "' ("
                + caDnDefault.hashCode()
                + ").");
        caInfo =
            caSession.getCAInfoInternal(caDnDefault.hashCode(), null, true);
      }
    }
