 ...
 *
 * <p>In RA mode, the authenticity is checked through a shared secret specified
 * either in the configuration file or in the CA.
 *
 * <p>In client mode, the authenticity is checked through the
 * clear-text-password of the pre-registered end entity from the database.
 *
 * @version $Id: HMACAuthenticationModule.java 28319 2018-02-19 09:14:59Z
 *     andresjakobs $
 */
public class HMACAuthenticationModule implements ICMPAuthenticationModule {
  ...
  public boolean verifyOrExtract(
      final PKIMessage pkiMessage, final String username) {
    if (pkiMessage == null) {
      this.errorMessage = "No PKIMessage was found";
      return false;
    }
    if (pkiMessage.getProtection() == null
        || pkiMessage.getHeader().getProtectionAlg() == null) {
      this.errorMessage =
          "PKI Message is not authenticated properly. No HMAC protection was"
              + " found.";
      return false;
    }
    try {
    ...
    try {
      if (this.cmpConfiguration.getRAMode(this.confAlias)) {
        ...
        // Note that this restriction only applies to HMAC and not
        // EndEntityCertificate because in the latter, the use of profiles can
        // be restricted through
        // Administrator privileges. Other authentication modules are not used
        // in RA mode
        final boolean useKeyIdForEndEntityProfile =
            StringUtils.equals(
                cmpConfiguration.getRAEEProfile(confAlias),
                CmpConfiguration.PROFILE_USE_KEYID);
        final boolean useKeyIdForCertificateProfile =
            StringUtils.equals(
                cmpConfiguration.getRACertProfile(confAlias),
                CmpConfiguration.PROFILE_USE_KEYID);
        if (useKeyIdForEndEntityProfile || useKeyIdForCertificateProfile) {
          final String keyId =
              CmpMessageHelper.getStringFromOctets(
                  pkiMessage.getHeader().getSenderKID());
          if ((useKeyIdForEndEntityProfile
                  && StringUtils.equals(
                      keyId, EndEntityConstants.EMPTY_ENDENTITYPROFILENAME))
              || (useKeyIdForCertificateProfile
                  && StringUtils.equals(
                      keyId, CertificateProfile.ENDUSERPROFILENAME))) {
            errorMessage = "Unaccepted KeyId '" + keyId + "' in CMP request";
            LOG.info(errorMessage);
            return false;
          }
        }
        // If we use a globally configured shared secret for all CAs we check it
        // right away
        if (globalSharedSecret != null) {
          if (LOG.isDebugEnabled()) {
            LOG.debug("Verifying message using Global Shared secret");
          }
          if (performPbeVerification(
              globalSharedSecret,
              "cmp.errorauthmessage",
              "Global auth secret")) {
            return true;
          }
        }
        ...
        if (caInfo instanceof X509CAInfo) {
          ...
          if (StringUtils.isNotEmpty(authSecret)) {
            if (LOG.isDebugEnabled()) {
              LOG.debug(
                  "Verify message using 'CMP RA Authentication Secret' from CA"
                      + " '"
                      + caInfo.getName()
                      + "'.");
            }
            ...
            if (LOG.isDebugEnabled()) {
              LOG.debug(
                  "CMP password is null from CA '" + caInfo.getName() + "'.");
            }
          }
        }
        // We have failed verification with CA authentication secret too.
        this.errorMessage =
            "Failed to verify message using both Global Shared Secret and CMP"
                + " RA Authentication Secret";
      } else {
        if (LOG.isDebugEnabled()) {
          LOG.debug("Verifying HMAC in Client mode");
        }
        // If client mode, we try to get the pre-registered endentity from the
        // DB, and if there is a
        ...
        try {
          if (username != null) {
            if (LOG.isDebugEnabled()) {
              LOG.debug(
                  "Searching for an end entity with username='"
                      + username
                      + "'.");
            }
            endEntityInformation =
                this.endEntityAccessSession.findUser(
                    authenticationToken, username);
          } else {
            // No username given, so we try to find from to extract the username
            // by the DN component specified,
            // if not found, it must be found by subject/issuerDN from the
            // certificate request.
            ...
            if (subjectDN != null) {
              ...
              final String extractedUsername =
                  getUsernameByDnComponent(subjectDN);
              ...
              if (endEntityInformation == null) {
                ...
                if (issuer == null) {
                  if (LOG.isDebugEnabled()) {
                    LOG.debug(
                        "Searching for an end entity with SubjectDN='"
                            + subjectDN
                            + "'.");
                  }
                  endEntityInformations =
                      this.endEntityAccessSession.findUserBySubjectDN(
                          authenticationToken, subjectDN);
                  if (endEntityInformations.size() > 1) {
                    LOG.warn(
                        "Multiple end entities with subject DN "
                            + subjectDN
                            + " were found. This may lead to unexpected"
                            + " behavior.");
                  }
                } else {
                  ...
                  endEntityInformations =
                      endEntityAccessSession.findUserBySubjectAndIssuerDN(
                          this.authenticationToken, subjectDN, issuerDN);
                  if (endEntityInformations.size() > 1) {
                    LOG.warn(
                        "Multiple end entities with subject DN "
                            + subjectDN
          ...
          LOG.info(
              "Not authorized to search for end entity: " + e.getMessage());
        }
        if (endEntityInformation == null) {
          LOG.info(
              INTRES.getLocalizedMessage(
                  "ra.errorentitynotexist",
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              "Comparing HMAC password authentication for user '"
        ...
        if (StringUtils.isEmpty(eepassword)) {
          this.errorMessage =
              "No clear text password for user '"
                  + endEntityInformation.getUsername()
      ...
      // We don't want to explain in detail to the client why the configured key
      // on the CA was invalid
   ...
   *
   * <p>Saves last error message on failure to validate. Saves
   * raAuthenticationSecret on successful validation (to be used later when
   * creating response protection)
   *
   ...
   */
  private boolean performPbeVerification(
      final String raAuthenticationSecret,
      final String errorMessageKey,
      final String errorMessageParameter)
    ...
    if (verifyer.verify(raAuthenticationSecret)) {
      ...
      String errmsg =
          INTRES.getLocalizedMessage(errorMessageKey, errorMessageParameter);
      LOG.info(errmsg);
   ...
   *
   * @param msg message
   * @return the certificate template embedded in msg. Null if no such template
   *     was found.
   */
