 ...
 *
 * <p>This class contains common methods for extracting the RA authentication
 * secret.
 *
 * @version $Id: BaseCmpMessageHandler.java 26734 2017-10-05 13:09:53Z anatom $
 */
public class BaseCmpMessageHandler {
   ...
   * @return the CA id to use for a request based on the current configuration,
   *     used end entity profile and keyId.
   * @throws CADoesntExistsException Fail
   * @throws AuthorizationDeniedException Fail
   */
  protected int getUsedCaId(final String keyId, final int eeProfileId)
    ...
    if (StringUtils.equals(caName, "ProfileDefault")) {
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Using default CA from End Entity Profile CA when adding users in"
                + " RA mode.");
      }
      // get default CA id from end entity profile
      final EndEntityProfile eeProfile =
          endEntityProfileSession.getEndEntityProfileNoClone(eeProfileId);
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Using keyId as CA name when adding users in RA mode: " + keyId);
      }
      if (keyId != null) {
        ...
        LOG.error(
            "Expecting the CA name to be specified in the KeyID parameter, but"
                + " the KeyID parameter is 'null'");
      }
    } else {
      // No need to do access control here, just to get the CAId
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Using fixed caName when adding users in RA mode: "
                + caName
                + "("
                + ret
                + ")");
   ...
   * @return the end entity profile name to use for a request based on the
   *     current configuration and keyId.
   * @throws EndEntityProfileNotFoundException Fail
   */
  protected int getUsedEndEntityProfileId(final String keyId)
      throws EndEntityProfileNotFoundException {
    final int eeProfileId;
    final String eeProfile =
   ...
   * @param keyId ID
   * @param eeProfileId Profile
   * @return the certificate profile name to use for a request based on the
   *     current configuration and keyId.
   * @throws NotFoundException Fail
   */
  protected String getUsedCertProfileName(
      final String keyId, final int eeProfileId) throws NotFoundException {
    // Get the configured string, may be a profile name or 'KeyId' or
    // 'ProfileDefault'
    String certificateProfile =
        cmpConfiguration.getRACertProfile(this.confAlias);
      ...
      if (eeProfile == null) {
        final String msg =
            INTRES.getLocalizedMessage(
                "store.errorcertprofilenotexist", eeProfileId);
        LOG.info(msg);
        throw new NotFoundException(msg);
      }
      certificateProfile =
          certificateProfileSession.getCertificateProfileName(
              eeProfile.getDefaultCertificateProfile());
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Using default certificate profile from End Entity Profile: "
                + certificateProfile);
      }
    } else if (StringUtils.equals(
        certificateProfile, CmpConfiguration.PROFILE_USE_KEYID)) {
      if (keyId != null) {
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              "Using Certificate Profile with same name as KeyId in request: "
                  + keyId);
        }
        certificateProfile = keyId;
      } else {
        LOG.error(
            "Expecting the Certificate Profile name to be specified in the"
                + " KeyID parameter, but the KeyID parameter is 'null'");
      }
    }
   ...
   * @return the certificate profile to use for a request based on the current
   *     configuration and keyId.
   * @throws NotFoundException Fail
   */
  protected int getUsedCertProfileId(final String certificateProfile)
      throws NotFoundException {
    final int ret =
        this.certificateProfileSession.getCertificateProfileId(
            certificateProfile);
    if (ret == 0) {
      final String msg =
          "No certificate profile found with name: " + certificateProfile;
      LOG.info(msg);
      throw new NotFoundException(msg);
    }
    return ret;
