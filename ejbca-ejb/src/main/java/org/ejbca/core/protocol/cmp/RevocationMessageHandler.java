...
public class RevocationMessageHandler extends BaseCmpMessageHandler
  ...
  public RevocationMessageHandler(
    ...
    this.certificateStoreSession =
        ejbBridgeSession.getCertificateStoreSession();
  ...
  public ResponseMessage handleMessage(
      final BaseCmpMessage msg, final boolean authenticated) {
    ...
    final String keyId =
        CmpMessageHelper.getStringFromOctets(msg.getHeader().getSenderKID());
    CA ca = null;
    try {
      ...
      return CmpMessageHelper.createUnprotectedErrorMessage(
          msg, FailInfo.INCORRECT_DATA, errMsg);
    } catch (CADoesntExistsException e) {
      final String errMsg =
          "CA with DN '"
              + msg.getHeader().getRecipient().getName().toString()
              + "' is unknown";
      LOG.info(errMsg);
      return CmpMessageHelper.createUnprotectedErrorMessage(
          msg, FailInfo.BAD_REQUEST, errMsg);
    } catch (AuthorizationDeniedException e) {
      LOG.info(INTRES.getLocalizedMessage(CMP_ERRORGENERAL, e.getMessage()), e);
      return CmpMessageHelper.createUnprotectedErrorMessage(
          msg, FailInfo.INCORRECT_DATA, e.getMessage());
    }
    ...
    ICMPAuthenticationModule authenticationModule =
        messageVerifyer.getUsedAuthenticationModule(
            msg.getMessage(), null, authenticated);
    if (authenticationModule == null) {
      LOG.info(messageVerifyer.getErrorMessage());
      return CmpMessageHelper.createUnprotectedErrorMessage(
          msg, FailInfo.BAD_REQUEST, messageVerifyer.getErrorMessage());
    }

    // If authentication was correct, we will now try to find the certificate to
    // revoke
    ...
    if (crlExt != null) {
      ...
      if (ext != null) {
        try {
          final ASN1InputStream ai =
              new ASN1InputStream(ext.getExtnValue().getOctets());
          final ASN1Primitive obj = ai.readObject();
          final ASN1Enumerated crlreason = ASN1Enumerated.getInstance(obj);
          // RevokedCertInfo.REVOCATION_REASON_AACOMPROMISE are the same integer
          // values as the CRL reason extension code
          ...
          LOG.info(
              INTRES.getLocalizedMessage(CMP_ERRORGENERAL, e.getMessage()), e);
          return CmpMessageHelper.createUnprotectedErrorMessage(
              msg, FailInfo.INCORRECT_DATA, e.getMessage());
        }
      } else {
    ...
    if (serno != null && issuer != null) {
      ...
      try {
        ...
        final String errMsg =
            INTRES.getLocalizedMessage("cmp.erroralreadyrequested");
        failText = errMsg;
        LOG.info(failText);
      } catch (AlreadyRevokedException e) {
        failInfo = FailInfo.CERT_REVOKED;
        final String errMsg =
            INTRES.getLocalizedMessage("cmp.erroralreadyrevoked");
        failText = errMsg;
        // This is already info logged in endEntityManagementSession.revokeCert
        // LOG.info(failText);
      }
      ...
      final String errMsg =
          INTRES.getLocalizedMessage(
              "cmp.errormissingissuerrevoke",
              (issuer != null ? issuer.toString() : "<no issuer in request>"),
              (serno != null
                  ? serno.getValue().toString(16)
                  : "<no serial number in request>"));
      failText = errMsg;
      LOG.info(failText);
    }
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "Creating a PKI revocation message response: " + responseProtection);
    }
    ...
    if (StringUtils.equals(responseProtection, "pbe")) {
      ...
      if (owfAlg != null && macAlg != null && cmpRaAuthSecret != null) {
        ...
        rresp.setPbeParameters(
            keyId, cmpRaAuthSecret, owfAlg, macAlg, iterationCount);
      }
  ...
  private CA getRecipientCA(final BaseCmpMessage msg, final String keyId)
      throws EndEntityProfileNotFoundException, CADoesntExistsException,
