...
public class CrmfMessageHandler extends BaseCmpMessageHandler
  ...
  public CrmfMessageHandler(
    ...
    this.endEntityManagementSession =
        ejbBridgeSession.getEndEntityManagementSession();
    ...
    if (this.cmpConfiguration.getRAMode(this.confAlias)) {
      ...
      this.usernameGenParams.setMode(
          this.cmpConfiguration.getRANameGenScheme(this.confAlias));
      this.usernameGenParams.setDNGeneratorComponent(
          this.cmpConfiguration.getRANameGenParams(this.confAlias));
      this.usernameGenParams.setPrefix(
          this.cmpConfiguration.getRANameGenPrefix(this.confAlias));
      this.usernameGenParams.setPostfix(
          this.cmpConfiguration.getRANameGenPostfix(this.confAlias));
      this.userPwdParams =
          this.cmpConfiguration.getRAPwdGenParams(this.confAlias);
      this.allowCustomCertSerno =
          this.cmpConfiguration.getAllowRACustomSerno(this.confAlias);
      this.responseProt =
    ...
    // Checks if an extended user data hander is configured and if so, creates
    // the handler class.
  ...
  /**
   * Gets the end entity with that subject DN either - by extracting the
   * username by a DN component ({@link
   * CmpConfiguration#getExtractUsernameComponent(String)} - or by matching its
   * DN exactly.
   *
   * @param dn the end entities DN (must match exactly).
   * @return the end entity that either has a matching username extracted from
   *     the DN, or a subjectDN that matches the dn exactly, or null if no user
   *     found.
   * @throws AuthorizationDeniedException if authorization was denied.
   */
  private EndEntityInformation getEndEntityFromCertReqRequest(final String dn)
    ...
    if (result == null) {
      // ECA-6435 Overwrite the EE DN with the request DN fails here,
  ...
  public ResponseMessage handleMessage(
      final BaseCmpMessage cmpRequestMessage, final boolean authenticated) {
    ...
    try {
      ...
      if (cmpRequestMessage instanceof CrmfRequestMessage) {
        ...
        // If we have usernameGeneratorParams we want to generate usernames
        // automagically for requests
        ...
        if (usernameGenParams != null) {
          ...
          // if extractUsernameComponent is null, we have to find the user from
          // the DN
          // if not empty the message will find the username itself, in the
          // getUsername method
          final String dn = crmfreq.getSubjectDN();
          final EndEntityInformation endEntityInformation =
              getEndEntityFromCertReqRequest(crmfreq.getSubjectDN());
          if (endEntityInformation != null) {
            if (LOG.isDebugEnabled()) {
              LOG.debug(
                  "Found username: " + endEntityInformation.getUsername());
            }
            final String username = endEntityInformation.getUsername();
            crmfreq.setUsername(username);
            final VerifyPKIMessage messageVerifyer =
                new VerifyPKIMessage(
                    null,
                    this.confAlias,
                    admin,
                    caSession,
            ...
            final ICMPAuthenticationModule authenticationModule =
                messageVerifyer.getUsedAuthenticationModule(
                    crmfreq.getPKIMessage(), username, authenticated);
            if (authenticationModule == null) {
              String errmsg = messageVerifyer.getErrorMessage();
              LOG.info(errmsg);
              return CmpMessageHelper.createUnprotectedErrorMessage(
                  cmpRequestMessage, FailInfo.BAD_REQUEST, errmsg);
            }

            crmfreq.setPassword(authenticationModule.getAuthenticationString());
            if (crmfreq.getHeader().getProtectionAlg() != null) {
              crmfreq.setPreferredDigestAlg(
                  AlgorithmTools.getDigestFromSigAlg(
                      crmfreq
                          .getHeader()
                          .getProtectionAlg()
                          .getAlgorithm()
            ...
            // Do we have a public key in the request? If not we may be trying
            // to do server generated keys
            enrichWithServerGeneratedKeyOrThrow(
                crmfreq, endEntityInformation.getCertificateProfileId());
            resp =
            ...
            final String errMsg =
                INTRES.getLocalizedMessage("cmp.infonouserfordn", dn);
            LOG.info(errMsg);
            ...
            LOG.info(failText);
            resp =
                signSession.createRequestFailedResponse(
                    admin,
                    crmfreq,
                    org.ejbca.core.protocol.cmp.CmpResponseMessage.class,
      ...
      // Thrown checking for the public key in the request, if these are thrown
      // there is something wrong with the key in the request
      final String errMsg =
          INTRES.getLocalizedMessage(CMP_ERRORGENERAL, e.getMessage());
      LOG.info(errMsg, e);
      resp =
          CmpMessageHelper.createUnprotectedErrorMessage(
              cmpRequestMessage, FailInfo.BAD_REQUEST, e.getMessage());
    } catch (NoSuchProviderException e) {
      // Thrown checking for the public key in the request, if this is thrown
      ...
      resp =
          CmpMessageHelper.createUnprotectedErrorMessage(
              cmpRequestMessage, FailInfo.SYSTEM_UNAVAILABLE, e.getMessage());
      ...
      resp =
          CmpMessageHelper.createUnprotectedErrorMessage(
              cmpRequestMessage, FailInfo.NOT_AUTHORIZED, e.getMessage());
    } catch (CADoesntExistsException e) {
      final String errMsg =
          INTRES.getLocalizedMessage(CMP_ERRORGENERAL, e.getMessage());
      LOG.info(
          errMsg,
          e); // info because this is something we should expect and we handle
              // it
      resp =
          CmpMessageHelper.createUnprotectedErrorMessage(
              cmpRequestMessage, FailInfo.WRONG_AUTHORITY, e.getMessage());
      ...
      resp =
          CmpMessageHelper.createUnprotectedErrorMessage(
              cmpRequestMessage, FailInfo.BAD_POP, e.getMessage());
  ...
  /**
   * Method that takes care of RA mode operations, i.e. when the message is
   * authenticated with a common secret using password based encryption (pbe).
   * This method will verify the pbe and if ok will automatically create/edit a
   * user and issue the certificate. In RA mode we assume that the RA knows what
   * it is doing.
   *
   ...
   */
  private ResponseMessage handleRaMessage(
      final CrmfRequestMessage crmfreq, final boolean authenticated)
      throws AuthorizationDeniedException, EjbcaException, CesecoreException {
    final int
        eeProfileId; // The endEntityProfile to be used when adding users in RA
                     // mode.
    final String
        certProfileName; // The certificate profile to use when adding users in
                         // RA mode.
    ...
    final String keyId =
        CmpMessageHelper.getStringFromOctets(
            crmfreq.getHeader().getSenderKID());
    ...
    try {
      ...
      return CmpMessageHelper.createErrorMessage(
          crmfreq,
          FailInfo.INCORRECT_DATA,
          e.getMessage(),
          requestId,
      ...
      final String errMsg =
          INTRES.getLocalizedMessage(CMP_ERRORGENERAL, e.getMessage());
      LOG.info(errMsg, e);
      // In case an EE profile or a cert profiles, or a CA can not be found,
      // this is a bad configuration or database is down.
      // In either case the system is unavailable due to CMP server, so client
      // should try again at some later point
      return CmpMessageHelper.createErrorMessage(
          crmfreq,
    ...
    final VerifyPKIMessage messageVerifyer =
        new VerifyPKIMessage(
            ...
            endEntityManagementSession,
            this.cmpConfiguration);
    ICMPAuthenticationModule authenticationModule =
        messageVerifyer.getUsedAuthenticationModule(
            crmfreq.getPKIMessage(), null, authenticated);
    if (authenticationModule == null) {
      String errmsg = messageVerifyer.getErrorMessage();
      LOG.info(errmsg);
      return CmpMessageHelper.createUnprotectedErrorMessage(
          crmfreq, FailInfo.BAD_REQUEST, errmsg);
    }

    try {
      // Create a username and password and register the new user in EJBCA
      final UsernameGenerator gen =
          UsernameGenerator.getInstance(this.usernameGenParams);
      // Don't convert this DN to an ordered EJBCA DN string with
      // CertTools.stringToBCDNString because we don't want double escaping of
      // some characters
      final RequestMessage req =
          this.extendedUserDataHandler != null
              ? this.extendedUserDataHandler.processRequestMessage(
                  crmfreq, certProfileName)
              : crmfreq;
      ...
      if (StringUtils.equals(
        ...
        pwd = authenticationModule.getAuthenticationString();
          ...
          authenticationModule.getName(), CmpConfiguration.AUTHMODULE_HMAC)) {
        ...
        // This should not run since an error would have occurred earlier if the
        // authentication module was unknown
        final String errMsg = "Unknown authentication module.";
        LOG.error(errMsg);
        return CmpMessageHelper.createUnprotectedErrorMessage(
            crmfreq, FailInfo.BAD_MESSAGE_CHECK, errMsg);
      }
      ...
      // Use rfc822name or first SubjectDN email address as user email address
      // if available
      ...
      if (this.allowCustomCertSerno) {
        ...
        if (customCertSerno != null) {
          // If we have a custom certificate serial number in the request, we
          // will pass it on to the UserData object
          ei = new ExtendedInformation();
          ei.setCertificateSerialNumber(customCertSerno);
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                "Custom certificate serial number: "
                    + customCertSerno.toString(16));
          }
        }
      }
      ...
      // Username and pwd in the EndEntityInformation and the IRequestMessage
      // must match
      crmfreq.setUsername(username);
      ...
      if (StringUtils.equals(
          authenticationModule.getName(), CmpConfiguration.AUTHMODULE_HMAC)) {
        final HMACAuthenticationModule hmacmodule =
            (HMACAuthenticationModule) authenticationModule;
        ...
        if (StringUtils.equals(this.responseProt, "pbe")) {
          crmfreq.setPbeParameters(
              keyId, raSecret, pbeDigestAlg, pbeMacAlg, pbeIterationCount);
        }
      }
      try {
        // Do we have a public key in the request? If not we may be trying to do
        // server generated keys
        enrichWithServerGeneratedKeyOrThrow(
            (ICrmfRequestMessage) req, certProfileId);
        try {
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                "Creating new request with eeProfileId '"
                    + eeProfileId
                    + "', certProfileId '"
                    + certProfileId
                    + "', caId '"
          ...
          resp =
              this.certificateRequestSession.processCertReq(
                  this.admin,
                  userdata,
                  req,
                  org.ejbca.core.protocol.cmp.CmpResponseMessage.class);
          ...
          final String updateMsg =
              INTRES.getLocalizedMessage("cmp.erroradduserupdate", username);
          LOG.info(updateMsg);
          // Try again
          resp =
              this.certificateRequestSession.processCertReq(
          ...
          | NoSuchAlgorithmException
          | InvalidAlgorithmParameterException e) {
        // Thrown checking for the public key in the request, if these are
        // thrown there is something wrong with the key in the request
        LOG.info(INTRES.getLocalizedMessage(CMP_ERRORGENERAL, username), e);
        resp =
            CmpMessageHelper.createErrorMessage(
                crmfreq,
      ...
      resp =
          CmpMessageHelper.createUnprotectedErrorMessage(
              crmfreq, FailInfo.BAD_MESSAGE_CHECK, e.getMessage());
    }
    return resp;
  }

  private void enrichWithServerGeneratedKeyOrThrow(
      final ICrmfRequestMessage req, final int certProfileID)
      throws InvalidKeyException, NoSuchAlgorithmException,
          NoSuchProviderException, InvalidAlgorithmParameterException {
    SubjectPublicKeyInfo pkInfo = req.getRequestSubjectPublicKeyInfo();

    // To trigger server generated keys, subjectPublicKeyInfo key could be null,
    // but subjectPublicKeyInfo could also still be there
    // with an AlgorithmIdentifier followed by a zero-length BIT STRING (RFC4210
    // Appendix D.4)
    if (pkInfo == null
        || (pkInfo.getAlgorithm() != null
            && pkInfo.getPublicKeyData() != null
            && pkInfo.getPublicKeyData().getBytes().length == 0)) {
      if (LOG.isDebugEnabled()) {
        LOG.debug("CRMF requests does not contain a request public key.");
        if (pkInfo != null) {
          LOG.debug(
              "CRMF requests contains a SubjectPublicKeyInfo without key bits"
                  + " and with algorithmId "
                  + pkInfo.getAlgorithm().getAlgorithm().getId());
        }
      }
      // Trying to request server generated keys? See if this is allowed, and
      // then try to figure out key generation parameters
      if (!this.cmpConfiguration.getAllowServerGeneratedKeys(this.confAlias)) {
        // Not possible to server generate key when there is no key to encrypt
        // the response with
        throw new InvalidKeyException("Server generated keys not allowed");
      }

      // Only allow server generated keys if we have a key to encrypt the
      // response with
      // Even though theoretically possible, we only support RSA key transport
      // protection/wrapping
      if (req.getProtocolEncrKey() == null
          || !req.getProtocolEncrKey().getAlgorithm().equals("RSA")) {
        // Not possible to server generate key when there is no key to encrypt
        // the response with
        throw new InvalidKeyException(
            "Request public key can not be empty without providing a suitable"
                + " protocolEncrKey (RSA)");
      } else {
        LOG.debug(
            "Found id_regCtrl_protocolEncrKey with an RSA key in request,"
                + " clear to generate keys.");
      }
      ...
      if (profile == null) {
        final String msg =
            "No certificate profile to get key specification for server"
                + " generated keys";
        LOG.debug(msg);
      ...
      if (pkInfo != null) {
        ...
        // If we come here we already know that there is an AlgorithmIdentifier,
        // but no public key bits
        ...
        if (PKCSObjectIdentifiers.rsaEncryption.equals(algId.getAlgorithm())) {
          if (!algs.contains(AlgorithmConstants.KEYALGORITHM_RSA)) {
            final String msg =
                "RSA key generation requested, but certificate profile"
                    + " specified does not allow RSA";
            if (LOG.isDebugEnabled()) {
          ...
          LOG.debug(
              "RSA algorithm found in SubjectPublicKeyInfo, using as server"
                  + " key generation alg.");
          algs.clear();
          algs.add(AlgorithmConstants.KEYALGORITHM_RSA);
        } else if (X9ObjectIdentifiers.id_ecPublicKey.equals(
            algId.getAlgorithm())) {
          if (!algs.contains(AlgorithmConstants.KEYALGORITHM_ECDSA)) {
            final String msg =
                "ECDSA key generation requested, but certificate profile"
                    + " specified does not allow ECDSA";
            if (LOG.isDebugEnabled()) {
              LOG.debug(msg + ": " + certProfileID + ": " + algs);
            }
            throw new InvalidKeyException(msg);
          ...
          LOG.debug(
              "ECDSA algorithm found in SubjectPublicKeyInfo, using as server"
                  + " key generation alg.");
          algs.clear();
          ...
          if (pkInfo.getAlgorithm().getParameters() == null) {
            final String msg =
                "ECDSA key generation requested, but no key parameters"
                    + " included";
            throw new InvalidKeyException(msg);
          } else {
            try {
              X962Parameters params =
                  X962Parameters.getInstance(
                      pkInfo.getAlgorithm().getParameters());
              if (params.isImplicitlyCA()) {
                LOG.debug(
                    "ECDSA key generation parameters is implicitlyCA, will try"
                        + " to use certificate profile parameters.");
                ...
                if (curveName == null) {
                  final String msg =
                      "ECDSA key generation requested, but X962Parameters is"
                          + " none of the supported named curves: "
                          + oid.getId();
                  LOG.debug(msg);
                  throw new InvalidKeyException(msg);
                }
                // Check that this curve is allowed in the Certificate Profile,
                // compare oids, since names have many aliases...and first
                // choice alias can change between different BC versions
                boolean found = false;
                for (String curve : curves) {
                  final ASN1ObjectIdentifier id =
                      ECNamedCurveTable.getOID(curve);
                ...
                if (!found) {
                  final String msg =
                      "ECDSA key generation requested, but X962Parameters"
                          + " curve is none of the allowed named curves: "
                          + curveName;
                  LOG.debug(msg);
                  throw new InvalidKeyException(msg);
                }
                if (LOG.isDebugEnabled()) {
                  LOG.debug(
                      "ECDSA key generation parameters is namedCurve, will try"
                          + " to generate key with named curve "
                          + oid.getId()
                          + ", "
                          + curveName);
                }
                ...
                final String msg =
                    "ECDSA key generation requested, but X962Parameters is"
                        + " none of the supported options implicitlyCA or"
                        + " namedCurve";
                throw new InvalidKeyException(msg);
              }
            } catch (IllegalArgumentException e) {
              final String msg =
                  "ECDSA key generation requested, but X962Parameters can not"
                      + " be decoded";
          ...
          final String msg =
              "Server key generation requested, but SubjectPublicKeyInfo"
                  + " specifies unsupported algorithm "
      ...
      if (algs.size() > 1) {
        final String msg =
            "Certificate profile specified more than one key algoritm, not"
                + " possible to server generate keys";
        if (LOG.isDebugEnabled()) {
          LOG.debug(msg + ": " + certProfileID + ": " + algs);
        }
        throw new InvalidKeyException(msg);
      ...
      if (AlgorithmConstants.KEYALGORITHM_RSA.equals(algs.get(0))) {
        ...
        if (sizes == null || sizes.length > 1) {
          final String msg =
              "Certificate profile specified more than one key size, not"
                  + " possible to server generate keys";
          if (LOG.isDebugEnabled()) {
        ...
        if (curves.size() > 1) {
          final String msg =
              "Certificate profile specified more than one EC curve, not"
                  + " possible to server generate keys";
          if (LOG.isDebugEnabled()) {
        ...
        final String msg =
            "Certificate profile an algorithm not supported for server"
                + " generated keys";
        LOG.debug(msg);
        throw new InvalidKeyException(msg);
      }
   ...
   * @throws AuthorizationDeniedException if authorization was denied.
   */
  private EndEntityInformation getEndEntityByDn(final String dn)
      throws AuthorizationDeniedException {
    EndEntityInformation endEntityInformation = null;
    if (LOG.isDebugEnabled()) {
      LOG.debug("looking for user with dn: " + dn);
    }
    List<EndEntityInformation> endEntityInformations =
        endEntityAccessSession.findUserBySubjectDN(admin, dn);
  ...
  /**
   * Gets the username by the DN component specified in the CMP configuration
   * 'extract username component' and adds the RA name generation prefix and
   * postfix.
   *
   * @param dn the DN.
   * @return the username with RA name generation prefix and postfix.
   */
  private String getUsernameByDnComponent(final String dn) {
    final String usernameComp =
