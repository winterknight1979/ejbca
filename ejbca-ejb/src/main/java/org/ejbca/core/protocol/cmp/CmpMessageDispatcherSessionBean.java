...
/**
 * Class that receives a CMP message and passes it on to the correct message
 * handler.
 *
 * <p>----- This processes does the following: 1. receive a CMP message 2. check
 * which message type it is 3. dispatch to the correct message handler 4. send
 * back the response received from the handler -----
 *
 * <p>Messages supported (@see <a
 * href="https://tools.ietf.org/html/rfc4210#section-5.3">RFC4210 5.3
 * Operation-Specific Data Structures</a>):
 *
 * <p>Implemented: - 5.3.1 Initialization Request - will return an
 * Initialization Response (-&gt; CertRepMessage). - 5.3.9 Revocation Request /
 * Response (-&gt; RevRepContent) - 5.3.17 PKI Confirmation - same as
 * certificate confirmation accept - will return a PKI Confirmation Content
 * (-&gt; PKIConfirmContent) - 5.3.18 Certificate Confirmation - accept or
 * reject by client - will return a PKI Confirmation Content (-&gt;
 * PKIConfirmContent) - 5.3.3 Certificate Request / Response (-&gt;
 * CertRepMessage) - 5.3.5 Key Update Request / Response (-&gt; CertRepMessage)
 *
 * <p>Responses of type 'CertRepMessage' may contain additional CA certificates
 * in its 'caPubs' field which can be configured in the CMP configuration
 * ({@link CmpConfiguration#getResponseCaPubsCA(String)}.
 *
 * @version $Id: CmpMessageDispatcherSessionBean.java 28471 2018-03-12 15:11:18Z
 *     henriks $
 */
@Stateless(
    mappedName =
        JndiConstants.APP_JNDI_PREFIX + "CmpMessageDispatcherSessionRemote")
public class CmpMessageDispatcherSessionBean
  ...
  /**
   * The message may have been received by any transport protocol, and is passed
   * here in it's binary ASN.1 form.
   *
   * @param authenticationToken the authentication token.
   * @param pkiMessage DER encoded CMP message received from the client.
   * @param pkiHeader DER encoded PKI header of the original CMP message
   ...
   * @return A response message containing the CMP response message or null if
   *     there is no message to send back or some internal error has occurred
   */
  private ResponseMessage dispatch(
      final AuthenticationToken authenticationToken,
      final PKIMessage pkiMessage,
      final PKIHeader pkiHeader,
      final CmpConfiguration cmpConfiguration,
      final String cmpConfigurationAlias,
      final int levelOfNesting) {
    if (levelOfNesting > CmpMessageHelper.MAX_LEVEL_OF_NESTING) {
      return CmpMessageHelper.createUnprotectedErrorMessage(
          ...
          "Rejected request due to unreasonable level of nesting.");
    }
    ...
    try {
      ...
      if (LOG.isDebugEnabled()) {
        final String message =
            "Received CMP message with pvno="
                + pkiHeader.getPvno()
                + ", sender="
                + pkiHeader.getSender().toString()
                + ", recipient="
                + pkiHeader.getRecipient().toString()
                + System.lineSeparator()
                + "Cmp configuration alias: "
                + cmpConfigurationAlias
                + System.lineSeparator()
                + "The CMP message is already authenticated: "
                + authenticated
                + System.lineSeparator()
                + "Body is of type: "
                + tagno
      ...
      switch (tagno) {
        case PKIBody.TYPE_INIT_REQ:
          // 0: ir, Initialization Request and 2 (cr, Certification Req) are
          // both certificate requests
        case PKIBody.TYPE_CERT_REQ:
          // 2:
          handler =
              new CrmfMessageHandler(
                  authenticationToken,
                  cmpConfiguration,
                  cmpConfigurationAlias,
                  ejbBridgeSession,
          ...
          cmpMessage =
              new CrmfRequestMessage(
                  pkiMessage,
                  cmpConfiguration.getCMPDefaultCA(cmpConfigurationAlias),
                  cmpConfiguration.getAllowRAVerifyPOPO(cmpConfigurationAlias),
                  cmpConfiguration.getExtractUsernameComponent(
                      cmpConfigurationAlias));
          break;
        case PKIBody.TYPE_KEY_UPDATE_REQ:
          // 7: Key Update request (kur, Key Update Request)
          handler =
              new CrmfKeyUpdateHandler(
                  authenticationToken,
                  cmpConfiguration,
                  cmpConfigurationAlias,
                  ejbBridgeSession);
        ...
        case PKIBody.TYPE_NESTED:
          ...
          if (nestedMessage.verify()) {
            ...
            try {
              ...
              final PKIMessage nestedPkiMessage =
                  nestedPkiMessages.toPKIMessageArray()[0];
              return dispatch(
                  authenticationToken,
                  nestedPkiMessage,
                  pkiHeader,
      ...
      addAdditionalResponseCaPubsCertificates(
          authenticationToken,
          cmpConfiguration,
          cmpConfigurationAlias,
          cmpMessage);
      addAdditionalResponseExtraCertsCertificates(
          authenticationToken,
          cmpConfiguration,
          cmpConfigurationAlias,
          cmpMessage);
  ...
  /**
   * Adds the list of additional CA certificates to the user certificates
   * signing CA certificate to be returned with the CMP response
   * 'CertRepMessage.caPubs' field.
   *
   ...
   */
  private void addAdditionalResponseCaPubsCertificates(
  ...
  /**
   * Adds the list of additional CA certificates to the message signing CA
   * certificate returned with the outer PKI response message
   * 'PKIMessage.extraCerts' field.
   *
   * @param admin the authentication token.
   * @param cmpConfiguration the CMP configuration list.
   * @param alias the CMP configuration alias.
   * @param message the request message.
   */
  private void addAdditionalResponseExtraCertsCertificates(
      final AuthenticationToken admin,
      final CmpConfiguration cmpConfiguration,
      final String alias,
      final BaseCmpMessage message) {
    if (message != null) {
      ...
      message.setAdditionalExtraCertsCertificates(
          getCaCertificates(admin, casToAdd));
    }
  }
   ...
   */
  private List<Certificate> getCaCertificates(
      final AuthenticationToken admin, final String caListString) {
    ...
    if (StringUtils.isNotBlank(caListString)) {
      ...
      for (String ca : StringUtils.split(caListString, ";")) {
        ...
        try {
          ...
          if (cainfo != null
              && CollectionUtils.isNotEmpty(cainfo.getCertificateChain())) {
