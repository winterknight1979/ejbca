...
/**
 * Nested Message Content according to RFC4210. The PKI message is signed by an
 * RA authority. The PKIMessage body is another PKIMessage containing the
 * request to be processed.
 *
 * @version $Id: NestedMessageContent.java 28096 2018-01-25 05:34:38Z
 *     andresjakobs $
 */
public class NestedMessageContent extends BaseCmpMessage
    ...
    try {
      ...
      for (final X509Certificate cert : racerts) {
        ...
        if (raSignedMessage.getProtection() != null) {
          ...
          Signature sig =
              Signature.getInstance(algId, BouncyCastleProvider.PROVIDER_NAME);
  ...
  /**
   * Reads the files in cmpConfiguration.getRaCertificatePath() and returns them
   * as a list of certificates.
   *
   * <p>The certificate files should be PEM encoded.
   *
   * @param raCertsPath Path
   * @return A list of the certificates in
   *     cmpConfiguration.getRaCertificatePath() that could be read and parsed.
   */
