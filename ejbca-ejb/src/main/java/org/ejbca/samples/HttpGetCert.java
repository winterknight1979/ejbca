 ...
 * sample generates a certificate request and uses POST to the same servlet as
 * used from a browser through
 * http://127.0.0.1:8080/ejbca/publicweb/apply/apply_man.jsp. The servlet url
 * used in the example is http://127.0.0.1:8080/ejbca/publicweb/apply/certreq.
 * The certificate reply containing a PEM-formatted certificate is printed to
 * the screen.
 *
 * <p>NOTE: Support for SSL has been commented out in this sample, since it
 * requires JSSE. This sample class generates a PKCS10 request and POSTs to the
 * CAs web interface. The reply is received and printed to stdout. Takes
 * arguments:
 *
 * <ul>
 *   <li>requesturl - URL to the CA web (servlet where requests are POSTed),
...
public class HttpGetCert {
   ...
   * @return DOCUMENT ME!
   * @exception java.security.cert.CertificateException if the certificate is
   *     not correct.
   * @throws IllegalArgumentException if webcert is not a self-signed
   *     certificate
   */
  ...
  */
   ...
   * @exception IllegalArgumentException if requesturl is not a vlid HTTP or
   *     HTTPS url.
   * @exception Exception if the trusted webcert is not a correct certificate.
   * @exception Exception if we get back a HTTP response code != 200 from the
   *     CA.
  ...
  public static void main(final String[] args) throws Exception {
    ...
    PKCS10CertificationRequest req =
        CertTools.genPKCS10CertificationRequest(
            "SHA1WithRSA",
            CertTools.stringToBcX500Name("C=SE,O=AnaTom,CN=HttpTest"),
            rsaKeys.getPublic(),
            new DERSet(),
            rsaKeys.getPrivate(),
            null);
