...
/**
 * Servlet to authenticate a user. Simple database using a file to keep users in
 * format: instance;username;password;DN DN is in form:
 * dn-c:dn-o:dn-ou:dn-ln:dn-gn:dn-cn where parts can be left out as desired.
 ...
 * </ul>
 *
 * <p>Returns a logic token stating that user is authenticated followed by the
 * information to use for this user's certificate.
 *
...
public class RemoteVerifyServlet extends HttpServlet {
  ...
  /**
   * Basic structure containing users. Top level keyed on instance gives new
   * HashMap keyed on username with String[] = { password, result } as data.
   */
   ...
   *
   * <p>Can be controlled via properties file.
   */
   ...
   */
  void addUserDataToResult(
      final AuthResult result, final String dnPartsString) {
    if (dnPartsString == null) {
      return;
    }

    Enumeration<Object> dnParts =
        new StringTokenizer(dnPartsString, DNPART_DELIMITER);
    ...
    while (dnParts.hasMoreElements()) {
      ...
      String dnValue =
          dnPart.substring(separatorPosition + 1); // skip separator
  ...
  /**
   * Authenticate a user given a querystring. <b>This is the only method a
   * customer should have to rewrite/override.</b>
   *
   ...
   */
  protected AuthResult authenticateUser(
   ...
   * @param e DOCUMENT ME!
   */
  protected void errorLog(final String s, final java.lang.Exception e) {
    log.error(s, e);
  }
   ...
   *
   * <p>Returns one of the following cases. (Apart from status being the first
   * line, order is not specified.)
   *
   * <ul>
   *   <li>A granted reply:
   *       <pre>
   ...
   * @exception IOException when something fails with basic I/O, such as
    ...
    try {
      ...
      if (dotAt == -1) {
        ...
        try {
          ...
          errorLog(
              "doPost: Got "
                  ...
                  + " for username '"
                  + username
                  + "'. Asuming version is OK. Tried to parse '"
                  + version
                  + "'");
        }
        ...
        try {
          ...
          minorversion =
              Integer.parseInt(version.substring(1 + dotAt, version.length()));
        } catch (NumberFormatException nfe) {
          errorLog(
              "doPost: Got "
                  + nfe
      ...
      if ((majorversion == PROTOCOL_VERSION_MAJOR)
        ...
        out.println(
            "message=Accepting at most "
                + PROTOCOL_VERSION_MAJOR
                + "."
                + PROTOCOL_VERSION_MINOR);
        errorLog(
            "PROTOCOL MISSMATCH. Got '"
                + version
                + "', but accepts only '"
                + PROTOCOL_VERSION_MAJOR
                + "."
   ...
   * @return <b>null</b> (if no user found) or String[] with [0] as passwd and
   *     [1] as certificate contents.
   */
  protected String[] findUserData(final String username) {
    if (users == null) {
      debugLog(
          "findUserData: No users found. Returning null for user '"
              + username
              + "'.");
    ...
    if (result != null) {
      ...
      debugLog(
          "findUserData: No information for user '" + username + "'found.");
    }
  ...
  /**
   * Load user DB at servlet load time, ie first access to servlet. It's ok to
   * call this method multiple times, since it simply clears the old cached data
  ...
  protected synchronized void loadUserDB() {
    ...
    debugLog(
        (new Date()).toString()
            + "loadUserDB: Loading from file: '"
            + getInitParameter("dbfilename")
            + "'.");

    InputStream is =
        getServletContext().getResourceAsStream(getInitParameter("dbfilename"));
    ...
    try {
      while (readMore) {
        ...
        if (line == null) {
          ...
          if (!line.startsWith(LINE_COMMENT)) {
            Enumeration<Object> lineParts =
                new StringTokenizer(line, RECORD_SEPARATOR);
            ...
            StringTokenizer st =
                new StringTokenizer(userDataString, DNPART_DELIMITER);
            ...
            debugLog(
                "loadUserDB: calling addUserData." + Arrays.toString(userData));
      ...
      errorLog(
          "loadUserDB: FAILED TO PARSE FILE: '"
              + getInitParameter("dbfilename")
              + "'.");
   ...
   *
   * @param username user to lookup.
   * @param userData String[] with [0] as passwd and [1] as certificate
   *     contents.
   */
  protected void addUserData(final String username, final String[] userData) {
    if (users == null) {
