...
/**
 * This class aims to be DN representation. It offers facilities to merge two
 * DN.
 *
...
public class DistinguishedName extends LdapName {
   ...
   */
  public DistinguishedName(final List<Rdn> rdns) {
    super(rdns);
  }
   ...
   */
  public DistinguishedName(final String name) throws InvalidNameException {
    super(name);
  }

  /**
   * Get a relative Distinguished Name. this method get the first occurence in
   * the case of mulitple.
   *
   * @param type type of the DN.
   * @return the requested DN.
   */
  public Rdn getRdn(final String type) {
   ...
   * @return the requested DN.
   */
  public Rdn getRdn(final String type, final int index) {
    return getRdn(this, type, index);
  }

  /**
   * Merge this DN with another provided DN. The result is a new DN resulting
   * from the merge of this DN and the
   *
   ...
   */
  public DistinguishedName mergeDN(
    ...
    for (ListIterator<Rdn> it = getRdns().listIterator(getRdns().size());
      ...
      if (providedRdnIteratorsMap.containsKey(localRdn.getType())
        ...
        Rdn providedRdn =
            (Rdn) (providedRdnIteratorsMap.get(localRdn.getType())).next();
        if (override) {
        ...
        if (useEntityEmailField && override) {
          ...
          if (finded) {
            ...
            try {
              localRdns.add(
                  new Rdn(
                      translateComponentName(localRdn.getType().toUpperCase()),
                      value));
    ...
    // loop on all remaining provided components and add them at the end of the
    // dn :
   ...
   * @return the requested DN.
   */
  public static Rdn getRdn(final DistinguishedName dn, final String type) {
   ...
   * @return the requested DN.
   */
  public static Rdn getRdn(
      final DistinguishedName dn, final String type, final int index) {
   ...
   */
  public String translateComponentName(final String name) {
