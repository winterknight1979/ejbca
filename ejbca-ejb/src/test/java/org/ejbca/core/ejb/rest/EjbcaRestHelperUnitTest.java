...
public class EjbcaRestHelperUnitTest {
  ...
  public void shouldFailOnFindingCertificateAuthorityByName() throws Exception {
    ...
    AuthenticationToken authenticationToken =
        EasyMock.mock(AuthenticationToken.class);

    expect(
            caSessionBean.getCAInfo(
                authenticationToken, certificateAuthorityName))
        .andReturn(null);
    replay(caSessionBean);
    EnrollPkcs10CertificateRequest request =
        new EnrollPkcs10CertificateRequest.Builder()
  ...
  public void shouldFailOnFindingEndEntityProfileByName() throws Exception {
    ...
    expect(
        ...
        .andReturn(1);
    expect(
            endEntityProfileSessionBean.getEndEntityProfileId(
                endEntityProfileName))
        .andThrow(new EndEntityProfileNotFoundException());
  ...
  public void shouldConvertToCorrectEndEntityInformation() throws Exception {
    ...
    expect(
            certificateProfileSessionBean.getCertificateProfileId(
                certificateProfileName))
        .andReturn(certificateProfileId);
    expect(
            endEntityProfileSessionBean.getEndEntityProfileId(
                endEntityProfileName))
        .andReturn(endEntityProfileId);
    expect(endEntityProfileSessionBean.getEndEntityProfile(endEntityProfileId))
        .andReturn(endEntityProfile);
    expect(endEntityProfile.useAutoGeneratedPasswd()).andReturn(false);
    expect(endEntityProfile.getValue(EndEntityProfile.SENDNOTIFICATION, 0))
        .andReturn(EndEntityProfile.TRUE);
    ...
    // then
    assertEquals(
        "The injected endentity profile id in 'given' section didnt get"
            + " converted into result properly",
        endEntityProfileId,
        endEntityInformation.getEndEntityProfileId());

    assertEquals(
        "The injected certificate profile id in 'given' section didnt get"
            + " converted into result properly",
        certificateProfileId,
        endEntityInformation.getCertificateProfileId());

    assertEquals(
        "End entiy DN got incorrectly parsed pkcs10 certificate request",
        "C=EE,ST=Alabama,L=tallinn,O=naabrivalve,CN=hello123server6",
  ...
  public void shouldParseCorrectDn() {
    PKCS10CertificationRequest pkcs10CertificateRequest =
        CertTools.getCertificateRequestFromPem(csr);
    String actualResult = testClass.getSubjectDn(pkcs10CertificateRequest);
    String expectedResult =
        "C=EE,ST=Alabama,L=tallinn,O=naabrivalve,CN=hello123server6";
    assertEquals(
        "End entiy DN got incorrectly parsed from pkcs10 certificate request",
        expectedResult,
        actualResult);
  }
  /**
  ...
  public void shouldParseCorrectAn() {
    ...
    String expectedResult =
        "dNSName=somedns.com, iPAddress=192.168.1.7,"
            + " dNSName=some.other.dns.com, directoryName=CN=Test\\,L=XX";
    assertEquals(
        "Subject AN got incorrectly parsed from pkcs10 certificate request",
        expectedResult,
        actualResult);
  }
}
