...
public class TestDatafields extends TestCase implements CVCTest {
  ...
  public void testParseError() throws Exception {
    try {
      CertificateParser.parseCertificate(
          new byte[] {'1', '2', '3', '4', '5'}); // Something unparseable
  ...
  public void testLengthEncoding() throws Exception {
    ...
    assertTrue(
        "Encoding of length " + len, Arrays.equals(ENCODED_LENGTH_7, derData));

    len = 127;
    derData = CVCObject.encodeLength(len);
    assertTrue(
        "Encoding of length " + len,
        Arrays.equals(ENCODED_LENGTH_127, derData));

    len = 128;
    derData = CVCObject.encodeLength(len);
    assertTrue(
        "Encoding of length " + len,
        Arrays.equals(ENCODED_LENGTH_128, derData));
  ...
  private int readLength(final byte[] buf) throws IOException {
  ...
  public void testArrayTrim() throws Exception {
    ...
    byte[] data4 =
        new byte[] {0x10, 0x7A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  ...
  public void testAuthorizationField() throws Exception {
    ...
    assertTrue(
        "rights did not include Write-DG17",
        rightsAT.getFlag(AccessRightAuthTerm.BIT_WRITE_DG17));
    assertFalse(
        "rights incorrectly included bit 36",
        rightsAT.getFlag(AccessRightAuthTerm.BIT_WRITE_DG18));
    assertFalse(
        "rights incorrectly included bit 2",
        rightsAT.getFlag(AccessRightAuthTerm.BIT_COMMUNITY_ID_VERIFICATION));
    assertTrue(
        "rights did not include Age Verification",
    ...
    assertTrue(
        ...
        auth4.getAuthRole().isSignatureTerminal());
  }
  ...
  public void testHolderReference() throws Exception {
    try {
      ...
      throw new Exception(
          "Empty countryCode should throw IllegalArgumentException!");
    ...
    try {
      ...
      throw new Exception(
          "Too long countryCode should throw IllegalArgumentException!");
    } catch (IllegalArgumentException e) {
      // This is expected
    }
    ...
    try {
      ...
      throw new Exception(
          "Too short countryCode should throw IllegalArgumentException!");
    } catch (IllegalArgumentException e) {
      // This is expected
    }
    ...
    try {
      ...
      throw new Exception(
          "Bad countryCode should throw IllegalArgumentException!");
    } catch (IllegalArgumentException e) {
      // This is expected
    }
    ...
    try {
      ...
      throw new Exception(
          "Empty mnemonic should throw IllegalArgumentException!");
    } catch (IllegalArgumentException e) {
      // This is expected
    }
    ...
    try {
      ...
      throw new Exception(
          "Empty sequence should throw IllegalArgumentException!");
    } catch (IllegalArgumentException e) {
      // This is expected
    }
  ...
  public void testIntegerField() throws Exception {
    try {
      new IntegerField(
          CVCTagEnum.PROFILE_IDENTIFIER, new byte[] {1, 2, 3, 4, 5});
      throw new Exception(
          "Too long byte array should throw IllegalArgumentException");
    } catch (IllegalArgumentException e) {
      // This is expected
    }
  ...
  public void testDateField() throws Exception {
    ...
    // Compare strings to avoid problems with seconds, minutes from current time
    // etc
    ...
    long millis =
        cal.getTime()
            .getTime(); // the millis from GMT (with only the date) that we want
                        // in a decoded time
  ...
  public void testOIDField() throws Exception {
    ...
    // First two numbers in the OID are encoded as 40*i1 + i2, the rest as i3,
    // i4, ...
