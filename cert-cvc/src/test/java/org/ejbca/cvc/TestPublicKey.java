 ...
 * @throws Exception fail*/
  public void testPubliKeyField() throws Exception {
    ...
    PublicKeyRSA rsa1 =
        (PublicKeyRSA)
            KeyFactory.createInstance(keyPair.getPublic(), "SHA1WITHRSA", null);
    byte[] der = rsa1.getEncoded();

    CVCObject cvcObj = CertificateParser.parseCVCObject(der);
    assertTrue(
        "Parsed array was not a PublicKeyRSA",
        (cvcObj instanceof PublicKeyRSA));

    RSAPublicKey rsaKey = (RSAPublicKey) keyPair.getPublic();

    RSAPublicKey rsa2 =
        (RSAPublicKey) cvcObj; // This casting should be successful
    assertEquals("Key modulus", rsaKey.getModulus(), rsa2.getModulus());
    assertEquals(
        "Key exponent", rsaKey.getPublicExponent(), rsa2.getPublicExponent());
    assertEquals("Key algorithm", "RSA", rsa2.getAlgorithm());

    PublicKeyRSA rsa3 = (PublicKeyRSA) rsa2;
    assertEquals(
        "OIDs", rsa1.getObjectIdentifier(), rsa3.getObjectIdentifier());
  }
 ...
 * @throws Exception fail
  ...
  public void testParseAndCreate() throws Exception {
    byte[] keydata =
        FileHelper.loadFile(
            new File("./src/test/resources/PUBLIC_KEY_RSA1024.cvc"));
    CVCPublicKey publicKey1 =
        (CVCPublicKey) CertificateParser.parseCVCObject(keydata);
    ...
    assertEquals(
        "Public keys as text differ",
        publicKey1.getAsText(""),
        publicKey2.getAsText(""));
  }
  ...
  public void testModulusValue() throws Exception {
    ...
    PublicKeyRSA rsaKey =
        (PublicKeyRSA)
            KeyFactory.createInstance(keyPair.getPublic(), "SHA1WITHRSA", null);
    byte[] modulusData =
        ((ByteField) rsaKey.getSubfield(CVCTagEnum.MODULUS)).getData();
 ...
 * @throws Exception fail */
  public void testPublicKeyEC() throws Exception {
    ...
    ECPoint decodedPoint =
        PublicKeyEC.decodePoint(
            new BigInteger(expectedByteStr, 16).toByteArray());
    assertEquals("AffineX", affineX, decodedPoint.getAffineX());

    // Test with a point with an affineY that is one byte shorted than the
    // affineX, should be 0 padded on the left
    String x =
        "9FDAB8773ADEE1735BB58E8D0A81C29924EC3F94D9F4B182E887CBDC7CDDD357";
    ...
    // affineY, should be 0 padded on the left
    ...
    PublicKeyEC ecKey =
        (PublicKeyEC)
            KeyFactory.createInstance(
                keyPair.getPublic(), "SHA1WITHECDSA", null);
 ...
 * @throws Exception Fail
  ...
  public void testPublicKeyECFields() throws Exception {
    ...
    CVCObject cvcObjIS =
        CertificateParser.parseCVCObject(bodyIS.getDEREncoded());
    CVCObject cvcObjCVCA =
        CertificateParser.parseCVCObject(bodyCVCA.getDEREncoded());
    assertTrue(
        "CVCObj not a CVCertificateBody",
        cvcObjIS.getTag() == CVCTagEnum.CERTIFICATE_BODY);

    // IS certificate must contain only two EC public key subfields
    PublicKeyEC ecKey1 =
        (PublicKeyEC) ((CVCertificateBody) cvcObjIS).getPublicKey();
    assertEquals(
        "Number of PublicKey subfields", 2, ecKey1.getSubfields().size());

    // CVCA certificate must contain all eight EC public key subfields
    PublicKeyEC ecKey2 =
        (PublicKeyEC) ((CVCertificateBody) cvcObjCVCA).getPublicKey();
    assertEquals(
        "Number of PublicKey subfields", 8, ecKey2.getSubfields().size());
  ...
  private CVCertificateBody createBody(final AuthorizationRoleEnum roleEnum)
      throws Exception {
    KeyPair keyPair = createECKeyPair();
    PublicKeyEC ecKey =
        (PublicKeyEC)
            KeyFactory.createInstance(
                keyPair.getPublic(), "SHA1WITHECDSA", roleEnum);
