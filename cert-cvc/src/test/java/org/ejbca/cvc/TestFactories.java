...
public class TestFactories extends TestCase implements CVCTest {
  ...
  public void testFieldFactory() throws Exception {
    AbstractDataField field =
        FieldFactory.decodeField(
            CVCTagEnum.COFACTOR_F, new byte[] {0x01, 0x01, 0x01, 0x01});
    ...
    OIDField oid2 =
        (OIDField) FieldFactory.decodeField(CVCTagEnum.OID, oid1.getEncoded());
    ...
    try {
      ...
      throw new Exception(
          "A sequence-type tag should throw IllegalArgumentException");
  ...
  public void testKeyFactory() throws Exception {
    ...
    try {
      ...
      throw new Exception(
          "Incomplete instance of GenericPublicKeyField should throw"
              + " ParseException");
    ...
    genKey.addSubfield(
        new ByteField(CVCTagEnum.MODULUS, modulus.toByteArray()));
    genKey.addSubfield(
        new ByteField(CVCTagEnum.EXPONENT, modulus.toByteArray()));

    // Create public key object and compare
    CVCPublicKey cvcPubkey = KeyFactory.createInstance(genKey);
    assertTrue(
        "cvcPubkey not a PublicKeyRSA", (cvcPubkey instanceof PublicKeyRSA));
    ...
    CVCPublicKey cvcKey2 =
        KeyFactory.createInstance(keyPair.getPublic(), algorithm, null);
    assertTrue(
        "cvcKey not instanceof PublicKeyRSA",
        (cvcKey2 instanceof PublicKeyRSA));

    PublicKeyRSA rsa2 = (PublicKeyRSA) cvcKey2;
    assertEquals(
        "Algorithm name differs",
        algorithm,
        AlgorithmUtil.getAlgorithmName(rsa2.getObjectIdentifier()));

    // This should return the exact same instance
    CVCPublicKey cvcPubkey2 =
        KeyFactory.createInstance(cvcPubkey, "DummyAlgorithm", null);
    assertTrue(
  ...
  public void testSequenceFactory() throws Exception {
    ...
    assertTrue(
        "seq not instance of CVCertificate", (seq instanceof CVCertificate));

    try {
      SequenceFactory.createSequence(CVCTagEnum.EXPIRATION_DATE);
      throw new Exception(
          "A datafield-type tag should throw IllegalArgumentException");
    } catch (IllegalArgumentException e) {
      // This is expected
    }
  }
