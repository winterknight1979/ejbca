 ...
 * @throws Exception fail*/
  public void testAlgorithmUtil() throws Exception {
    ...
    assertEquals(
        "OID values not equal", oid.getValue(), "0.4.0.127.0.7.2.2.2.2.2");

    String algorithmName2 = AlgorithmUtil.getAlgorithmName(oid);
    assertEquals(
        "algorithm names not equal",
        algorithmName1.toUpperCase(),
        algorithmName2);

    try {
      AlgorithmUtil.getOIDField("NonExistingAlgorithm");
      throw new Exception(
          "Illegal algorithm name should throw IllegalArgumentException");
