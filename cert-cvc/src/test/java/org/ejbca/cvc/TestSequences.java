 ...
 * @throws Exception fail*/
  public void testAddUnexpectedSubfield() throws Exception {
    ...
    try {
      AbstractDataField subfield =
          new DateField(CVCTagEnum.EFFECTIVE_DATE, new Date());
      generic.addSubfield(subfield);
      throw new Exception(
          "Field is not allowed in GenericPublicKeyField: "
              + subfield.getTag());
 ...
 * @throws Exception fail*/
  public void testAddSubfieldTwice() throws Exception {
    ...
    AbstractDataField subfield =
        new ByteField(
            CVCTagEnum.MODULUS, new byte[] {1, 2, 3, 4, 5, 6, 7, 8, 9});
    generic.addSubfield(subfield);
    try {
      // This shouln't work since MODULUS has already been added
      generic.addSubfield(subfield);
      throw new Exception(
          "Cannot add same field typ again: " + subfield.getTag());
    } catch (ConstructionException e) {
      // This is expected
    }
  }
 ...
 * @throws Exception fail
  ...
  public void testEncodeAuthorizationTemplate() throws Exception {
    CVCAuthorizationTemplate authTemplate =
        new CVCAuthorizationTemplate(
            AuthorizationRoleEnum.IS, AccessRightEnum.READ_ACCESS_DG3);
