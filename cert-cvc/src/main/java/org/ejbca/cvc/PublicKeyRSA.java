 ...
 * @throws ConstructionException fail
  ...
  PublicKeyRSA(final GenericPublicKeyField genericKey)
    ...
    ByteField modulusField =
        (ByteField) genericKey.getSubfield(CVCTagEnum.MODULUS);
 ...
 * @throws ConstructionException Fail
  ...
  PublicKeyRSA(final OIDField oid, final RSAPublicKey rsaKey)
    ...
    addSubfield(
        new ByteField(
            CVCTagEnum.MODULUS,
            trimByteArray(rsaKey.getModulus().toByteArray()),
            true));
    addSubfield(
        new ByteField(
            CVCTagEnum.EXPONENT,
            trimByteArray(rsaKey.getPublicExponent().toByteArray())));
  }
