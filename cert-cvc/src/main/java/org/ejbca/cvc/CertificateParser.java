...
public final class CertificateParser {
  ...
  private static CVCObject decode(
      final byte[] data, final CVCTagEnum expectedTag)
      throws ParseException, ConstructionException {
  ...
  private static CVCObject decode(
      ...
      final CVCTagEnum expectedTag,
      final CVCTagEnum tagIfAmbiguous)
    ...
    if (expectedTag != null && tag != expectedTag) {
      throw new ParseException(
          "Expected first tag " + expectedTag + " but found " + tag);
    }
    // Special handling for certain tags that have the same value, e.g.
    // ARBITRARY_DATA and ROLE_AND_ACCESS_RIGHTS
    ...
    if (tag.isSequence()) {
      ...
      while (din.available() > sequenceEnd) {
        // Special handling for ARBITRARY_DATA which has the same value as
   ...
   * be returned.
   */
  private static CVCTagEnum findTagFromValue(final int tagvalue)
    ...
    if (wantedType != null) {
      ...
      throw new ParseException(
          "Unknown CVC tag value " + Integer.toHexString(tagvalue));
    }
  }
