...
public class CVCAuthenticatedRequest extends AbstractSequence
  ...
  public byte[] getTBS() throws ConstructionException {
    try {
      ...
      // "The signature SHALL be created over the concatenation of the encoded
      // CV
      // Certificate
      // and the encoded Certification Authority Reference (i.e. both including
      // tag
      // and length)."
  ...
  public void verify(final PublicKey pubKey)
    ...
    try {
      ...
      if (pubKey instanceof CVCPublicKey) {
        // If pubKey is an instance of CVCPublicKey then the algorithm can be
        // extracted
        // using its OID
        CVCPublicKey cvcKey = (CVCPublicKey) pubKey;
        algorithm =
            AlgorithmUtil.getAlgorithmName(cvcKey.getObjectIdentifier());
      } else {
        // Otherwise we assume that the inner signature is calculated using the
        // same
