...
public class WebServiceConfiguration {
   ...
   * authorized denied exception.
   *
   * @return the id of the approval profile (not the name as that can be
   *     changed), or -1 if approvals shall not be used (setting is empty)
   * @throws IllegalArgumentException if the value in jaxws.approvalprofileid is
   *     set, but not an integer
   */
  public static int getApprovalProfileId() {
    int id = -1;
    final String idString =
        EjbcaConfigurationHolder.getString("jaxws.approvalprofileid");
    if (!StringUtils.isEmpty(idString)) {
      try {
        id = Integer.valueOf(idString);
      } catch (NumberFormatException e) {
        throw new IllegalArgumentException(
            "Invalid value in jaxws.approvalprofileid, must be an integer.", e);
      }
    }
    return id;
  }
    ...
    return "true"
        .equalsIgnoreCase(
            EjbcaConfigurationHolder.getExpandedString(
                "jaxws.noauthonfetchuserdata"));
  }

  /**
   * If this is true, all certificates on the old token will be put on hold. If
   * this is false, MS SmartCard Logon certificates will not be put on hold.
  ...
  public static boolean getTransactionLogEnabled() {
    return "true"
        .equalsIgnoreCase(
            EjbcaConfigurationHolder.getExpandedString("ejbcaws.trx-log"));
  }
  ...
  /**
   * Returns the the base string that will be substituted during transaction
   * logging.
   *
   * @return Order
   */
