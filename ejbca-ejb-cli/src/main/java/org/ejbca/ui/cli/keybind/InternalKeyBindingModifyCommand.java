 ...
 * See getDescription().
 *
 * @version $Id: InternalKeyBindingModifyCommand.java 27740 2018-01-05 07:24:53Z
 *     mikekushner $
 */
public class InternalKeyBindingModifyCommand
  ...
  public CommandResult executeCommand(
    ...
    final InternalKeyBinding internalKeyBinding =
        internalKeyBindingMgmtSession.getInternalKeyBinding(
            getAdmin(), internalKeyBindingId.intValue());
    ...
    final List<InternalKeyBindingTrustEntry> removeTrustList =
          ...
          if (value == null) {
            removeTrustList.add(
                new InternalKeyBindingTrustEntry(
                    Integer.valueOf(caInfo.getCAId()), null));
          } else {
            try {
              final BigInteger serialNumber = new BigInteger(value, 16);
              removeTrustList.add(
                  new InternalKeyBindingTrustEntry(
                      Integer.valueOf(caInfo.getCAId()), serialNumber));
            } catch (NumberFormatException e) {
              getLogger()
                  .info(
                      " Ignoring trustEntry with invalid certificate serial"
                          + " number: "
                          + value);
            }
          }
    ...
    final List<InternalKeyBindingTrustEntry> addTrustList =
        new ArrayList<InternalKeyBindingTrustEntry>();
    ...
    if (nextKeyPairAlias != null) {
      if ("null".equals(nextKeyPairAlias)) {
        ...
        final List<String> availableKeyPairAliases =
            EjbRemoteHelper.INSTANCE
                .getRemoteSession(CryptoTokenManagementSessionRemote.class)
                .getKeyPairAliases(
                    getAdmin(), internalKeyBinding.getCryptoTokenId());
        if (internalKeyBinding.getKeyPairAlias().equals(nextKeyPairAlias)) {
          getLogger()
              .info(
                  " Ignoring --nextkeypair with value "
                      + nextKeyPairAlias
                      + ". The value is already used as the current"
                      + " keyPairAlias.");
          ...
          getLogger()
              .info(
                  " Ignoring --nextkeypair with value "
                      + nextKeyPairAlias
                      + ". The alias is not present in the bound CryptoToken."
                      + " Available aliases: "
                      + availableKeyPairAliases.toString());
        } else {
          getLogger()
              .info(
                  " Setting nextKeyPairAlias to \"" + nextKeyPairAlias + "\".");
    ...
    for (final InternalKeyBindingTrustEntry internalKeyBindingTrustEntry :
      ...
      if (!internalKeyBindingTrustEntries.remove(
        ...
        getLogger()
            .info(
                " Unable to remove non-existing trustEntry: "
                    + internalKeyBindingTrustEntry.toString());
      } else {
        getLogger()
            .info(
                " Removed trustEntry: "
    ...
    for (final InternalKeyBindingTrustEntry internalKeyBindingTrustEntry :
      ...
      if (internalKeyBindingTrustEntries.contains(
        ...
        getLogger()
            .info(
                " Unable to add existing trustEntry: "
                    + internalKeyBindingTrustEntry.toString());
      } else {
    ...
    Map<String, Serializable> validatedProperties =
        validateProperties(
            internalKeyBinding.getImplementationAlias(), propertyMap);
    if (validatedProperties == null) {
      return CommandResult.FUNCTIONAL_FAILURE;
    }
    for (Entry<String, Serializable> entry : validatedProperties.entrySet()) {
      DynamicUiProperty<? extends Serializable> oldProperty =
          internalKeyBinding.getProperty(entry.getKey());
    ...
    if (modified) {
      internalKeyBindingMgmtSession.persistInternalKeyBinding(
          getAdmin(), internalKeyBinding);
      getLogger()
          .info(
              "InternalKeyBinding with id "
                  + internalKeyBindingId.intValue()
                  + " modified successfully.");
      return CommandResult.SUCCESS;
    } else {
      getLogger()
          .error(
              "No changes were made to InternalKeyBinding with id "
                  + internalKeyBindingId.intValue()
                  + ".");
      return CommandResult.FUNCTIONAL_FAILURE;
    }
  ...
  public String getFullHelpText() {
    ...
    sb.append(
        getCommandDescription()
            + "\n\n"
            + "Optional Type specific properties are listed below and are"
            + " written as -propertyname=value, e.g."
