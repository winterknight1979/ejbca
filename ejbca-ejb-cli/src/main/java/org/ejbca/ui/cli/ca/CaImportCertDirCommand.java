...
public class CaImportCertDirCommand extends BaseCaAdminCommand {
...

  {
    registerParameter(
        new Parameter(
            ...
            "\"DN\" means use certificate's SubjectDN as username, \"CN\""
                + " means use certificate subject's common name as username"
                + " and \"FILE\" means user the file's name as username"));
    registerParameter(
        new Parameter(
            CA_NAME_KEY,
            "CA Name",
    ...
    registerParameter(
        new Parameter(
            ...
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "Directory of PEM certificates, which must be PEM encoded"));
    registerParameter(
        new Parameter(
            EE_PROFILE_KEY,
            "Profile Name",
    ...
    registerParameter(
        Parameter.createFlag(
            RESUME_ON_ERROR_KEY,
            "Set if the import should resume in case of errors, or stop on"
    ...
    registerParameter(
        new Parameter(
            ...
            StandaloneMode.FORBID,
            ParameterMode.ARGUMENT,
            "Revocation reason, if the certificates are to be imported as"
    ...
    registerParameter(
        new Parameter(
            ...
            StandaloneMode.FORBID,
            ParameterMode.ARGUMENT,
            "Revocation time, if the certificates are to be imported as"
                + " revoked. Will be set to the current time if this option is"
                + " not set. Format is "
                + DATE_FORMAT
  ...
  public CommandResult execute(final ParameterContainer parameters) {
    ...
    // Parse arguments into more coder friendly variable names and validate
    // switches
    ...
    final int threadCount =
        parameters.get(THREAD_COUNT) == null
            ? 1
    ...
    try {
      ...
      // Get all files in the directory to import from and try to read and
      ...
      final Queue<Future<CertificateImporter.Result>> futures =
          new LinkedList<>();
  ...
  public String getCommandDescription() {
    return "Imports a directory with PEM encoded certficate file(s) to the"
               + " database.";
  }
  ...
  public String getFullHelpText() {
    ...
    sb.append(
        "The import will proceed as long as the only error encountered is a"
            + " mismatch between the signer of end entity certificate and the"
            + " specified certification authority. If the -resumeonerror"
            + " options is provided, the import will resume even in case of"
            + " more critical errors (such as violation of end entity"
            + " constraints, malformed files etc). The offending files will be"
            + " printed-out to the standard output, and a summary will contain"
            + " exact numbers on imported and failed certificates.\n\n");
    // List available CAs by name
    sb.append("Available CAs: " + getAvailableCasString() + "\n");
    sb.append(
        "Available end entity profiles: "
            + getAvailableEepsString(AccessRulesConstants.CREATE_END_ENTITY)
            + "\n");
    sb.append(
        "Available certificate profiles: "
            + getAvailableEndUserCpsString()
            + "\n");
