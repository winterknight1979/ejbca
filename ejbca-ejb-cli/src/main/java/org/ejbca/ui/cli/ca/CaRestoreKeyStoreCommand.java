 ...
 *
 * @version $Id: CaRestoreKeyStoreCommand.java 20925 2015-03-18 19:24:38Z
 *     mikekushner $
 */
...

  {
    ...
    registerParameter(
        new Parameter(
            ...
            ParameterMode.ARGUMENT,
            "Alias for the Signature key. If left out the existing alias will"
                + " be used, and if multiple are available a list will be"
                + " displayed."));
    registerParameter(
        new Parameter(
            ENC_ALIAS_KEY,
            ...
            StandaloneMode.FORBID,
            ParameterMode.ARGUMENT,
            "Alias for the Encryption key. If left out the existing alias will"
                + " be used, and if multiple are available a list will be"
                + " displayed."));
    registerParameter(
        new Parameter(
    ...
    if (alias == null) {
      ...
      try {
        ...
        throw new IllegalStateException(
            "Algorithm used to check the integrity of the keystore could npt"
                + " be found.",
            e);
      } catch (CertificateException e) {
        throw new IllegalStateException(
            "Certificate could not be loaded from keystore.", e);
      } catch (IOException e) {
