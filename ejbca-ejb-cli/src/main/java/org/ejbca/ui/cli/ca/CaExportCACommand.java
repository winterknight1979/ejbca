...
public class CaExportCACommand extends BaseCaAdminCommand {
...

  {
    ...
    registerParameter(
        new Parameter(
            ...
            ParameterMode.ARGUMENT,
            "They keystore password. If not set then it will be prompted"
                + " for."));
    registerParameter(
        new Parameter(
            SIGNATURE_ALIAS_KEY,
            ...
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "The signature key alias. Will default to "
                + SIGNATURE_ALIAS_DEFAULT
                + " if not set."));
    registerParameter(
        new Parameter(
            ...
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "The encryption key alias. Will default to "
                + ENCRYPTION_ALIAS_DEFAULT
                + " if not set. "));
  }
  ...
  public CommandResult execute(final ParameterContainer parameters) {
    ...
    if ((signatureKeyAlias == null
            ...
            && parameters.isStandalone(ENCRYPTION_ALIAS_KEY))
  ...
  public String getFullHelpText() {
    return getCommandDescription()
        + "X.509 CAs are exported as PKCS#12 files while for CVC CAs only the"
        + " private certificate signing key is exported as a PKCS#8 key. You"
        + " will be prompted for keystore password to protect stored keystore,"
        + " but can optionally specify it on command line using the optional"
        + " argument '-kspassword <password>'.\n\n"
        + "Do not set only one of SignatureKeyAlias or EncryptionKeyAlias"
        + " implicitely (without a switch). Either set both, or if only doing"
        + " one then with a switch. Setting only one without a switch may lead"
