...
public class ChangeRuleCommand extends BaseRolesCommand {
...

  {
    ...
    registerParameter(
        Parameter.createFlag(
            RECURSIVE_KEY,
            "(Deprecated) Set this switch if rule is to be recursive. Default"
                + " is false."));
    registerParameter(
        new Parameter(
            ROLE_NAMESPACE_KEY,
  ...
  public CommandResult execute(final ParameterContainer parameters) {
    ...
    try {
      final String resourceName =
          AccessRulesHelper.normalizeResource(parameters.get(RULE_KEY));
      final String resource =
          super.getResourceNameToResourceMap().get(resourceName);
      if (resource == null) {
        getLogger()
            .error(
                "ERROR: No resource with name '"
                    + resourceName
                    + "' is available.");
        return CommandResult.FUNCTIONAL_FAILURE;
      }
      final AccessRuleState rule =
          AccessRuleState.matchName(parameters.get(STATE_KEY));
      if (rule == null) {
        getLogger()
            .error(
                "ERROR: No such state \"" + parameters.get(STATE_KEY) + "\".");
        return CommandResult.FUNCTIONAL_FAILURE;
      }
      if (parameters.containsKey(RECURSIVE_KEY)) {
        // Be nice and log, but don't fail existing scripts depending on this
        // command
        getLogger()
            .warn(
                "Setting "
                    + RECURSIVE_KEY
                    + " is not needed from EJBCA 6.8.0, since all rules are"
                    + " always recursive.");
      }
      ...
      if (rule == AccessRuleState.RULE_NOTUSED) {
        if (role.getAccessRules().remove(resource) == null) {
          getLogger()
              .info("No rule for resource '" + resourceName + "' found.");
        }
      } else if (rule == AccessRuleState.RULE_ACCEPT) {
        if (role.getAccessRules().put(resource, Role.STATE_ALLOW) == null) {
          getLogger()
              .info("Added accept rule for resource '" + resourceName + "'.");
        } else {
          getLogger()
              .info(
                  "Replaces existing access rule with allow rule for resource '"
                      + resourceName
                      + "'.");
        }
      } else {
        if (role.getAccessRules().put(resource, Role.STATE_DENY) == null) {
          getLogger()
              .info("Added deny rule for resource '" + resourceName + "'.");
        } else {
          getLogger()
              .info(
                  "Replaces existing access rule with deny rule for resource '"
                      + resourceName
                      + "'.");
        }
      }
  ...
  public String getFullHelpText() {
      ...
      if (StringUtils.isEmpty(role.getNameSpace())) {
        availableRoles.append(
            (availableRoles.length() == 0 ? "" : ", ")
                + super.getFullRoleName(
                    role.getNameSpace(), role.getRoleName()));
      }
    }
    sb.append("Available roles: " + availableRoles + "\n");
    final List<String> resourceNames =
        new ArrayList<>(super.getResourceNameToResourceMap().keySet());
    ...
    for (AccessRuleState current : AccessRuleState.values()) {
      availableRuleStates.append(
          (availableRuleStates.length() == 0 ? "" : ", ") + current.getName());
    }
