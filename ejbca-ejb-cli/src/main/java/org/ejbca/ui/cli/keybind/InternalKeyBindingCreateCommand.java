 ...
 * See getDescription().
 *
 * @version $Id: InternalKeyBindingCreateCommand.java 23648 2016-06-09 15:19:55Z
 *     mikekushner $
 */
public class InternalKeyBindingCreateCommand
...

  {
    ...
    registerParameter(
        new Parameter(
            ...
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "Fingerprint of a certificate from the bound keypair. Set as"
                + " 'null' to set later."));
    registerParameter(
        new Parameter(
            CRYPTO_TOKEN_KEY,
    ...
    registerParameter(
        new Parameter(
            ...
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "The signature algorithm that this InternalKeyBinding will use for"
                + " signatures."));
  ...
  public CommandResult execute(final ParameterContainer parameters) {
    ...
    Map<String, Serializable> validatedProperties =
        validateProperties(type, dataMap);
    if (validatedProperties == null) {
      return CommandResult.FUNCTIONAL_FAILURE;
    }
    final InternalKeyBindingStatus status =
        InternalKeyBindingStatus.valueOf(
            parameters.get(STATUS_KEY).toUpperCase());
    final String certificateId =
        "null".equalsIgnoreCase(parameters.get(CERTIFICATE_FINGERPRINT_KEY))
            ? null
            : parameters.get(CERTIFICATE_FINGERPRINT_KEY);
