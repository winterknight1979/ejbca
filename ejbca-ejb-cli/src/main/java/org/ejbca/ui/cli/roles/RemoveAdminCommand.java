...
public class RemoveAdminCommand extends BaseRolesCommand {
...

  {
    ...
    registerParameter(
        new Parameter(
            ...
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "(Deprected) Match operator type. Kept to prevent legacy scripts"
                + " from breaking. Currently implied by "
                + MATCH_WITH_KEY
                + " switch."));
    registerParameter(
        new Parameter(
  ...
  public CommandResult execute(final ParameterContainer parameters) {
    ...
    try {
      role =
          EjbRemoteHelper.INSTANCE
              ...
              .getRole(getAuthenticationToken(), namespace, roleName);
    } catch (AuthorizationDeniedException e) {
      getLogger()
          .error(
              "Not authorized to role "
    ...
    if (StringUtils.isEmpty(matchWithKeyParam)) {
      getLogger()
          .error(
              "No such thing to match with as '"
    ...
    if (matchWithKeySplit.length == 1) {
      ...
      if (authenticationTokenMetaData == null
          || !authenticationTokenMetaData.isUserConfigurable()) {
    ...
    if (X509CertificateAuthenticationTokenMetaData.TOKEN_TYPE.equals(
      ...
      try {
        caInfo =
            EjbRemoteHelper.INSTANCE
                ...
                .getCAInfo(getAuthenticationToken(), caNameParam);
      } catch (AuthorizationDeniedException e) {
      ...
      // To be backwards compatible with existing scripts in the wild, we will
      // still have to require parameter but we ignore the value if is unused
      // getLogger().info("Ignoring mandatory '" + CA_NAME_KEY + "' parameter
      // value for the selected Token Type '" + tokenType + "'.");
      tokenIssuerId = RoleMember.NO_ISSUER;
      caName = "-";
    }
    final AccessMatchValue accessMatchValue =
        AccessMatchValueReverseLookupRegistry.INSTANCE
            .lookupMatchValueFromTokenTypeAndName(tokenType, matchWithKey);
    if (accessMatchValue == null) {
      getLogger()
          .error("No such thing to match with as '" + matchWithKey + "'.");
      return CommandResult.FUNCTIONAL_FAILURE;
    }
    ...
    if (accessMatchTypeParam == null) {
      if (accessMatchValue.getAvailableAccessMatchTypes().isEmpty()) {
        ...
        accessMatchType =
            accessMatchValue.getAvailableAccessMatchTypes().get(0);
      }
      getLogger()
          .info(
              "Using '"
      ...
      accessMatchType =
          AccessMatchType.matchFromName(parameters.get(MATCH_TYPE_KEY));
      if (accessMatchType == null) {
        getLogger()
            .error(
                "No such type to match with as '"
                    + parameters.get(MATCH_TYPE_KEY)
                    + "'.");
        return CommandResult.FUNCTIONAL_FAILURE;
      }
    }
    ...
    try {
      ...
      for (final RoleMember roleMember
        ...
        if (tokenType.equals(roleMember.getTokenType())
            ...
            && accessMatchValue.getNumericValue()
          ...
          roleMemberSession.remove(
              getAuthenticationToken(), roleMember.getId());
          foundMatch = true;
          getLogger()
              .info(
                  "Removed role member: "
      ...
      if (!foundMatch) {
        getLogger()
            .info(
                "Could not find any matching admin in role "
                    + super.getFullRoleName(namespace, roleName)
                    + ".");
      }
    } catch (AuthorizationDeniedException e) {
      getLogger()
          .info(
              "Not authorized to members of role "
                  + super.getFullRoleName(namespace, roleName)
                  + ".");
      return CommandResult.AUTHORIZATION_FAILURE;
    }
