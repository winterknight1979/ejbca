...
public class AddRoleMemberCommand extends BaseRolesCommand {
...

  {
    ...
    registerParameter(
        new Parameter(
            ...
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "(Ignored) Match operator type. Kept to prevent legacy scripts"
  ...
  public CommandResult execute(final ParameterContainer parameters) {
    ...
    if (accessMatchValue.getAvailableAccessMatchTypes().isEmpty()) {
      ...
      // Just grab the first one, since we according to ECA-3164 only will have
      // a single allowed match operator for each match key
      ...
      getLogger()
          .info(
              "Parameter "
                  ...
                  + MATCH_WITH_KEY
                  + " value "
                  + accessMatchValue.name()
                  + " implies "
                  + accessMatchType.name()
                  + ".");
    }
    ...
    try {
      EjbRemoteHelper.INSTANCE
          .getRemoteSession(RoleMemberSessionRemote.class)
          .persist(getAuthenticationToken(), roleMember);
    } catch (AuthorizationDeniedException e) {
        ...
        for (final String accessMatchValueName :
          ...
          availableAccessMatchValues +=
              (availableAccessMatchValues.isEmpty() ? "" : ", ")
                  + tokenType
                  + ":"
                  + accessMatchValueName;
        }
      }
    }
    sb.append(
        "Match with is one of ('CertificateAuthenticationToken:' can be"
            + " omitted): "
            + availableAccessMatchValues
            + "\n");
    return sb.toString();
