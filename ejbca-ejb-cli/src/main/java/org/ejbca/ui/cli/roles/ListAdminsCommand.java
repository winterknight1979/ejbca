    ...
    Collections.sort(
        ...
        new Comparator<RoleMember>() {
          ...
          public int compare(
            ...
            final int compareTokenType =
                roleMember1
                    .getTokenType()
                    .compareTo(roleMember2.getTokenType());
            if (compareTokenType != 0) {
              return compareTokenType;
            }
            final int compareTokenMatchKey =
                Integer.valueOf(roleMember1.getTokenMatchKey())
                    .compareTo(Integer.valueOf(roleMember2.getTokenMatchKey()));
            if (compareTokenMatchKey != 0) {
              return compareTokenMatchKey;
            }
            final int compareTokenMatchValue =
                roleMember1
                    .getTokenMatchValue()
                    .compareTo(roleMember1.getTokenMatchValue());
        ...
        try {
          final CAInfo info =
              EjbRemoteHelper.INSTANCE
                  ...
                  .getCAInfo(
                      getAuthenticationToken(), roleMember.getTokenIssuerId());
          if (info == null) {
            caName =
                "[Unknown CA with ID " + roleMember.getTokenIssuerId() + "]";
          } else {
            caName = "'" + info.getName() + "'";
          }
        } catch (AuthorizationDeniedException e) {
          caName =
              "[(Name redacted) CA with ID "
