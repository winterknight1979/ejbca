  ...
  public Object getBeanValue(final String field, final Object obj)
    ...
    if (prop == null) {
      throw new FieldNotFoundException(
          "Field '"
              + field
              + "' does not exist. Did you use correct case for every"
    ...
    if (prop.getType().isInterface()) {
      logger.info(
          "Converting value '"
              ...
              + ArrayList.class
              + "', ");
      // If the value can be converted into an integer, we will use an
      // ArrayList<Integer>
      // Our problem here is that the type of a collection (<Integer>, <String>)
      // is only compile time, it can not be determined in runtime.
