...
/**
 * Clears caches used internally by EJBCA. The caches are used to limit the
 * number of database queries issued to the database. See
 * conf/cache.properties.sample for configuration of caches.
 *
...
public class ClearCacheCommand extends EjbcaCommandBase {
  ...
  // Register parameters
  {
    ...
    registerParameter(
        new Parameter(
            ...
            StandaloneMode.FORBID,
            ParameterMode.FLAG,
            "Clear Certificate Profile cache."));
    registerParameter(
        new Parameter(
            AUTHORIZATION,
            "Authorization",
            ...
            StandaloneMode.FORBID,
            ParameterMode.FLAG,
            "Clear Authorization cache. This parameter will also flush the"
                + " roles and role member caches."));
    registerParameter(
        new Parameter(
            CA_CACHE,
            "CA Cache",
    ...
    registerParameter(
        new Parameter(
            ...
            StandaloneMode.FORBID,
            ParameterMode.FLAG,
            "Clear CT caches (OCSP and Fail Fast cache)."));
    registerParameter(
        new Parameter(
            VALIDATOR_CACHE,
            "Validator Cache",
    ...
    registerParameter(
        Parameter.createFlag(
            APPROVAL_PROFILE_CACHE, "Clear the approval profile cache."));
  }
  ...
  public String getFullHelpText() {
    return "Clears caches used internally by EJBCA. See"
               + " conf/cache.properties.sample for config options. This"
               + " command should only be needed if cache times are set to"
               + " very high values. All arguments are optional, but you have"
  ...
  public CommandResult execute(final ParameterContainer parameters) {
    ...
    final boolean globalconf =
        (parameters.get(GLOBAL_CONFIGURATION) != null) || all;
    final boolean eeprofile = (parameters.get(EE_PROFILES) != null) || all;
    final boolean certprofile =
        (parameters.get(CERTIFICATE_PROFILE) != null) || all;
    final boolean authorization =
        (parameters.get(AUTHORIZATION) != null) || all;
    final boolean cacache = (parameters.get(CA_CACHE) != null) || all;
    final boolean ctcache = (parameters.get(CT_CACHE) != null) || all;
    final boolean approvalProfileCache =
        (parameters.get(APPROVAL_PROFILE_CACHE) != null) || all;
    final boolean validatorCache =
        (parameters.get(VALIDATOR_CACHE) != null) || all;
    ...
    if (authorization) {
      ...
      EjbRemoteHelper.INSTANCE
          .getRemoteSession(RoleDataSessionRemote.class)
          .forceCacheExpire();
      // Flush RoleMemberData cache
      EjbRemoteHelper.INSTANCE
          .getRemoteSession(RoleMemberDataSessionRemote.class)
          .forceCacheExpire();
    }
    if (cacache) {
    ...
    if (ctcache) {
      ...
      final OcspResponseGeneratorSessionRemote ocspResponseGeneratorSession =
          EjbRemoteHelper.INSTANCE.getRemoteSession(
              OcspResponseGeneratorSessionRemote.class);
