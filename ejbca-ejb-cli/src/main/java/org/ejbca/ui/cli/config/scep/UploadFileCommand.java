...
public class UploadFileCommand extends BaseScepConfigCommand {
  ...
  public String getFullHelpText() {
    ...
    sb.append("\n");
    sb.append(
        "Only one alias will be read. If one file contains configurations of"
            + " several aliases, you have to repeat the command with a"
            + " different alias each time to have all configurations of all"
            + " aliases read.\n");
    sb.append("\n");
    // sb.append(
    //        "Note that the alias 'scep' is the default alias used when sending
    // the SCEP request through the URL " +
    //        "'http://HOST:PORT/ejbca/publicweb/apply/scep/pkiclient.exe'.
    // \n");
    // sb.append("Note also that the alias 'scep' does not exist per default and
    // should be created before using this URL\n");
    sb.append(
    ...
    sb.append(
        "Note that if you have an old SCEP properties file containing the key"
            + " 'scep.ra.createOrEditUser', this key will automatically be"
            + " read and stored as 'scep.operationmode'. A 'true' value will"
            + " be intepreted as 'ra' and a 'false' value will be interpreted"
            + " as 'ca'.");
  ...
  private void readConfigurations(
      final CompositeConfiguration config, final String alias) {
    ...
    while (itr.hasNext()) {
      ...
      if (StringUtils.contains(key, ScepConfiguration.SCEP_RAMODE_OLD)) {
        value =
            StringUtils.equalsIgnoreCase(config.getString(key), "true")
                ? "ra"
                : "ca";
