 ...
 *
 * @version $Id: CryptoTokenUpdatePinCommand.java 19902 2014-09-30 14:32:24Z
 *     anatom $
 */
public class CryptoTokenUpdatePinCommand extends BaseCryptoTokenCommand {
...

  {
    ...
    registerParameter(
        Parameter.createFlag(
            SWITCH_UPDATE_ONLY_KEY,
            "Set the auto-activation pin and make the token auto-activated if"
                + " it was so previously."));
    registerParameter(
        Parameter.createFlag(
            SWITCH_REMOVE_AUTO_KEY,
            "Will remove any auto-activation pin if present (new pin is not"
  ...
  public CommandResult executeCommand(
    ...
    final char[] currentAuthenticationCode =
        getAuthenticationCode(parameters.get(OLD_PIN_KEY));
    final char[] newAuthenticationCode =
        removeAuto ? null : getAuthenticationCode(parameters.get(NEW_PIN_KEY));
    try {
      ...
      boolean result =
          cryptoTokenManagementSession.updatePin(
              getAdmin(),
      ...
      if (result) {
        ...
        getLogger()
            .info("Auto-activation is now not in use for this CryptoToken.");
      }
      ...
      getLogger()
          .info(
              "CryptoToken is " + (isActive ? "active" : "deactivated") + ".");
  ...
  public String getFullHelpText() {
    return getCommandDescription()
        ...
        + " modified and this requires the current activation PIN. For PKCS#11"
        + " CryptoTokens this will only modify the auto-activation pin and"
        + " requires the current (auto-activation or) activation PIN.";
  }
