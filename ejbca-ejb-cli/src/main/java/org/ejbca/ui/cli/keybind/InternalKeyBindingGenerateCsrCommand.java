 ...
 * See getDescription().
 *
 * @version $Id: InternalKeyBindingGenerateCsrCommand.java 22335 2015-12-01
 *     17:25:14Z jeklund $
 */
public class InternalKeyBindingGenerateCsrCommand
...

  {
    registerParameter(
        Parameter.createFlag(
            GENKEYPAIR_KEY,
            "Set to generate a \"next\" key pair with the same key"
                + " specification as the current and a new alias. If a nextKey"
                + " reference already exists, it will be replaced with a"
    ...
    registerParameter(
        new Parameter(
            ...
            ParameterMode.ARGUMENT,
            "Subject DN in the generated CSR, if left out default subject DN"
                + " will be used (CN=Should be ignored by CA)."));
    registerParameter(
        Parameter.createFlag(
            SUBJECTDN_ORDER_KEY,
            "(when using --subjectdn) Use this flag to specify that the DN"
                + " order in the CSR should be X500 (C=SE,O=org,CN=name)"
                + " instead of the default (CN=name,O=org,C=SE)"));
  }
  ...
  public CommandResult executeCommand(
      ...
      throws AuthorizationDeniedException, IOException, InvalidKeyException,
    ...
    // If contains key we want to boolean to be false, because LDAP DN order is
    // "true"
    ...
    if (csrSubjectDN != null) {
      ...
      getLogger()
          .info(
              "Using subject DN from argument '"
                  + x500Name.toString()
                  + "', with order "
                  + x500dnorder);
    } else {
      ...
      getLogger()
          .info(
              "Using default subject DN, from existing mapped certificate if"
                  + " it is present");
      x500Name = null;
    }
    String nextKeyAlias;
    if (switchGenKeyPair) {
      ...
      getLogger()
          .info("A new key pair has been generated with alias " + nextKeyAlias);
    } else {
    ...
    final byte[] certificateRequestBytes =
        internalKeyBindingMgmtSession.generateCsrForNextKey(
            ...
            x500Name != null ? x500Name.getEncoded() : null);
    if (certificateRequestBytes == null) {
      getLogger().error("Unable to generate CSR for " + nextKeyAlias);
      return CommandResult.FUNCTIONAL_FAILURE;
    } else {
      final byte[] pemEncodedPublicKey =
          CertTools.getPEMFromCertificateRequest(certificateRequestBytes);
      final OutputStream fos =
          new FileOutputStream(parameters.get(CSR_FILE_KEY));
      fos.write(pemEncodedPublicKey);
      fos.close();
      getLogger()
          .info(
              "Stored PEM encoded PKCS#10 request for \""
                  + parameters.get(KEYBINDING_NAME_KEY)
                  + "\" as "
                  + parameters.get(CSR_FILE_KEY));
  ...
  public String getFullHelpText() {
    return getCommandDescription()
        + " Optionally generates a new \"next\" key pair and otherwise returns"
        + " the current public key. \n"
        + "The subjectDN in the CSR will be set to the currently mapped"
        + " certificate's SubjectDN (if present) and if not a default DN of"
        + " (CN=Internal Key Binding Name). A subjectDN can also be set using"
        + " the parameter --subjectdn (see --help)";
  }
