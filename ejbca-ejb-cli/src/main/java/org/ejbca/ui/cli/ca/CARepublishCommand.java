...
public class CARepublishCommand extends BaseCaAdminCommand {
...

  {
    ...
    registerParameter(
        Parameter.createFlag(
            ALL_KEY,
            "Publish all certificates for each end entity instead of only the"
                + " latest (default only publishes the latest)."));
    registerParameter(
        Parameter.createFlag(CACERT_KEY, "Publish CA certificate."));
    registerParameter(Parameter.createFlag(CACRL_KEY, "Publish CA CRL."));
    registerParameter(
        Parameter.createFlag(EECERT_KEY, "Publish End Entity certificates."));
  }
      ...
      if (caiter.hasNext()) {
        ...
        final byte[] crlbytes =
            EjbRemoteHelper.INSTANCE
                ...
                .getLastCRL(cainfo.getSubjectDN(), false);
        // Get the CRLnumber
        X509CRL crl;
        try {
        ...
        if (capublishers != null) {
          ...
          getLogger()
              .info(
                  "No publishers configured for the CA, no CA certificate or"
                      + " CRL published.");
        }
      } else {
        getLogger()
            .info(
                "CA does not have a certificate, no certificate or CRL"
                    + " published!");
      }

      if (eecertmode) {
        // Get all users for this CA
        Collection<EndEntityInformation> coll =
            EjbRemoteHelper.INSTANCE
        ...
        while (iter.hasNext()) {
          ...
          if (data.getCertificateProfileId() > 0) { // only if we find a
            ...
            if (certProfile == null) {
              getLogger()
                  .error(
                      "Can not get certificate profile with id: "
                          + data.getCertificateProfileId());
              continue;
            }
              ...
              if (certProfile.getPublisherList() != null) {
                getLogger()
                    .info(
                        "Re-publishing user "
                            + data.getUsername()
                            + " to publishers in users certificate profile "
                            + data.getCertificateProfileId());
                if (addAll) {
                  getLogger()
                      .info(
                          "Re-publishing all certificates ("
                              + certCol.size()
                              + ").");
                  // Reverse the collection so we publish the latest certificate
                  // last
                  Collections.reverse(
                      certCol); // now the latest (last expire date) certificate
                                // is last in the List
                  ...
                  // The latest one is the first in the List according to
                  // findCertificatesByUsername()
                  publishCert(
                      getAuthenticationToken(),
                      data,
                      certProfile,
                      (X509Certificate) certCol.iterator().next());
                }
              } else {
                getLogger()
                    .info(
                        "Not publishing certificate for user "
                            + data.getUsername()
                            + ", no publisher in certificate profile.");
              }
            } else {
              getLogger()
                  .info(
                      "No certificate to publish for user "
                          + data.getUsername());
            }
          } else {
            getLogger()
                .info(
                    "No certificate profile is set for user "
                        + data.getUsername());
          }
        }
      } // if (eecertmode)
  ...
  private void publishCert(
    ...
    try {
      ...
      if (!ret) {
        getLogger()
            .error(
                "Failed to publish certificate for user "
                    + data.getUsername()
                    + ", continuing with next user. Publish returned false.");
      }
    } catch (Exception e) {
  ...
  public String getCommandDescription() {
    return "Re-publishes the certificates of a CA and/or all users issued by a"
               + " particular CA. ";
  }
  ...
  public String getFullHelpText() {
    return getCommandDescription()
        + "Default if none of cacert, cacrl or eecert is specific is to"
        + " publish all types, you can specify one or several on the command"
        + " line. For example to only publish CA certificate and CRL, no end"
        + " entity certificates: ca republish ManagementCA -cacert"
        + " -cacrlExample to only publish CA certificate and CRL, and latest"
        + " end entity certificates: ca republish ManagementCA";
  }
