 ...
 *
 * @version $Id: KeyRecoveryCommand.java 26210 2017-08-03 10:12:32Z samuellb $
 */
public class KeyRecoveryCommand extends BaseRaCommand {
  ...
  public CommandResult execute(final ParameterContainer parameters) {
    ...
    BigInteger certificatesn =
        new BigInteger(parameters.get(SERIAL_NUMBER_KEY), 16);
      ...
      if (userdata == null) {
        getLogger()
            .error(
                "ERROR: The user "
                    + username
                    + " defined by certificate serial number doesn't exist.");
        return CommandResult.FUNCTIONAL_FAILURE;
      }
      if (EjbRemoteHelper.INSTANCE
        ...
        getLogger()
            .info(
                "Key pair corresponding to the specified certificate has been"
                    + " marked for recovery.");
        return CommandResult.SUCCESS;
      } else {
        getLogger()
            .error(
                "Failed to mark keys corresponding to given certificate for"
                    + " recovery.");
      }
