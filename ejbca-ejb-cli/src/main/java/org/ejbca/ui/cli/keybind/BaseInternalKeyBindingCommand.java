...
/**
 * @version $Id: BaseInternalKeyBindingCommand.java 23648 2016-06-09 15:19:55Z
 *     mikekushner $
 */
public abstract class BaseInternalKeyBindingCommand
  ...
  protected Map<String, Serializable> validateProperties(
    ...
    InternalKeyBindingPropertyValidationWrapper validatedProperties =
        InternalKeyBindingFactory.INSTANCE.validateProperties(type, dataMap);
    if (!validatedProperties.arePropertiesValid()) {
      ...
      if (validatedProperties.getUnknownProperties().size() > 0) {
        stringBuffer.append(
            "The following properties were unknown for the type: "
                + type
                + "\n");
  ...
  protected String showTypesProperties() {
    ...
    Map<String, Map<String, DynamicUiProperty<? extends Serializable>>>
        typesAndProperties =
            internalKeyBindingMgmtSession.getAvailableTypesAndProperties();
    final StringBuilder sb = new StringBuilder();
    sb.append(
        "Registered implementation types and implemention specific"
            + " properties:\n");
