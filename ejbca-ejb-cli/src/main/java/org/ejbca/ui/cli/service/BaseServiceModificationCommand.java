 ...
 *
 * @version $Id: BaseServiceModificationCommand.java 23457 2016-05-16 17:18:31Z
 *     mikekushner $
 */
public abstract class BaseServiceModificationCommand
    extends BaseServiceCommand {
  ...
  protected final String FIELDS_HELP =
      "The \"fields\" are related directly to the service, e.g. it's"
          ...
          + "Note that no properties are set by default with the \"create\""
          + " command. You must either use the Admin Web when creating the"
          + " service or know in advance which properties are required. It is"
          + " NOT possible to list non-existent (but required) properties from"
          + " the CLI.";

  /**
   * Modifies the fields in the given ServiceConfiguration from command-line
   * arguments. The syntax is:
   *
   * <p>servicename [-listFields|-listProperties] fieldOrProperty=value...
   *
   * <p>This method handles the -list arguments as well, which show which fields
   ...
   * @return Bool
   */
  protected boolean modifyFromArgs(
      final ServiceConfiguration serviceConfig, final String[] args) {
    ...
    for (String property : params) {
      ...
      if (arr.length != 2) {
        getLogger()
            .info(
                "ERROR: Property is missing a value (the syntax is"
                    + " property=value):  "
                    + arr[0]);
        success = false;
        continue;
  ...
  /**
   * Modifies a given field or property in a ServiceConfiguration, using the a
   * FieldEditor.
   *
   * <p>First it tries to find a field with the given name, then it tries with
   * the worker/interval/action properties.
   *
  ...
  protected boolean modify(
    ...
    try {
      ...
      fieldEditor.setValue(
          ServiceConfiguration.class.getName(), field, value, serviceConfig);
      getLogger().info("Updated field: " + field);
      getLogger()
          .info(
              "New field value: "
                  + fieldEditor.getBeanValue(field, serviceConfig));
      found = true;
      ...
      if (props.containsKey(field) || field.startsWith("worker.")) {
        ...
        getLogger()
            .info(
                "New worker property value: "
                    + serviceConfig.getWorkerProperties().getProperty(field));
        found = true;
      }
      ...
      if (props.containsKey(field) || field.startsWith("interval.")) {
        ...
        getLogger()
            .info(
                "New interval property value: "
                    + serviceConfig.getIntervalProperties().getProperty(field));
        found = true;
      }
      ...
      if (props.containsKey(field) || field.startsWith("action.")) {
        ...
        getLogger()
            .info(
                "New action property value: "
                    + serviceConfig.getActionProperties().getProperty(field));
        found = true;
      }
   ...
   * @return bool
   */
  protected boolean handleListOptions(
      final ServiceConfiguration serviceConfig,
        ...
        getLogger()
            .info(
                "create".equals(command)
                    ? "The -listProperties option can presently only be used"
   ...
   * @param errors Errors
   */
  private void displayNotFound(final List<String> errors) {
    getLogger().info("");
    getLogger()
        .info(
            "ERROR: One or more names didn't exist either as a field or"
                + " property:");
    getLogger().info("");
