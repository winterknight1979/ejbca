...
public class CaImportCACommand extends BaseCaAdminCommand {
...

  {
    registerParameter(
        new Parameter(
            ...
            ParameterMode.FLAG,
            "Set this flag if importing a hard keystore (PKCS#11), default is"
                + " a soft keystore (PKCS#12)"));
    registerParameter(
        new Parameter(
            CA_NAME_KEY,
            "CA Name",
    ...
    registerParameter(
        new Parameter(
            ...
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "(PKCS#12) The PKCS#12 file to import from. Mandatory for soft"
                + " keys."));
    registerParameter(
        new Parameter(
            ...
            StandaloneMode.FORBID,
            ParameterMode.ARGUMENT,
            "(PKCS#12) The keystore password. If not set then it will be"
                + " prompted for."));
    registerParameter(
        new Parameter(
            SIGNATURE_ALIAS_KEY,
            ...
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "(PKCS#12) If left blank, will use the only available alias, or if"
                + " multiple are available a list will be shown."));
    registerParameter(
        new Parameter(
            ENCRYPTION_ALIAS_KEY,
            "Encryption Alias",
            MandatoryMode.OPTIONAL,
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "(PKCS#12) If left blank, will use the only available alias, or if"
                + " multiple are available a list will be shown. If no"
                + " encryption alias is given, the encryption keys will be"
                + " generated."));
    // CA Certificate arguments
    registerParameter(
        new Parameter(
            CA_TOKEN_CLASSPATH_KEY,
            "CA Token Classpath",
            MandatoryMode.OPTIONAL,
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "(PKCS#11) Example: org.cesecore.keys.token.PKCS11CryptoToken for"
                + " PKCS11 HSMs."));
    registerParameter(
        new Parameter(
            CA_TOKEN_PASSWORD_KEY,
            "CA Token Password",
            MandatoryMode.OPTIONAL,
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "(PKCS#11) Password for the CA Token."));
    registerParameter(
        new Parameter(
            CA_TOKEN_PROPERTIES_FILE_KEY,
            "CA Token Properties File",
            MandatoryMode.OPTIONAL,
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "(PKCS#11) A file were you define key name, password and key alias"
                + " for the HSM. Same as the Hard CA Token Properties in admin"
                + " GUI."));
    registerParameter(
        new Parameter(
            ...
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "(PKCS#11) A file containing CA-certificates. One or more"
                + " CA-certificates, with this CA's certificate first, and"
                + " others following in certificate chain order."));
  }
  ...
  public CommandResult execute(final ParameterContainer parameters) {
    ...
    if (!importHardToken) {
      ...
      try {
        ...
        if (alias == null) {
          ...
          try {
            ...
            throw new IllegalStateException(
                "BouncyCastle provider couldn't be found.", e);
          }
          ...
          try {
            ...
            if (e.getCause() instanceof UnrecoverableKeyException) {
              ...
              throw new IllegalStateException(
                  "Uknown IOException was caught", e);
            }
          }
          try {
            fis.close();
      ...
      Certificate[] cacertarray =
          cacerts.toArray(new Certificate[cacerts.size()]);
      try {
        EjbRemoteHelper.INSTANCE
            .getRemoteSession(CAAdminSessionRemote.class)
            .importCAFromHSM(
  ...
  public String getFullHelpText() {
    return getCommandDescription()
        + " This command has two modes: importing a CA from a PKCS#12 keystore"
        + " (default) or importing from a CA certificate. PKCS#12 keystore is"
        + " the default option, while CA certificate can be chosen by"
        + " specifying the flag "
        + HARD_SWITCH_KEY
        + "\n"
        + "The two usages are: \n"
        ...
        + "<CA name> "
        + HARD_SWITCH_KEY
        + " <catokenclasspath> <catokenpassword> <catokenproperties>"
        + " <ca-certificate-file>";
  }
