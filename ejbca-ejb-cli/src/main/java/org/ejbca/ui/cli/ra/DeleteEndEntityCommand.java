 ...
 *
 * @version $Id: DeleteEndEntityCommand.java 29136 2018-06-07 11:11:07Z
 *     andresjakobs $
 */
...

  {
    ...
    registerParameter(
        Parameter.createFlag(
            FORCE_KEY, "Don't ask if the end entity has been revoked."));
  }
    ...
    try {
      ...
      if ((inp == 121) || (inp == 89)) {
        try {
          ...
          getLogger()
              .error("ERROR: User could not be removed. " + e.getMessage());
          return CommandResult.FUNCTIONAL_FAILURE;
        }
      } else {
