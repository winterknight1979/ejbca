...
public class KeyRecoveryNewestCommand extends BaseRaCommand {
  ...
  public CommandResult execute(final ParameterContainer parameters) {
    ...
    if (!usekeyrecovery) {
      getLogger()
          .error(
              "ERROR: Keyrecovery have to be enabled in the system"
                  + " configuration in order to use this command.");
    ...
    try {
      ...
      if (EjbRemoteHelper.INSTANCE
        ...
        getLogger()
            .info(
                "Key corresponding to users newest certificate has been marked"
                    + " for recovery.");
        return CommandResult.SUCCESS;
      } else {
        getLogger()
            .info(
                "ERROR: Failed to mark key corresponding to users newest"
