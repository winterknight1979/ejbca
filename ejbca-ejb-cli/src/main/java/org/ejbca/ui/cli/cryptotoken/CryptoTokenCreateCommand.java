...
public class CryptoTokenCreateCommand extends EjbcaCliUserCommandBase {
...

  {
    ...
    registerParameter(
        new Parameter(
            ...
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "Set to true|false to allow|disallow whether crypto token should"
                + " be autoactivated or not."));
    registerParameter(
        new Parameter(
            TYPE_KEY,
            "Type",
            ...
            "Available types: "
                ...
                + ", "
                + PKCS11CryptoToken.class.getSimpleName()));
    // Soft params
    registerParameter(
        new Parameter(
            PRIVATE_KEY_EXPORT_KEY,
            "true|false",
            ...
            "("
                + SoftCryptoToken.class.getSimpleName()
                + ") Set to true|false to allow|disallow private key export."));
    // PKCS#11
    registerParameter(
        new Parameter(
            PKCS11_LIB_KEY,
            ...
            "("
                + PKCS11CryptoToken.class.getSimpleName()
                + ") PKCS#11 library file. Required if type is "
                + PKCS11CryptoToken.class.getSimpleName()));
    registerParameter(
        new Parameter(
            SLOT_REFERENCE_TYPE_KEY,
            "Slot Reference Type",
            MandatoryMode.OPTIONAL,
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
    ...
    registerParameter(
        new Parameter(
            PKCS11_SLOTCOLLIDE_IGNORE,
            "Ignore used P11 slots",
            MandatoryMode.OPTIONAL,
            StandaloneMode.FORBID,
            ParameterMode.FLAG,
            "Ignore warnings, and confirm yes, for P11 slots that are already"
                + " used."));
    // ePassport CSCA only
    registerParameter(
        new Parameter(
            USE_EXPLICIT_KEY_PARAMETERS,
            "true|false",
            MandatoryMode.OPTIONAL,
            StandaloneMode.ALLOW,
            ParameterMode.ARGUMENT,
            "Set to true|false to allow|disallow usage of explicit ECC"
                + " parameters( Only for ICAO CSCA and DS certificates)."));
  }
  ...
  public CommandResult execute(final ParameterContainer parameters) {
    ...
    final boolean autoActivate =
        Boolean.valueOf(parameters.get(AUTOACTIVATE_KEY));
    final boolean ignoreslotwarning =
        (parameters.get(PKCS11_SLOTCOLLIDE_IGNORE) != null);
    ...
    if (SoftCryptoToken.class.getSimpleName().equals(type)) {
      ...
      cryptoTokenPropertes.setProperty(
          CryptoToken.ALLOW_EXTRACTABLE_PRIVATE_KEY,
          Boolean.toString(
              Boolean.valueOf(parameters.get(PRIVATE_KEY_EXPORT_KEY))));
      cryptoTokenPropertes.setProperty(
      ...
      cryptoTokenPropertes.setProperty(
          PKCS11CryptoToken.SHLIB_LABEL_KEY, pkcs11LibFilename);
      ...
      if (slotPropertyValue == null) {
        getLogger()
            .info(
                "Slot reference key ("
                    + SLOT_REFERENCE_KEY
                    + ") needs to be defined for PKCS#11 tokens.");
        return CommandResult.CLI_FAILURE;
      }
      ...
      Pkcs11SlotLabelType labelType =
          Pkcs11SlotLabelType.getFromKey(
              parameters.get(SLOT_REFERENCE_TYPE_KEY));
      if (labelType == null) {
        getLogger()
            .info(
                parameters.get(SLOT_REFERENCE_TYPE_KEY)
                    + " was not a valid slot reference type.");
        return CommandResult.CLI_FAILURE;
      }
      cryptoTokenPropertes.setProperty(
          PKCS11CryptoToken.SLOT_LABEL_VALUE, slotPropertyValue);
        ...
        getLogger()
            .info(
                "Invalid value "
                    + slotPropertyValue
                    + " given for slot type "
                    + labelType.getDescription());
        return CommandResult.CLI_FAILURE;
      } else {
        cryptoTokenPropertes.setProperty(
            PKCS11CryptoToken.SLOT_LABEL_TYPE, labelType.getKey());
      }
      // Parse attribute file
      String attributeFileName = parameters.get(PKCS11_ATTR_FILE_KEY);
      if ((attributeFileName != null)
          && (!"null".equalsIgnoreCase(attributeFileName))) {
        ...
        cryptoTokenPropertes.setProperty(
            PKCS11CryptoToken.ATTRIB_LABEL_KEY, attributeFileName);
      }
      ...
      try {
        List<String> usedBy =
            cryptoTokenManagementSession.isCryptoTokenSlotUsed(
                ...
                cryptoTokenName,
                className,
                cryptoTokenPropertes);
        if (!usedBy.isEmpty() && !ignoreslotwarning) {
          for (String usedByName : usedBy) {
            String name = usedByName;
            if (StringUtils.isNumeric(name)) {
              // if the crypto token name is purely numeric, it is likely to be
              // a database protection token
              name = name + " (database protection?)";
            }
            getLogger()
                .info(
                    "The P11 slot is already used by another crypto token: "
                        + name);
          }
      ...
      } catch (CryptoTokenNameInUseException
        ...
        getLogger()
            .info(
                "There is an error creating the Crypto Token: "
                    + e.getMessage());
        getLogger().info("Do you want to continue anyhow? [yes/no]: ");
        String yes = System.console().readLine();
        if (!StringUtils.equalsIgnoreCase("yes", yes)) {
    ...
    String useExplicitKeyParameter =
        parameters.get(USE_EXPLICIT_KEY_PARAMETERS);
    ...
    if (autoActivate) {
      BaseCryptoToken.setAutoActivatePin(
          cryptoTokenPropertes, new String(authenticationCode), true);
    }
    try {
      final Integer cryptoTokenIdNew =
   ...
   * @return a decrypted version of the parameter or use input if the parameter
   *     equals "null"
   */
  private char[] getAuthenticationCode(final String commandLineArgument) {
    final char[] authenticationCode;
    if (commandLineArgument == null
        || "null".equalsIgnoreCase(commandLineArgument)) {
