...
/**
 * Remove a Publisher from the system. If there are references from CAs and/or
 * certificate profiles, you can optionally also remove these, or just list
 * them.
 *
 * @version $Id: CaRemovePublisherCommand.java 30470 2018-11-12 11:02:11Z
 *     samuellb $
 */
public class CaRemovePublisherCommand extends BaseCaAdminCommand {
...

  {
    registerParameter(
        new Parameter(
            ...
            "Name of the Publisher. If no flags are provided, removing the"
                + " publisher will work if there are no references to the"
                + " publisher from CAs or Certificate Profiles."));
    registerParameter(
        Parameter.createFlag(
            LIST_REF_KEY,
            "Lists all references (you are authorized to) to the publisher,"
                + " without removing the publisher."));
    registerParameter(
        Parameter.createFlag(
            REMOVE_REF_KEY,
            "Removes all references (you are authorized to) to the publisher,"
                + " without removing the publisher."));
    registerParameter(
        Parameter.createFlag(
            REMOVE_ALL_KEY,
            "Removes all references (you are authorized to) to the publisher,"
                + " and removes the publisher as well."));
  }
  ...
  public CommandResult execute(final ParameterContainer parameters) {
    ...
    try {
          ...
          if (publisherIds.contains(publisherId)) {
            getLogger()
                .info(
                    "CA '"
                        + cainfo.getName()
                        + "' contains a reference to the publisher '"
                        + name
                        + "'.");
      ...
      Collection<Integer> cps =
          cpSession.getAuthorizedCertificateProfileIds(
              getAuthenticationToken(), 0);
      ...
      for (Integer cpid : cps) {
        ...
        if (publisherIds.contains(publisherId)) {
          getLogger()
              .info(
                  "Certificate profile '"
                      + cpSession.getCertificateProfileName(cpid)
                      + "' contains a reference to the publisher '"
                      + name
                      + "'.");
          if (removerefmode || removeallmode) {
            ...
            publisherIds.remove(publisherId);
            cpSession.changeCertificateProfile(
                getAuthenticationToken(), cpName, profile);
            getLogger()
                .info(
                    "Removed publisher reference '"
                        + name
  ...
  public String getCommandDescription() {
    return "Remove the Publisher from the system, optionally also removing"
               + " references from CAs and Certificate Profiles.";
  }
  ...
  public String getFullHelpText() {
    return getCommandDescription()
        + "\n"
        + "Note: Can only list/remove references in CAs and Certificate"
        + " Profiles that you are authorized to.\n"
        + "Command can be used to list references, remove references, or"
        + " remove both references and Publisher.\n"
        + "References are 'CRL Publishers' in CA configuration, and"
        + " 'Publishers' in Certificate Profiles.";
  }
