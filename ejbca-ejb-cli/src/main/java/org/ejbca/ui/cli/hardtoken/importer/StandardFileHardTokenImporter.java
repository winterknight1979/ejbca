 ...
 *
 * <p>The Importer have the following properties: separator : indicates which
 * column separator that should be used.
 *
 * <p>columnorder : A list of columnnames separated by ',' of how the columns
 * are ordered. Valid values are 'tokensn', 'pin1', 'pin2', 'bothpin', 'puk1',
 * 'puk2', 'bothpuk' Example the line: 'columnorder=tokensn, pin1, pin2,
 * bothpuk' will generate a hardtokendata with the tokensn from first column,
 * basicpin from the secondcolumn, signaturepin from third and finally both puk
 * codes from the forth column.
 *
 * <p>tokentype : either 'enhancedeid' or 'swedisheid'. And indicates what type
 * of token that will be created.
 *
 * <p>import.properties:
 *
 * <pre>
 ...
 * @version $Id: StandardFileHardTokenImporter.java 19902 2014-09-30 14:32:24Z
 *     anatom $
 */
public class StandardFileHardTokenImporter extends FileReadHardTokenImporter {
  ...
  public void startImport(final Properties props) throws IOException {
    ...
    if (props.getProperty(PROPERTY_SEPARATOR) == null) {
      throw new IOException(
          "Error property " + PROPERTY_SEPARATOR + " not set.");
    }
    columnSeparator = props.getProperty(PROPERTY_SEPARATOR);

    if (props.getProperty(PROPERTY_TOKENTYPE) == null) {
      throw new IOException(
          "Error property " + PROPERTY_TOKENTYPE + " not set.");
    }
    hardTokenType = props.getProperty(PROPERTY_TOKENTYPE);
    if (!hardTokenType.equalsIgnoreCase("enhancedeid")
        && !hardTokenType.equalsIgnoreCase("swedisheid")) {
      throw new IOException(
          "Error property "
              + PROPERTY_TOKENTYPE
              + " must have either the value 'enhancedeid' or 'swedisheid'.");
    }
  }

  private void getColumns(final Properties props) throws IOException {
    if (props.getProperty("columnorder") == null) {
      throw new IOException(
          "Error the required property 'columnorder' isn't set.");
    }
  ...
  private int getColumn(final String column) throws IOException {
    ...
    throw new IOException(
        "Error illegal column "
            + column
            + " in the "
            + PROPERTY_COLUMNORDER
            + " property.");
  }

  /**
   * @see
   *     org.ejbca.ui.cli.hardtoken.importer.FileReadHardTokenImporter#readHardTokenData()
   */
  @Override
    ...
    if (line != null) {
      ...
      for (int i = 0; i < lineColumns.length; i++) {
        ...
        switch (columns[i]) {
          ...
          default:
            throw new IOException(
                "Error reading column + " + i + " of hard token import data.");
        }
      }
      ...
      HardToken ht =
          getHardTokenType(basicPIN, basicPUK, signaturePIN, signaturePUK);
      retval =
  ...
  private HardToken getHardTokenType(
      final String basicPIN,
      final String basicPUK,
      final String signaturePIN,
      final String signaturePUK) {
    if (hardTokenType.equalsIgnoreCase("enhancedeid")) {
      return new EnhancedEIDHardToken(
          signaturePIN, signaturePUK, basicPIN, basicPUK, false, 0);
    }
    return new SwedishEIDHardToken(
        basicPIN, basicPUK, signaturePIN, signaturePUK, 0);
  }
}
