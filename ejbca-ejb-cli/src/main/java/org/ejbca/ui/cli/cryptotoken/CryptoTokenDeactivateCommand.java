 ...
 *
 * @version $Id: CryptoTokenDeactivateCommand.java 19902 2014-09-30 14:32:24Z
 *     anatom $
 */
public class CryptoTokenDeactivateCommand extends BaseCryptoTokenCommand {
  ...
  private static final Logger LOG =
      Logger.getLogger(CryptoTokenDeactivateCommand.class);
    ...
    try {
      ...
      if (cryptoTokenManagementSession.isCryptoTokenStatusActive(
        ...
        if (usingAutoActivation) {
          ...
          getLogger()
              .info(
                  "This is the expected outcome since the CryptoTokens is"
                      + " instantly auto-activated after the deactivation"
                      + " request.");
        } else {
          getLogger().error(msg);
        }
        ...
        if (usingAutoActivation) {
          getLogger()
              .error(
                  "CryptoTokens was deactivated despite auto-activation being"
                      + " used. This is an unexpected outcome.");
        } else {
