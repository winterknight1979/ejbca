 ...
 *
 * @version $Id: CaEditCaCommand.java 23457 2016-05-16 17:18:31Z mikekushner $
 */
public class CaEditCaCommand extends BaseCaAdminCommand {
    ...
    try {
      ...
      try {
        if (field.equals(CA.NAME)) {
          // The CA name field is a bit of a special case. Since there's a
          // CESeCore method specifically for it, we should use
          // it instead
          try {
            EjbRemoteHelper.INSTANCE
                .getRemoteSession(CaSessionRemote.class)
  ...
  public String getFullHelpText() {
    return getCommandDescription()
        + "Fields that can be set are derived from setFieldName of the CA java"
        + " code. If there is a 'setFieldName(type)' method, the values to use"
        + " in this command should be 'fieldName value'\n"
