...
public class UpdateCommand extends BaseScepConfigCommand {
  ...
  public CommandResult execute(final ParameterContainer parameters) {
    ...
    if (StringUtils.equals(key, ScepConfiguration.SCEP_RAMODE_OLD)
        || StringUtils.equals(key, ScepConfiguration.SCEP_OPERATIONMODE)) {
    ...
    try {
      ...
      LOG.info(
          "Configuration updated: "
              + key
              + "="
              + getScepConfiguration().getValue(key, alias));
      getGlobalConfigurationSession()
  ...
  public String getFullHelpText() {
    ...
    sb.append(
        "    "
            + ScepConfiguration.SCEP_OPERATIONMODE
            + " - possible values: ca "
            + divider
            + " ra"
            + "\n");
    sb.append(
        "    "
            + ScepConfiguration.SCEP_INCLUDE_CA
    ...
    sb.append(
        "    "
            + ScepConfiguration.SCEP_RA_ENTITYPROFILE
            + " - possible values: "
            + existingEeps
            + "\n");
    ...
    for (Integer profileId
      ...
      existingCps.append(
          (existingCps.length() == 0 ? "" : divider)
              + certificateprofileidtonamemap.get(profileId));
    }
    sb.append(
        "    "
            + ScepConfiguration.SCEP_RA_CERTPROFILE
    ...
    sb.append(
        "    "
            + ScepConfiguration.SCEP_RA_DEFAULTCA
            + " - possible values: ProfileDefault"
            + divider
            + existingCas
            + "\n");
    sb.append(
        "    "
            + ScepConfiguration.SCEP_RA_AUTHPWD
    ...
    sb.append(
        "    "
            + ScepConfiguration.SCEP_RA_NAME_GENERATION_SCHEME
            + " - possible values: DN "
            + divider
            + " RANDOM "
            + divider
    ...
    sb.append(
        "    "
            + ScepConfiguration.SCEP_RA_NAME_GENERATION_PARAMETERS
            + " - possible values: See CMP Configurations in the Admin GUI"
            + "\n");
    sb.append(
        "    "
            + ScepConfiguration.SCEP_RA_NAME_GENERATION_PREFIX
    ...
    sb.append(
        "    "
            + ScepConfiguration.SCEP_RA_NAME_GENERATION_POSTFIX
            + " - possible values: ${RANDOM} "
            + divider
            + " any alphanumeric string"
            + "\n");
    sb.append(
        "    "
            + ScepConfiguration.SCEP_CLIENT_CERTIFICATE_RENEWAL
            + " - (Enterprise Edition Only) possible values: true "
            + divider
            + " false"
            + "\n");
    sb.append(
        "    "
            + ScepConfiguration.SCEP_CLIENT_CERTIFICATE_RENEWAL_WITH_OLD_KEY
            + " - (Enterprise Edition Only) possible values: true "
            + divider
            + " false"
            + "\n");
