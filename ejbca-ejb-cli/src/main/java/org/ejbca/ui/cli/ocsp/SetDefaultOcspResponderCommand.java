 ...
 * See getDescription().
 *
 * @version $Id: SetDefaultOcspResponderCommand.java 21875 2015-09-16 19:57:31Z
 *     samuellb $
 */
public class SetDefaultOcspResponderCommand extends EjbcaCliUserCommandBase {
  ...
  public String getFullHelpText() {
    ...
    stringBuilder.append(
        " The value is saved in the form of a DN, either the subject DN of the"
            + " CA or the issuer DN of the Internal Keybinding chosen as"
            + " default responder.");
    stringBuilder.append(
        " Note that input from this command is not vetted, so be sure to be"
            + " exact in setting the value. DNs inputed in reverse order will"
            + " be saved in standard order.");
    stringBuilder.append(
        " If no default responder is to be set, simply enter an emptry string"
            + " within quotes (\"\"). Local CAs with active OCSP Keybindings"
            + " will not be listed.\n\n");
    InternalKeyBindingMgmtSessionRemote internalKeyBindingMgmtSession =
        EjbRemoteHelper.INSTANCE.getRemoteSession(
    ...
    for (InternalKeyBindingInfo info :
      ...
      if (info.getStatus().equals(InternalKeyBindingStatus.ACTIVE)) {
        ...
        ikbContents.add(
            new String[] {info.getName(), CertTools.getIssuerDN(certificate)});
    ...
    if (!ikbContents.isEmpty()) {
      stringBuilder.append(
          bold(
              "The following Internal Keybindings are available as"
                  + " signers:\n"));
      stringBuilder.append(
          formatTable(
              1, new String[] {"Keybinding Name:", "Issuer DN:"}, ikbContents));
      stringBuilder.append("\n");
    }
    ...
    if (!caContents.isEmpty()) {
      stringBuilder.append(
          bold("The following CAs are available as signers:\n"));
      stringBuilder.append(
          formatTable(1, new String[] {"CA Name:", "Subject DN:"}, caContents));
      stringBuilder.append("\n");
    }
    ...
    GlobalOcspConfiguration conf =
        (GlobalOcspConfiguration)
            globalConfigurationSession.getCachedConfiguration(
                GlobalOcspConfiguration.OCSP_CONFIGURATION_ID);
    stringBuilder.append(
        "The current default responder is: "
            + conf.getOcspDefaultResponderReference());
