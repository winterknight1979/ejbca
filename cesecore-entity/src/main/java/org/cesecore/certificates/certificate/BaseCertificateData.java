...
public abstract class BaseCertificateData extends ProtectedData {
  ...
  /**
   * @return the number of milliseconds since 1970-01-01 00:00:00 GMT until the
   *     certificate was issued or null if the information is not known.
   */
  ...
  /**
   * Set to date when revocation occured if status == CERT_REVOKED. Format ==
   * Date.getTime()
   *
  ...
  /**
   * @return the end entity profile this certificate was issued under or null if
  ...
  //
  // The setters below should in general only be used when adding the
  ...
  /**
   * Return the certificate. From this table if contained here. From {link
   * Base64CertData} if contained there.
   *
   * @param entityManager To be used if the cert is in the {@link
   *     Base64CertData} table.
   * @return The certificate
   */
  @Transient
  public String getBase64Cert(final EntityManager entityManager) {
    if (getBase64Cert() != null && getBase64Cert().length() > 0) {
      return getBase64Cert(); // the cert was in this table.
    }
    // try the other table.
    final Base64CertData res =
        Base64CertData.findByFingerprint(entityManager, getFingerprint());
    if (res == null) {
      String message =
          "No "
              + getClassName()
   ...
   * @return The certificate or null if it doesn't exist or is blank/null in the
   *     database
   */
  @Transient
  public Certificate getCertificate(final EntityManager entityManager) {
    try {
  ...
  /**
   * Serialnumber formated as BigInteger.toString(16).toUpperCase(), or just as
   * it is in DB if not encodable to hex.
   *
  ...
  /**
   * @return the end entity profile this certificate was issued under or 0 if
   *     the information is not available.
   */
  @Transient
  public int getEndEntityProfileIdOrZero() {
    return getEndEntityProfileId() == null
   ...
   * @param strict will treat NOTIFIED as ACTIVE and ARCHIVED as REVOKED if set
   *     to false
   * @return status
   */
  public boolean equalsStatus(
      final BaseCertificateData certificateData, final boolean strict) {
    ...
    if ((status == CertificateConstants.CERT_ACTIVE
        ...
        && (otherStatus == CertificateConstants.CERT_ACTIVE
            || otherStatus
