 ...
 *
 * @version $Id: RoleTest.java 25031 2017-01-18 12:51:36Z jeklund $
 */
public class RoleTest {
  ...
  public void testHasAccessToResource() {
    ...
    hasAccessToResourcesInternal(role);
    assertNull(
        "Minimization did not remove deny rule.",
        role.getAccessRules().get("/xyz"));
    assertNull(
        "Minimization did not remove allow rule.",
        role.getAccessRules().get("/1/2"));
    assertNull(
  ...
  private void hasAccessToResourcesInternal(final Role role) {
    ...
    assertTrue(
        ERRMSG_DENIED_TO_ALLOWED, role.hasAccessToResource("/fuu/anything"));
    assertTrue(
        ERRMSG_DENIED_TO_ALLOWED,
        role.hasAccessToResource("/fuu/anything/something"));
    assertFalse(ERRMSG_ALLOWED_TO_DENIED, role.hasAccessToResource("/xyz/abc"));
    assertTrue(
        ERRMSG_DENIED_TO_ALLOWED, role.hasAccessToResource("/xyz_abc/foo"));
    assertTrue(
        ERRMSG_DENIED_TO_ALLOWED, role.hasAccessToResource("/a/b/c/d/e"));
    ...
    assertFalse(
        ERRMSG_ALLOWED_TO_DENIED, role.hasAccessToResource("/1/2/3/4/5"));
    // Check explicitly configured access (normalized form)
    assertTrue(ERRMSG_DENIED_TO_ALLOWED, role.hasAccessToResource("/fuu/"));
    assertFalse(
        ERRMSG_ALLOWED_TO_DENIED, role.hasAccessToResource("/foo/bar/"));
    assertFalse(ERRMSG_ALLOWED_TO_DENIED, role.hasAccessToResource("/xyz/"));
    assertTrue(ERRMSG_DENIED_TO_ALLOWED, role.hasAccessToResource("/xyz_abc/"));
    assertTrue(ERRMSG_DENIED_TO_ALLOWED, role.hasAccessToResource("/a/b/c/d/"));
    assertFalse(ERRMSG_ALLOWED_TO_DENIED, role.hasAccessToResource("/a/b/"));
    ...
    assertTrue(ERRMSG_DENIED_TO_ALLOWED, role.hasAccessToResource("/1/2/"));
    assertFalse(
        ERRMSG_ALLOWED_TO_DENIED, role.hasAccessToResource("/1/2/3/4/"));
    ...
    assertTrue(
        ...
        role.hasAccessToResource("/fuu/anything/something/"));
    assertFalse(
        ERRMSG_ALLOWED_TO_DENIED, role.hasAccessToResource("/xyz/abc/"));
    assertTrue(
        ERRMSG_DENIED_TO_ALLOWED, role.hasAccessToResource("/xyz_abc/foo/"));
    assertTrue(
        ERRMSG_DENIED_TO_ALLOWED, role.hasAccessToResource("/a/b/c/d/e/"));
