...
public class RaAuthenticationHelper implements Serializable {
  ...
  public AuthenticationToken getAuthenticationToken(
    ...
    if (authenticationToken == null
      ...
      if (x509Certificate == null
        ...
        LOG.warn(
            "Suspected session hijacking attempt from "
                ...
                + " previously authenticated with client certificate.");
      ...
      if (x509Certificate != null) {
          ...
          if (!StringUtils.equals(
            ...
            LOG.warn(
                "Suspected session hijacking attempt from "
                    ...
                    + " the same HTTP session. new certificate had subject '"
        ...
        // No need to perform re-authentication if the client certificate was
        // the same
        if (authenticationToken == null) {
          authenticationToken =
              webAuthenticationProviderSession
                  .authenticateUsingClientCertificate(x509Certificate);
        }
        x509AuthenticationTokenFingerprint =
            authenticationToken == null ? null : fingerprint;
      }
      if (authenticationToken == null) {
        // Instead of checking httpServletRequest.isSecure() (connection deemed
        // secure by container), we check if a TLS session is present
        final boolean confidentialTransport = currentTlsSessionId != null;
        authenticationToken =
            webAuthenticationProviderSession.authenticateUsingNothing(
                httpServletRequest.getRemoteAddr(), confidentialTransport);
      }
    }
  ...
  /**
   * Invoke once the session is started to prevent security leak via HTTP
   * headers related.
   *
   * @param httpServletRequest req
   * @param httpServletResponse resp
   */
  private void resetUnwantedHttpHeaders(
      final HttpServletRequest httpServletRequest,
      final HttpServletResponse httpServletResponse) {
    // Ensure that we never send the JSESSIONID over an insecure (HTTP)
    // connection
    // By default JBoss will send the JSESSIONID cookie over HTTP with the
    // "Secure;" option. Since this is sent in clear from the server to the
    // broswer
    // it does not really help security much that it is only sent over HTTPS
