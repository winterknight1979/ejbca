 ...
 *
 * @version $Id: RaSessionKeepAliveServlet.java 26057 2017-06-22 08:08:34Z
 *     anatom $
 */
...
public class RaSessionKeepAliveServlet extends HttpServlet {
  ...
  protected void service(
    ...
    if (maxInactiveIntervalSeconds > MAX_INACTIVE_INTERVAL_JSCLIENT_SECONDS) {
      // Since the client has JavaScript enabled (pretty safe to assume since
      // this is accessed via AJAX), we can set the session timeout more
      // aggressively
    ...
    // Use a fail safe of 10000 milliseconds when calculating the time to the
    // next check
