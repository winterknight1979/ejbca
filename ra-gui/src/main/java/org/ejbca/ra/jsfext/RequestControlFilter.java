 ...
 *
 * <p>If a user makes two requests, A and B, then A will be processed first
 * while B waits. When A finishes, B will be processed.
 *
 * <p>If a user makes three or more requests (e.g. A, B, and C), then the first
 ...
 *       request matches one of those patterns.
 *   <li>Requests wait a maximum of 30 seconds, which can be overridden per URI
 *       pattern in the filter's configuration.
 * </ul>
 *
 * @author Kevin Chipalowsky and Ivelin Ivanov Changed to WebFilter and
 *     modernized by PrimeKey
 * @version $Id: RequestControlFilter.java 25084 2017-01-24 10:08:30Z anatom $
 */
@WebFilter(
    filterName = "RequestControlFilter",
    urlPatterns = {"/*"},
    initParams = {
...
public class RequestControlFilter implements Filter {
    ...
    while (en.hasMoreElements()) {
      ...
      if (paramName.startsWith("excludePattern")) {
        ...
        String durationString =
            paramName.substring("maxWaitMilliseconds.".length());
    ...
    synchronized (getSynchronizationObject(session)) {
      ...
      if (isRequestInProcess(session)) {
        ...
        if (!waitForRelease(httpRequest)) {
          ...
          // this request was replaced in the queue by another request, so it
   ...
   */
  private boolean waitForRelease(final HttpServletRequest request) {
    HttpSession session = request.getSession();
   ...
   */
  private void enqueueRequest(final HttpServletRequest request) {
    HttpSession session = request.getSession();
