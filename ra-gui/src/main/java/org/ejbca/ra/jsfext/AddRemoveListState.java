...
/**
 * Backing state for the addremovelist.xhtml component, which is a list with an
 * "Enabled" and an "Available" list that items may be moved between.
 *
 * <p>The getEnabledItems() and getItemStates() methods returns the enabled
 * items. Note that the type in the generic parameter must implement the
 * equals() method.
 *
 * @version $Id: AddRemoveListState.java 25492 2017-03-15 12:59:25Z samuellb $
 * @param <T> Type
 */
public final class AddRemoveListState<T extends Serializable>
  ...
  public void addListItem(
      final T value, final String label, final boolean enabled) {
  ...
  /**
   * Internal method that returns the currently selected items in the enabled
   * list. These can be removed with the "Remove" button.
   *
   * @return List
   */
  public List<T> getSelectedInEnabledList() {
   ...
   * list. These can be added with the "Add" button.
   *
   * @return List
   */
  public List<T> getSelectedInAvailableList() {
