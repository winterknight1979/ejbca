...
/**
 * Managed bean that backs up the enrollwithusername.xhtml page. Extends
 * EnrollWithRequestIdBean to make use of common code
 *
...
@ManagedBean
@ViewScoped
public class EnrollWithUsernameBean extends EnrollWithRequestIdBean
  ...
  protected void postConstruct() {
    ...
    username =
        httpServletRequest.getParameter(EnrollWithUsernameBean.PARAM_USERNAME);
    paramEnrollmentCode =
        httpServletRequest.getParameter(
            EnrollWithUsernameBean.PARAM_ENROLLMENT_CODE);
    super.setRaAuthenticationBean(raAuthenticationBean);
    super.postConstruct();
  }

  /**
   * Disable the username field if we have passwed username as a parameter in
   * the URL (i.e. &amp;username=tomas). User friendly as the user can not
   * accidentally change the pre defined username
   *
   * @return bool
   */
  public boolean isUsernameDisabled() {
    ...
    return httpServletRequest.getParameter(
            EnrollWithUsernameBean.PARAM_USERNAME)
        != null;
  }
  ...
  public void checkUsernameEnrollmentCode() {
    if (StringUtils.isNotEmpty(username)) {
      ...
      // Code for handling the case where we put enrollment code as a URL GET
      ...
      try {
        raMasterApiProxyBean.checkUserStatus(
            raAuthenticationBean.getAuthenticationToken(), username, password);
  ...
  public boolean isFinalizeEnrollmentRendered() {
    ...
    return (endEntityInformation != null
