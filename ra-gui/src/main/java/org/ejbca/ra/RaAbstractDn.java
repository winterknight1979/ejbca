...
/**
 * Represents two APIs: list (needed for JSF) and map for the DN fields of the
 * specified end entity profile. It is extended by Subject DN,
 * SubjectAlternateName and Subject Directory Attributes
 *
 * @version $Id: RaAbstractDn.java 26570 2017-09-18 18:42:48Z oskareriksson $
 */
public abstract class RaAbstractDn {
  ...
  /**
   * @return one of the DnComponents.getDnProfileFields,
   *     DnComponents.getAltNameFields() or DnComponents.getDirAttrFields
   */
  ...
  public RaAbstractDn(
    ...
    if (dn != null) {
      dnFieldExtractor =
          new DNFieldExtractor(dn, getAbstractDnFieldExtractorType());
    }
        ...
        if (dnFieldExtractor != null) {
          fieldInstance.setValue(
              dnFieldExtractor.getField(
                  DnComponents.profileIdToDnId(fieldInstance.getProfileId()),
                  fieldInstance.getNumber()));
        }
        fieldInstances.add(fieldInstance);
        fieldInstancesMap
            .get(key)
            .put(fieldInstance.getNumber(), fieldInstance);
      }
    }
  }
  ...
  public Map<String, Map<Integer, EndEntityProfile.FieldInstance>>
