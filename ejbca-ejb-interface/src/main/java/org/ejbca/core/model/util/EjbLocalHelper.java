...
public class EjbLocalHelper implements EjbBridgeSessionLocal {
  ...
  /**
   * Requires a "ejb-local-ref" definition in web.xml and ejb-jar.xml from all
   * accessing components or an application server that support global JNDI
   * names (introduced in EJB 3.1).
   *
  ...
  private EjbBridgeSessionLocal getEjbLocal() {
    ...
    try {
      if (!useEjb31GlobalJndiName) {
        ret =
            (EjbBridgeSessionLocal)
                getInitialContext().lookup("java:comp/env/EjbBridgeSession");
      }
  ...
  public StatedumpSessionLocal getStatedumpSession() {
    try {
      if (!useEjb31GlobalJndiName) {
        return (StatedumpSessionLocal)
            getInitialContext().lookup("java:comp/env/StatedumpSession");
      }
    } catch (NamingException e) {
  ...
  /**
   * Dynamically loads the UnidfnrSession with JNDI. It's usually not available
   * in the EJBCA source tree, and in this case this is properly handled by
   * returning null.
   *
  ...
  public UnidfnrSessionLocal getUnidfnrSession() {
    try {
      if (!useEjb31GlobalJndiName) {
        return (UnidfnrSessionLocal)
            getInitialContext().lookup("java:comp/env/UnidfnrSession");
      }
    } catch (NamingException e) {
      // NOPMD ignore and continue
    }
