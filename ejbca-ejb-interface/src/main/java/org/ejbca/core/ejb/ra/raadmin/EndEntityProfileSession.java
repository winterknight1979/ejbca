 ...
 *
 * @version $Id: EndEntityProfileSession.java 29589 2018-08-08 12:02:53Z
 *     bastianf $
 */
public interface EndEntityProfileSession {
   ...
   * @throws AuthorizationDeniedException if admin was not authorized to add end
   *     entity profiles
   * @throws EndEntityProfileExistsException if a profile of the given name
   *     already exists
   */
  int addEndEntityProfile(
      AuthenticationToken admin, String profilename, EndEntityProfile profile)
  ...
  /**
   * Adds a end entity profile to a group with the same content as the original
   * profile.
   *
   * @param admin admin
  ...
  /**
   * Removes an end entity profile from the database, does not throw any errors
   ...
   * @throws AuthorizationDeniedException fail
   */
  void renameEndEntityProfile(
      AuthenticationToken admin, String oldprofilename, String newprofilename)
      throws EndEntityProfileExistsException, AuthorizationDeniedException;
   ...
   * @throws AuthorizationDeniedException fail
   * @throws EndEntityProfileNotFoundException if sought end entity profile was
   *     not found.
   */
  void changeEndEntityProfile(
      AuthenticationToken admin, String profilename, EndEntityProfile profile)
      throws AuthorizationDeniedException, EndEntityProfileNotFoundException;

  /**
   * Do NOT use, use changeEndEntityProfile instead. Used internally for testing
   * only. Updates a profile without flushing caches.
   *
   * <p>TODO: Move this method out of production code.
   *
   * @param admin admin
   ...
   *     not found.
   */
  void internalChangeEndEntityProfileNoFlushCache(
      AuthenticationToken admin, String profilename, EndEntityProfile profile)
      throws AuthorizationDeniedException, EndEntityProfileNotFoundException;
   ...
   *
   * @param admin the administrator for whom to get the profile Ids he/she has
   *     access to
   * @param endentityAccessRule an access rule which is required on the profile
   *     in order for it to be returned, for example
   *     AccessRulesConstants.CREATE_END_ENTITY to only return profiles for
   *     which the admin have create rights
   * @return Collection of end entity profile id:s (Integer)
   */
  Collection<Integer> getAuthorizedEndEntityProfileIds(
      AuthenticationToken admin, String endentityAccessRule);

  /**
   * Retrives a list of ids to profiles with non-existent CA ids, if the admin
   * has root rule access. Otherwise an empty list is returned.
   *
   * @param admin admin
   * @return list
   */
   ...
   */
  int getEndEntityProfileId(String profilename)
   ...
   * @return a collection of EndEntityProfile names using the sought
   *     CertificateProfile
   */
  List<String> getEndEntityProfilesUsingCertificateProfile(
      int certificateprofileid);

  /**
   * Fetches available certificate profiles associated with an end entity
   * profile.
   *
   * <p>No authorization required.
   *
   * @param admin the authentication of the caller.
   * @param entityProfileId id of the end entity profile.
   * @return a map of available certificate profiles names and IDs or an empty
   *     map.
   * @throws EndEntityProfileNotFoundException if a profile with the ID
   *     entityProfileId could not be found
   */
   ...
   * @throws AuthorizationDeniedException if the authorization was denied (still
   *     not thrown).
   * @throws EndEntityProfileNotFoundException if the end entity could not be
   *     found.
   */
   ...
   * @throws AuthorizationDeniedException if the requesting admin wasn't
   *     authorized to the profile or any of the CA's therein
   * @throws EndEntityProfileNotFoundException if the profile was not found
   */
  byte[] getProfileAsXml(AuthenticationToken authenticationToken, int profileId)
      throws AuthorizationDeniedException, EndEntityProfileNotFoundException;
}
