...
/**
 * Session bean for managing hard tokens, hard token profiles and hard token
 * issuers. A hard token is a smart card, usb token and similar. A generic thing
 * actually.
 *
 * @version $Id: HardTokenSession.java 19902 2014-09-30 14:32:24Z anatom $
 */
public interface HardTokenSession {
   ...
   */
  void addHardTokenProfile(
      AuthenticationToken admin, String name, HardTokenProfile profile)
      throws HardTokenProfileExistsException, AuthorizationDeniedException;

  /**
   * Adds a hard token profile to the database. Used for importing and exporting
   ...
   * @throws AuthorizationDeniedException fail
   */
  void removeHardTokenProfile(AuthenticationToken admin, String name)
      throws AuthorizationDeniedException;
   ...
   * @throws AuthorizationDeniedException fail
   */
  void renameHardTokenProfile(
      AuthenticationToken admin, String oldname, String newname)
      throws HardTokenProfileExistsException, AuthorizationDeniedException;

  /**
   * Retrieves a Collection of id:s (Integer) to authorized profiles. Authorized
   * hard token profiles are profiles containing only authorized certificate
   * profiles and caids.
   *
   * @param admin admin
   * @return Collection of id:s (Integer)
   */
  Collection<Integer> getAuthorizedHardTokenProfileIds(
      AuthenticationToken admin);
   ...
   */
  boolean changeHardTokenIssuer(
      AuthenticationToken admin, String alias, HardTokenIssuer issuerdata)
      throws AuthorizationDeniedException;
   ...
   * @throws AuthorizationDeniedException fail
   */
  boolean cloneHardTokenIssuer(
      AuthenticationToken admin, String oldalias, String newalias, int roleId)
      throws AuthorizationDeniedException;
   ...
   * @throws AuthorizationDeniedException fail
   */
  void removeHardTokenIssuer(AuthenticationToken admin, String alias)
      throws AuthorizationDeniedException;
   ...
   * @throws AuthorizationDeniedException fail
   */
  boolean renameHardTokenIssuer(
      AuthenticationToken admin, String oldalias, String newalias, int roleId)
      throws AuthorizationDeniedException;

  /**
   * Checks if the provided token is authorized to the issuer identified by the
   * alias, and if she is authorized to edit hard token issuers.
   *
   * @param token An AuthenticationToken
   * @param alias The alias of the hard token being checked against
   * @return true if the token is authorized to edit the issuer identified by
   *     the alias.
   */
  boolean isAuthorizedToEditHardTokenIssuer(
      AuthenticationToken token, String alias);
   ...
   *     alias.
   */
  boolean isAuthorizedToHardTokenIssuer(
      AuthenticationToken admin, String alias);
   ...
   */
  Collection<HardTokenIssuerInformation> getHardTokenIssuerDatas(
      AuthenticationToken admin);
   ...
   */
  TreeMap<String, HardTokenIssuerInformation> getHardTokenIssuers(
      AuthenticationToken admin);
   ...
   */
  org.ejbca.core.model.hardtoken.HardTokenIssuerInformation
      getHardTokenIssuerInformation(String alias);
   ...
   */
  org.ejbca.core.model.hardtoken.HardTokenIssuerInformation
      getHardTokenIssuerInformation(int id);
   ...
   * @throws UnavailableTokenException if users tokentype isn't among hard token
   *     issuers available tokentypes.
   */
  void getIsHardTokenProfileAvailableToIssuer(
      int issuerid, EndEntityInformation userdata)
   ...
   * @param significantissuerdn indicates which CA the hard token should belong
   *     to.
  ...
  void addHardToken(
      ...
      String tokensn,
      String username,
      String significantissuerdn,
      int tokentype,
      HardToken hardtokendata,
   ...
   * @param hardtokendata the hard token data
   * @throws HardTokenDoesntExistsException if tokensn does not exist in
   *     database.
   */
  void changeHardToken(
      AuthenticationToken admin,
      String tokensn,
      int tokentype,
      HardToken hardtokendata)
   ...
   *     database.
   */
  void removeHardToken(AuthenticationToken admin, String tokensn)
      throws HardTokenDoesntExistsException;
   ...
   * @throws AuthorizationDeniedException fail
   */
  HardTokenInformation getHardToken(
      AuthenticationToken admin, String tokensn, boolean includePUK)
      throws AuthorizationDeniedException;
   ...
   */
  Collection<HardTokenInformation> getHardTokens(
      AuthenticationToken admin, String username, boolean includePUK);

  /**
   * Method that searches the database for a tokensn. It returns all HardTokens
   * with a serial number that begins with the given search-pattern.
   *
   ...
   */
  void addHardTokenCertificateMapping(
      AuthenticationToken admin, String tokensn, Certificate certificate);
   ...
   * @param certificate the certificate to map to.
   */
  void removeHardTokenCertificateMapping(
      AuthenticationToken admin, Certificate certificate);
   ...
   */
  String findHardTokenByCertificateSNIssuerDN(
      BigInteger certificatesn, String issuerdn);
   ...
   */
  List<String> getHardTokenProfileUsingCertificateProfile(
