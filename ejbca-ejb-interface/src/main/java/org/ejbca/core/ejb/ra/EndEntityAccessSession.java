 ...
 *
 * @version $Id: EndEntityAccessSession.java 29301 2018-06-21 10:27:38Z
 *     andresjakobs $
 */
public interface EndEntityAccessSession {
   ...
   */
  EndEntityInformation findUser(AuthenticationToken admin, String username)
   ...
   * @param subjectdn DN
   * @param issuerdn DN
   * @return A list of all EndEntityInformations found matching those DNs, or an
   *     empty list
   * @throws AuthorizationDeniedException fail
   */
  List<EndEntityInformation> findUserBySubjectAndIssuerDN(
      AuthenticationToken admin, String subjectdn, String issuerdn)
      throws AuthorizationDeniedException;
   ...
   * @param admin admin
   * @param subjectdn DN
   * @return A list of all EndEntityInformations matching the given DN, or an
   *     empty list
   * @throws AuthorizationDeniedException if the admin was not authorized to the
   *     end entity profile or issuing CA
   */
  List<EndEntityInformation> findUserBySubjectDN(
      AuthenticationToken admin, String subjectdn)
      throws AuthorizationDeniedException;
   ...
   *     end entity profile or issuing CA
   */
  List<EndEntityInformation> findUserByEmail(
      AuthenticationToken admin, String email)
      throws AuthorizationDeniedException;
  ...
  Collection<EndEntityInformation> findAllUsersByStatus(
      AuthenticationToken admin, int status);
   ...
   * @return Collection of EndEntityInformation, or empty collection if the
   *     query is illegal or no users exist
   */
  Collection<EndEntityInformation> findAllUsersByCaId(
      AuthenticationToken admin, int caid);

  /**
   * Finds all batch users with a specified status. Limited by the maximum query
   * count define in the global configuration.
   *
   ...
   * @param caauthorizationstring is a string placed in the where clause of SQL
   *     query indication which CA:s the administrator is authorized to view.
   * @param endentityprofilestring is a string placed in the where clause of SQL
   *     query indication which endentityprofiles the administrator is
   *     authorized to view.
   * @param numberofrows the number of rows to fetch, use 0 for the maximum
   *     query count define in the global configuration.
   * @param endentityAccessRule The end entity access rule that is necessary to
   *     execute the query
   ...
   * @param onlyValid only return valid certificates not revoked or expired
   *     ones.
   * @param now the current time as long value since epoch.
   * @return a collection of certificate wrappers or an empty list if no
   *     certificates, or no user, could be found.
