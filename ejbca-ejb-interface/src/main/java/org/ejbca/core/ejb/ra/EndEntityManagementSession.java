...
/**
 * Session bean handling end entity administration, i.e. adding and editing end
 * entities.
 *
 * @version $Id: EndEntityManagementSession.java 29540 2018-08-01 08:20:04Z
 *     jekaterina_b_helmes $
 */
public interface EndEntityManagementSession {

  /**
   * Check if user is authorized for specified an EE access rule for specified
   * EEP.
   *
   * @param admin authenticationToken to be check for authorization
   * @param profileid id of EEP for which operation authorization needs to be
   *     checked
   * @param rights EE access rule to be checked (etc.
   *     AccessRulesConstants.DELETE_END_ENTITY...)
   * @return true if user is authorized for specified EE access rules for
   ...
   * @param tokentype the type of token to be generated, one of SecConst.TOKEN
   *     constants
   ...
   * @throws EndEntityExistsException if an end entity by the specified username
   *     already exists
   */
  void addUser(
      ...
      throws AuthorizationDeniedException, EndEntityProfileValidationException,
          ...
          EndEntityExistsException, CustomFieldException, IllegalNameException,
   ...
   *     timecreated and timemodified will not be used.
   * @param clearpwd true if the password will be stored in clear form in the
   *     db, otherwise it is hashed.
   ...
   * @param clearpwd true if the password will be stored in clear form in the
   *     db, otherwise it is hashed.
   * @return returned object contains the same sensitive information as was
   *     passed in clearpwd, or an autogenerated pwd.
   * @throws AuthorizationDeniedException if the administrator isn't authorized
   *     to add user
   * @throws EndEntityProfileValidationException if data doesn't fulfill
   *     requirements of the end entity profile
   * @throws EndEntityExistsException if user already exists or some other
   *     database error occurs during commit
   * @throws WaitingForApprovalException if approval is required and the action
   *     has been added in the approval queue.
   * @throws CADoesntExistsException fail
   * @throws IllegalNameException if the Subject DN or SAN failed constraints
   * @throws CustomFieldException if the end entity was not validated by a
   *     locally defined field validator
   * @throws ApprovalException if an approval already exists for this request.
   * @throws CertificateSerialNumberException if the CA requires that Subject DN
   *     Serial Numbers be unique, and the one specified here already exists.
   */
  EndEntityInformation addUser(
      AuthenticationToken admin,
      EndEntityInformation userdata,
      boolean clearpwd)
      throws AuthorizationDeniedException, EndEntityProfileValidationException,
          EndEntityExistsException, WaitingForApprovalException,
          CADoesntExistsException, IllegalNameException, CustomFieldException,
          ApprovalException, CertificateSerialNumberException;
   ...
   * @param clearpwd true if the password will be stored in clear form in the
   *     db, otherwise it is hashed.
   * @param lastApprovingAdmin The last administrator to have approved the
   *     request
   * @throws AuthorizationDeniedException if administrator isn't authorized to
   *     add user
   * @throws EndEntityProfileValidationException if data doesn't fulfill
   ...
   * @throws AuthorizationDeniedException if administrator isn't authorized to
   *     add user
   * @throws EndEntityProfileValidationException if data doesn't fullfil
   *     requirements of end entity profile
   ...
   * @throws AuthorizationDeniedException if administrator isn't authorized to
   *     add user
   * @throws EndEntityProfileValidationException if data doesn't fllfil
   *     requirements of end entity profile
   * @throws WaitingForApprovalException if approval is required and the action
   *     have been added in the approval queue.
   * @throws CADoesntExistsException if the caid of the user does not exist
   ...
   * @param clearpwd true if the password will be stored in clear form in the
   *     db, otherwise it is hashed.
   * @param approvalRequestId the unique ID of the approval request (not the
   *     hash)
   * @param lastApprovingAdmin the last administrator to have approved the
   *     request
   * @throws AuthorizationDeniedException if administrator isn't authorized to
   ...
   * @throws CertificateSerialNumberException if SubjectDN serial number already
   *     exists.
   * @throws NoSuchEndEntityException NoSuchEndEntityException if the user does
   *     not exist
   * @throws CustomFieldException if the end entity was not validated by a
   *     locally defined field validator
   */
  void changeUserAfterApproval(
      AuthenticationToken admin,
      EndEntityInformation userdata,
      boolean clearpwd,
   ...
   * @throws AuthorizationDeniedException if admin was not authorized to remove
   *     end entities
   ...
   */
  void deleteUser(AuthenticationToken admin, String username)
   ...
   *     have been added in the approval queue.
   */
  void setUserStatus(AuthenticationToken admin, String username, int status)
      throws AuthorizationDeniedException, NoSuchEndEntityException,
          ApprovalException, WaitingForApprovalException;

  /**
   * Changes status of a user. This method is called mainly when executing a
   * ChangeStatusEndEntityApprovalRequest
   *
   * @param admin An authentication token
   * @param username the unique username.
   * @param status the new status, from 'UserData'.
   * @param approvalRequestID The ID of the approval request submitted to change
   *     the status
   * @param lastApprovingAdmin the last administrator to have approved the
   *     request
   * @throws AuthorizationDeniedException fail
   ...
   * @throws NoSuchEndEntityException if the end entity was not found
   */
  void setUserStatusAfterApproval(
      AuthenticationToken admin,
      String username,
      int status,
      int approvalRequestID,
   ...
   * @throws NoSuchEndEntityException if the end entity was not found
   */
  void setPassword(AuthenticationToken admin, String username, String password)
  ...
  /**
   * Verifies a password for a user, decreasing the remaining login attempts if
   * verification fails and if caller wants (and it is not set to unlimited).
   *
   * @param admin the administrator performing the action
   * @param username the unique username.
   * @param password the password to be verified.
   * @param decRemainingLoginAttemptsOnFailure if true and password verification
   *     fails, will try to decrease remaining login attempts (which can be
   *     unlimited)
  ...
  boolean verifyPassword(
      ...
      String username,
      String password,
      boolean decRemainingLoginAttemptsOnFailure)
      throws EndEntityProfileValidationException, AuthorizationDeniedException,
          NoSuchEndEntityException;
   ...
   * @throws CouldNotRemoveEndEntityException if the user could not be deleted.
   */
  void revokeAndDeleteUser(
      AuthenticationToken admin, String username, int reason)
      throws AuthorizationDeniedException, ApprovalException,
          WaitingForApprovalException, NoSuchEndEntityException,
          CouldNotRemoveEndEntityException;

  /**
   * Method that revokes a user. Revokes all users certificates and then sets
   * user status to revoked. If user status is already revoked it still revokes
   * all users certificates, ignoring the ones that are already revoked.
   *
   * @param admin admin
   ...
   * @throws AuthorizationDeniedException fail
   * @throws NoSuchEndEntityException fail
   * @throws AlreadyRevokedException if user is revoked and unrevocation is
   *     attempted by sending revocation reason NOTREVOKED or REMOVEFROMCRL
   ...
   * @param reason for revocation, one of {@link
   *     org.cesecore.certificates.crl.RevokedCertInfo}.REVOKATION_REASON_
   *     constants or use {@link
   *     org.cesecore.certificates.crl.RevokedCertInfo}.NOT_REVOKED to un-revoke
   *     a certificate on hold.
   ...
   * Method that revokes a user. Revokes all users certificates and then sets
   * user status to revoked. If user status is already revoked it still revokes
   * all users certificates, ignoring the ones that are already revoked. This
   * method is called mainly when executing a RevocationApprovalRequest
   *
   * @param admin admin
   * @param username the username to revoke.
   * @param reason revocation reason to use in certificate revocations
   * @param approvalRequestID the ID of the approval request submitted to revoke
   *     the user
   * @param lastApprovingAdmin the last administrator to have approved the
   *     request
   * @throws AuthorizationDeniedException fail
  ...
  /**
   * Same as {@link #revokeCert(AuthenticationToken, BigInteger, String, int)}
   * but also sets the revocation date.
   *
   * @param admin admin
   * @param certserno sn
   ...
   * @param issuerdn dn
   * @param reason reason
   * @param checkPermission if true and if 'revocationdate' is not null then the
   *     certificate profile must allow back dating otherwise a {@link
   *     RevokeBackDateNotAllowedForProfileException} is thrown.
   * @throws AuthorizationDeniedException fail
   ...
   * un-revoke a certificate that has been revoked with reason ON_HOLD. This is
   * done by giving reason RevokedCertInfo.NOT_REVOKED (or
   ...
   *     constants. Use RevokedCertInfo.NOT_REVOKED to re-activate a certificate
   *     on hold.
   * @throws AuthorizationDeniedException fail
   ...
   * RevokedCertInfo.REVOCATION_REASON_REMOVEFROMCRL).
   *
   * <p>reason, certificateProfileId and revocationdate can be included as input
   * parameters wrapped into CertRevocationDto dto class.
   *
   * @param admin token of the administrator performing the action
   * @param certRevocationDto wrapper object of the input parameters for the
   *     revoke.
   * @throws AuthorizationDeniedException fail
   * @throws AlreadyRevokedException if the certificate was already revoked
   * @throws NoSuchEndEntityException if certificate to revoke can not be found
  ...
  void revokeCertWithMetadata(
      AuthenticationToken admin, CertRevocationDto certRevocationDto)
      throws AuthorizationDeniedException, NoSuchEndEntityException,
          ApprovalException, WaitingForApprovalException,
          AlreadyRevokedException, RevokeBackDateNotAllowedForProfileException,
   ...
   * RevokedCertInfo.REVOCATION_REASON_REMOVEFROMCRL).
   *
   * <p>This method is called mainly when executing a RevocationApprovalRequest
   *
   * @param admin the administrator performing the action
   * @param certserno the serno of certificate to revoke.
   * @param issuerdn DN
   ...
   * @return true if certificate belongs to a user, false if no user with
   *     specified certificate exists
   */
  boolean checkIfCertificateBelongToUser(
      BigInteger certificatesnr, String issuerdn);

  /**
   * Method checking if username already exists in database. WARNING: do not use
   * this method where an authorization check is needed, use findUser there
   * instead.
   *
   * @param username user
   ...
   * @param certificate is the certificate to recover the keys for. Use 'null'
   *     to recovery the certificate with latest not before date.
  ...
  /**
   * Marks a user for key recovery and performs authorization checks. This
   * method will not mark the certificate for key recover. Method is intended to
   * be used when KeyRecoveryData is not present in the instance database (eg.
   * when local key generation is enabled).
   *
   * @param admin used to authorize this action
   * @param username is the user to key recover a certificate for
   * @param endEntityProfileId of the end entity related to certificate
   * @param certificate used to check if approval is required to recover this
   *     certificate
   * @return true if operation was completed successfully
   * @throws AuthorizationDeniedException if requesting administrator isn't
   *     authorized to perform key recovery
  ...
  /**
   * Selects a list of specific list of EndEntityInformation entities, as
   * filtered by the below parameters.
   *
   * @param caIds The list of CAIDs to filter by. If this list is empty, all the
   *     UserData objects that match the given expiration and status are
   *     returned.
   ...
   * @return list
   */
  List<EndEntityInformation> findUsers(
      List<Integer> caIds, long timeModified, int status);

  /**
   * Rename an end entity. Updates existing references in the database to this
   * username.
   *
   * <p>No re-publishing with updated username will take place.
   *
   ...
   * @throws AuthorizationDeniedException if the user was not authorized to edit
   *     this end entity
   * @throws EndEntityExistsException the newUsername is already taken by
   *     another end entity
   */
  boolean renameEndEntity(
      AuthenticationToken admin, String currentUsername, String newUsername)
