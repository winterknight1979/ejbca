...
/**
 * @version $Id: HardTokenBatchJobSession.java 19902 2014-09-30 14:32:24Z anatom
 *     $
 */
public interface HardTokenBatchJobSession {
  ...
  /**
   * Returns a Collection of users scheduled for batch generation for the given
   * issuer. A maximum of MAX_RETURNED_QUEUE_SIZE users will be returned by
   * call.
   *
   * @param alias alias
   ...
   *
   * @param alias alias
   * @param index index in queue of user to retrieve. (First position is 1
   *     according to old JDBC implementation.)
   * @return The next token to generate or NULL if the given user doesn't exist
   *     in queue.
   * @throws UnavailableTokenException fail
   * @throws javax.ejb.EJBException if a communication or other error occurs.
   */
   ...
   * token issuer id. This function is mainly for avoiding desynchronisation
   * when a hard token issuer is deleted.
   *
