...
public interface AdminPreferenceSession {
   ...
   *
   * @param certificatefingerprint The certificate finger print of the
   *     certificate the admin uses as a credential.
   ...
   *
   * @param admin An {@link X509CertificateAuthenticationToken} representing the
   *     admin the preference should cover.
   * @param adminpreference the admin preference to add.
   * @return false if admin already exists.
   */
  boolean addAdminPreference(
   ...
   * @return false if admin does not exist.
   */
  boolean changeAdminPreferenceNoLog(
      X509CertificateAuthenticationToken admin,
      AdminPreference adminpreference);
   ...
   * @param defaultadminpreference The {@link AdminPreference} to save as
   *     default.
   * @throws AuthorizationDeniedException if the local {@link
   *     AuthenticationToken} wasn't authorized to
   *     /system_functionality/edit_systemconfiguration
   */
  void saveDefaultAdminPreference(
      AuthenticationToken admin, AdminPreference defaultadminpreference)
