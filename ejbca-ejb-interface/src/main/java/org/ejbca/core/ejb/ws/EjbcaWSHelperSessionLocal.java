...
/**
 * Local interface for EjbcaWSHelperSession. These methods are used from EjbcaWS
 * only
 *
 * @version $Id: EjbcaWSHelperSessionLocal.java 29326 2018-06-25 12:45:51Z
 *     bastianf $
 */
@Local
public interface EjbcaWSHelperSessionLocal extends EjbcaWSHelperSession {
   ...
   * with the given client certificate. - Checks (through
   ...
   * mapped in an admin role. Does not check any other authorization though,
   * other than that it is an administrator.
   *
   ...
   * @throws AuthorizationDeniedException if searching by a CA which the admin
   *     is not authorized to.
   * @throws CADoesntExistsException if searching for a non-existent CA.
   * @throws EndEntityProfileNotFoundException if usermatch was for and end
   *     entity profile, and that profile didn't exist
   */
  Query convertUserMatch(AuthenticationToken admin, UserMatch usermatch)
      throws CADoesntExistsException, AuthorizationDeniedException,
          EndEntityProfileNotFoundException;

  /**
   * Checks authorization for each certificate and optionally check that it's
   * valid. Does not check revocation status.
   *
  ...
  /**
   * Verifies that the given certificate has a correct signature and is
   * currently valid, and if so, sets the end entity status to NEW and sets its
   * password. Used from EjbcaWS.cvcRequest during renewals.
   *
   ...
   * @throws EndEntityProfileValidationException if the password does not
   *     satisfy the requirements in the end entity profile
   * @throws NoSuchEndEntityException if there's no end entity with the given
   *     username
   * @throws AuthorizationDeniedException if not authorized to the given end
   *     entity
   * @throws ApprovalException if an approval error occurred while setting end
   *     entity status
   * @throws WaitingForApprovalException if approval is required for setting the
   *     end entity status
   */
  void checkValidityAndSetUserPassword(
      AuthenticationToken admin,
  ...
  /**
   * Resets the password and restores the status for the given end entity. Used
   * for restoring the end entity status if a certificate request operation
   * failed. All exceptions are swallowed and error-logged, since this method is
   * intended to be called from catch blocks.
   *
   * @param admin authentication token.
   * @param username username of end entity
   * @param status original end entity status to restore
   */
  void resetUserPasswordAndStatus(
      AuthenticationToken admin, String username, int status);
   ...
   */
  void importCaCert(AuthenticationToken admin, String caname, byte[] certbytes)
   ...
   */
  void updateCaCert(AuthenticationToken admin, String caname, byte[] certbytes)
   ...
   * @throws CryptoTokenOfflineException fail
   */
  void rolloverCACert(AuthenticationToken admin, String caname)
      throws AuthorizationDeniedException, CADoesntExistsException,
  ...
  /**
   * Help method returning a subset of certificates containing only valid
   * certificates expiredate and revocation status is checked.
   *
   * @param admin admin
   * @param certs carts
   * @return valid certs
   ...
   */
  Collection<java.security.cert.Certificate> returnOnlyValidCertificates(
      AuthenticationToken admin,
      Collection<java.security.cert.Certificate> certs);
   ...
   * @throws EjbcaException fail
   */
  HardTokenDataWS convertHardTokenToWS(
      HardTokenInformation data,
