 ...
 *
 * @version $Id: ApprovalSessionLocal.java 26119 2017-07-03 15:02:08Z
 *     oskareriksson $
 */
@Local
public interface ApprovalSessionLocal extends ApprovalSession {
  ...
  /**
   * Send all work-flow related approval notifications given the last added
   * Approval to the approvalsPerformed.
   *
   * @param approvalRequest the approval request
   * @param approvalProfile the profile determining the approval work flow
   * @param approvalData the ApprovalData object referring to the approval
   *     request
   * @param expired should be set to true if the notification is due to
   *     expiration of the approval request
   */
  void sendApprovalNotifications(
  ...
  /**
   * Returns a list of all approval requests whose status is
   * WAITING_FOR_APPROVAL only (aka. not including status APPROVED). This method
   * is called mainly when upgrading older approval requests to EJBCA 6.6.0 or
   * later.
   *
   * @return A list of all approval requests whose status is
  ...
  /**
   * Updates the approval request field for an approval. Doesn't do any
   * authorization checks.
   *
   * @param approvalDataId the ID of an approvalData object (not the approvalID
   *     hash)
   ...
   * Approval. Doesn't do any authorization checks.
   *
   * @param authenticationToken The administrator requesting extension of the
   *     request. Used for audit logging only.
   * @param approvalDataId Id of the approval request
   * @param extendForMillis The new expiration date will be set to current time
   *     plus this number of milliseconds
   * @throws IllegalStateException if the request has been approved or denied
   *     already.
   */
  void extendApprovalRequestNoAuth(
   ...
   * @param caAuthorizationString a list of authorized CA Ids in the form
   *     'cAId=... OR cAId=...'
   * @param endEntityProfileAuthorizationString a list of authorized end entity
   *     profile ids in the form '(endEntityProfileId=... OR
   *     endEntityProfileId=...) objects only
   ...
   *
   * @param includeUnfinished Includes requests that haven't been executed or
   *     rejected yet.
   * @param includeProcessed Includes requests that have been approved and
   *     executed, or rejected.
   * @param includeExpired Includes requests that have expired.
   * @param startDate Include requests from this date and later, or null for no
   *     limit.
   * @param endDate Include requests up to this date, or null for no limit.
   * @param expiresBefore When searching unfinished requests, include only those
   *     that expire before this date. Set to null to include all.
   * @param index where the ResultSet should start
   * @param numberofrows maximum number of rows
   * @param caAuthorizationString a list of authorized CA Ids in the form
   *     'cAId=... OR cAId=...'
   ...
   * @return a List of ApprovalDataVO, never null
   */
  List<ApprovalDataVO> queryByStatus(
