...
/**
 * EJBCA specific security audit event types, for audit using CESecore's audit
 * log.
 *
 * <p>These event types extend the list of already existing event types of
 * CESeCore.
 *
 ...
 * @version $Id: EjbcaEventTypes.java 26323 2017-08-15 17:24:53Z anatom $
 */
public enum EjbcaEventTypes implements EventType {
  ...
  APPROVAL_ADD,
  /**
   * Action that requires approval was approved by one of the required
   * administrator(s).
   */
  APPROVAL_APPROVE,
  ...
  APPROVAL_EDIT,
  /**
   * Action that requires approval was rejected by one of the required
   * administrator(s).
   */
  APPROVAL_REJECT,
  ...
  CA_REMOVETOKEN,
  /**
   * Renewal of a Certificate Authority's certificate, optionally using a
   * different key pair.
   */
  CA_RENEWED,
  ...
  CA_ROLLEDOVER,
  /**
   * Restoration of a Certificate Authority's previously removed (soft) Crypto
   * Token.
   */
  CA_RESTORETOKEN,
  /**
  ...
  CA_REVOKED,
  /**
   * Certificate Authority signs (attests) a provided certificate signing
   * request.
   */
  CA_SIGNREQUEST,
  ...
  CA_USERAUTH,
  /**
   * Certificate Authority's signing certificate is not valid yet or not valid
   * any longer.
   */
  CA_VALIDITY,
  ...
  HARDTOKEN_ADD,
  /**
   * Creation of link from a (client) hardware token representation to a
   * certificate.
   */
  ...
  HARDTOKEN_EDIT,
  /**
   * Modification or name change of an existing issuer for (client) hardware
   * tokens.
   */
  HARDTOKEN_EDITISSUER,
  /**
   * Modification or name change of an existing template for (client) hardware
   * tokens.
   */
  HARDTOKEN_EDITPROFILE,
  /**
   * Outcome of provisioning of a (client) hardware token reported by external
  ...
  HARDTOKEN_REMOVE,
  /**
   * Removal of link from a (client) hardware token representation to a
   * certificate.
   */
  ...
  /**
   * Administrator views the content of a (client) hardware token
   * representation.
   */
  HARDTOKEN_VIEWED,
  /**
   * Administrator views the PUK code of a (client) hardware token
   * representation.
   */
  HARDTOKEN_VIEWEDPUK,
  /**
   * Persistence of encrypted key material and meta data that can be used for
   * recovering a server-side generated client key pair.
   */
  KEYRECOVERY_ADDDATA,
  /**
   * Modification of encrypted key material and meta data that can be used for
   * recovering a server-side generated client key pair.
   */
  KEYRECOVERY_EDITDATA,
  /**
   * Change status of meta data for encrypted key material to allow extraction
   * of server-side generated client key pair.
   */
  KEYRECOVERY_MARKED,
  /**
   * Removal of specific or all encrypted key material and meta data that can be
   * used for recovering a server-side generated client key pair.
   */
  ...
  RA_RENAMEEEPROFILE,
  /**
   * Change status of an existing end entity and all the end entity's
   * certificates to revoked.
   */
  RA_REVOKEDENDENTITY,
  ...
  /**
   * Publishing of a certificate and/or related certificate meta data when
   * certificate is activated after being on hold.
   */
