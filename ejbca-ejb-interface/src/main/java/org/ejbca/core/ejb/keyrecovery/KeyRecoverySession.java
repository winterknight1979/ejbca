...
/**
 * @version $Id: KeyRecoverySession.java 26210 2017-08-03 10:12:32Z samuellb $
 */
public interface KeyRecoverySession {
   ...
   *
   * <p>If /superadmin -&gt; true
   *
   * <p>Other must have both AccessRulesConstants. /ra_functionality/keyrecovery
   ...
   * @throws WaitingForApprovalException if approval is required. Expected to be
   *     thrown in this case
   ...
   * @throws AuthorizationDeniedException if not authorized to administer keys
   */
  void removeKeyRecoveryData(
      AuthenticationToken admin, CertificateWrapper certificate)
      throws AuthorizationDeniedException;
   ...
   */
  KeyRecoveryInformation recoverKeys(
      AuthenticationToken admin, String username, int endEntityProfileId)
      throws AuthorizationDeniedException;

  /**
   * Marks a users newest certificate for key recovery. Newest means certificate
   * with latest not before date.
   *
   * @param admin the administrator calling the function
   * @param username or the user.
   * @param endEntityProfileId the end entity profile of the user, used for
   *     access control
   * @return true if operation went successful or false if no certificates could
   *     be found for user, or user already marked.
   ...
   *     exist
   */
  boolean markNewestAsRecoverable(
      AuthenticationToken admin, String username, int endEntityProfileId)
   ...
   * @return true if operation went successful or false if certificate couldn't
   *     be found.
   * @throws AuthorizationDeniedException fail
   * @throws WaitingForApprovalException fail
   * @throws ApprovalException fail
