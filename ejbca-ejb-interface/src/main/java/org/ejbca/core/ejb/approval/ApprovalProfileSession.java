 ...
 *
 * <p>In order to add or edit ApprovalProfiles the admin needs access to: -
 * /ca_functionality/ (StandardRules.CAFUNCTIONALITY)
 *
 * <p>TODO: Generate a separate session beans specifically for performing CRUD
 * operations on ProfileData objects, agnostic of their end purpose. This bean
 * is sufficient for now though.
 *
 * @version $Id: ApprovalProfileSession.java 24037 2016-07-27 15:00:00Z anatom $
 */
public interface ApprovalProfileSession {
   ...
   * @throws ApprovalProfileExistsException trying to add a profile with a name
   *     or id that already exists
  ...
  /**
   * Adds an approval profile with the same content as the original approval
   * profile.
   *
   * @param admin Administrator performing the operation
   * @param profile name of original approval profile
   * @param newname name of new approval profile
   * @throws ApprovalProfileExistsException if a profile with newname already
   *     exists
   * @throws ApprovalProfileDoesNotExistException if a profile with orgname does
   *     not exist
   * @throws AuthorizationDeniedException if current administrator is not
   *     authorized to modify profiles
   */
   ...
   * profiles that the specified administrator is authorized to will be
   * returned.
   *
   * @param admin Administrator performing the operation
   ...
   * @throws ApprovalProfileExistsException if a profile with name newname
   *     already exists
   * @throws ApprovalProfileDoesNotExistException if the profile to be renamed
  ...
  /**
   * Removes an approval profile from the database, does not throw any errors if
