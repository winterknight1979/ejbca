 ...
 *
 * @version $Id: BlacklistSessionLocal.java 26315 2017-08-15 08:16:04Z anatom $
 */
@Local
public interface BlacklistSessionLocal extends BlacklistSession {
   ...
   *
   * @param id ID
   * @return a Blacklist or null if a public key blacklist with the given id
   *     does not exist. Uses cache to get the object as quickly as possible.
   */
  BlacklistEntry getBlacklistEntry(int id);

  /**
   * Returns the id of the public key blacklist entry with the given
   * fingerprint.
   *
   * @param type type
   * @param value value
   ...
   *
   * @param id ID
   * @return the fingerprint of the public key blacklist with the given id or
   *     null if none was found.
   */
  String getBlacklistEntryFingerprint(int id);

  /**
   * Adds a public key blacklist entry to the database. Used for importing and
   * exporting entries from xml-files.
   *
   ...
   *     ca_functionality/edit_publickeyblacklist
   * @throws BlacklistExistsException if public key blacklist already exists.
   */
  void addBlacklistEntry(
      AuthenticationToken admin, int id, BlacklistEntry entry)
      throws AuthorizationDeniedException, BlacklistExistsException;

  /**
   * Flushes the public key blacklist entry cache to ensure that next time they
