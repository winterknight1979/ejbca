...
public enum CommandLibrary {
  ...
  CommandLibrary() {
    synchronized (this) {
      if (root == null) {
        root = new Branch("");
        ServiceLoader<? extends CliCommandPlugin> serviceLoader =
            ServiceLoader.load(CliCommandPlugin.class);
        try {
          for (Iterator<? extends CliCommandPlugin> iterator =
            ...
            if (!command.getCommandPathAliases().isEmpty()) {
              Iterator<String[]> aliasIterator =
                  command.getCommandPathAliases().iterator();
          ...
          if (e.getCause() instanceof IllegalStateException
              && e.getCause()
                  .getLocalizedMessage()
                  .contains("No EJB receiver")) {
            log.error(
                "Error: CLI could not contact EJBCA instance. Either your"
                    + " application server is not up and running, EJBCA has"
                    + " not been deployed successfully, or some firewall rule"
                    + " is blocking the CLI from the application server.");
            System.exit(1);
          } else {
            throw e;
          }
   ...
   * @return Result
   */
  public CommandResult findAndExecuteCommandFromParameters(
  ...
  /**
   * @param args The full argument list from a command. Just pass the whole
   *     thing in there and the library will try to figure out if a command is
   *     hidden in there somewhere.
   * @return true if a command can be found that matches the path in the args
   */
  public boolean doesCommandExist(final String... args) {
    return root.doesCommandExist(args);
  }

  /**
   * A private subclass which describes a branch in the command tree. A branch
   * can contain both a number of subbranches as well as a number of direct
   * commands.
   *
   * @version $Id: CommandLibrary.java 26134 2017-07-06 06:12:16Z oskareriksson
   *     $
  ...
  private static class Branch {
    ...
    // Denotes that a branch is an alias branch, i.e it won't show up in the man
    // page.
    /** Param. */
    private final boolean isAlternate;
    /** Param. */
    private final Map<String, Branch> subBranches =
        new HashMap<String, Branch>();
    /** Param. */
    private final Map<String, CliCommandPlugin> commands =
        new HashMap<String, CliCommandPlugin>();
    /** Param. */
    private final Map<String, CliCommandPlugin> alternateCommands =
        new HashMap<String, CliCommandPlugin>();

    /**
     * @param aname name
     */
    Branch(final String aname) {
      this.name = aname;
     ...
     *
     * @param aname name
     * @param aisAlternate bool
     */
   Branch(final String aname, final boolean aisAlternate) {
      this.name = aname;
      this.isAlternate = aisAlternate;
    }
     ...
     */
    public void addChild(final CliCommandPlugin command, final String... path) {
    ...
    public void addChild(
        final CliCommandPlugin command,
        final boolean isAlternatePath,
        final String... path) {
      if (path.length == 0) {
        ...
        if (commands.containsKey(command.getMainCommand())) {
          throw new CliCommandLibraryConflictException(
              "Command "
                  ...
                  + "("
                  + command.getClass()
                  + ") has been added twice. Conflicts with "
                  + commands.get(command.getMainCommand()).getClass());
        } else {
        ...
        if (childName == command.getMainCommand()
            || command.getMainCommandAliases().contains(childName)) {
          throw new CliCommandLibraryConflictException(
              "Naming conflict when attempting to construct command library: "
                  + childName);
        }
     ...
     * @return true if command exists, or if a request to print the man page of
     *     a subbranch has come in.
     */
    public boolean doesCommandExist(final String... args) {
      if (args.length == 0) {
        return false;
    ...
    public void printManPage() {
      ...
      if (sortedSubCommandList.size() > 0) {
        ...
        stringBuffer.replace(
            stringBuffer.length() - subcommandDelimiter.length(),
            stringBuffer.length(),
            " ]\n");
      }
      ...
      if (sortedCommandList.size() > 0) {
        stringBuffer.append(DELIMITER);
        stringBuffer.append(
            (sortedSubCommandList.size() > 0 ? "And the " : "The ")
                + "following commands are available:\n");
        for (String command : sortedCommandList) {
          stringBuffer.append(
              TAB
                  + command
                  + indent.substring(command.length())
                  + commands.get(command).getCommandDescription()
                  + "\n");
        }
      }
      stringBuffer.append(
    ...
    private String tab(final int indentation) {
    ...
    public CommandResult execute(final String... parameters) {
      if (parameters.length == 0) {
        // We only got to a branch
        if (isAlternate) {
          INSTANCE.log.warn(
              "WARNING: The path used is an unlisted alternate path and may be"
                  + " deprecated, and may cease to exist at any point. Please"
                  + " start using the updated path as soon as possible.\n");
        }
        printManPage();
        ...
        if (commands.containsKey(key) || alternateCommands.containsKey(key)) {
          if (isAlternate) {
            INSTANCE.log.warn(
                "WARNING: The path used is an unlisted alternate path and may"
                    + " be deprecated, and may cease to exist at any point."
                    + " Please start using the updated path as soon as"
                    + " possible.\n");
          }
          if (alternateCommands.containsKey(key)) {
            INSTANCE.log.warn(
                "WARNING: The command \""
                    + key
                    + "\" used is an unlisted alternate command and may be"
                    + " deprecated, and may cease to exist at any point."
                    + " Please start using the updated command \""
                    + alternateCommands.get(key).getMainCommand()
                    + "\" as soon as possible.\n");
            return alternateCommands
                .get(key)
                .execute(Arrays.copyOfRange(parameters, 1, parameters.length));
          } else {
            return commands
                .get(key)
                .execute(Arrays.copyOfRange(parameters, 1, parameters.length));
