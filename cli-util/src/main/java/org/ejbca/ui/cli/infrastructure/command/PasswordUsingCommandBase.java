...
/**
 * @version $Id: PasswordUsingCommandBase.java 19968 2014-10-09 13:13:58Z
 *     mikekushner $
 */
public abstract class PasswordUsingCommandBase extends CommandBase {
  ...
  private static final Logger LOG =
      Logger.getLogger(PasswordUsingCommandBase.class);
  ...
  /** Param. */
  private static final String PASSWORD_MAN_TEXT =
      "Use the syntax "
          + USERNAME_KEY
          + " <username> "
          + PASSWORD_KEY
          + "=<password> to specify password explicitly or "
          + USERNAME_KEY
          + " <username> "
          + PASSWORD_PROMPT_KEY
          + " to prompt";
   ...
   * @throws CliUsernameException If no username was supplied, and the default
   *     user is disabled.
   * @throws CliAuthenticationFailedException for any password related issues
   */
  private ParameterContainer stripUsernameAndPasswordFromParameters(
      final ParameterContainer parameters)
      throws CliUsernameException, CliAuthenticationFailedException {
    GlobalConfigurationSessionRemote gcsession =
    ...
    if (gcsession == null) {
      throw new CliAuthenticationFailedException(
          "Can not get configuration from server. Is server started and"
              + " communication working?");
    }
    GlobalConfiguration configuration =
        (GlobalConfiguration)
            gcsession.getCachedConfiguration(
    ...
    if (parameters.containsKey(PASSWORD_KEY)) {
      ...
      if (password.startsWith("file:") && (password.length() > len)) {
        final String fileName = password.substring(len);
        // Read the password file and just take the first line as being the
        // password
        try {
          ...
          br.close();
          if (password != null) {
            // Trim it, it's so easy for people to include spaces after a line,
            // and a password should never end with a space
            password = password.trim();
          }
          if ((password == null) || (password.length() == 0)) {
            LOG.error("File '" + fileName + "' does not contain any lines.");
            throw new CliAuthenticationFailedException(
                "File '" + fileName + "' does not contain any lines.");
          }
        } catch (IOException e) {
          throw new CliAuthenticationFailedException(
              "File '" + fileName + "' can not be read: " + e.getMessage());
        }
      }
    ...
    boolean defaultUserEnabled =
        configuration.getEnableCommandLineInterfaceDefaultUser();
    if ((username == null || password == null)) {
      if (defaultUserEnabled) {
        ...
        LOG.info(
            "No CLI user was supplied, and use of the default CLI user is"
                + " disabled.");
        throw new CliUsernameException(
            "No CLI user was supplied, and use of the default CLI user is"
                + " disabled.");
      }
    } else if (username.equals(EjbcaConfiguration.getCliDefaultUser())
        && !defaultUserEnabled) {
    ...
    if (!EjbRemoteHelper.INSTANCE
        ...
        .existsUser(username)) {
      // We only check for username here, but it's needless to give too much
      // info.
      LOG.info(
          "CLI authentication failed. The user '"
              + username
              + "' with the given password does not exist.");
      throw new CliAuthenticationFailedException(
          "Authentication failed. User " + username + " not exist.");
    }
  ...
  public CommandResult execute(final String... arguments) {
    ...
    if (gcsession == null) {
      LOG.error(
          "ERROR: Can not get configuration from server. Is server started and"
              + " communication working?");
    ...
    // Check this before parsing so that user doesn't have to enter CLI password
    // for a nonfunctional command
    ...
    try {
      ParameterContainer parameters =
          parameterHandler.parseParameters(arguments);
      if (parameters == null) {
        // Parameters couldn't be parsed, but this should already be handled.
        return CommandResult.CLI_FAILURE;
      }

      ParameterContainer strippedParameters =
          stripUsernameAndPasswordFromParameters(parameters);
   ...
   *
   * <p>Worth noting in this method is that the password is not sent as a
   * credential, because this would imply sending it (or its hash) in cleartext
   * over the network. Instead, it's only sent as part of a SHA1 hash (as part
   * of the CliAuthenticationToken specification). Actual check of the
   * password's validity will formally occur at the first time that the
   * authentication token is checked for authorization.
   *
   * <p>Note also that the CliAuthenticationToken may only be used for a single
  ...
  protected AuthenticationToken getAuthenticationToken() {
    ...
    CliAuthenticationToken authenticationToken =
        (CliAuthenticationToken)
