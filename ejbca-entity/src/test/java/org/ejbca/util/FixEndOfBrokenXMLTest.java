 ...
 *
 * @version $Id: FixEndOfBrokenXMLTest.java 34163 2020-01-02 15:00:17Z samuellb
 *     $
 */
public class FixEndOfBrokenXMLTest {
   ...
   *
   * <p>This test does not reproduce the production failure that the fix was
   * written for, but instead tests the general ability to repair XML under the
   * current JDK.
   *
  ...
  public void test01() throws Exception {
    ...
    for (int nrOfBytesMissing = 0;
      ...
      try (SecureXMLDecoder decoder =
          new SecureXMLDecoder(new ByteArrayInputStream(fixedXml), true)) {
        ...
        if (nrOfBytesMissing < limit) {
          assertEquals(
              "Only possible to fix "
                  + nrOfBytesMissing
                  + " missing bytes. We should be able to handle "
                  + limit
                  + " missing bytes.",
              new String(xml),
              new String(decodedXml));
        }
        notPossibleToRemoveMoreBytes(nrOfBytesMissing, brokenXml, limit);
        return;
      }
  ...
  private byte[] readXmlFromFile() throws IOException {
    final InputStream is =
        FixEndOfBrokenXMLTest.class.getResourceAsStream("/test.xml");
    final ByteArrayOutputStream baos = new ByteArrayOutputStream();
    while (true) {
  ...
  private void notPossibleToRemoveMoreBytes(
    ...
    assertFalse(
        "Only possible to fix "
            ...
            + limit
            + " missing bytes.",
        nrOfBytesMissing < limit);
  }

  private byte[] decodeAndEncode(final byte[] testXml) throws IOException {
    final ByteArrayOutputStream baos = new ByteArrayOutputStream();
    try (SecureXMLDecoder dec =
        new SecureXMLDecoder(new ByteArrayInputStream(testXml), true)) {
      try (XMLEncoder encoder = new XMLEncoder(baos)) {
   ...
   *
   * <p>This might not be a good thing since we loose more data than necessary,
  ...
  public void test02MissingLastStartOfString() throws Exception {
    ...
    assertEquals(
        "XMLFix has chnaged behaviour. Did not remove objects as originally"
            + " designed.",
        expected,
        fixed);
    log.trace("<test02MissingLastStartOfString");
