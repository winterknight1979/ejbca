 ...
 *
 * @author lars
 * @version $Id: TcpReturnMessage.java 19902 2014-09-30 14:32:24Z anatom $
 */
public final class TcpReturnMessage {
    /** Logger. */
  ...
  /**
   * true if the socket should be closed after returning the message to the
   * client.
   */
  private final boolean doClose;

  private TcpReturnMessage(final byte[] m, final boolean c) {
    this.message = m;
    this.doClose = c;
  }
  /**
   * @param inMsg resp message to be returned to the client.
   * @param close true if the session to the client should be closed.
   * @return the message to be sent to the client
   * @throws IOException Fail
   */
  public static TcpReturnMessage createMessage(
      final byte[] inMsg, final boolean close) throws IOException {
    final byte[] msg;
    final int t3 = 3;
    final int t5 = 5;
    final int msgType;
    ...
    // errorMsgRep should only be used for TCP protocol errors, see 3.5.6 in
    // cmp-transport-protocols
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "Wrote length: '"
              + msg.length
              + t3
              + "' Wrote version: '10' Wrote flags: '"
              + flags
              + "' Wrote msgType: '"
              + msgType
              + "' Wrote msg with length: "
              + msg.length);
    }
